...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.enclawedCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.enclawedCanvasA2UIAction &&
        typeof window.enclawedCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.enclawedSendUserAction === "function";
  ...
  window.addEventListener("enclawed:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Enclawed node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.enclawedSendUserAction === "function"
        ? window.enclawedSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ENCLAWED_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.ENCLAWED_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Enclawed Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
