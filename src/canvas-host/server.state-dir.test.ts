...
describe("canvas host state dir defaults", () => {
  ...
  it("uses ENCLAWED_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("enclawed-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Enclawed Canvas");
      } finally {
        await handler.close();
      }
    });
