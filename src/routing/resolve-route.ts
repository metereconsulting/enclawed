...
function listAgents(cfg: EnclawedConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: EnclawedConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<EnclawedConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: EnclawedConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: EnclawedConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: EnclawedConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<EnclawedConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  EnclawedConfig,
  {
    bindingsRef: EnclawedConfig["bindings"];
    agentsRef: EnclawedConfig["agents"];
    sessionRef: EnclawedConfig["session"];
...
function resolveRouteCacheForConfig(cfg: EnclawedConfig): Map<string, ResolvedAgentRoute> {
