...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Enclawed is a hobby project and still in beta. Expect sharp edges.",
      "By default, Enclawed is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Enclawed is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Enclawed.",
      ...
      "",
      "Run regularly:",
      formatCliCommand("enclawed security audit --deep"),
      formatCliCommand("enclawed security audit --fix"),
      "",
      "Must read: https://docs.enclawed.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("Enclawed setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: EnclawedConfig = snapshot.valid
    ? snapshot.exists
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.enclawed.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("enclawed doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (compatibilityNotices.length > 0) {
    await prompter.note(
      [
        ...
        `Inspect: ${formatCliCommand("enclawed plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("enclawed configure")}.`;
  ...
  let localGatewayToken = process.env.ENCLAWED_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.ENCLAWED_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: EnclawedConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
