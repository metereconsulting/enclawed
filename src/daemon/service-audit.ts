...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ENCLAWED_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `enclawed gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ENCLAWED_GATEWAY_TOKEN does not match gateway.auth.token in enclawed.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.ENCLAWED_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.ENCLAWED_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `enclawed gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
