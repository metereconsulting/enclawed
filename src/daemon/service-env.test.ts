...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ENCLAWED_SERVICE_MARKER).toBe("openclaw");
    expect(env.ENCLAWED_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ENCLAWED_SERVICE_VERSION).toBe("string");
    expect(env.ENCLAWED_SYSTEMD_UNIT).toBe("openclaw-gateway.service");
    expect(env.ENCLAWED_WINDOWS_TASK_NAME).toBe("OpenClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.ENCLAWED_LAUNCHD_LABEL).toBe("ai.openclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.ENCLAWED_SYSTEMD_UNIT).toBe("openclaw-gateway-work.service");
    expect(env.ENCLAWED_WINDOWS_TASK_NAME).toBe("OpenClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.ENCLAWED_LAUNCHD_LABEL).toBe("ai.openclaw.work");
    }
  });
