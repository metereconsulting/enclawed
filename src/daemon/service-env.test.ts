...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ENCLAWED_GATEWAY_PORT).toBe("18789");
    expect(env.ENCLAWED_GATEWAY_TOKEN).toBeUndefined();
    expect(env.ENCLAWED_SERVICE_MARKER).toBe("enclawed");
    expect(env.ENCLAWED_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ENCLAWED_SERVICE_VERSION).toBe("string");
    expect(env.ENCLAWED_SYSTEMD_UNIT).toBe("enclawed-gateway.service");
    expect(env.ENCLAWED_WINDOWS_TASK_NAME).toBe("Enclawed Gateway");
    if (process.platform === "darwin") {
      expect(env.ENCLAWED_LAUNCHD_LABEL).toBe("ai.enclawed.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ENCLAWED_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ENCLAWED_SYSTEMD_UNIT).toBe("enclawed-gateway-work.service");
    expect(env.ENCLAWED_WINDOWS_TASK_NAME).toBe("Enclawed Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.ENCLAWED_LAUNCHD_LABEL).toBe("ai.enclawed.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through ENCLAWED_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ENCLAWED_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.ENCLAWED_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits ENCLAWED_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".enclawed"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ENCLAWED_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".enclawed-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ENCLAWED_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".enclawed"));
  });

  it("uses ENCLAWED_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ENCLAWED_STATE_DIR: "/var/lib/enclawed" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/enclawed"));
  });

  it("expands ~ in ENCLAWED_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ENCLAWED_STATE_DIR: "~/enclawed-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/enclawed-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ENCLAWED_STATE_DIR: "C:\\State\\enclawed" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\enclawed");
  });
});
