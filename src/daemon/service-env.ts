...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.ENCLAWED_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ENCLAWED_PROFILE: profile,
    ENCLAWED_GATEWAY_PORT: String(port),
    ENCLAWED_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ENCLAWED_SYSTEMD_UNIT: systemdUnit,
    ENCLAWED_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    ENCLAWED_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ENCLAWED_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    ENCLAWED_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.ENCLAWED_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ENCLAWED_GATEWAY_TOKEN: gatewayToken,
    ENCLAWED_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    ENCLAWED_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    ENCLAWED_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    ENCLAWED_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    ENCLAWED_LOG_PREFIX: "node",
    ENCLAWED_SERVICE_MARKER: NODE_SERVICE_MARKER,
    ENCLAWED_SERVICE_KIND: NODE_SERVICE_KIND,
    ENCLAWED_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    ENCLAWED_STATE_DIR: sharedEnv.stateDir,
    ENCLAWED_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.ENCLAWED_STATE_DIR;
  const configPath = env.ENCLAWED_CONFIG_PATH;
