...
const TEST_MANAGED_HOME = "/tmp/enclawed-test-home";
const GATEWAY_SERVICE = "enclawed-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/enclawed gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/enclawed", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/enclawed-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "enclawed-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "enclawed-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/enclawed-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled enclawed-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled enclawed-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when ENCLAWED_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/enclawed-gateway.service",
    },
    {
      name: "uses profile-specific service name when ENCLAWED_PROFILE is set to a custom value",
      env: { HOME: "/home/test", ENCLAWED_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/enclawed-gateway-jbphoenix.service",
    },
    {
      name: "prefers ENCLAWED_SYSTEMD_UNIT over ENCLAWED_PROFILE",
      env: {
        HOME: "/home/test",
        ENCLAWED_PROFILE: "jbphoenix",
        ENCLAWED_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles ENCLAWED_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        ENCLAWED_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from ENCLAWED_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        ENCLAWED_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/enclawed gateway start --name "My Bot"')).toEqual([
      "/usr/bin/enclawed",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('enclawed --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["enclawed", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('enclawed --path "C:\\\\Program Files\\\\Enclawed"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["enclawed", "--path", "C:\\\\Program Files\\\\Enclawed"]);

    expect(
      splitArgsPreservingQuotes('enclawed --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["enclawed", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/enclawed gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads ENCLAWED_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/enclawed gateway run", "EnvironmentFile=%h/.enclawed/.env"],
      { [`${TEST_SERVICE_HOME}/.enclawed/.env`]: "ENCLAWED_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.ENCLAWED_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/enclawed gateway run",
        "EnvironmentFile=%h/.enclawed/.env",
        'Environment="ENCLAWED_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.enclawed/.env`]: "ENCLAWED_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.ENCLAWED_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.ENCLAWED_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.enclawed/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/enclawed-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/enclawed gateway run",
          'EnvironmentFile=%h/.enclawed/first.env "%h/.enclawed/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.enclawed/first.env") {
        return "ENCLAWED_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.enclawed/second env.env") {
        return 'ENCLAWED_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ENCLAWED_GATEWAY_TOKEN: "first-token",
      ENCLAWED_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "ENCLAWED_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "ENCLAWED_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "ENCLAWED_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      ENCLAWED_GATEWAY_TOKEN: "override-token",
      ENCLAWED_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/enclawed-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/enclawed gateway run",
          "EnvironmentFile=%h/.enclawed/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.enclawed/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'ENCLAWED_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "ENCLAWED_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      ENCLAWED_GATEWAY_TOKEN: "quoted token",
      ENCLAWED_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      ENCLAWED_GATEWAY_TOKEN: "file",
      ENCLAWED_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("stageSystemdService", () => {
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-systemd-stage-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".enclawed");
    const env = {
      HOME: home,
      ENCLAWED_STATE_DIR: stateDir,
      ENCLAWED_SYSTEMD_UNIT: "enclawed-gateway-stage-test",
    };
    ...
    await fs.writeFile(
      ...
      ["ENCLAWED_GATEWAY_TOKEN=dotenv-token", "LLM_API_KEY=dotenv-key"].join("\n"),
      "utf8",
    );
    ...
    try {
      await stageSystemdService({
        ...
        environment: {
          ENCLAWED_GATEWAY_TOKEN: "dotenv-token",
      ...
      expect(unit).not.toContain("Environment=ENCLAWED_GATEWAY_TOKEN=dotenv-token");
      expect(unit).not.toContain("Environment=LLM_API_KEY=dotenv-key");
      expect(envFile).toBe("ENCLAWED_GATEWAY_TOKEN=dotenv-token\nLLM_API_KEY=dotenv-key\n");
  ...
  it("keeps inline overrides out of the generated env file", async () => {
    ...
    await fs.writeFile(
      path.join(stateDir, ".env"),
      ["ENCLAWED_GATEWAY_TOKEN=stale-token", "LLM_API_KEY=dotenv-key"].join("\n"),
      "utf8",
    );
    ...
    try {
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          ENCLAWED_GATEWAY_TOKEN: "fresh-token",
          LLM_API_KEY: "dotenv-key",
        },
      });
      ...
      expect(unit).toContain(`EnvironmentFile=-${envFilePath}`);
      expect(unit).toContain("Environment=ENCLAWED_GATEWAY_TOKEN=fresh-token");
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "enclawed-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ ENCLAWED_PROFILE: "work" });
  });
