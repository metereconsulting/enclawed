...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when ENCLAWED_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".enclawed", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when ENCLAWED_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", ENCLAWED_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".enclawed-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers ENCLAWED_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        ENCLAWED_PROFILE: "rescue",
        ENCLAWED_STATE_DIR: "C:\\State\\enclawed",
      },
      expected: path.join("C:\\State\\enclawed", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", ENCLAWED_PROFILE: "default" },
      expected: path.join("/home/test", ".enclawed", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Enclawed Gateway",
          "cd /d C:\\Projects\\enclawed",
          "set NODE_ENV=production",
          "set ENCLAWED_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\enclawed",
          environment: {
            NODE_ENV: "production",
            ENCLAWED_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\enclawed\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\enclawed\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Enclawed Share\\node.exe" "\\\\fileserver\\Enclawed Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Enclawed Share\\node.exe",
            "\\\\fileserver\\Enclawed Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from ENCLAWED_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ ENCLAWED_STATE_DIR: path.join(tmpDir, "custom-state") }),
