...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `enclawed-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ENCLAWED_LAUNCHD_LABEL: `ai.enclawed.launchd-int-${testId}`,
      ENCLAWED_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
