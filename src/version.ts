...
declare const __ENCLAWED_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "enclawed";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["ENCLAWED_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["ENCLAWED_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["ENCLAWED_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.ENCLAWED_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Enclawed version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __ENCLAWED_VERSION__ === "string" ? __ENCLAWED_VERSION__ : undefined,
  bundledVersion: process.env.ENCLAWED_BUNDLED_VERSION,
});
