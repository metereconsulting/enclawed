...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  resolveAllowlist: (params: { cfg: EnclawedConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: EnclawedConfig; resolved: TResolved }) => EnclawedConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: EnclawedConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: EnclawedConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: EnclawedConfig, policy: ChannelAccessPolicy): EnclawedConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: EnclawedConfig, policy: ChannelAccessPolicy): EnclawedConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: EnclawedConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: EnclawedConfig; resolved: string[] }) => {
      calls.push("apply");
