 ...
 *
 * `parameters` accepts a schema from either typebox surface in use across the
 * codebase (`@sinclair/typebox` for core tools, `typebox` v1 for bundled
 * channel plugins) since both emit structurally equivalent JSON-schema nodes.
 */
export type ChannelAgentTool = Omit<AgentTool<TSchema, unknown>, "parameters"> & {
  parameters: TSchema | TypeBoxSchema;
  ownerOnly?: boolean;
};

/** Lazy agent-tool factory used when tool availability depends on config. */
export type ChannelAgentToolFactory = (params: { cfg?: EnclawedConfig }) => ChannelAgentTool[];
...
export type ChannelMessageToolSchemaContribution = {
  /**
   * JSON-schema property fragments contributed to the shared `message` tool.
   *
   * Accepts schemas from either typebox surface in use across the codebase
   * (`@sinclair/typebox` in core agent tools, `typebox` v1 in bundled channel
   * plugins) since both are merged structurally before reaching the runtime.
   */
  properties: Record<string, TSchema | TypeBoxSchema>;
  /**
   * Optional action ids these schema properties are scoped to. When omitted,
   * the fragment applies to all of the plugin's discovered actions.
   */
...
export type ChannelAccountSnapshot = {
  ...
  healthState?: string;
  /** Coarse status state string surfaced by channel status issue collectors. */
  statusState?: string;
  ...
  /**
   * Last time the transport (polling loop, websocket, webhook listener) saw
   * activity. Used by status surfaces that distinguish transport health from
   * agent-run activity.
   */
...
/**
 * TTS-related per-channel capability metadata advertised by a channel plugin.
 *
 * Channels that support sending generated speech (voice notes, audio
 * messages) declare the preferred synthesis target and audio file formats so
 * the auto-reply layer can route TTS output through a path the channel will
 * render natively.
 */
export type ChannelTtsCapabilities = {
  voice?: {
    synthesisTarget?: "audio-file" | "voice-note" | "telephony";
    audioFileFormats?: readonly string[];
    preferAudioFileFormat?: string;
    /**
     * When true, the channel can transcode synthesized audio into a
     * channel-native voice format before delivery.
     */
...
export type ChannelCapabilities = {
  ...
  /** Optional TTS routing metadata for channels that support voice output. */
  tts?: ChannelTtsCapabilities;
};
...
export type ChannelThreadingAdapter = {
  ...
  /**
   * Resolves the canonical channel id for a target+thread pair. Used by
   * channels (Telegram topics) that encode thread identity into the channel id
...
export type ChannelMessagingAdapter = {
  ...
  /**
   * Builds a presentation block describing the cross-context origin of a
   * relayed message (for example "From #general"). Channels that render
   * presentation blocks natively implement this instead of (or in addition to)
   * `buildCrossContextComponents`.
   */
  buildCrossContextPresentation?: (params: { originLabel: string; message: string }) => {
    tone?: "neutral" | "muted" | "success" | "warn" | "error";
    blocks: ReadonlyArray<Record<string, unknown>>;
  };
  /**
   * When true, preserve the heartbeat thread id when routing to a group target.
   * Used by channels (Telegram topics) where the heartbeat thread should stay
   * pinned to the originating topic during group routing.
   */
...
export type ChannelAgentPromptAdapter = {
  messageToolHints?: (params: { cfg: EnclawedConfig; accountId?: string | null }) => string[];
