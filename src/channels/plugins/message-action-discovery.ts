...
export function listChannelMessageActions(cfg: EnclawedConfig): ChannelMessageActionName[] {
...
export function listChannelMessageCapabilities(cfg: EnclawedConfig): ChannelMessageCapability[] {
...
function mergeToolSchemaProperties(
  target: Record<string, MessageToolSchema>,
  source: Record<string, MessageToolSchema> | undefined,
) {
  if (!source) {
    return;
  }
...
export function resolveChannelMessageToolSchemaProperties(params: {
  ...
  const properties: Record<string, MessageToolSchema> = {};
