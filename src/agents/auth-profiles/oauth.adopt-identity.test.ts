...
describe("OAuth credential adoption is identity-gated", () => {
  ...
  beforeEach(async () => {
    ...
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-oauth-adopt-identity-"));
    process.env.ENCLAWED_STATE_DIR = tempRoot;
    mainAgentDir = path.join(tempRoot, "agents", "main", "agent");
    process.env.ENCLAWED_AGENT_DIR = mainAgentDir;
