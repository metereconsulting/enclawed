...
type LoadAuthProfileStoreLocalOptions = { readOnly?: boolean; allowKeychainPrompt?: boolean };
...
export function loadAuthProfileStoreWithoutExternalProfiles(agentDir?: string): AuthProfileStore {
  const options: LoadAuthProfileStoreLocalOptions = {
    readOnly: true,
...
export function resolvePersistedAuthProfileOwnerAgentDir(params: {
  ...
  if (requestedProfile) {
    // Conservative oss policy: if the main store has the same profile, prefer it.
