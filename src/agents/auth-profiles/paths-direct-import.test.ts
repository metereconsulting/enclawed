...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  const envSnapshot = captureEnv(["ENCLAWED_STATE_DIR"]);
  let stateDir = "";

  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to resolveEnclawedAgentDir when agentDir is omitted", () => {
    // Omitting agentDir exercises the `agentDir ?? resolveEnclawedAgentDir()`
    // nullish branch. With ENCLAWED_STATE_DIR set to our tempdir, the
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-enclawed-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-path-ensure-"));
    process.env.ENCLAWED_STATE_DIR = stateDir;
  });
