...
describe("createOpenAIWebSocketStreamFn", () => {
  ...
  it("keeps native turn metadata stable across websocket retries and increments attempt", async () => {
    ...
    expect(firstPayload.metadata?.enclawed_session_id).toBe("sess-turn-metadata-retry");
    expect(firstPayload.metadata?.enclawed_transport).toBe("websocket");
    expect(firstPayload.metadata?.enclawed_turn_id).toBeTruthy();
    expect(secondPayload.metadata?.enclawed_turn_id).toBe(firstPayload.metadata?.enclawed_turn_id);
    expect(firstPayload.metadata?.enclawed_turn_attempt).toBe("1");
    expect(secondPayload.metadata?.enclawed_turn_attempt).toBe("2");
  });
