...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: EnclawedConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<EnclawedConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<EnclawedConfig["agents"]>["list"]>[number]["tools"];
  }): EnclawedConfig {
    return {
      tools: params.tools,
      agents: {
