...
async function createBundleProbeRuntime(params?: { reservedToolNames?: string[] }) {
  const workspaceDir = await makeTempDir("enclawed-bundle-mcp-tools-");
  const pluginRoot = path.join(workspaceDir, ".enclawed", "extensions", "bundle-probe");
...
describe("createBundleMcpToolRuntime", () => {
  ...
  it("loads configured SSE MCP tools via url", async () => {
    ...
    try {
      const workspaceDir = await makeTempDir("enclawed-bundle-mcp-sse-");
      const runtime = await createBundleMcpToolRuntime({
        workspaceDir,
        cfg: {
          mcp: {
