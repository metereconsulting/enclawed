...
describe("createEnclawedCodingTools", () => {
  const testConfig: EnclawedConfig = {};

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createEnclawedCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createEnclawedCodingTools({
      config: testConfig,
    ...
    const codexTools = createEnclawedCodingTools({
      config: testConfig,
    ...
    const disabledConfig: EnclawedConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createEnclawedCodingTools({
    ...
    const anthropicTools = createEnclawedCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: EnclawedConfig = {
      tools: {
        exec: {
    ...
    const allowed = createEnclawedCodingTools({
    ...
    const denied = createEnclawedCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createEnclawedCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createEnclawedCodingTools({ config: testConfig });
