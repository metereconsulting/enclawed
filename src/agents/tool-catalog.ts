...
type CoreToolDefinition = {
  ...
  includeInEnclawedGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const enclawedTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInEnclawedGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:enclawed": enclawedTools,
