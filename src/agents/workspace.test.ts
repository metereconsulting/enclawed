...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ENCLAWED_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ENCLAWED_HOME: "/srv/enclawed-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/enclawed-home"), ".enclawed", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".enclawed", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("enclawed-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("enclawed-workspace-");
    await fs.mkdir(path.join(tempDir, ".enclawed"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-workspace-hardlink-"));
    try {
