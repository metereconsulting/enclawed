...
function resolveAnthropicOpusThinking(cfg: EnclawedConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: EnclawedConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<EnclawedConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<EnclawedConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as EnclawedConfig,
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    ...
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as EnclawedConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as EnclawedConfig)).toBe(false);
    });
  });
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<EnclawedConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", () => {
      const warnLogs = createWarnLogCapture("enclawed-model-selection-test");
      try {
        const cfg: Partial<EnclawedConfig> = {
          agents: {
            defaults: {
