...
describe("tool_result_persist hook", () => {
  ...
  it("loads tool_result_persist hooks without breaking persistence", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-toolpersist-"));
  ...
  it("reapplies the cap after tool_result_persist expands a tool result", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-toolpersist-expand-"));
    process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
...
describe("before_message_write hook", () => {
  it("continues persistence when a before_message_write hook throws", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-before-write-"));
    process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("reapplies the cap after before_message_write expands a tool result", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-before-write-expand-"));
    process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
