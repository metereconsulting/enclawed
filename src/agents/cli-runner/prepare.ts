...
const prepareDeps = {
  ...
  createMcpLoopbackServerConfig,
  resolveEnclawedDocsPath: async (
    params: Parameters<typeof import("../docs-path.js").resolveEnclawedDocsPath>[0],
  ) => (await import("../docs-path.js")).resolveEnclawedDocsPath(params),
};
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          ENCLAWED_MCP_TOKEN: mcpLoopbackRuntime.token,
          ENCLAWED_MCP_AGENT_ID: sessionAgentId ?? "",
          ENCLAWED_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          ENCLAWED_MCP_SESSION_KEY: params.sessionKey ?? "",
          ENCLAWED_MCP_MESSAGE_CHANNEL: params.messageProvider ?? "",
          ENCLAWED_MCP_SENDER_IS_OWNER: params.senderIsOwner === true ? "true" : "false",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  const docsPath = await prepareDeps.resolveEnclawedDocsPath({
    workspaceDir,
