...
const ENCLAWED_CLAUDE_PLUGIN_NAME = "enclawed-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredEnclawedTmpDir(), "enclawed-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, ENCLAWED_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: ENCLAWED_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Enclawed skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
