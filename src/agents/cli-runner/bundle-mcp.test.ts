...
describe("prepareCliBundleMcpConfig", () => {
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    try {
      ...
      expect(raw.mcpServers?.openclaw?.headers?.Authorization).toBe("Bearer ${ENCLAWED_MCP_TOKEN}");
  ...
  it("injects codex MCP config overrides with env-backed loopback headers", async () => {
    ...
    expect(prepared.backend.args).toEqual([
      ...
      "--json",
      "-c",
      'mcp_servers={ openclaw = { url = "http://127.0.0.1:23119/mcp", bearer_token_env_var = "ENCLAWED_MCP_TOKEN", env_http_headers = { x-session-key = "ENCLAWED_MCP_SESSION_KEY", x-openclaw-sender-is-owner = "ENCLAWED_MCP_SENDER_IS_OWNER" } } }',
    ]);
