...
const contextTestState = vi.hoisted(() => {
  const state = {
    ...
    ensureEnclawedModelsJson: vi.fn(async () => {}),
...
vi.mock("./models-config.js", () => ({
  ensureEnclawedModelsJson: contextTestState.ensureEnclawedModelsJson,
}));

vi.mock("./agent-paths.js", () => ({
  resolveEnclawedAgentDir: () => "/tmp/enclawed-agent",
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureEnclawedModelsJson.mockClear();
  return { ensureEnclawedModelsJson: contextTestState.ensureEnclawedModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real enclawed startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "enclawed", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "enclawed", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "enclawed", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureEnclawedModelsJson } = mockContextModuleDeps(loadConfigMock);
        ...
        expect(ensureEnclawedModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
