...
function isLikelyEnclawedCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "enclawed" ||
    entryBasename === "enclawed.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Enclawed CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureEnclawedModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyEnclawedCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): EnclawedConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureEnclawedModelsJson(cfg);
    } catch {
