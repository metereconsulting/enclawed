...
let configOverride: EnclawedConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: EnclawedConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/enclawed/enclawed/issues/31298
    previousFastTestEnv = process.env.ENCLAWED_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.ENCLAWED_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // ENCLAWED_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Enclawed runtime context (internal):");
