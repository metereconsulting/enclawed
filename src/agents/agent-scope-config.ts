...
export function listAgentEntries(cfg: EnclawedConfig): AgentEntry[] {
...
export function listAgentIds(cfg: EnclawedConfig): string[] {
...
export function resolveDefaultAgentId(cfg: EnclawedConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: EnclawedConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: EnclawedConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
