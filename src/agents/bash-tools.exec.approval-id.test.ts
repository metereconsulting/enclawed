...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".enclawed", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    previousBundledPluginsDir = process.env.ENCLAWED_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.ENCLAWED_DISABLE_BUNDLED_PLUGINS;
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-test-"));
  ...
  afterEach(() => {
    ...
    if (previousDisableBundledPlugins === undefined) {
      ...
      process.env.ENCLAWED_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
    }
  });
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-test-bin-"));
  ...
  it("auto-continues the same Discord session after approval resolves without a second user turn", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-exec-followup-discord-"));
  ...
  it("executes approved commands and emits a session-only followup in webchat-only mode", async () => {
    const agentCalls: Array<Record<string, unknown>> = [];
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-exec-followup-sidefx-"));
    const markerPath = path.join(tempDir, "marker.txt");
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".enclawed", "skills", "gog");
