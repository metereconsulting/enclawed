...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousEnclawedAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousEnclawedAgentDir = process.env.ENCLAWED_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousEnclawedAgentDir === undefined) {
      delete process.env.ENCLAWED_AGENT_DIR;
    } else {
      process.env.ENCLAWED_AGENT_DIR = previousEnclawedAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "ENCLAWED_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.ENCLAWED_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.ENCLAWED_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.ENCLAWED_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: EnclawedConfig = {
  models: {
    providers: {
      "custom-proxy": {
