...
import "./test-helpers/fast-enclawed-tools-sessions.js";
...
function createEnclawedTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("uses number (not integer) in tool schemas for Gemini compatibility", () => {
    const tools = createEnclawedTools();
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createEnclawedTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createEnclawedTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
