...
import { createEnclawedTools } from "./enclawed-tools.js";
...
function resolveExecConfig(params: { cfg?: EnclawedConfig; agentId?: string }) {
...
export function createEnclawedCodingTools(options?: {
  agentId?: string;
  ...
  /**
   * When true, suppress the managed web-search tool for this run (for example
   * when a harness provides its own provider-native web search).
   */
  ...
  /** If true, include the heartbeat tool in the tool list. */
  enableHeartbeatTool?: boolean;
  /** If true, force-include the heartbeat tool regardless of other gating. */
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createEnclawedReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
