...
vi.mock("../plugins/provider-runtime.js", async () => {
  ...
  return {
    ...actual,
    applyProviderConfigDefaultsWithPlugin: (config: EnclawedConfig) => config,
...
let ensureEnclawedModelsJson: typeof import("./models-config.js").ensureEnclawedModelsJson;
...
beforeAll(async () => {
  ...
  ({ ensureEnclawedModelsJson, resetModelsJsonReadyCacheForTest } =
...
function createOpenAiApiKeySourceConfig(): EnclawedConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): EnclawedConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): EnclawedConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): EnclawedConfig {
  return {
    models: {
      providers: {
        openai: {
...
function withGatewayTokenMode(config: EnclawedConfig): EnclawedConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function withGeneratedModelsFromRuntimeSource(
  params: {
    ...
    candidateConfig?: EnclawedConfig;
  },
  ...
  await withTempHome(async () => {
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      try {
        ...
        await ensureEnclawedModelsJson(params.candidateConfig ?? loadConfig());
        await runAssertions();
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
...
describe("models-config runtime source snapshot", () => {
  ...
  it("uses non-env marker from runtime source snapshot for file refs", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        unsetEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS);
        const sourceConfig: EnclawedConfig = {
          models: {
            providers: {
              moonshot: {
        ...
        const runtimeConfig: EnclawedConfig = {
          models: {
            providers: {
              moonshot: {
                baseUrl: "https://api.moonshot.ai/v1",
        ...
        try {
          ...
          await ensureEnclawedModelsJson(loadConfig());
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const clonedRuntimeConfig: EnclawedConfig = {
          ...runtimeConfig,
          agents: {
            defaults: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureEnclawedModelsJson(clonedRuntimeConfig);
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const sourceConfig = createOpenAiApiKeySourceConfig();
        const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
        const firstCandidate: EnclawedConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        const secondCandidate: EnclawedConfig = {
          ...runtimeConfig,
          models: {
            providers: {
              openai: {
        ...
        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureEnclawedModelsJson(firstCandidate);
          ...
          expect(parsed.providers.openai?.headers?.["X-Enclawed-Test"]).toBe("one");

          // Header changes still rewrite models.json, but merge mode preserves the existing baseUrl.
          await ensureEnclawedModelsJson(secondCandidate);
          ...
          expect(parsed.providers.openai?.baseUrl).toBe("https://api.openai.com/v1");
          expect(parsed.providers.openai?.apiKey).toBe("OPENAI_API_KEY"); // pragma: allowlist secret
          expect(parsed.providers.openai?.headers?.["X-Enclawed-Test"]).toBe("two");
        } finally {
          clearRuntimeConfigSnapshot();
          clearConfigCache();
        }
  ...
  it("keeps source markers when runtime projection is skipped for incompatible top-level shape", async () => {
    await withTempHome(async () => {
      await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
        ...
        const incompatibleCandidate: EnclawedConfig = {
          ...createOpenAiApiKeyRuntimeConfig(),
        };

        try {
          setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
          await ensureEnclawedModelsJson(incompatibleCandidate);
          await expectGeneratedProviderApiKey("openai", "OPENAI_API_KEY"); // pragma: allowlist secret
        } finally {
          clearRuntimeConfigSnapshot();
          clearConfigCache();
