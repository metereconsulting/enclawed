...
import { markEnclawedExecEnv } from "../../infra/enclawed-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "enclawed.sandbox=1");
  args.push("--label", `enclawed.sessionKey=${params.scopeKey}`);
  args.push("--label", `enclawed.createdAtMs=${createdAtMs}`);
  args.push("--label", `enclawed.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `enclawed.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markEnclawedExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "enclawed.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`enclawed sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`enclawed sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("enclawed sandbox recreate --all");
}
