...
describe("provider attribution", () => {
  it("resolves the canonical Enclawed product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Enclawed runtime wrapper.",
      product: "Enclawed",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { ENCLAWED_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { ENCLAWED_VERSION: "2026.3.22" })).toEqual({
      originator: "enclawed",
      version: "2026.3.22",
      "User-Agent": "enclawed/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { ENCLAWED_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ ENCLAWED_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
