...
import "./test-helpers/fast-enclawed-tools.js";
...
function createExecTool(workspaceDir: string) {
  const tools = createEnclawedCodingTools({
    workspaceDir,
...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("enclawed-ws-", async (workspaceDir) => {
      await withTempDir("enclawed-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createEnclawedCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "enclawed" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("enclawed-ws-", async (workspaceDir) => {
      await withTempDir("enclawed-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("enclawed-ws-", async (workspaceDir) => {
      const cfg: EnclawedConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createEnclawedCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-enclawed.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("enclawed-sandbox-", async (sandboxDir) => {
      await withTempDir("enclawed-workspace-", async (workspaceDir) => {
        ...
        const tools = createEnclawedCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
