...
export async function resolveApiKeyForProvider(params: {
  ...
  throw new Error(
    [
      ...
      `Configure auth for this agent (${formatCliCommand("enclawed agents add <id>")}) or copy auth-profiles.json from the main agentDir.`,
    ].join(" "),
  );
}
...
export function resolveModelAuthMode(
  ...
  const authStore = store ?? ensureAuthProfileStore(options?.workspaceDir);
