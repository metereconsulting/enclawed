...
export type RunEmbeddedPiAgentParams = {
  ...
  sessionKey?: string;
  /**
   * Sandbox-scoped session key used by realtime-voice agent consult to keep
   * the embedded pi-agent's sandbox cache distinct from the outer session.
   * Optional: when omitted the embedded runner falls back to
   * `sessionKey ?? sessionId` internally (see attempt.ts).
   */
