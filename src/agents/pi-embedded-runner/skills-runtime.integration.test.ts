...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("enclawed-bundled-");
  const workspaceDir = await createTempDir("enclawed-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: EnclawedConfig) {
