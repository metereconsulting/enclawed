...
/**
 * Optional per-message queue options forwarded to harness-owned steering
 * queues (for example the Codex app-server batching/debounce policy).
 */
 ...
 */
const EMBEDDED_RUN_STATE_KEY = Symbol.for("enclawed.embeddedRunState");
...
export function queueEmbeddedPiMessage(
  ...
  logMessageQueued({ sessionId, source: "pi-embedded-runner" });
  void handle.queueMessage(text, options);
  return true;
}
