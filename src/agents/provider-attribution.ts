...
const ENCLAWED_ATTRIBUTION_PRODUCT = "Enclawed";
const ENCLAWED_ATTRIBUTION_ORIGINATOR = "enclawed";
...
function formatEnclawedUserAgent(version: string): string {
  return `${ENCLAWED_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: ENCLAWED_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: ENCLAWED_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatEnclawedUserAgent(identity.version),
    },
  };
}
