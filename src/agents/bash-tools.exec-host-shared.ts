...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.enclawed/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "enclawed doctor" and "enclawed approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
