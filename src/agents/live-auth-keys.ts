...
const GOOGLE_LIVE_SINGLE_KEY = "ENCLAWED_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ENCLAWED_LIVE_ANTHROPIC_KEY",
    listVar: "ENCLAWED_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ENCLAWED_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ENCLAWED_LIVE_${base}_KEY`;
