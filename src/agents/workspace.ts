...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ENCLAWED_PROFILE?.trim();
  if (profile && normalizeOptionalLowercaseString(profile) !== "default") {
    return path.join(home, ".enclawed", `workspace-${profile}`);
  }
  return path.join(home, ".enclawed", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".enclawed";
