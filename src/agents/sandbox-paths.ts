...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const enclawedTmpDir = path.resolve(resolvePreferredEnclawedTmpDir());
  if (!isPathInside(enclawedTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: enclawedTmpDir });
  return resolved;
}
