...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when ENCLAWED_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const enclawedHome = path.join(tmpDir, "enclawed-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(enclawedHome, { recursive: true });
    ...
    process.env.ENCLAWED_HOME = enclawedHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(enclawedHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
