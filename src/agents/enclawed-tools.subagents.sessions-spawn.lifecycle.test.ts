...
} from "./enclawed-tools.subagents.sessions-spawn.test-harness.js";
import {
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.ENCLAWED_TEST_FAST;
...
describe("enclawed-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnAnnounceFlowOverride();
    resetSessionsSpawnHookRunnerOverride();
    resetSessionsSpawnConfigOverride();
  ...
  afterAll(() => {
    ...
    process.env.ENCLAWED_TEST_FAST = fastModeEnv.previous;
  });
