...
async function withOutsideHardlinkInEnclawedTmp<T>(
  params: {
    enclawedTmpDir: string;
  ...
  const hardlinkPath = path.join(params.enclawedTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.enclawedTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.enclawedTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.enclawedTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const enclawedTmpDir = resolvePreferredEnclawedTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Enclawed tmp root",
      media: path.join(enclawedTmpDir, "image.png"),
      expected: path.join(enclawedTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Enclawed tmp root",
      media: pathToFileURL(path.join(enclawedTmpDir, "photo.png")).href,
      expected: path.join(enclawedTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Enclawed tmp root",
      media: path.join(enclawedTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(enclawedTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(enclawedTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside enclawed tmp root",
      media: path.join(os.tmpdir(), "outside-enclawed", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Enclawed tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(enclawedTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(enclawedTmpDir, { recursive: true });
      const symlinkPath = path.join(enclawedTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Enclawed tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInEnclawedTmp(
      {
        enclawedTmpDir,
  ...
  it("rejects symlinked Enclawed tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInEnclawedTmp(
