...
import { planEnclawedModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: EnclawedConfig): {
  config: EnclawedConfig;
  sourceConfigForSecrets: EnclawedConfig;
} {
...
export async function ensureEnclawedModelsJson(
  config?: EnclawedConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveEnclawedAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planEnclawedModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
