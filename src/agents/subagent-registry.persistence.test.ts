...
describe("subagent registry persistence", () => {
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-subagent-"));
  ...
  it("uses isolated temp state when ENCLAWED_STATE_DIR is unset in tests", async () => {
    delete process.env.ENCLAWED_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "enclawed-test-state"));
  });
});
