...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__enclawed__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__enclawed__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__enclawed__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Enclawed handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Enclawed docs: ${docsPath}`,
    "Mirror: https://docs.enclawed.ai",
    "Source: https://github.com/enclawed/enclawed",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For Enclawed behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `enclawed status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Enclawed process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Enclawed agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Enclawed agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Enclawed.";
  }

  const lines = [
    "You are a personal assistant running inside Enclawed.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Enclawed's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    ...safetySection,
    "## Enclawed CLI Quick Reference",
    "Enclawed is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- enclawed gateway status",
    "- enclawed gateway start",
    "- enclawed gateway stop",
    "- enclawed gateway restart",
    "If unsure, ask the user to run `enclawed help` (or `enclawed gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Enclawed Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Enclawed pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Enclawed and included below in Project Context.",
    "",
