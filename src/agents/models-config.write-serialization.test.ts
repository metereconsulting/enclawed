...
const planEnclawedModelsJsonMock = vi.fn();
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planEnclawedModelsJson: (...args: unknown[]) => planEnclawedModelsJsonMock(...args),
  }));
  ({ ensureEnclawedModelsJson } = await import("./models-config.js"));
});

beforeEach(() => {
  planEnclawedModelsJsonMock
    .mockReset()
...
describe("models-config write serialization", () => {
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      try {
        await Promise.all([ensureEnclawedModelsJson(first), ensureEnclawedModelsJson(second)]);
      } finally {
