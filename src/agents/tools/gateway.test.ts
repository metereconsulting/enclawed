...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    enclawed: process.env.ENCLAWED_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.enclawed === undefined) {
      delete process.env.ENCLAWED_GATEWAY_TOKEN;
    } else {
      process.env.ENCLAWED_GATEWAY_TOKEN = envSnapshot.enclawed;
    }
  });
  ...
  it("uses ENCLAWED_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ENCLAWED_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
