...
import { isEnclawedOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `enclawed security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as EnclawedConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as EnclawedConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isEnclawedOwnerOnlyCoreToolName("gateway"),
    description:
