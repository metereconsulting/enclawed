...
function buildMessageToolSchemaProps(options: {
  ...
  extraProperties?: Record<string, TSchema | TypeBoxSchema>;
}) {
  return {
    ...buildRoutingSchema(),
...
function buildMessageToolSchemaFromActions(
  ...
  // Channel-contributed schema fragments may come from the `typebox` v1 package
  // while core builds with `@sinclair/typebox`. Both emit structurally
  // equivalent JSON-schema nodes; normalize the merged bag to the builder's
  // schema type for the `Type.Object` assembly.
  const properties = props as Record<string, TSchema>;
  return Type.Object({
