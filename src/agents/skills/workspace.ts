...
import { resolveEnclawedMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
function resolveSkillsLimits(config?: EnclawedConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "enclawed-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "enclawed-bundled") {
    return {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "enclawed-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "enclawed-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values())
    ...
    .map((skill) => {
      ...
      return {
        skill,
        frontmatter,
        metadata: resolveEnclawedMetadata(frontmatter),
        invocation,
        exposure: {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`enclawed skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`enclawed skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
