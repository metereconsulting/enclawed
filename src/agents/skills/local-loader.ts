...
export function loadSkillsFromDirSafe(params: { dir: string; source: string; maxBytes?: number }): {
  ...
  // Enclaved-flavor gate (paper §1.3, §3.3): SKILL.md skills are unsigned
  // by construction and untrusted code by definition. The enclaved flavor
  // refuses to admit them through this path; a deploying organization
  // wanting a skill in an enclaved deployment must run it through the
  // SkillRuntime bootstrap with a signed manifest (see
  // src/enclawed/skill-runtime.ts). The open flavor admits them; the
  // SkillRuntime gate then routes their irreversible calls through HITL.
  if (getFlavor() === "enclaved") {
    return { skills: [] };
  }
