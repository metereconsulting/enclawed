...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ENCLAWED_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns ENCLAWED_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-bundled-override-"));
    process.env.ENCLAWED_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "enclawed" }));
