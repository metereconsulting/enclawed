...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseEnclawedManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyEnclawedManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): EnclawedSkillMetadata | undefined {
