...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("enclawed-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("enclawed-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("enclawed-tilde-test-mkdir-");
  ...
  it("ignores ENCLAWED_HOME for write operations", async () => {
    const enclawedHome = await createTempDir("enclawed-home-override-", os.tmpdir());
    const dir = await createTempDir("enclawed-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("ENCLAWED_HOME", enclawedHome);

    createHostWorkspaceWriteTool(enclawedHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(enclawedHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores ENCLAWED_HOME for mkdir operations", async () => {
    const enclawedHome = await createTempDir("enclawed-home-override-", os.tmpdir());
    const dir = await createTempDir("enclawed-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(enclawedHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores ENCLAWED_HOME for readFile operations", async () => {
    const enclawedHome = await createTempDir("enclawed-home-override-", os.tmpdir());
    const dir = await createTempDir("enclawed-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(enclawedHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores ENCLAWED_HOME for access operations", async () => {
    const enclawedHome = await createTempDir("enclawed-home-override-", os.tmpdir());
    const dir = await createTempDir("enclawed-tilde-test-edit-");
