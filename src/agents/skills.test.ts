...
const fixtureSuite = createFixtureSuite("enclawed-skills-suite-");
...
beforeAll(async () => {
  ...
  tempHome = await createTempHomeEnv("enclawed-skills-home-");
  skillsHomeEnv = setMockSkillsHomeEnv(tempHome.home);
  await fs.mkdir(path.join(tempHome.home, ".enclawed", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native Enclawed slash commands", async () => {
    const workspaceDir = await makeWorkspace();
    const config = {
      plugins: {
        entries: {
    ...
    const pluginRoot = path.join(tempHome!.home, ".enclawed", "extensions", "compound-bundle");
...
describe("applySkillEnvOverrides", () => {
  ...
  it("prefers resolved caller skill config when the active runtime snapshot is still raw", async () => {
    ...
    const callerConfig: EnclawedConfig = {
      skills: {
        entries: {
          "env-skill": {
            apiKey: "resolved-key",
