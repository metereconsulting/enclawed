...
describe("auth profile store cache", () => {
  ...
  it("recomputes runtime-only external auth overlays even while the base store is cached", async () => {
    await withAgentDirEnv("enclawed-auth-store-cache-", (agentDir) => {
  ...
  it("refreshes the cached auth store after auth-profiles.json changes", async () => {
    await withAgentDirEnv("enclawed-auth-store-refresh-", async (agentDir) => {
  ...
  it("keeps runtime-only external auth out of persisted auth-profiles.json files", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-auth-store-missing-"));
    const previousAgentDir = process.env.ENCLAWED_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
