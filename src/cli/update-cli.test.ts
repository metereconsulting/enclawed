...
import type { EnclawedConfig, ConfigFileSnapshot } from "../config/types.enclawed.js";
...
vi.mock("../infra/enclawed-root.js", () => ({
  resolveEnclawedPackageRoot: vi.fn(),
  resolveEnclawedPackageRootSync: vi.fn(() => process.cwd()),
}));
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/enclawed-config",
  };
});
...
const { resolveEnclawedPackageRoot } = await import("../infra/enclawed-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/enclawed-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as EnclawedConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/enclawed-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveEnclawedPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("enclawed-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("enclawed-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveEnclawedPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("enclawed");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/enclawed-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "enclawed-gateway" }],
      hints: [],
    });
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          ENCLAWED_UPDATE_POST_CORE: "1",
          ENCLAWED_UPDATE_POST_CORE_CHANNEL: "dev",
        }),
      }),
    );
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
          root: createCaseDir("enclawed-downgraded-root"),
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        ENCLAWED_UPDATE_POST_CORE: "1",
        ENCLAWED_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("Enclawed update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "enclawed", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as EnclawedConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "enclawed@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    mockPackageInstallStatus(createCaseDir("enclawed-update"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "latest",
      version: "2026.3.23-2",
      nodeEngine: ">=22.14.0",
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g enclawed` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:enclawed/enclawed#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("enclawed-update"));
        await updateCommand({ yes: true, tag: "github:enclawed/enclawed#main" });
      },
      expectedSpec: "github:enclawed/enclawed#main",
    },
    {
      name: "ENCLAWED_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("enclawed-update"));
        await withEnvAsync(
      ...
      expectedSpec: "http://10.211.55.2:8138/enclawed-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "enclawed", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("enclawed-update");
    const localAppData = createCaseDir("enclawed-localappdata");
  ...
  it("skips plugin sync in the old process after switching from package to git", async () => {
    ...
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "git",
        root: path.join(tempDir, "..", "enclawed"),
        after: { version: "2026.4.10" },
      }),
    );
    serviceLoaded.mockResolvedValue(true);
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `enclawed update`.",
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            ENCLAWED_CONFIG_PATH: "./config/enclawed.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          cwd: root,
          env: expect.objectContaining({
            ENCLAWED_STATE_DIR: path.resolve("./state"),
            ENCLAWED_CONFIG_PATH: path.resolve("./config/enclawed.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            ENCLAWED_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ENCLAWED_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ENCLAWED_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("enclawed-update-wizard");
    await withEnvAsync({ ENCLAWED_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/enclawed as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ ENCLAWED_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "enclawed"));
    });
