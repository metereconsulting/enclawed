...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "enclawed", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "enclawed", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "enclawed", "--profile", "work", "gateway", "status"],
