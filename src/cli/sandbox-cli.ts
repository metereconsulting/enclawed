...
const SANDBOX_EXAMPLES = {
  main: [
    ["enclawed sandbox list", "List all sandbox containers."],
    ["enclawed sandbox list --browser", "List only browser containers."],
    ["enclawed sandbox recreate --all", "Recreate all containers."],
    ["enclawed sandbox recreate --session main", "Recreate a specific session."],
    ["enclawed sandbox recreate --agent mybot", "Recreate agent containers."],
    ["enclawed sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["enclawed sandbox list", "List all sandbox containers."],
    ["enclawed sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["enclawed sandbox recreate --all", "Recreate all containers."],
    ["enclawed sandbox recreate --session main", "Recreate a specific session."],
    ["enclawed sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["enclawed sandbox recreate --browser --all", "Recreate only browser containers."],
    ["enclawed sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["enclawed sandbox explain", "Show effective sandbox config."],
    ["enclawed sandbox explain --session agent:main:main", "Explain a specific session."],
    ["enclawed sandbox explain --agent work", "Explain an agent sandbox."],
    ["enclawed sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.enclawed.ai/cli/sandbox")}\n`,
    )
    .action(() => {
