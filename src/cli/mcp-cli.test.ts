...
const serveEnclawedChannelMcp = mocks.serveEnclawedChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveEnclawedChannelMcp: mocks.serveEnclawedChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("enclawed-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("enclawed-cli-mcp-home-", async () => {
      ...
      expect(serveEnclawedChannelMcp).toHaveBeenCalledWith({
