...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-bundled-skills-test-"));
    process.env.ENCLAWED_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
