...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Enclawed and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["enclawed update", "Update a source checkout (git)"],
        ["enclawed update --channel beta", "Switch to beta channel (git + npm)"],
        ["enclawed update --channel dev", "Switch to dev channel (git + npm)"],
        ["enclawed update --tag beta", "One-off update to a dist-tag or version"],
        ["enclawed update --tag main", "One-off package install from GitHub main"],
        ["enclawed update --dry-run", "Preview actions without changing anything"],
        ["enclawed update --no-restart", "Update without restarting the service"],
        ["enclawed update --json", "Output result as JSON"],
        ["enclawed update --yes", "Non-interactive (accept downgrade prompts)"],
        ["enclawed update wizard", "Interactive update wizard"],
        ["enclawed --update", "Shorthand for enclawed update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run enclawed update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.enclawed.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["enclawed update status", "Show channel + version status."],
          ["enclawed update status --json", "JSON output."],
          ["enclawed update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.enclawed.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
