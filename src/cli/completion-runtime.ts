...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Enclawed Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# Enclawed Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(enclawed completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "enclawed") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
