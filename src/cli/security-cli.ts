...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["enclawed security audit", "Run a local security audit."],
          ["enclawed security audit --deep", "Include best-effort live Gateway probe checks."],
          ["enclawed security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "enclawed security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["enclawed security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["enclawed security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.enclawed.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Enclawed security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("enclawed security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("enclawed security audit --fix")}`));
        if (!fixResult) {
