...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["enclawed.bash", "enclawed.fish", "enclawed.ps1", "enclawed.zsh"]),
    );
