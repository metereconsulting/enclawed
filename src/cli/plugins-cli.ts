...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Enclawed plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.enclawed.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "enclawed"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      const cfg = (snapshot.sourceConfig ?? snapshot.config) as EnclawedConfig;
      const enableResult = enablePluginInConfig(cfg, id);
      let next: EnclawedConfig = enableResult.config;
  ...
  plugins
    ...
    .option("--json", "Print machine-readable JSON instead of styled lines")
    .action((opts: { json?: boolean } = {}) => {
      ...
      if (opts.json) {
        // Machine-readable contract consumed by CI smoke tests (see
        // scripts/ci/post-bundle-smoke.mjs). Keep the shape narrow and stable:
        // adding fields is fine; renaming/removing requires updating callers.
        const payload = {
          workspaceDir: report.workspaceDir,
          summary: {
            total: report.plugins.length,
            loaded: report.plugins.filter((p) => p.status === "loaded").length,
            errored: errors.length,
            errorDiagnostics: diags.length,
            compatibilityNotices: compatibility.length,
          },
          plugins: report.plugins.map((entry) => ({
            id: entry.id,
            status: entry.status,
            source: entry.source,
      ...
      const docs = formatDocsLink("/plugin", "docs.enclawed.ai/plugin");
      lines.push("");
