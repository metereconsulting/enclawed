...
describe("renderGatewayServiceStartHints", () => {
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        ENCLAWED_CONTAINER: "openclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("openclaw-demo-container");
    expect(
      resolveDaemonContainerContext({
        ENCLAWED_CONTAINER_HINT: "openclaw-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("openclaw-demo-container");
  });
  ...
  it("drops the generic container foreground hint when ENCLAWED_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        ...
        { ENCLAWED_CONTAINER: "openclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toEqual([]);
  });
  ...
  it("drops the generic container foreground hint when ENCLAWED_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        ...
        { ENCLAWED_CONTAINER_HINT: "openclaw-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toEqual([]);
  });
});
