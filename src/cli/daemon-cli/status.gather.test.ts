...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ENCLAWED_STATE_DIR: "/tmp/enclawed-daemon",
    ENCLAWED_CONFIG_PATH: "/tmp/enclawed-daemon/enclawed.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.ENCLAWED_STATE_DIR ?? "/tmp/enclawed-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.ENCLAWED_CONFIG_PATH ?? `${stateDir}/enclawed.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/enclawed-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ENCLAWED_STATE_DIR = "/tmp/enclawed-cli";
    process.env.ENCLAWED_CONFIG_PATH = "/tmp/enclawed-cli/enclawed.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/enclawed-daemon/enclawed.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/enclawed-cli/enclawed.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ENCLAWED_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.ENCLAWED_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.ENCLAWED_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "enclawed-gateway" }],
        hints: [],
      },
      healthy: false,
