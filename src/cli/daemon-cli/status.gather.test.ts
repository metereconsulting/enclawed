...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ENCLAWED_STATE_DIR: "/tmp/openclaw-daemon",
    ENCLAWED_CONFIG_PATH: "/tmp/openclaw-daemon/openclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.ENCLAWED_STATE_DIR ?? "/tmp/openclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.ENCLAWED_CONFIG_PATH ?? `${stateDir}/openclaw.json`;
});
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ENCLAWED_STATE_DIR = "/tmp/openclaw-cli";
    process.env.ENCLAWED_CONFIG_PATH = "/tmp/openclaw-cli/openclaw.json";
