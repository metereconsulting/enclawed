...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("enclawed-daemon-install-int-");
    configPath = path.join(tempHome, "enclawed.json");
    process.env.HOME = tempHome;
    process.env.ENCLAWED_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.ENCLAWED_GATEWAY_TOKEN).toBeUndefined();
  });
});
