...
export function resolveMissingPluginCommandMessage(
  ...
  if (parentPluginId) {
    ...
    if (config?.plugins?.entries?.[parentPluginId]?.enabled === false) {
      return (
        `The \`enclawed ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`enclawed ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.ENCLAWED_CONTAINER) ?? null;
  ...
  try {
    ...
    process.on("uncaughtException", (error) => {
      console.error("[enclawed] Uncaught exception:", formatUncaughtError(error));
