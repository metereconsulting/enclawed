  ...
  plugins: [
    {
      ...
      source: "bundled",
      manifestPath: "/tmp/memory-wiki/enclawed.plugin.json",
...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "enclawed", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "enclawed", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "enclawed", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "enclawed", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "enclawed", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "enclawed", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "enclawed", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "enclawed", "models", "status", "--json"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "enclawed", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "enclawed", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "enclawed", "acp", "-v"])).toBe(true);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "enclawed", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "enclawed", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "enclawed", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "enclawed", "--help", "status"])).toBe(false);
  });
});
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("enclawed memory");
  });
