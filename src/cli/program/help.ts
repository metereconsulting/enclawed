...
const EXAMPLES = [
  ...
  [
    "enclawed channels login --verbose",
    "Link personal WhatsApp Web and show QR + connection logs.",
  ],
  [
    'enclawed message send --target +15555550123 --message "Hi" --json',
  ...
  [
    'enclawed agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'enclawed message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env ENCLAWED_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.enclawed-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates ENCLAWED_STATE_DIR/ENCLAWED_CONFIG_PATH under ~/.enclawed-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  if (
    ...
    console.log(
      commit ? `enclawed ${ctx.programVersion} (${commit})` : `enclawed ${ctx.programVersion}`,
    );
    process.exit(0);
  }
