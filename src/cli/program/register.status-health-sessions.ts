...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["enclawed status", "Show channel health + session summary."],
          ["enclawed status --all", "Full diagnosis (read-only)."],
          ["enclawed status --json", "Machine-readable output."],
          ["enclawed status --usage", "Show model provider usage/quota snapshots."],
          [
            "enclawed status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["enclawed status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.enclawed.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.enclawed.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["enclawed sessions", "List all sessions."],
          ["enclawed sessions --agent work", "List sessions for one agent."],
          ["enclawed sessions --all-agents", "Aggregate sessions across agents."],
          ["enclawed sessions --active 120", "Only last 2 hours."],
          ["enclawed sessions --json", "Machine-readable output."],
          ["enclawed sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.enclawed.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["enclawed sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "enclawed sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["enclawed sessions cleanup --enforce", "Apply maintenance now."],
          ["enclawed sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["enclawed sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "enclawed sessions cleanup --enforce --store ./tmp/sessions.json",
