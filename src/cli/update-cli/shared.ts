...
import { resolveEnclawedPackageRoot } from "../../infra/enclawed-root.js";
...
const ENCLAWED_REPO_URL = "https://github.com/enclawed/enclawed.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "enclawed";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["enclawed", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.ENCLAWED_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "enclawed");
  }
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveEnclawedPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ENCLAWED_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ENCLAWED_GIT_DIR points at a non-git directory: ${params.dir}. Set ENCLAWED_GIT_DIR to an empty folder or an enclawed checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ENCLAWED_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "enclawed.mjs");
