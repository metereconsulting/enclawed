    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["enclawed channels list", "List configured channels and auth profiles."],
          ["enclawed channels status --probe", "Run channel status checks and probes."],
          [
            "enclawed channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["enclawed channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.enclawed.ai/cli/channels",
        )}\n`,
    );
