...
describe("proxy cli runtime", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "enclawed-proxy-cli-runtime-"));
    ...
    process.env.ENCLAWED_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.ENCLAWED_DEBUG_PROXY_ENABLED;
    delete process.env.ENCLAWED_DEBUG_PROXY_SESSION_ID;
