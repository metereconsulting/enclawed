    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "enclawed node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["enclawed node status", "Check node host service status."],
          ["enclawed node install", "Install the node host service."],
          ["enclawed node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.enclawed.ai/cli/node")}\n`,
    );
