...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.enclawed${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ENCLAWED_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.ENCLAWED_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.ENCLAWED_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.ENCLAWED_CONFIG_PATH)) {
    env.ENCLAWED_CONFIG_PATH = path.join(stateDir, "enclawed.json");
  }

  if (profile === "dev" && !env.ENCLAWED_GATEWAY_PORT?.trim()) {
    env.ENCLAWED_GATEWAY_PORT = "19001";
  }
}
