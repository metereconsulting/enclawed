...
describe("applyCliProfileEnv", () => {
  it("fills env defaults for dev profile", () => {
    ...
    expect(env.ENCLAWED_CONFIG_PATH).toBe(path.join(expectedStateDir, "openclaw.json"));
  ...
  it("does not override explicit env values", () => {
    ...
    expect(env.ENCLAWED_CONFIG_PATH).toBe(path.join("/custom", "openclaw.json"));
  });
  ...
  it("uses ENCLAWED_HOME when deriving profile state dir", () => {
    ...
    expect(env.ENCLAWED_STATE_DIR).toBe(path.join(resolvedHome, ".openclaw-work"));
...
describe("formatCliCommand", () => {
  ...
  it("inserts --profile flag when profile is set", () => {
    expect(formatCliCommand("openclaw doctor --fix", { ENCLAWED_PROFILE: "work" })).toBe(
      "openclaw --profile work doctor --fix",
    );
  });

  it("trims whitespace from profile", () => {
    expect(formatCliCommand("openclaw doctor --fix", { ENCLAWED_PROFILE: "  jbopenclaw  " })).toBe(
      "openclaw --profile jbopenclaw doctor --fix",
    );
  });

  it("handles command with no args after openclaw", () => {
    expect(formatCliCommand("openclaw", { ENCLAWED_PROFILE: "test" })).toBe(
      "openclaw --profile test",
    );
  });

  it("handles pnpm wrapper", () => {
    expect(formatCliCommand("pnpm openclaw doctor", { ENCLAWED_PROFILE: "work" })).toBe(
  ...
  it("inserts --container when a container hint is set", () => {
    expect(
      formatCliCommand("openclaw gateway status --deep", { ENCLAWED_CONTAINER_HINT: "demo" }),
  ...
  it("does not prepend --container for update commands", () => {
    expect(formatCliCommand("openclaw update", { ENCLAWED_CONTAINER_HINT: "demo" })).toBe(
      "openclaw update",
    );
    expect(
      formatCliCommand("pnpm openclaw update --channel beta", { ENCLAWED_CONTAINER_HINT: "demo" }),
