    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["enclawed nodes status", "List known nodes with live status."],
          ["enclawed nodes pairing pending", "Show pending node pairing requests."],
          [
            'enclawed nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["enclawed nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.enclawed.ai/cli/nodes")}\n`,
    );
