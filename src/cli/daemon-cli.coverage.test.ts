...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      ENCLAWED_GATEWAY_PORT: String(params.port),
      ...(params.token ? { ENCLAWED_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ENCLAWED_STATE_DIR = "/tmp/enclawed-cli-state";
    process.env.ENCLAWED_CONFIG_PATH = "/tmp/enclawed-cli-state/enclawed.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        ENCLAWED_STATE_DIR: "/tmp/enclawed-daemon-state",
        ENCLAWED_CONFIG_PATH: "/tmp/enclawed-daemon-state/enclawed.json",
        ENCLAWED_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.enclawed.gateway.plist",
    });
