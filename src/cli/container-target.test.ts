...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to ENCLAWED_CONTAINER", () => {
    expect(
...
describe("maybeRunCliInContainer", () => {
  ...
  it("uses ENCLAWED_CONTAINER when the flag is absent", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "openclaw", "status"], {
        env: { ENCLAWED_CONTAINER: "demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("clears inherited host routing and gateway env before execing into the child CLI", () => {
    ...
    maybeRunCliInContainer(["node", "openclaw", "status"], {
      env: {
        ...
        ENCLAWED_GATEWAY_PASSWORD: "password",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "openclaw", ENCLAWED_CONTAINER: "" },
      },
    );
  });
  ...
  it("checks docker after podman and before failing", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "somalley", ENCLAWED_CONTAINER: "" },
      },
    );
  ...
  it("prefers --container over ENCLAWED_CONTAINER", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "openclaw", "--container", "flag-demo", "health"], {
        env: { ENCLAWED_CONTAINER: "env-demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      maybeRunCliInContainer(["node", "openclaw", "--container", "demo", "status"], {
        env: { ENCLAWED_CLI_CONTAINER_BYPASS: "1" } as NodeJS.ProcessEnv,
      }),
    ).toEqual({
      handled: false,
