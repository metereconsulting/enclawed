...
let compiledSecretTargetRegistryState: {
  ...
  enclawedCompiledSecretTargets: CompiledTargetRegistryEntry[];
  enclawedTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreEnclawedTargetState: {
  knownTargetIds: Set<string>;
  enclawedCompiledSecretTargets: CompiledTargetRegistryEntry[];
  enclawedTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledBundledChannelEnclawedTargets = new Map<
  string,
...
function getCompiledSecretTargetRegistryState() {
  ...
  const enclawedCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "enclawed.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    enclawedCompiledSecretTargets,
    enclawedTargetsById: buildConfigTargetIdIndex(enclawedCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreEnclawedTargetState() {
  if (compiledCoreEnclawedTargetState) {
    return compiledCoreEnclawedTargetState;
  }
  const enclawedCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "enclawed.json")
    .map(compileTargetRegistryEntry);
  compiledCoreEnclawedTargetState = {
    knownTargetIds: new Set(enclawedCompiledSecretTargets.map((entry) => entry.id)),
    enclawedCompiledSecretTargets,
    enclawedTargetsById: buildConfigTargetIdIndex(enclawedCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(enclawedCompiledSecretTargets),
  };
  return compiledCoreEnclawedTargetState;
}

function getCompiledBundledChannelEnclawedTargets(
  channelId: string,
): CompiledTargetRegistryEntry[] | null {
  ...
  if (compiledBundledChannelEnclawedTargets.has(normalizedChannelId)) {
    return compiledBundledChannelEnclawedTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadBundledChannelSecretContractApi(normalizedChannelId)
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "enclawed.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledBundledChannelEnclawedTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreEnclawedTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreEnclawedTargetState().enclawedCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitBundledChannelEntries = explicitBundledChannelId
    ? getCompiledBundledChannelEnclawedTargets(explicitBundledChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().enclawedCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreEnclawedTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreEnclawedTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.enclawedCompiledSecretTargets,
    entriesById: registryState.enclawedTargetsById,
  });
