...
import type { EnclawedConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.enclawed.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: EnclawedConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: EnclawedConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: EnclawedConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = EnclawedSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as EnclawedConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: EnclawedConfig | null | undefined;
  ...
  const ensureCompatConfig = (): EnclawedConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Enclawed config schema.
