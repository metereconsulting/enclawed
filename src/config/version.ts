...
export function parseEnclawedVersion(raw: string | null | undefined): EnclawedVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeEnclawedVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseEnclawedVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameEnclawedStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseEnclawedVersion(a);
  const parsedB = parseEnclawedVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareEnclawedVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseEnclawedVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseEnclawedVersion(current);
  const parsedTouched = parseEnclawedVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameEnclawedStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareEnclawedVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: EnclawedVersion): number {
