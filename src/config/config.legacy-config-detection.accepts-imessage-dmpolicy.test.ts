...
function expectEnclawedSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = EnclawedSchema.safeParse(params.config);
  expect(res.success).toBe(false);
  if (!res.success) {
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectEnclawedSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = EnclawedSchema.safeParse(config);
    expect(res.success).toBe(true);
    if (res.success) {
