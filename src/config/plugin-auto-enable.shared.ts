...
function collectModelRefs(cfg: EnclawedConfig): string[] {
  const refs: string[] = [];
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: EnclawedConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: EnclawedConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: EnclawedConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: EnclawedConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: EnclawedConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: EnclawedConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: EnclawedConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: EnclawedConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: EnclawedConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: EnclawedConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: EnclawedConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: EnclawedConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: EnclawedConfig): boolean {
  const entries = cfg.plugins?.entries;
...
function hasPluginEntries(cfg: EnclawedConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function configMayNeedPluginManifestRegistry(cfg: EnclawedConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: EnclawedConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: EnclawedConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isBuiltInChannelAlreadyEnabled(cfg: EnclawedConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: EnclawedConfig, pluginId: string): EnclawedConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
