...
export type CliBackendConfig = {
  ...
  /**
   * Flag used to pass a path to a file containing the system prompt
   * (e.g. `--append-system-prompt-file`).
   */
  ...
  serialize?: boolean;
  /**
   * Optional live-session driver id for this backend.
   *
   * When set, runtime uses a long-lived transport instead of the per-turn
   * exec/spawn path. The well-known driver id `"claude-stdio"` keeps a single
   * Claude CLI process attached over stdin/stdout for the whole session.
   */
...
export type AgentDefaultsConfig = {
  ...
  /**
   * Default agent-runtime selection policy.
   *
   * Use `id` to pin a specific runtime (for example `claude-cli`) or leave
   * unset / `"auto"` to let Enclawed pick based on auth/availability.
   */
  agentRuntime?: {
    /** Runtime id, for example `"claude-cli"` or `"auto"`. */
    id?: string;
  };
  ...
  /**
   * Per-model prompt-overlay configuration that plugins read to apply
   * personality or other prompt-level overlays before the model call. Keyed
   * by overlay family (e.g. `gpt5`) with overlay-specific options nested
   * under the family key.
   */
  promptOverlays?: {
    gpt5?: {
...
export type AgentLlmConfig = {
   ...
   * If unset, Enclawed uses the default LLM idle timeout.
   */
