...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): EnclawedConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: EnclawedConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        ENCLAWED_STATE_DIR: envStateDir,
      };
      const cfg: EnclawedConfig = {};
