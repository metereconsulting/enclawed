...
// Keep integration tests deterministic: never read a real enclawed.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "enclawed-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.ENCLAWED_SESSION_CACHE_TTL_MS;
    process.env.ENCLAWED_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.ENCLAWED_SESSION_CACHE_TTL_MS;
    } else {
      process.env.ENCLAWED_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
