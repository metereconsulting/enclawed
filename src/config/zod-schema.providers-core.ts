...
export const TelegramAccountSchemaBase = z
  .object({
    ...
    network: z
      ...
      .strict()
      .optional(),
    pollingStallThresholdMs: z
      .number()
      .optional()
      .describe(
        "Long-polling stall watchdog threshold in ms. When no successful update fetch occurs within this window, Enclawed restarts the poll loop.",
      ),
...
const DiscordVoiceSchema = z
  .object({
    ...
    reconnectGraceMs: z.number().int().min(0).optional(),
  })
  .strict()
  .optional();
...
export const DiscordAccountSchema = z
  .object({
    ...
    gatewayInfoTimeoutMs: z.number().int().positive().optional(),
    thread: z
      .object({
...
export const SlackAccountSchema = z
  .object({
    ...
    socketMode: z
      .object({
        clientPingTimeout: z.number().optional(),
        serverPingTimeout: z.number().optional(),
