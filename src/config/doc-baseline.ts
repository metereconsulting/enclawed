...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.ENCLAWED_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveEnclawedPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    ENCLAWED_STATE_DIR: path.join(os.tmpdir(), "enclawed-config-doc-baseline-state"),
    ENCLAWED_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
