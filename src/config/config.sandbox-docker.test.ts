...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("enclawed-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "enclawed-sandbox-browser-global" },
      agentBrowser: { network: "enclawed-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("enclawed-sandbox-browser-agent");
  });
