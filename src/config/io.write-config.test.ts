...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "enclawed-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  it("keeps writes inside an ENCLAWED_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".enclawed", "enclawed.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "enclawed.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "enclawed.json"), "utf-8"),
      ) as {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/enclawed-test-required-plugin",
          source: "/tmp/enclawed-test-required-plugin/index.ts",
          manifestPath: "/tmp/enclawed-test-required-plugin/enclawed.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
