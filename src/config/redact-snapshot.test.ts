...
import type { ConfigFileSnapshot } from "./types.enclawed.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/enclawed/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/enclawed/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/enclawed/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/enclawed/nickserv-password.txt");
