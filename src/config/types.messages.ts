...
export type GroupChatConfig = {
  ...
  historyLimit?: number;
  /**
   * How the host should surface agent replies in group/channel chats when the
   * message tool is also available. "automatic" delivers directly;
   * "message_tool" defers visible delivery to the agent's message-tool path.
   */
...
export type DmConfig = {
  historyLimit?: number;
  /** Optional per-DM system prompt override. */
...
export type MessagesConfig = {
  ...
  tts?: TtsConfig;
  /**
   * How the host should surface agent replies for direct/private chats when
   * the message tool is also available. "automatic" delivers directly;
   * "message_tool" defers visible delivery to the agent's message-tool path.
   * Group chats use `groupChat.visibleReplies` instead.
   */
  visibleReplies?: "automatic" | "message_tool";
};
...
export type CommandsConfig = {
  ...
  config?: boolean;
  /** Allow /mcp command for Enclawed-managed MCP settings (default: false). */
  mcp?: boolean;
