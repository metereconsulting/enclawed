...
function warnLegacyToolsBySenderKey(rawKey: string) {
  ...
  process.emitWarning(
    `toolsBySender key "${trimmed}" is deprecated. Use explicit prefixes (id:, e164:, username:, name:). Legacy unprefixed keys are matched as id only.`,
    {
      ...
      code: "ENCLAWED_TOOLS_BY_SENDER_UNTYPED_KEY",
    },
  );
}
...
export function resolveChannelGroupRequireMention(params: {
  ...
  /**
   * When true and no explicit config/override mention requirement applies,
   * default to not requiring a mention (used by channels where group messages
   * are addressed to the bot by default).
   */
  ...
  return params.configuredGroupDefaultsToNoMention ? false : true;
}
