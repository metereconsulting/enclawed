...
export type DiscordIntentsConfig = {
  ...
  /** Enable Guild Voice States intent (needed for voice channel features). Default: false. */
  voiceStates?: boolean;
};
...
export type DiscordVoiceConfig = {
  ...
  tts?: TtsConfig;
  /** Realtime voice model ref (provider/model) used for voice conversations. */
  model?: string;
  /** Voice gateway connect timeout in ms. */
  connectTimeoutMs?: number;
  /** Grace period in ms before tearing down a voice connection on reconnect. */
...
export type DiscordAccountConfig = {
  ...
  token?: SecretInput;
  /**
   * Explicit Discord application (client) id. When omitted, Enclawed derives it
   * from the bot token or the Discord API.
   */
  applicationId?: string;
  /** Timeout in ms for Discord gateway info / app-id lookups. Default: 5000. */
  ...
  /** Thread handling behavior (parent-channel context inheritance, etc.). */
  thread?: {
    /** Inherit parent channel routing/context for messages in threads. Default: false. */
  ...
  inboundWorker?: {
    /**
     * Max time (ms) a queued inbound run may execute before Enclawed aborts it.
