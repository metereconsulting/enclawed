...
function collectConfigEnvVarsByTarget(cfg?: EnclawedConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: EnclawedConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: EnclawedConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: EnclawedConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.ENCLAWED_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
