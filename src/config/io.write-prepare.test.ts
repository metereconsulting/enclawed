...
describe("config io write prepare", () => {
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("enclawed config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('enclawed config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/enclawed.json",
        expectedConfigPath: "/tmp/enclawed.json",
  ...
  it("keeps plugin AJV defaults out of the persisted candidate", () => {
    ...
    const nextConfig: EnclawedConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("preserves normalized nested channel enabled keys during unrelated writes", () => {
    ...
    const nextConfig: EnclawedConfig = {
