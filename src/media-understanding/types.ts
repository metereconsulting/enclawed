...
export type ImageDescriptionRequest = {
  ...
  /**
   * Optional pre-resolved auth-profile store. When provided, image-description
   * runs forward it to model-auth resolution so profile-locked describes do
   * not re-open the auth store from disk.
   */
