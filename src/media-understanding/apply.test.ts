...
const TEMP_MEDIA_PREFIX = "enclawed-media-";
...
function createGroqAudioConfig(): EnclawedConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): EnclawedConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): EnclawedConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: EnclawedConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredEnclawedTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        ENCLAWED_AGENT_DIR: isolatedAgentDir,
