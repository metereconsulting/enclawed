...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.ENCLAWED_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: EnclawedConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: EnclawedConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: EnclawedConfig, requestedAgentId: string) => {
