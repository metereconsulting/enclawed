...
export function resolveHooksConfig(cfg: EnclawedConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: EnclawedConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-enclawed-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-enclawed-idempotency-key"]) ||
