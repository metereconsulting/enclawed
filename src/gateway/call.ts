...
function loadGatewayConfig(): EnclawedConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: EnclawedConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.ENCLAWED_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as EnclawedConfig) : loadGatewayConfig());
