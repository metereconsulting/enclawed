...
import { ensureEnclawedCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.ENCLAWED_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.ENCLAWED_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "ENCLAWED_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "ENCLAWED_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `enclawed config set gateway.auth.mode token` and `enclawed config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: EnclawedConfig) =>
  ...
  try {
    ...
    const gatewayRequestContext = createGatewayRequestContext({
      ...
      clients,
      enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: EnclawedConfig) => {
        enforceSharedGatewaySessionGenerationForConfigWrite({
