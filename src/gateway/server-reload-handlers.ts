...
export function createGatewayReloadHandlers(params: {
  ...
  createHealthMonitor: (config: EnclawedConfig) => ChannelHealthMonitor | null;
}) {
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: EnclawedConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (ENCLAWED_SKIP_GMAIL_WATCHER=1)"),
      });
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.ENCLAWED_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.ENCLAWED_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (ENCLAWED_SKIP_CHANNELS=1 or ENCLAWED_SKIP_PROVIDERS=1)",
        );
      } else {
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: EnclawedConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
...
export function startManagedGatewayConfigReloader(params: {
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: EnclawedConfig) => string | undefined;
