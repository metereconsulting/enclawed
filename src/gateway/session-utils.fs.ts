...
export function attachEnclawedTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__enclawed && typeof record.__enclawed === "object" && !Array.isArray(record.__enclawed)
      ? (record.__enclawed as Record<string, unknown>)
      : {};
  return {
    ...record,
    __enclawed: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachEnclawedTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "enclawed" && model === "delivery-mirror";
