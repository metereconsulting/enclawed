...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([ENCLAWED_MODEL_ID, ENCLAWED_DEFAULT_MODEL_ID]);
  ids.add(`enclawed/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`enclawed/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== ENCLAWED_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
