...
let lastCreateEnclawedToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/enclawed-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateEnclawedToolsContext?.agentTo,
          agentThreadId: lastCreateEnclawedToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createEnclawedTools: (ctx: Record<string, unknown>) => {
      lastCreateEnclawedToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateEnclawedToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-enclawed-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-enclawed-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateEnclawedToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateEnclawedToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateEnclawedToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateEnclawedToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateEnclawedToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateEnclawedToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-enclawed-message-to": "channel:24514",
        "x-enclawed-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
