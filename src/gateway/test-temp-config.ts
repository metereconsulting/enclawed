...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "enclawed-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.ENCLAWED_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as EnclawedConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "enclawed-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.ENCLAWED_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
