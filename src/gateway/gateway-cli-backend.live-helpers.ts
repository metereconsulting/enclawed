...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("ENCLAWED_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.ENCLAWED_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.ENCLAWED_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.ENCLAWED_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.ENCLAWED_SKIP_PROVIDERS,
    ...
    bundledPluginsDir: process.env.ENCLAWED_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.ENCLAWED_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.ENCLAWED_SKIP_PROVIDERS = "1";
  ...
  process.env.ENCLAWED_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  ...
  restoreEnvVar("ENCLAWED_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("ENCLAWED_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("ENCLAWED_SKIP_PROVIDERS", snapshot.skipProviders);
  ...
  restoreEnvVar("ENCLAWED_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("ENCLAWED_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
