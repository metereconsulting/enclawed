...
import { attachEnclawedTranscriptMeta, readSessionMessages } from "./session-utils.js";
...
export type SessionHistoryMessage = Record<string, unknown> & {
  __enclawed?: SessionHistoryTranscriptMeta;
};
...
export function resolveMessageSeq(message: SessionHistoryMessage | undefined): number | undefined {
  const seq = message?.__enclawed?.seq;
...
export class SessionHistorySseState {
  ...
  appendInlineMessage(update: {
    ...
    const nextMessage = attachEnclawedTranscriptMeta(update.message, {
