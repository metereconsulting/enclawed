...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ENCLAWED_SKIP_BROWSER_CONTROL_SERVER",
  "ENCLAWED_SKIP_GMAIL_WATCHER",
  "ENCLAWED_SKIP_CANVAS_HOST",
  "ENCLAWED_BUNDLED_PLUGINS_DIR",
  "ENCLAWED_SKIP_CHANNELS",
  "ENCLAWED_SKIP_PROVIDERS",
  "ENCLAWED_SKIP_CRON",
  "ENCLAWED_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.ENCLAWED_CONFIG_PATH) {
    configPaths.add(process.env.ENCLAWED_CONFIG_PATH);
  }
  if (process.env.ENCLAWED_STATE_DIR) {
    configPaths.add(path.join(process.env.ENCLAWED_STATE_DIR, "enclawed.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "enclawed-test-no-bundled-extensions")
    : "enclawed-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  const stateDir = process.env.ENCLAWED_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".enclawed-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".enclawed-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".enclawed-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>enclawed-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__enclawedTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__enclawedTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["ENCLAWED_GATEWAY_TOKEN"]);
  const prev = process.env.ENCLAWED_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ENCLAWED_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.ENCLAWED_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
