...
type GatewayInstallAuthMode = NonNullable<NonNullable<EnclawedConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: EnclawedConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.ENCLAWED_GATEWAY_PASSWORD) ||
