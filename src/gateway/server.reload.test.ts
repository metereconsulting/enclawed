...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ENCLAWED_SKIP_CHANNELS;
    prevSkipGmail = process.env.ENCLAWED_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.ENCLAWED_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.ENCLAWED_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ENCLAWED_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ENCLAWED_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ENCLAWED_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.ENCLAWED_SKIP_PROVIDERS;
    } else {
      process.env.ENCLAWED_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    const configPath = process.env.ENCLAWED_CONFIG_PATH;
    if (!configPath) {
      throw new Error("ENCLAWED_CONFIG_PATH is not set");
    }
...
`,
    ...
    const previousGatewayTokenEnv = process.env.ENCLAWED_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.ENCLAWED_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
