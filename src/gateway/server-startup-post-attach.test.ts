...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/enclawed-state",
    resolveConfigPath: vi.fn(() => "/tmp/enclawed-state/enclawed.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/enclawed-state"),
  };
});
...
function createPostAttachParams(overrides: Partial<PostAttachParams> = {}): PostAttachParams {
  return {
    ...
    defaultWorkspaceDir: "/tmp/enclawed-workspace",
    deps: {} as never,
