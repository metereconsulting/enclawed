...
function cfg(input: Partial<EnclawedConfig>): EnclawedConfig {
  return input as EnclawedConfig;
}
...
const DEFAULT_ENV = {
  ENCLAWED_GATEWAY_TOKEN: "env-token",
  ENCLAWED_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when ENCLAWED env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when ENCLAWED env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when ENCLAWED env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
