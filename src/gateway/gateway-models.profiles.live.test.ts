...
import { resolveEnclawedAgentDir } from "../agents/agent-paths.js";
...
import { ensureEnclawedModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import type { ModelsConfig, EnclawedConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ENCLAWED_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.ENCLAWED_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.ENCLAWED_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.ENCLAWED_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.ENCLAWED_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.ENCLAWED_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["ENCLAWED_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.ENCLAWED_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.ENCLAWED_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.ENCLAWED_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.ENCLAWED_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.ENCLAWED_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.ENCLAWED_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.ENCLAWED_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.ENCLAWED_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("ENCLAWED_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("ENCLAWED_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("ENCLAWED_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.ENCLAWED_LIVE_GATEWAY_MODELS;
    delete process.env.ENCLAWED_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ENCLAWED_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.ENCLAWED_LIVE_GATEWAY_MODELS = "openai/gpt-5.4,anthropic/claude-opus-4-6";
    delete process.env.ENCLAWED_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.ENCLAWED_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.ENCLAWED_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: EnclawedConfig;
  agentDir: string;
}): Promise<EnclawedConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<EnclawedConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.ENCLAWED_CONFIG_PATH,
    token: process.env.ENCLAWED_GATEWAY_TOKEN,
    skipChannels: process.env.ENCLAWED_SKIP_CHANNELS,
    skipGmail: process.env.ENCLAWED_SKIP_GMAIL_WATCHER,
    skipCron: process.env.ENCLAWED_SKIP_CRON,
    skipCanvas: process.env.ENCLAWED_SKIP_CANVAS_HOST,
    disableBonjour: process.env.ENCLAWED_DISABLE_BONJOUR,
    logLevel: process.env.ENCLAWED_LOG_LEVEL,
    agentDir: process.env.ENCLAWED_AGENT_DIR,
  ...
  process.env.ENCLAWED_SKIP_CHANNELS = "1";
  process.env.ENCLAWED_SKIP_GMAIL_WATCHER = "1";
  process.env.ENCLAWED_SKIP_CRON = "1";
  process.env.ENCLAWED_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.ENCLAWED_DISABLE_BONJOUR = "1";
    process.env.ENCLAWED_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.ENCLAWED_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveEnclawedAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-live-state-"));
  process.env.ENCLAWED_STATE_DIR = tempStateDir;
  ...
  process.env.ENCLAWED_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.enclawed-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveEnclawedAgentDir();
  const sanitizedCfg: EnclawedConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-live-"));
  const tempConfigPath = path.join(tempDir, "enclawed.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.ENCLAWED_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "Enclawed live tool probe (local, safe): " +
    ...
    process.env.ENCLAWED_CONFIG_PATH = previous.configPath;
    process.env.ENCLAWED_GATEWAY_TOKEN = previous.token;
    process.env.ENCLAWED_SKIP_CHANNELS = previous.skipChannels;
    process.env.ENCLAWED_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ENCLAWED_SKIP_CRON = previous.skipCron;
    process.env.ENCLAWED_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ENCLAWED_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.ENCLAWED_LOG_LEVEL = previous.logLevel;
    process.env.ENCLAWED_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ENCLAWED_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await ensureEnclawedModelsJson(cfg);
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ENCLAWED_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.enclawed-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
