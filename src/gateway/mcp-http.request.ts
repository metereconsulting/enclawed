...
function resolveScopedSessionKey(cfg: EnclawedConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  const senderIsOwnerRaw = normalizeOptionalLowercaseString(
    getHeader(req, "x-enclawed-sender-is-owner"),
  );
  return {
    sessionKey: resolveScopedSessionKey(cfg, getHeader(req, "x-session-key")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-enclawed-message-channel")) ?? undefined,
    accountId: normalizeOptionalString(getHeader(req, "x-enclawed-account-id")),
    senderIsOwner:
