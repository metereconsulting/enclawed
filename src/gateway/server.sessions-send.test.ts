...
const { createEnclawedTools } = await import("../agents/enclawed-tools.js");
...
type SessionSendTool = ReturnType<typeof createEnclawedTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createEnclawedTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ENCLAWED_GATEWAY_PORT", "ENCLAWED_GATEWAY_TOKEN"]);
  ...
  process.env.ENCLAWED_GATEWAY_PORT = String(gatewayPort);
  process.env.ENCLAWED_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("ENCLAWED_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createEnclawedTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
