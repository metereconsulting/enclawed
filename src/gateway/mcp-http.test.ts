  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-enclawed-account-id": "work",
        "x-enclawed-message-channel": "telegram",
      },
    ...
    const sendToolsList = async (senderIsOwner: "true" | "false") =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-enclawed-message-channel": "matrix",
          "x-enclawed-sender-is-owner": senderIsOwner,
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.enclawed?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.enclawed?.headers?.Authorization).toBe(
      "Bearer ${ENCLAWED_MCP_TOKEN}",
    );
    expect(config.mcpServers?.enclawed?.headers?.["x-enclawed-message-channel"]).toBe(
      "${ENCLAWED_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.enclawed?.headers?.["x-enclawed-sender-is-owner"]).toBe(
      "${ENCLAWED_MCP_SENDER_IS_OWNER}",
    );
  });
});
