...
import type { loadEnclawedPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureEnclawedModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: EnclawedConfig;
  pluginRegistry: ReturnType<typeof loadEnclawedPlugins>;
  ...
  if (!skipChannels) {
    ...
    params.logChannels.info(
      "skipping channel start (ENCLAWED_SKIP_CHANNELS=1 or ENCLAWED_SKIP_PROVIDERS=1)",
    );
  }
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: EnclawedConfig;
    pluginRegistry: ReturnType<typeof loadEnclawedPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
