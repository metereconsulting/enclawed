...
export function resolveGatewayReloadSettings(cfg: EnclawedConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: EnclawedConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: EnclawedConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: EnclawedConfig | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: EnclawedConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: EnclawedConfig) => {
