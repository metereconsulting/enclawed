...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "enclawed-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".enclawed", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["ENCLAWED_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "enclawed");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".enclawed");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "enclawed-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-wizard-home-"));
      process.env.HOME = tempHome;
      process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".enclawed", "enclawed.json");
      const bundledPluginsDir = path.join(tempHome, "enclawed-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.ENCLAWED_STATE_DIR = path.join(tempHome, ".enclawed");
      process.env.ENCLAWED_CONFIG_PATH = configPath;
      process.env.ENCLAWED_SKIP_CHANNELS = "1";
      ...
      process.env.ENCLAWED_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.ENCLAWED_SKIP_PROVIDERS = "1";
      process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.ENCLAWED_TEST_MINIMAL_GATEWAY = "1";
