...
const CODEX_HARNESS_LIVE = isTruthyEnvValue(process.env.ENCLAWED_LIVE_CODEX_HARNESS);
const CODEX_HARNESS_DEBUG = isTruthyEnvValue(process.env.ENCLAWED_LIVE_CODEX_HARNESS_DEBUG);
const CODEX_HARNESS_IMAGE_PROBE = isTruthyEnvValue(
  process.env.ENCLAWED_LIVE_CODEX_HARNESS_IMAGE_PROBE,
);
const CODEX_HARNESS_MCP_PROBE = isTruthyEnvValue(process.env.ENCLAWED_LIVE_CODEX_HARNESS_MCP_PROBE);
...
function snapshotEnv(): EnvSnapshot {
  return {
    agentRuntime: process.env.ENCLAWED_AGENT_RUNTIME,
    configPath: process.env.ENCLAWED_CONFIG_PATH,
    gatewayToken: process.env.ENCLAWED_GATEWAY_TOKEN,
    openaiApiKey: process.env.OPENAI_API_KEY,
    skipBrowserControl: process.env.ENCLAWED_SKIP_BROWSER_CONTROL_SERVER,
...
function restoreEnv(snapshot: EnvSnapshot): void {
  restoreEnvVar("ENCLAWED_AGENT_RUNTIME", snapshot.agentRuntime);
  restoreEnvVar("ENCLAWED_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("ENCLAWED_GATEWAY_TOKEN", snapshot.gatewayToken);
  restoreEnvVar("OPENAI_API_KEY", snapshot.openaiApiKey);
  restoreEnvVar("ENCLAWED_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("ENCLAWED_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("ENCLAWED_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("ENCLAWED_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("ENCLAWED_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("ENCLAWED_STATE_DIR", snapshot.stateDir);
}
...
describeLive("gateway live (Codex harness)", () => {
  it(
    ...
    async () => {
      const modelKey = process.env.ENCLAWED_LIVE_CODEX_HARNESS_MODEL ?? DEFAULT_CODEX_MODEL;
      ...
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-live-codex-harness-"));
      const stateDir = path.join(tempDir, "state");
      const workspace = await createLiveWorkspace(tempDir);
      const configPath = path.join(tempDir, "enclawed.json");
      const token = `test-${randomUUID()}`;
      const port = await getFreeGatewayPort();

      clearRuntimeConfigSnapshot();
      process.env.ENCLAWED_AGENT_RUNTIME = "codex";
      process.env.ENCLAWED_AGENT_HARNESS_FALLBACK = "none";
      process.env.ENCLAWED_CONFIG_PATH = configPath;
      process.env.ENCLAWED_GATEWAY_TOKEN = token;
      process.env.ENCLAWED_SKIP_BROWSER_CONTROL_SERVER = "1";
      ...
      try {
        ...
        const modelsText = await requestCodexCommandText({
          ...
          expectedText: [
            ...
            "Current Enclawed session status reports the active model as:",
          ],
        });
