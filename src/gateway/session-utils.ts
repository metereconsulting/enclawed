...
function listConfiguredAgentIds(cfg: EnclawedConfig): string[] {
...
export function listAgentsForGateway(cfg: EnclawedConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
export function loadCombinedSessionStoreForGateway(cfg: EnclawedConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: EnclawedConfig): GatewaySessionsDefaults {
