...
function gatewayTokenConfig(config: EnclawedConfig): EnclawedConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function asConfig(value: unknown): EnclawedConfig {
  return value as EnclawedConfig;
}

function buildSnapshot(config: EnclawedConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/enclawed-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: EnclawedConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.ENCLAWED_SKIP_CHANNELS;
  const previousSkipProviders = process.env.ENCLAWED_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.ENCLAWED_SKIP_CHANNELS;
    } else {
      process.env.ENCLAWED_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.ENCLAWED_SKIP_PROVIDERS;
    } else {
      process.env.ENCLAWED_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
