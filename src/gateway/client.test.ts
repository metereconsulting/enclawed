...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("enclawed doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with ENCLAWED_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with ENCLAWED_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ENCLAWED_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://enclawed-gateway.ai:18789",
      onConnectError,
    });
