...
export function withCachedMigrationConfigRuntime(
  ...
  const currentFn = configApi?.current;
  const mutateFn = configApi?.mutateConfigFile;
  if (!currentFn || !mutateFn) {
    return runtime;
  }
  const replaceFn = configApi?.replaceConfigFile;
  let cachedConfig: MigrationProviderContext["config"] | undefined;
  const current = (): ReturnType<typeof currentFn> => {
    cachedConfig ??= structuredClone(
      (currentFn() ?? fallbackConfig),
    );
    return cachedConfig;
  };
  return {
      ...
      ...(replaceFn
        ? {
            replaceConfigFile: async (params) => {
              const result = await replaceFn(params);
              cachedConfig = structuredClone(result.nextConfig);
              return result;
            },
          }
