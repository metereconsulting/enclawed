...
export type SingleProviderPluginCatalogOptions =
  | {
      ...
      /**
       * Optional offline-catalog factory used when no API key is configured.
       *
       * Single-provider plugins set this to expose a static fallback catalog
       * (matching the top-level `staticCatalog` slot on `ProviderPlugin`) so
       * setup/onboarding surfaces can still list models before credentials are
       * available.
       */
  ...
  return definePluginEntry({
    ...
    register(api) {
      ...
      if (provider) {
        ...
        let staticCatalog: ProviderPluginCatalog | undefined;
        if ("run" in provider.catalog) {
          ...
          const buildStaticProvider = provider.catalog.buildStaticProvider;
          if (buildStaticProvider) {
            staticCatalog = {
              order: "simple",
              run: async () => ({ provider: await buildStaticProvider() }),
            };
          }
        }
