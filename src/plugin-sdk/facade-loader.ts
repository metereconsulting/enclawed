...
let cachedEnclawedPackageRoot: string | undefined;
...
function getEnclawedPackageRoot() {
  if (cachedEnclawedPackageRoot) {
    return cachedEnclawedPackageRoot;
  }
  cachedEnclawedPackageRoot =
...
export function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      params.location.boundaryRoot === getEnclawedPackageRoot()
        ? "Enclawed package root"
        : (() => {
...
export async function loadBundledPluginPublicSurfaceModule<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === getEnclawedPackageRoot() ? "Enclawed package root" : "plugin root",
    rejectHardlinks: false,
  });
  if (!opened.ok) {
...
export function resetFacadeLoaderStateForTest(): void {
  ...
  cachedEnclawedPackageRoot = undefined;
}
