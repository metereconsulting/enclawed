...
type BuildProviderReplayFamilyHooksOptions =
  | {
      ...
      /**
       * Override tool-call-id sanitization for the OpenAI-compatible replay
       * policy. Some compatible transports require stable provider tool-call
       * ids preserved across multi-turn replay.
       */
    ...
    case "openai-compatible": {
      ...
      return {
        buildReplayPolicy: (ctx: ProviderReplayPolicyContext) =>
          buildOpenAICompatibleReplayPolicy(
            ctx.modelApi,
            sanitizeToolCallIds === undefined ? {} : { sanitizeToolCallIds },
          ),
      };
    }
