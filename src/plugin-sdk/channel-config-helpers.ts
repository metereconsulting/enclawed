...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`enclawed pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`enclawed pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends EnclawedConfig = EnclawedConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends EnclawedConfig = EnclawedConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends EnclawedConfig = EnclawedConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: EnclawedConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends EnclawedConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends EnclawedConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends EnclawedConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
...
>(params: {
  ...
  resolveAllowFrom: (account: ResolvedAccount) => Array<string | number> | null | undefined;
  /**
   * Optional cfg-aware access resolver. When provided, its `dmPolicy` /
   * `allowFrom` override the account-only `resolvePolicy` / `resolveAllowFrom`
   * results (used by channels that derive effective DM access from full config
   * inheritance rather than the resolved account alone).
   */
  resolveAccess?: (params: { cfg: EnclawedConfig; account: ResolvedAccount }) => {
  ...
  normalizeEntry?: (raw: string) => string;
  /**
   * When true, fall back to the default-account shared DM defaults when the
   * resolved account does not define its own DM policy/allowlist. Used by
   * channels (WhatsApp) that share defaults across the default account.
   */
  ...
  return ({
    ...
    const access = params.resolveAccess?.({ cfg, account });
...
export type { ChannelDmPolicy } from "../channels/plugins/dm-access.js";
