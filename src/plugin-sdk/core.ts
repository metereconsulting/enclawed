...
import type { EnclawedPluginApi } from "../plugins/types.js";
...
export type { EnclawedPluginToolContext, EnclawedPluginToolFactory } from "../plugins/types.js";
export type {
...
type DefineChannelPluginEntryOptions<TPlugin = ChannelPlugin> = {
  ...
  registerCliMetadata?: (api: EnclawedPluginApi) => void;
  registerFull?: (api: EnclawedPluginApi) => void;
};
...
type ChatChannelThreadingReplyModeOptions<TResolvedAccount> =
  ...
  | {
      scopedAccountReplyToMode: {
        resolveAccount: (cfg: EnclawedConfig, accountId?: string | null) => TResolvedAccount;
        resolveReplyToMode: (
...
import { resolveThreadSessionKeys as _resolveThreadSessionKeysInternal } from "../routing/session-key.js";
import { normalizeOptionalLowercaseString as _normalizeOptionalLowercaseString } from "../shared/string-coerce.js";
...
export function recoverCurrentThreadSessionId(params: {
  ...
  if (
    _normalizeOptionalLowercaseString(current.baseSessionKey) !==
    _normalizeOptionalLowercaseString(params.route.baseSessionKey)
  ) {
    return undefined;
  }
  const context = {
...
export function buildThreadAwareOutboundSessionRoute(params: {
  ...
  const threadKeys = _resolveThreadSessionKeysInternal({
