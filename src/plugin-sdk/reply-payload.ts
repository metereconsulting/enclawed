...
import { normalizeLowercaseStringOrEmpty as _normalizeLowercaseStringOrEmpty } from "../shared/string-coerce.js";

const REASONING_PREFIX = "reasoning:";

function _trimLeadingMarkdownQuoteMarkers(text: string): string {
...
/**
 * Returns true when the reply payload should be rendered as agent reasoning
 * (e.g. extended thinking / scratchpad output) rather than a direct user-facing
 * reply. Plugins use this gate to route reasoning frames to a hidden channel
 * surface and keep them out of the visible reply pipeline.
 */
export function isReasoningReplyPayload(payload: ReasoningReplyPayload): boolean {
  ...
  const normalized = _normalizeLowercaseStringOrEmpty(text.trimStart());
  if (normalized.startsWith(REASONING_PREFIX)) {
    return true;
  }
  return _normalizeLowercaseStringOrEmpty(_trimLeadingMarkdownQuoteMarkers(text)).startsWith(
    REASONING_PREFIX,
  );
}
