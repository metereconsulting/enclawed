...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  Enclawed  ", 20)).toBe("Enclawed");
    expect(coerceIdentityValue("  Enclawed  ", 8)).toBe("Enclawed");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  Enclawed Assistant  ", 8)).toBe("Enclawed");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  Enclawed  ", 0)).toBe("");
    expect(coerceIdentityValue("  Enclawed  ", -1)).toBe("Enclawe");
  });
});
