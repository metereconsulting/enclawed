...
function parseToolCallTagAt(
  text: string,
  start: number,
  tagNames: ReadonlySet<string> = TOOL_CALL_TAG_NAMES,
  ...
  if (!tagNames.has(tagName) || !isToolCallBoundary(text[cursor])) {
    return null;
  }
...
export function stripToolCallXmlTags(
  ...
  // `stripFunctionCallsXmlPayloads` (default true) requests that
  // `<function_call>` / `<function_calls>` XML payloads be stripped alongside
  // the canonical tool-call tags. The default tag set already includes those,
  // so callers that pass `true` get the standard behavior.
  const activeToolCallTagNames =
    options?.stripFunctionCallsXmlPayloads === false
      ? new Set(
          [...TOOL_CALL_TAG_NAMES].filter(
            (name) => name !== "function_call" && name !== "function_calls",
          ),
        )
      : TOOL_CALL_TAG_NAMES;
  ...
  for (let idx = 0; idx < text.length; idx += 1) {
    ...
    const tag = parseToolCallTagAt(text, idx, activeToolCallTagNames);
    if (!tag) {
      continue;
    }
