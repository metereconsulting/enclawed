...
function summarizeGroupPolicy(cfg: EnclawedConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: EnclawedConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: EnclawedConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: EnclawedConfig): boolean {
...
function isBrowserEnabled(cfg: EnclawedConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: EnclawedConfig): SecurityAuditFinding[] {
