...
import type { ConfigFileSnapshot, EnclawedConfig } from "../config/config.js";
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Enclawed state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Enclawed state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.ENCLAWED_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.ENCLAWED_GATEWAY_PASSWORD);
...
export function collectLoggingFindings(cfg: EnclawedConfig): SecurityAuditFinding[] {
...
export function collectElevatedFindings(cfg: EnclawedConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExecRuntimeFindings(cfg: EnclawedConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: EnclawedConfig): string[] {
