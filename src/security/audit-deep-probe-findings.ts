...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("enclawed status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("enclawed security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set ENCLAWED_GATEWAY_TOKEN/ENCLAWED_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("enclawed security audit --deep")}".`,
    });
  }
  return findings;
}
