...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = makeTempDir(tempRoots, "enclawed-install-sh-");
  const cliPath = path.join(root, "enclawed");
...
function resolveInstallerVersionCases(params: {
  ...
  const output = execFileSync(
    ...
    [
      "-c",
      `${versionHelperSource}
for enclawed_bin in "\${@:3}"; do
  ENCLAWED_BIN="$enclawed_bin"
  resolve_enclawed_version
done
(
  cd "$2"
  FAKE_ENCLAWED_BIN="\${@:1:1}" bash -s <<'ENCLAWED_STDIN_INSTALLER'
${versionHelperSource}
ENCLAWED_BIN="$FAKE_ENCLAWED_BIN"
resolve_enclawed_version
ENCLAWED_STDIN_INSTALLER
)`,
      "enclawed-version-test",
      params.stdinCliPath,
      params.stdinCwd,
      ...params.cliPaths,
    ],
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const decorated = withFakeCli("Enclawed 2026.3.10 (abcdef0)");
      const raw = withFakeCli("Enclawed dev's build");
      const stdinFixture = withFakeCli("Enclawed 2026.3.10 (abcdef0)");

      const hostileCwd = makeTempDir(tempRoots, "enclawed-install-stdin-");
...
`,
      ...
      ).toEqual(["2026.3.10", "Enclawed dev's build", "2026.3.10"]);
    },
  );
});
