...
let originalEnclawedLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.ENCLAWED_TEST_FILE_LOG;
  originalEnclawedLogLevel = process.env.ENCLAWED_LOG_LEVEL;
  delete process.env.ENCLAWED_TEST_FILE_LOG;
  delete process.env.ENCLAWED_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.ENCLAWED_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalEnclawedLogLevel === undefined) {
    delete process.env.ENCLAWED_LOG_LEVEL;
  } else {
    process.env.ENCLAWED_LOG_LEVEL = originalEnclawedLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
