...
import { redact as enclawedRedact } from "../enclawed/dlp-scanner.js";
...
export function createSubsystemLogger(subsystem: string): SubsystemLogger {
  ...
  const emitLog = (level: LogLevel, message: string, meta?: Record<string, unknown>) => {
    // enclawed: when classified mode is active, redact every log line through
    // the DLP scanner and tee to the hash-chained audit log. No-op otherwise.
    const enclawed = getEnclawedRuntime();
    if (enclawed) {
      message = enclawedRedact(message);
      if (meta && typeof meta === "object") {
        const redacted: Record<string, unknown> = {};
        for (const [k, v] of Object.entries(meta)) {
          redacted[k] = typeof v === "string" ? enclawedRedact(v) : v;
        }
        meta = redacted;
      }
      enclawed.audit
        .append({ type: `log.${level}`, actor: subsystem, level: null, payload: { message, meta } })
        .catch(() => {});
    }
