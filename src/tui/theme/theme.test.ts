...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      ENCLAWED_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when ENCLAWED_THEME=light", async () => {
    const mod = await importThemeWithEnv({ ENCLAWED_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when ENCLAWED_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ ENCLAWED_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats ENCLAWED_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ ENCLAWED_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("ENCLAWED_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      ENCLAWED_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ ENCLAWED_THEME: "light" });
    const darkMod = await importThemeWithEnv({ ENCLAWED_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.ENCLAWED_THEME = "light";
