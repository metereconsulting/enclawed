...
const createTempDir = () => tempDirs.make("enclawed-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets ENCLAWED_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "enclawed",
    });

    expect(env.ENCLAWED_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("enclawed");
  });

  it("overrides pre-existing ENCLAWED_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      ENCLAWED_SHELL: "wrong",
    });
    expect(env.ENCLAWED_SHELL).toBe("acp-client");
  });
  ...
  it("preserves ENCLAWED_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        ENCLAWED_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["ENCLAWED_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Enclawed bridge", () => {
    ...
    expect(env.ENCLAWED_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.ENCLAWED_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "enclawed",
        serverArgs: ["acp"],
        defaultServerCommand: "enclawed",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("ENCLAWED_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "enclawed", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "enclawed", "dist", "entry.js");
    const shimPath = path.join(dir, "enclawed.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\enclawed\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/enclawed-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/enclawed-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/enclawed-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
