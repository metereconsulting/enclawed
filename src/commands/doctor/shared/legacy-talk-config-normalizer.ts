...
export function normalizeLegacyTalkConfig(cfg: EnclawedConfig, changes: string[]): EnclawedConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as EnclawedConfig["talk"]) ?? {};
