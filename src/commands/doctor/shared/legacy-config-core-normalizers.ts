...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as EnclawedConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as EnclawedConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<EnclawedConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<EnclawedConfig["models"]>>;
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<EnclawedConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as EnclawedConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<EnclawedConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyMistralModelMaxTokens(
  ...
  return {
    ...
    models: {
      ...cfg.models,
      providers: nextProviders as NonNullable<EnclawedConfig["models"]>["providers"],
    },
  };
}
