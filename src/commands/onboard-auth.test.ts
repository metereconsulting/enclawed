...
vi.mock("../agents/agent-paths.js", () => ({
  resolveEnclawedAgentDir: () => process.env.ENCLAWED_AGENT_DIR ?? "/tmp/enclawed-agent",
}));

vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.ENCLAWED_STATE_DIR ?? "/tmp/enclawed-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.ENCLAWED_AGENT_DIR ?? "/tmp/enclawed-agent";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ENCLAWED_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("enclawed-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-oauth-sync-"));
    ...
    process.env.ENCLAWED_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-oauth-nosync-"));
    process.env.ENCLAWED_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ENCLAWED_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-oauth-external-"));
    process.env.ENCLAWED_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ENCLAWED_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("enclawed-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("enclawed-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to ENCLAWED_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("enclawed-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
