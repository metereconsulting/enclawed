...
vi.mock("../agents/agent-paths.js", () => ({
  resolveOpenClawAgentDir: () => process.env.ENCLAWED_AGENT_DIR ?? "/tmp/openclaw-agent",
}));

vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.ENCLAWED_STATE_DIR ?? "/tmp/openclaw-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.ENCLAWED_AGENT_DIR ?? "/tmp/openclaw-agent";
