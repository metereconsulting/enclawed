...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.ENCLAWED_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.ENCLAWED_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from enclawed.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<EnclawedConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Enclawed update wizard" : "Enclawed configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: EnclawedConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("enclawed doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.ENCLAWED_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.ENCLAWED_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
