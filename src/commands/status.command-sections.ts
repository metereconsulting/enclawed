...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("enclawed security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("enclawed security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.enclawed.ai/faq",
    "Troubleshooting: https://docs.enclawed.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("enclawed status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("enclawed logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("enclawed node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("enclawed status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("enclawed gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`enclawed devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("enclawed devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("enclawed devices list")}`),
  ];
}
