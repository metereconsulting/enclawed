...
beforeAll(() => {
  envSnapshot = captureEnv(["ENCLAWED_PROFILE"]);
  process.env.ENCLAWED_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByEnclawed: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/enclawed/pnpm-lock.yaml",
        markerPath: "/tmp/enclawed/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.ENCLAWED_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.enclawed.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.enclawed.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.ENCLAWED_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/enclawed-root.js", () => ({
  resolveEnclawedPackageRoot: vi.fn().mockResolvedValue("/tmp/enclawed"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("enclawed status --all") ||
          line.includes("enclawed --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("enclawed --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("ENCLAWED_GATEWAY_TOKEN", "abcd1234", async () => {
