...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.ENCLAWED_GATEWAY_TOKEN;
    prevPassword = process.env.ENCLAWED_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    if (prevToken === undefined) {
      ...
      process.env.ENCLAWED_GATEWAY_TOKEN = prevToken;
    }
    if (prevPassword === undefined) {
      delete process.env.ENCLAWED_GATEWAY_PASSWORD;
    } else {
      process.env.ENCLAWED_GATEWAY_PASSWORD = prevPassword;
    }
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".enclawed"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".enclawed", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as EnclawedConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.ENCLAWED_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as EnclawedConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "ENCLAWED_GATEWAY_TOKEN" },
        },
      },
    } as EnclawedConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as EnclawedConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as EnclawedConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as EnclawedConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("enclawed approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as EnclawedConfig);
    const message = lastMessage();
    expect(message).toContain("[secrets]");
    expect(message).toContain("failed to resolve account");
    expect(message).toContain("Run: enclawed security audit --deep");
  });
