...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("enclawed tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("enclawed tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: EnclawedConfig) {
