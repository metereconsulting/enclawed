...
const ensureEnclawedModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveEnclawedAgentDir = vi.fn().mockReturnValue("/tmp/enclawed-agent");
...
beforeEach(() => {
  ...
  ensureEnclawedModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureEnclawedModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureEnclawedModelsJson).toHaveBeenCalled();
    expect(ensureEnclawedModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
