...
import type { EnclawedConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: EnclawedConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: EnclawedConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Enclawed default (full)";
  const defaultRequestedAskSource = "Enclawed default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("enclawed approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: EnclawedConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: EnclawedConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("enclawed security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.enclawed/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("enclawed approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.enclawed.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("enclawed configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("enclawed config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("enclawed doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("enclawed config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('enclawed config set session.dmScope "per-channel-peer"') +
