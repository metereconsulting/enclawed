...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/enclawed-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = EnclawedConfig>(
  overrides: T = {} as T,
): EnclawedConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): EnclawedConfig {
