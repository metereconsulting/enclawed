...
export async function loadValidConfigOrThrow(): Promise<EnclawedConfig> {
...
export async function updateConfig(
  mutator: (cfg: EnclawedConfig) => EnclawedConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: EnclawedConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: EnclawedConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("enclawed agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
