...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveEnclawedAgentDir: vi.fn().mockReturnValue("/tmp/enclawed-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/enclawed-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/enclawed-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/enclawed-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/enclawed-dev/enclawed.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveEnclawedAgentDir: mocks.resolveEnclawedAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/enclawed-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveEnclawedAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/enclawed-dev/enclawed.json");
    expect(payload.auth.storePath).toBe("/tmp/enclawed-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/enclawed-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/enclawed-agent-custom");
