...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.ENCLAWED_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: EnclawedConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: EnclawedConfig) => {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: EnclawedConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: EnclawedConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.ENCLAWED_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<EnclawedConfig> {
