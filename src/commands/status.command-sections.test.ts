...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:enclawed security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:enclawed security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:enclawed status --all",
      "  Need to debug live? cmd:enclawed logs --follow",
      "  Fix reachability first: cmd:enclawed gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:enclawed devices approve req-123)",
      "muted(Fallback: cmd:enclawed devices approve --latest)",
      "muted(Inspect: cmd:enclawed devices list)",
    ]);
  });
