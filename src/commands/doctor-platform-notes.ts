...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".enclawed", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: EnclawedConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["ENCLAWED_GATEWAY_TOKEN", await getenv("ENCLAWED_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["ENCLAWED_GATEWAY_PASSWORD", await getenv("ENCLAWED_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "ENCLAWED_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.ENCLAWED_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- ENCLAWED_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
