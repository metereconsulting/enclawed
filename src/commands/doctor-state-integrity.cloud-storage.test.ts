...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("anchors cloud detection to OS homedir when ENCLAWED_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".openclaw");
    const originalOpenClawHome = process.env.ENCLAWED_HOME;
    process.env.ENCLAWED_HOME = "/tmp/openclaw-home-override";
    ...
    try {
      ...
      if (originalOpenClawHome === undefined) {
        ...
        process.env.ENCLAWED_HOME = originalOpenClawHome;
      }
    }
  });
