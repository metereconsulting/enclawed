...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Enclawed", ".enclawed");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/enclawed-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".enclawed");
  ...
  it("anchors cloud detection to OS homedir when ENCLAWED_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".enclawed");
    const originalEnclawedHome = process.env.ENCLAWED_HOME;
    process.env.ENCLAWED_HOME = "/tmp/enclawed-home-override";
