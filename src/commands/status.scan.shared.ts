...
export function hasExplicitMemorySearchConfig(cfg: EnclawedConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: EnclawedConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: EnclawedConfig, agentId: string) => { store: { path: string } } | null;
