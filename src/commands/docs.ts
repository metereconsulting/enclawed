...
const SEARCH_TOOL = "https://docs.enclawed.ai/mcp.SearchEnclawed";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.enclawed.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('enclawed docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.enclawed.ai/");
      runtime.log(`Search: ${formatCliCommand('enclawed docs "your query"')}`);
    }
    return;
  }
