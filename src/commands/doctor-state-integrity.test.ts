...
type EnvSnapshot = {
  HOME?: string;
  ENCLAWED_HOME?: string;
  ENCLAWED_STATE_DIR?: string;
  ENCLAWED_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ENCLAWED_HOME: process.env.ENCLAWED_HOME,
    ENCLAWED_STATE_DIR: process.env.ENCLAWED_STATE_DIR,
    ENCLAWED_OAUTH_DIR: process.env.ENCLAWED_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: EnclawedConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("ENCLAWED_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: EnclawedConfig) {
...
async function runStateIntegrityText(cfg: EnclawedConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.ENCLAWED_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when ENCLAWED_OAUTH_DIR is explicitly configured", async () => {
    process.env.ENCLAWED_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: EnclawedConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.ENCLAWED_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it("prints enclawed-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: EnclawedConfig = {};
    ...
    expect(text).toMatch(/enclawed sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/enclawed sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /enclawed sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
