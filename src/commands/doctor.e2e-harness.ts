...
export const resolveEnclawedPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadEnclawedPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../infra/enclawed-root.js", () => ({
  ...
  resolveEnclawedPackageRootSync: vi.fn(() => "/tmp/enclawed"),
}));
...
beforeEach(() => {
  ...
  resolveEnclawedPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ENCLAWED_STATE_DIR;
  originalUpdateInProgress = process.env.ENCLAWED_UPDATE_IN_PROGRESS;
  process.env.ENCLAWED_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-doctor-state-"));
