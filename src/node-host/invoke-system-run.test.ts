...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  let sharedEnclawedHome = "";
  let sharedRuntimeBinDir = "";
  let sharedFixtureId = 0;
  let previousEnclawedHome: string | undefined;
  const sharedRuntimeBins = new Set<string>();

  beforeAll(() => {
    sharedFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-node-host-fixtures-"));
    sharedEnclawedHome = path.join(sharedFixtureRoot, "enclawed-home");
    sharedRuntimeBinDir = path.join(sharedFixtureRoot, "bin");
    fs.mkdirSync(sharedEnclawedHome, { recursive: true });
  ...
  beforeEach(() => {
    previousEnclawedHome = process.env.ENCLAWED_HOME;
    process.env.ENCLAWED_HOME = sharedEnclawedHome;
  ...
  afterEach(() => {
    clearRuntimeConfigSnapshot();
    if (previousEnclawedHome === undefined) {
      delete process.env.ENCLAWED_HOME;
    } else {
      process.env.ENCLAWED_HOME = previousEnclawedHome;
    }
  });
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = sharedEnclawedHome;
    const previousEnclawedHome = process.env.ENCLAWED_HOME;
    process.env.ENCLAWED_HOME = tempHome;
  ...
  for (const testCase of approvedEnvShellWrapperCases) {
    it.runIf(process.platform !== "win32")(testCase.name, async () => {
      const tmp = createFixtureDir("enclawed-approved-wrapper-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "enclawed-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "enclawed-prepare-run-path-pin-",
        run: async ({ expected }) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "enclawed-allowlist-path-pin-",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = createFixtureDir("enclawed-approval-cwd-link-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = createFixtureDir("enclawed-approval-cwd-parent-link-");
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = createFixtureDir("enclawed-approval-cwd-real-");
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = createFixtureDir("enclawed-approval-cwd-drift-");
    const fallback = createFixtureDir("enclawed-approval-cwd-drift-alt-");
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("denies approval-based execution when a script operand changes after approval", async () => {
    const tmp = createFixtureDir("enclawed-approval-script-drift-");
  ...
  it("keeps approved shell script execution working when the script is unchanged", async () => {
    const tmp = createFixtureDir("enclawed-approval-script-stable-");
    const fixture = createMutableScriptOperandFixture(tmp);
    fs.writeFileSync(fixture.scriptPath, fixture.initialBody);
    if (process.platform !== "win32") {
      fs.chmodSync(fixture.scriptPath, 0o755);
  ...
  it("validates approved runtime script operand stability at dispatch", async () => {
    await withFakeRuntimeOnPath({
      runtime: "tsx",
      run: async () => {
        const tmp = createFixtureDir("enclawed-approval-tsx-script-drift-");
        ...
        const stableTmp = createFixtureDir("enclawed-approval-tsx-script-stable-");
  ...
  it("denies approval-based execution when tsx is missing a required mutable script binding", async () => {
    await withFakeRuntimeOnPath({
      runtime: "tsx",
      run: async () => {
        const tmp = createFixtureDir("enclawed-approval-tsx-missing-binding-");
        const fixture = createRuntimeScriptOperandFixture({ tmp, runtime: "tsx" });
        fs.writeFileSync(fixture.scriptPath, fixture.initialBody);
        try {
          const prepared = buildSystemRunApprovalPlan({
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `enclawed-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("denies semicolon-chained shell payloads in allowlist mode without explicit approval", async () => {
    const payloads = ["enclawed status; id", "enclawed status; cat /etc/passwd"];
  ...
  it("does not persist allow-always approvals for strict inline-eval carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("enclawed-inline-eval-bin-");
          for (const testCase of cases) {
  ...
  it("persists benign awk allow-always approvals in strict inline-eval mode without reopening inline carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("enclawed-inline-eval-awk-");
          try {
            const executablePath = createTempExecutable({
              dir: tempDir,
              name: "awk",
  ...
  it("does not persist allow-always approvals for strict inline-eval make carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("enclawed-inline-eval-make-");
          try {
            const executablePath = createTempExecutable({
              dir: tempDir,
              name: "make",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("enclawed-shell-wrapper-inner-");
      try {
  ...
  it("keeps cmd.exe transport wrappers approval-gated on Windows", async () => {
    ...
    const tempDir = createFixtureDir("enclawed-cmd-wrapper-allow-");
    try {
  ...
  it.each([
    ...
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const tempDir = createFixtureDir("enclawed-env-cmd-wrapper-allow-");
    try {
      const scriptPath = path.join(tempDir, "check_mail.cmd");
      fs.writeFileSync(scriptPath, "@echo off\r\necho ok\r\n");
  ...
  it("reuses exact-command durable trust for shell-wrapper reruns", async () => {
    ...
    const tempDir = createFixtureDir("enclawed-shell-wrapper-allow-");
    try {
      const prepared = buildSystemRunApprovalPlan({
