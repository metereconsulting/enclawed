...
import { z } from "zod";
import { createEnclawedChannelMcpServer, EnclawedChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createEnclawedChannelMcpServer({
...
describe("enclawed channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new EnclawedChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
