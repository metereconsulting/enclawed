...
vi.mock("./channel-bridge.js", () => ({
  EnclawedChannelBridge: class MockEnclawedChannelBridge {
...
describe("serveEnclawedChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveEnclawedChannelMcp } = await import("./channel-server.js");

    const servePromise = serveEnclawedChannelMcp({ verbose: false });
