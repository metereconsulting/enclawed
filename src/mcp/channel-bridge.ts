...
export class EnclawedChannelBridge {
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClient({
      ...
      clientDisplayName: "Enclawed MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`enclawed mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `enclawed mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
