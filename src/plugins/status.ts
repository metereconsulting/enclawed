...
import { normalizeEnclawedVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeEnclawedVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeEnclawedVersionBase(plugin.version) ??
    plugin.version
  );
}
