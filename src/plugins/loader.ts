...
import { discoverEnclawedPlugins } from "./discovery.js";
...
export type PluginLoadOptions = {
  ...
  /**
   * Optional pre-loaded manifest registry. When provided the loader skips
   * the discovery pass entirely and uses this registry directly. Used by
   * agent-tool-result-middleware loader and other scoped activations that
   * already resolved their owning plugin set.
   */
...
function createGuardedPluginRegistrationApi(api: EnclawedPluginApi): {
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<EnclawedPluginDefinition["register"]>>[0],
): void {
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadEnclawedPlugins(...)
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: EnclawedPluginDefinition;
  register?: EnclawedPluginDefinition["register"];
} {
  const resolved = unwrapDefaultModuleExport(moduleExport);
  if (typeof resolved === "function") {
    return {
      register: resolved as EnclawedPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as EnclawedPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.ENCLAWED_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverEnclawedPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverEnclawedPlugins>["candidates"][number];
  right: ReturnType<typeof discoverEnclawedPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadEnclawedPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadEnclawedPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      let mod: EnclawedPluginModule | null = null;
      try {
        ...
        mod = profilePluginLoaderSync({
          ...
          pluginId: record.id,
          source: safeSource,
          run: () => getJiti(safeSource)(safeImportSource) as EnclawedPluginModule,
        });
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: EnclawedPluginModule | null = null;
            try {
              runtimeMod = profilePluginLoaderSync({
                ...
                run: () =>
                  getJiti(safeRuntimeSource)(safeRuntimeImportSource) as EnclawedPluginModule,
              });
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run '${formatCliCommand("enclawed plugins list")}' for details.`,
        );
      }
    }
...
export async function loadEnclawedPluginCliRegistry(
  options: PluginLoadOptions = {},
