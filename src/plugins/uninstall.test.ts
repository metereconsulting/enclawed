...
type PluginConfig = NonNullable<EnclawedConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: EnclawedConfig["channels"];
}): EnclawedConfig {
...
function createSinglePluginWithEmptySlotsConfig(): EnclawedConfig {
...
function createSingleNpmInstallConfig(installPath: string): EnclawedConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "enclawed-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-enclawed-plugin-install/my-plugin",
      },
      extensionsDir,
    });
