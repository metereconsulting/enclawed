...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-enclawed",
    targetDir: params?.targetDir ?? "/tmp/opik-enclawed",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "enclawed-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/enclawed-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@enclawed/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["enclawed-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/enclawed-codex-app-server",
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-enclawed",
        spec: "@opik/opik-enclawed",
        integrity: "sha512-old",
        installPath: "/tmp/opik-enclawed",
      }),
      pluginIds: ["opik-enclawed"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-enclawed",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "enclawed-codex-app-server",
        spec: "enclawed-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/enclawed-codex-app-server",
      }),
      pluginIds: ["enclawed-codex-app-server"],
      specOverrides: {
        "enclawed-codex-app-server": "enclawed-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "enclawed-codex-app-server",
        targetDir: "/tmp/enclawed-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "enclawed-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @enclawed/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@enclawed/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @enclawed/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "enclawed-codex-app-server@beta",
        resolvedName: "enclawed-codex-app-server",
        resolvedSpec: "enclawed-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "enclawed-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "enclawed-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "enclawed-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "enclawed-codex-app-server",
      }),
      specOverrides: {
        "enclawed-codex-app-server": "enclawed-codex-app-server@beta",
      },
      expectedSpec: "enclawed-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "enclawed-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "enclawed-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@enclawed/voice-call",
      targetDir: "/tmp/enclawed-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@enclawed/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@enclawed/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@enclawed/voice-call");
    expect(result.config.plugins?.entries?.["@enclawed/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@enclawed/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@enclawed/voice-call",
      installPath: "/tmp/enclawed-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { ENCLAWED_HOME: "/srv/enclawed-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/enclawed-home";
    mockBundledSources(
      createBundledSource({
