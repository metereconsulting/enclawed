...
import type {
  EnclawedPluginHookOptions,
  EnclawedPluginToolContext,
  EnclawedPluginToolFactory,
  EnclawedPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type EnclawedPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<EnclawedConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
   * When true, the host clears existing `agents.defaults.models` keys before
   * applying `configPatch.agents.defaults.models`, instead of merging. Used by
   * providers that rewrite the agent model allowlist (for example switching
   * between provider-native ids and CLI-shim aliases).
   */
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Enclawed uses this to expose
 ...
 *
 * Runs after Enclawed resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Enclawed auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Enclawed wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Enclawed builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Enclawed no
...
/**
 * Provider-owned stream wrapper hook after Enclawed applies its generic
 ...
 *
 * Return `true` or `false` to override Enclawed's built-in provider cache TTL
 ...
 *
 * Runs only after Enclawed exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Enclawed should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Enclawed loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Enclawed falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
  ...
  /**
   * Static fallback catalog used when the live `catalog` returns null or fails.
   *
   * Use this when a provider wants a built-in offline catalog snapshot to back
   * up live discovery — for example before OAuth credentials are available.
   */
  staticCatalog?: ProviderPluginCatalog;
  /**
   * Provider-owned thinking-policy profile.
   *
   * Use this to expose the available reasoning ladder for a given provider/model
   * pair. Consumed by /think autocomplete and provider-policy surfaces.
   */
  resolveThinkingProfile?: (
    ctx: ProviderThinkingPolicyContext,
   ...
   *
   * Enclawed calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Enclawed performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Enclawed registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Enclawed wrappers.
   *
   ...
   *
   * Called after Enclawed resolves the raw configured credential but before the
   ...
   * hint than Enclawed's generic auth-store guidance.
   */
   ...
   * Enclawed's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Enclawed should surface a provider-specific hint for
   ...
   * merging. Enclawed deduplicates by `provider/id`, so plugins only need to
   ...
   * without replacing the full Enclawed-owned system prompt.
   */
   ...
   * rewrites after Enclawed has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Enclawed handles
   ...
   *
   * Enclawed uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `enclawed doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `enclawed doctor` should offer.
   *
   ...
   *
   * Enclawed calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Enclawed's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Enclawed falls back to that stored profile.
   */
...
export type SpeechProviderPlugin = {
  ...
  /**
   * Optional pre-synthesis hook for providers that need to inject persona
   * instructions or rewrite the prompt text before the synthesize call.
   */
  prepareSynthesis?: (
...
export type RealtimeVoiceProviderPlugin = {
  ...
  /**
   * Optional browser-facing realtime session factory. Providers that support
   * client-side WebRTC/WebSocket sessions implement this to return a
   * provider-shaped browser session descriptor.
   */
...
export type EnclawedPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  /** Whether the sender is an owner (required for owner-only commands). */
  ...
  sessionId?: string;
  /** Transcript file for the active Enclawed session when available. */
  sessionFile?: string;
  /**
   * Diagnostics command flow flags. Set by diagnostics-capable command surfaces
   * to indicate preview/upload approval state and the routing context the
   * diagnostics output should respect.
   */
  diagnosticsPreviewOnly?: boolean;
  diagnosticsUploadApproved?: boolean;
  diagnosticsPrivateRouted?: boolean;
  /**
   * Additional candidate sessions the diagnostics command should inspect when
   * resolving runtime threads across multiple bound conversations.
   */
  diagnosticsSessions?: Array<{
    sessionKey?: string;
    sessionId?: string;
    sessionFile?: string;
    channel?: string;
  ...
  commandBody: string;
  /** Current Enclawed configuration */
  config: EnclawedConfig;
 ...
 */
export type EnclawedPluginCommandDefinition = {
  ...
  description: string;
  /**
   * Command-name ownership policy. `"reserved"` indicates the plugin owns the
   * command namespace and core should not let another surface shadow it.
   */
  ownership?: "reserved";
  /**
   * Optional guidance lines surfaced to the agent's system prompt so the model
   * knows when to prefer this plugin command over alternative flows.
   */
...
export type EnclawedPluginHttpRouteAuth = "gateway" | "plugin";
export type EnclawedPluginHttpRouteMatch = "exact" | "prefix";
export type EnclawedPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type EnclawedPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type EnclawedPluginHttpRouteParams = {
  path: string;
...
export type EnclawedPluginCliContext = {
  program: Command;
  config: EnclawedConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type EnclawedPluginCliRegistrar = (ctx: EnclawedPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Enclawed to keep a plugin command lazy-loaded while still
 ...
 */
export type EnclawedPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type EnclawedPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type EnclawedPluginNodeHostCommand = {
  command: string;
  cap?: string;
  /**
   * Marks the command as security-sensitive. Dangerous node-host commands
   * require explicit operator approval / capability grant before the host will
   * dispatch them to a remote node.
   */
  dangerous?: boolean;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type EnclawedPluginSecurityAuditContext = {
  config: EnclawedConfig;
...
export type EnclawedPluginSecurityAuditCollector = (
  ctx: EnclawedPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

/**
 * Metadata accompanying a diagnostic event delivered to internal-diagnostics
 * subscribers.
 *
 * `trusted` is true when the event originated from in-process Enclawed code
 * paths. External/forwarded events are marked `trusted: false` so plugin
 * exporters can decide whether to emit them.
 */
...
/**
 * Privileged diagnostics capability surface exposed only to bundled
 * service-grade plugins (for example diagnostics exporters).
 *
 * Plugins receive this via `EnclawedPluginServiceContext.internalDiagnostics`
 * when the host opts in. The capability is intentionally narrow: plugins can
 * subscribe to typed event payloads and emit their own typed events.
 */
export type EnclawedPluginInternalDiagnostics = {
  onEvent: (
...
export type EnclawedPluginServiceContext = {
  config: EnclawedConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
  /**
   * Optional internal-diagnostics capability for service plugins that export
   * Enclawed diagnostic events (Prometheus, OTEL, etc.). Undefined when the
   * host has not granted the capability.
   */
  internalDiagnostics?: EnclawedPluginInternalDiagnostics;
};

/** Background service registered by a plugin during `register(api)`. */
export type EnclawedPluginService = {
  id: string;
  start: (ctx: EnclawedPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: EnclawedPluginServiceContext) => void | Promise<void>;
};

/** Context passed to a gateway-discovery advertise hook. */
export type EnclawedGatewayDiscoveryAdvertiseContext = {
...
/**
 * Gateway discovery service registered by a plugin during `register(api)`.
 *
 * The `advertise` hook is invoked at gateway-startup time with the current
 * gateway context. Implementations may run an mDNS/Bonjour advertiser, a
 * Tailscale service registration, or any other lookup-side-channel; the
 * returned `stop` function (if any) is called on gateway shutdown.
 */
export type EnclawedGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: EnclawedGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

// --- Node-invoke policy surface (used by file-transfer + paired nodes) -----

export type EnclawedPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type EnclawedPluginNodeInvokeApprovalDecision =
  | "allow-once"
  | "allow-always"
  | "deny";

export type EnclawedPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: EnclawedPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type EnclawedPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: EnclawedPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<EnclawedPluginNodeInvokeTransportResult>;
};

export type EnclawedPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type EnclawedPluginNodeInvokePolicy = {
  commands: string[];
  handle: (
    ctx: EnclawedPluginNodeInvokePolicyContext,
  ) => Promise<EnclawedPluginNodeInvokePolicyResult> | EnclawedPluginNodeInvokePolicyResult;
};

export type EnclawedPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type EnclawedPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: EnclawedPluginReloadRegistration;
  nodeHostCommands?: EnclawedPluginNodeHostCommand[];
  securityAuditCollectors?: EnclawedPluginSecurityAuditCollector[];
  register?: (api: EnclawedPluginApi) => void;
  activate?: (api: EnclawedPluginApi) => void;
};

export type EnclawedPluginModule = EnclawedPluginDefinition | ((api: EnclawedPluginApi) => void);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

export type PluginConfigMigration = (config: EnclawedConfig) =>
  | {
      config: EnclawedConfig;
      changes: string[];
    }
...
export type EnclawedPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | EnclawedPluginToolFactory,
    opts?: EnclawedPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: EnclawedPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: EnclawedPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: EnclawedPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: EnclawedPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Enclawed can keep
  ...
  registerReload: (registration: EnclawedPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: EnclawedPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: EnclawedPluginSecurityAuditCollector) => void;
  registerService: (service: EnclawedPluginService) => void;
  /**
   * Register a gateway discovery service (e.g. Bonjour/mDNS advertiser).
   * Called by the gateway at startup with the current gateway context.
   */
  registerGatewayDiscoveryService: (service: EnclawedGatewayDiscoveryService) => void;
  /** Register a migration provider (consumed by `enclawed migrate`). */
  registerMigrationProvider: (provider: MigrationProviderPlugin) => void;
  /** Register a node-invoke policy used by paired-node transport handlers. */
  registerNodeInvokePolicy: (policy: EnclawedPluginNodeInvokePolicy) => void;
   ...
   */
  registerCommand: (command: EnclawedPluginCommandDefinition) => void;
  ...
  /**
   * Register an agent-tool-result-middleware handler that observes/rewrites
   * tool results as they flow back to the active harness (pi/codex). Plugins
   * declare which runtimes the handler applies to via the options bag; the
   * default is all known runtimes.
   */
  registerAgentToolResultMiddleware: (
    handler: import("./agent-tool-result-middleware-types.js").AgentToolResultMiddleware,
    opts?: import("./agent-tool-result-middleware-types.js").AgentToolResultMiddlewareOptions,
  ) => void;
...
// Migration plugin types — used by plugin-sdk/migration.ts + migration-runtime.ts
// Ported from upstream Enclawed so the migration-provider subpaths resolve in oss.
