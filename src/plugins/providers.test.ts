...
type LoadEnclawedPlugins = typeof import("./loader.js").loadEnclawedPlugins;
...
const loadEnclawedPluginsMock = vi.fn<LoadEnclawedPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/enclawed.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadEnclawedPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: EnclawedConfig = {
    plugins: {},
  };
  const autoEnabledConfig: EnclawedConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadEnclawedPlugins: (...args: Parameters<LoadEnclawedPlugins>) =>
        loadEnclawedPluginsMock(...args),
  ...
  beforeEach(() => {
    ...
    loadEnclawedPluginsMock.mockReturnValue(registry);
