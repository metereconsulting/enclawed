...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_ENCLAWED_EXTENSIONS: "missing_openclaw_extensions",
  EMPTY_ENCLAWED_EXTENSIONS: "empty_openclaw_extensions",
...
async function installPluginFromPackageDir(
  ...
  if (!minHostVersionCheck.ok) {
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires OpenClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set ENCLAWED_VERSION and retry.`,
