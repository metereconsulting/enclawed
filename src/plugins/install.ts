...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing enclawed.extensions; update the plugin package to include enclawed.extensions (for example ["./dist/index.js"]). See https://docs.enclawed.ai/help/troubleshooting#plugin-install-fails-with-missing-enclawed-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "enclawed.plugin.json",
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_ENCLAWED_EXTENSIONS: "missing_enclawed_extensions",
  EMPTY_ENCLAWED_EXTENSIONS: "empty_enclawed_extensions",
...
function ensureEnclawedExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_ENCLAWED_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json enclawed.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_ENCLAWED_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "${formatCliCommand("enclawed security audit --deep")}" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureEnclawedExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureEnclawedExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from enclawed.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-enclawed"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json enclawed.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires enclawed >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set ENCLAWED_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires enclawed >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade enclawed and retry.`,
...
export async function installPluginFromNpmSpec(
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "enclawed-npm-pack-",
    spec,
    timeoutMs,
