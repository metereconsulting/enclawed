...
function createInstalledRuntimePluginDir(
  ...
  const bundledDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `enclawed-runtime-contract-bundled-${pluginId}-`),
  );
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `enclawed-runtime-contract-state-${pluginId}-`),
  );
  ...
  fs.writeFileSync(
    path.join(pluginRoot, "enclawed.plugin.json"),
    JSON.stringify({
      id: pluginId,
      channels: [pluginId],
