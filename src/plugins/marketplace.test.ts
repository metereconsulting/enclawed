...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("enclawed-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("enclawed-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("enclawed-marketplace-test-", async (homeDir) => {
      const enclawedHome = path.join(homeDir, "enclawed-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, ENCLAWED_HOME: enclawedHome },
