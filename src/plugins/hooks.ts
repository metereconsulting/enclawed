...
export function createHookRunner(
   ...
   * Allows plugins to influence finalize/revise decisions before an agent run
   * commits its last turn. Runs sequentially; first non-"continue" action wins.
   */
  async function runBeforeAgentFinalize(
    event: PluginHookBeforeAgentFinalizeEvent,
    ctx: PluginHookAgentContext,
  ): Promise<PluginHookBeforeAgentFinalizeResult | undefined> {
    const hooks = getHooksForName(registry, "before_agent_finalize");
    if (hooks.length === 0) {
      return undefined;
    }
    logger?.debug?.(
      `[hooks] running before_agent_finalize (${hooks.length} handlers, first-decision wins)`,
    );
    for (const hook of hooks) {
      try {
        const handlerResult = await (
        ...
        if (handlerResult && handlerResult.action && handlerResult.action !== "continue") {
          return handlerResult;
        }
      } catch (err) {
        handleHookError({
