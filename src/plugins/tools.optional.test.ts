...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadEnclawedPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { ENCLAWED_HOME: "/srv/enclawed-home" },
      },
    },
    {
