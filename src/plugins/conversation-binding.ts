...
const APPROVALS_PATH = "~/.enclawed/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "enclawed-app-server:thread:",
  "enclawed-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("enclawed.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("enclawed.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Enclawed instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
