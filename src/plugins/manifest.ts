...
/**
 * Canonical plugin manifest filename. New plugins MUST write this name.
 *
 * The legacy `openclaw.plugin.json` filename is still accepted by the
 * discovery layer (see PLUGIN_MANIFEST_FILENAMES) so existing installs that
 * still ship the old name keep working during the transition. New code should
 * always read/write `PLUGIN_MANIFEST_FILENAME`.
 */
export const PLUGIN_MANIFEST_FILENAME = "enclawed.plugin.json";
/** Legacy filename still readable for backward compatibility. */
export const LEGACY_PLUGIN_MANIFEST_FILENAME = "openclaw.plugin.json";
/**
 * Discovery candidates in resolution order. The new filename wins on tie so
 * plugins that migrated keep using the new name; legacy installs that only
 * ship `openclaw.plugin.json` continue to be found.
 */
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `enclawed qa`, for example `matrix`. */
  commandName: string;
...
export type PluginManifestContracts = {
  ...
  tools?: string[];
  /**
   * Names of agent-tool-result-middleware runtimes this plugin contributes
   * handlers for ("pi", "codex"). Used by the middleware loader to scope
   * activation of bundled plugins that opt into the contract.
   */
...
// package.json "enclawed" metadata (used for setup/catalog)
...
export type EnclawedPackageStartup = {
  /**
...
export type EnclawedPackageSetupFeatures = {
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type EnclawedPackageManifest = {
  extensions?: string[];
  setupEntry?: string;
  setupFeatures?: EnclawedPackageSetupFeatures;
  channel?: PluginPackageChannel;
  install?: PluginPackageInstall;
  startup?: EnclawedPackageStartup;
};
...
export type PackageManifest = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, EnclawedPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): EnclawedPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
