...
export type MemoryEmbeddingProviderCreateOptions = {
  ...
  taskType?:
    ...
    | "FACT_VERIFICATION";
  /**
   * Provider-specific input-type hint forwarded to embedding APIs that
   * distinguish query vs passage/document inputs (for example Voyage/Cohere
   * `input_type`).
   */
  inputType?: string;
  /** Override input-type hint used specifically for query embeddings. */
  queryInputType?: string;
  /** Override input-type hint used specifically for document/passage embeddings. */
