...
const mocks = vi.hoisted(() => ({
  ...
  loadEnclawedPluginCliRegistry: vi.fn(),
  loadEnclawedPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadEnclawedPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadEnclawedPluginCliRegistry(...args),
  loadEnclawedPlugins: (...args: unknown[]) => mocks.loadEnclawedPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  ...
  autoEnabledConfig: EnclawedConfig;
  ...
  expect(mocks.loadEnclawedPlugins).toHaveBeenCalledWith(
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadEnclawedPluginCliRegistry.mockReset();
    mocks.loadEnclawedPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadEnclawedPlugins.mockReset();
    mocks.loadEnclawedPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as EnclawedConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as EnclawedConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as EnclawedConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadEnclawedPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadEnclawedPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadEnclawedPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("lazy-registers descriptor-backed plugin commands on first invocation", async () => {
    ...
    await registerPluginCliCommands(program, {} as EnclawedConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as EnclawedConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as EnclawedConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
  ...
  it("skips plugin CLI registration from validated config when the snapshot is invalid", async () => {
    ...
    expect(mocks.loadEnclawedPlugins).not.toHaveBeenCalled();
  });
});
