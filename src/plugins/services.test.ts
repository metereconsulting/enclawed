...
import type { EnclawedPluginService, EnclawedPluginServiceContext } from "./types.js";
...
function createRegistry(services: EnclawedPluginService[]) {
...
function expectServiceContext(
  ctx: EnclawedPluginServiceContext,
...
function expectServiceLogger(ctx: EnclawedPluginServiceContext) {
...
function expectServiceContexts(
  contexts: EnclawedPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: EnclawedPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: EnclawedPluginServiceContext[] = [];
