...
const mocks = vi.hoisted(() => ({
  loadEnclawedPlugins: vi.fn<typeof import("../loader.js").loadEnclawedPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadEnclawedPlugins: (...args: Parameters<typeof mocks.loadEnclawedPlugins>) =>
    mocks.loadEnclawedPlugins(...args),
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadEnclawedPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadEnclawedPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadEnclawedPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
});
