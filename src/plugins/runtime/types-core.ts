...
export type PluginRuntimeCore = {
  version: string;
  config: {
    loadConfig: () => import("../../config/types.enclawed.js").EnclawedConfig;
    writeConfigFile: (
      cfg: import("../../config/types.enclawed.js").EnclawedConfig,
      options?: RuntimeWriteConfigOptions,
    ) => Promise<void>;
    /**
     * Optional snapshot accessor. Migration providers that need a stable
     * pre-mutation config view can supply this; the default narrow runtime
     * factory does not populate it.
     */
    current?: () =>
      | import("../../config/types.enclawed.js").EnclawedConfig
      | undefined;
    /**
     * Optional config-mutation entrypoint used by migration providers that
     * round-trip drafts through `src/config/mutate.ts`. The default narrow
     * runtime factory does not populate this.
     */
    mutateConfigFile?: typeof import("../../config/mutate.js").mutateConfigFile;
    /**
     * Optional whole-file replacement entrypoint used by migration providers.
     * The default narrow runtime factory does not populate this.
     */
    replaceConfigFile?: typeof import("../../config/mutate.js").replaceConfigFile;
  };
  agent: {
    defaults: {
  ...
  state: {
    ...
    /**
     * Optional persistent keyed-store factory for plugins that need durable,
     * namespaced, TTL-bounded key/value storage (for example Discord component
     * registries). Undefined when the host has not wired a backing store —
     * callers must check for presence and degrade gracefully.
     */
  ...
  tasks: {
    ...
    /**
     * Session-bound managed TaskFlow access used by plugins that bind a flow to
     * a host session (for example webhook-triggered flows). Provides
     * `bindSession(...)` to obtain a session-scoped flow runtime.
     */
