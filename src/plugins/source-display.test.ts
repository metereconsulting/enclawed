...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "enclawed", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".enclawed", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".enclawed", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "enclawed-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".enclawed", "extensions"),
      },
    });
  });
});
