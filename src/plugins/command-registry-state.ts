...
export type RegisteredPluginCommand = EnclawedPluginCommandDefinition & {
  pluginId: string;
  pluginName?: string;
  pluginRoot?: string;
};
...
const PLUGIN_COMMAND_STATE_KEY = Symbol.for("enclawed.pluginCommandsState");
...
export function getPluginCommandSpecs(
  provider?: string,
  _options?: { config?: import("../config/types.enclawed.js").EnclawedConfig },
): Array<{
  name: string;
  description: string;
  acceptsArgs: boolean;
}> {
  // `_options.config` is accepted for caller ergonomics; native command spec
  // resolution is config-independent (driven by registered plugin metadata).
  void _options;
  const providerName = normalizeOptionalLowercaseString(provider);
  if (
    providerName &&
