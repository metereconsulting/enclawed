...
import { discoverEnclawedPlugins, type PluginCandidate } from "./discovery.js";
import type { PluginManifestCommandAlias } from "./manifest-command-aliases.js";
import { admitPluginCandidate } from "../enclawed/integration/plugin-admit.js";
import {
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ENCLAWED_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.ENCLAWED_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  channelConfigs?: Record<string, PluginManifestChannelConfig>;
  packageChannel?: EnclawedPackageManifest["channel"];
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "enclawed",
...
export function loadPluginManifestRegistry(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "enclawed") === "bundle";
    const manifestRes:
    ...
    // Enclawed host-level admission gate (paper §1.3). Run before any
    // further plugin processing so that in enclaved flavor an unsigned or
    // untrusted plugin is rejected before its activation, command set,
    // capabilities, or runtime barrels touch the registry. In open flavor
    // this is warn-only.
    const admission = admitPluginCandidate({ pluginId: manifest.id });
    if (!admission.admit) {
      diagnostics.push({
        level: "error",
        pluginId: manifest.id,
        source: manifestRes.manifestPath,
        message: `enclawed gate: ${admission.reason}`,
      });
      continue;
    }
        ...
        message:
          minHostVersionCheck.kind === "invalid"
            ...
            : minHostVersionCheck.kind === "unknown_host_version"
              ? `plugin requires enclawed >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load`
              : `plugin requires enclawed >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`,
      });
      continue;
    }
