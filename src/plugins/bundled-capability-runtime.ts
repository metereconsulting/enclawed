...
import type { EnclawedPluginDefinition, EnclawedPluginModule } from "./types.js";
 ...
 */
export const CAPABILITY_VITEST_SHIM_ALIASES = [
  {
    subpath: "llm-task",
...
export function buildVitestCapabilityShimAliasMap(): Record<string, string> {
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        [`@enclawed/plugin-sdk/${subpath}`, targetPath],
        [`@enclawed/plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    ["@enclawed/plugin-sdk"]: _ignoredLegacyRootAlias,
    ["@enclawed/plugin-sdk"]: _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as EnclawedPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getJiti(safeSource)(safeSource) as EnclawedPluginModule;
    } catch (error) {
