...
import { resolveEnclawedPackageRootSync } from "../infra/enclawed-root.js";
...
function hasTrustedEnclawedRootIndicator(params: {
  packageRoot: string;
  ...
  const hasEnclawedBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("enclawed")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.enclawed === "string");
  const hasEnclawedEntrypoint = fs.existsSync(path.join(params.packageRoot, "enclawed.mjs"));
  return hasCliEntryExport || hasEnclawedBin || hasEnclawedEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedEnclawedRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedEnclawedRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedEnclawedRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveEnclawedPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedEnclawedRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["@enclawed/plugin-sdk", "@enclawed/plugin-sdk"] as const;
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.ENCLAWED_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `@enclawed/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias
      ? { "@enclawed/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.enclawedPackageRoot, "extensions"),
    path.join(params.enclawedPackageRoot, "dist", "extensions"),
    path.join(params.enclawedPackageRoot, "dist-runtime", "extensions"),
