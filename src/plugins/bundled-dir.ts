...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "enclawed-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.ENCLAWED_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "enclawed.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.ENCLAWED_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveEnclawedPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveEnclawedPackageRootSync({ argv1: process.argv[1] }),
      resolveEnclawedPackageRootSync({ cwd: process.cwd() }),
      resolveEnclawedPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
