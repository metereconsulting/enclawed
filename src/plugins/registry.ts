...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("enclawed.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: EnclawedPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: EnclawedPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: EnclawedPluginHookOptions | undefined,
  ...
  const registerHttpRoute = (record: PluginRecord, params: EnclawedPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: EnclawedPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as EnclawedPluginChannelRegistration).plugin === "object"
        ? (registration as EnclawedPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: EnclawedPluginCliCommandDescriptor[] },
  ) => {
  ...
  const registerReload = (record: PluginRecord, registration: EnclawedPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: EnclawedPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: EnclawedPluginNodeHostCommand,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: EnclawedPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    ...
    if (existing) {
      // Idempotent: same-plugin re-registration silently dedupes (see registerService).
      if (existing.pluginId === record.id) {
        return;
      }
      pushDiagnostic({
    ...
    if (!record.gatewayDiscoveryServiceIds) {
      record.gatewayDiscoveryServiceIds = [];
    }
  ...
  const registerMigrationProvider = (
    ...
    if (existing) {
      // Idempotent: same-plugin re-registration silently dedupes (mirrors
      // registerService / registerGatewayDiscoveryService).
      if (existing.pluginId === record.id) {
        return;
      }
      pushDiagnostic({
  ...
  const registerNodeInvokePolicy = (
    ...
    // Same-plugin re-registration silently dedupes by checking if the
    // existing entry has identical commands. Cross-plugin overlap on any
    // single command surfaces as an error diagnostic.
    const existing = bucket.find((entry) => entry.pluginId === record.id);
    if (existing) {
      const sameCommands =
        existing.policy.commands.length === commands.length &&
        existing.policy.commands.every((cmd, i) => cmd === commands[i]);
      if (sameCommands) {
        return;
      }
    }
    for (const cmd of commands) {
      const conflict = bucket.find(
        (entry) => entry.pluginId !== record.id && entry.policy.commands.includes(cmd),
      );
      if (conflict) {
        pushDiagnostic({
          level: "error",
  ...
  const registerCommand = (record: PluginRecord, command: EnclawedPluginCommandDefinition) => {
        ...
        ...(registrationMode === "full"
          ? {
              ...
              registerMigrationProvider: (provider) =>
                registerMigrationProvider(record, provider),
              ...
              registerCompactionProvider: (
                provider: Parameters<EnclawedPluginApi["registerCompactionProvider"]>[0],
              ) => {
              ...
              registerMemoryEmbeddingProvider: (adapter) => {
                ...
                if (existing) {
                  // Idempotent: same-plugin re-registration silently dedupes,
                  // matching registerService's behaviour (see #62033). The same
                  // plugin can hit this path twice across snapshot vs activating
                  // loads, and during the activate pass the previous snapshot's
                  // global registration is still present.
