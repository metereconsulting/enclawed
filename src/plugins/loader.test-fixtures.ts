...
import { clearPluginLoaderCache, loadEnclawedPlugins } from "./loader.js";
import { clearPluginManifestRegistryCache } from "./manifest-registry.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadEnclawedPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadEnclawedPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "enclawed-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.ENCLAWED_BUNDLED_PLUGINS_DIR;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "enclawed.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".enclawed", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ ENCLAWED_STATE_DIR: stateDir, ...params.env }, () =>
    loadEnclawedPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
}
