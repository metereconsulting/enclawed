...
function collectConfiguredPluginEntryIds(config: EnclawedConfig): string[] {
...
function resolveRegister(mod: EnclawedPluginModule): {
...
export function resolvePluginSetupRegistry(params?: {
  ...
  for (const record of manifestRegistry.plugins) {
    ...
    let mod: EnclawedPluginModule;
    try {
      mod = getJiti(setupSource)(setupSource) as EnclawedPluginModule;
    } catch {
      continue;
    }

    const resolved = resolveRegister((mod as { default?: EnclawedPluginModule }).default ?? mod);
