...
vi.mock("./enclawed-root.js", () => ({
  resolveEnclawedPackageRoot: vi.fn(async () => null),
  resolveEnclawedPackageRootSync: vi.fn(() => null),
}));
...
let enclawedRoot: typeof import("./enclawed-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    enclawedRoot = await import("./enclawed-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/enclawed");
    const wrapperArgv1 = abs("fixtures/bin/enclawed");
  ...
  it("uses resolveEnclawedPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/enclawed");
    (
      enclawedRoot.resolveEnclawedPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/enclawed"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "enclawed" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "enclawed.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-enclawed");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/enclawed-bundle");
    (
      enclawedRoot.resolveEnclawedPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/Enclawed.app/Contents/MacOS/Enclawed");
    const bundledUiDir = abs("fixtures/Enclawed.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/enclawed-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
