...
vi.mock("./enclawed-root.js", async () => {
  const actual = await vi.importActual<typeof import("./enclawed-root.js")>("./enclawed-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "enclawed-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ENCLAWED_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ENCLAWED_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveEnclawedPackageRoot } = await import("./enclawed-root.js"));
    ...
    vi.mocked(resolveEnclawedPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveEnclawedPackageRoot).mockResolvedValue("/opt/enclawed");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/enclawed/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
