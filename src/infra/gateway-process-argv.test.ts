...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\Enclawed\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/enclawed/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/enclawed/enclawed.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/enclawed/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/enclawed/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the enclawed executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\enclawed.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/enclawed-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/enclawed-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\enclawed-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/enclawed/custom.js", "gateway"])).toBe(false);
