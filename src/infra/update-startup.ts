...
import { resolveEnclawedPackageRoot } from "./enclawed-root.js";
...
function resolveAutoUpdatePolicy(cfg: EnclawedConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: EnclawedConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveEnclawedPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("enclawed update")}`,
      );
