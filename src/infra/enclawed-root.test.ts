...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__enclawed_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "enclawed-root");
...
function setPackageRoot(root: string, name = "enclawed") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./enclawed-root.js").resolveEnclawedPackageRootSync,
  asyncResolver: typeof import("./enclawed-root.js").resolveEnclawedPackageRoot,
  opts: Parameters<typeof import("./enclawed-root.js").resolveEnclawedPackageRootSync>[0],
...
vi.mock("./enclawed-root.fs.runtime.js", () => ({
  enclawedRootFsSync: mockFsModule(),
  enclawedRootFs: mockFsPromisesModule(),
}));

describe("resolveEnclawedPackageRoot", () => {
  let resolveEnclawedPackageRoot: typeof import("./enclawed-root.js").resolveEnclawedPackageRoot;
  let resolveEnclawedPackageRootSync: typeof import("./enclawed-root.js").resolveEnclawedPackageRootSync;
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ resolveEnclawedPackageRoot, resolveEnclawedPackageRootSync } =
      await import("./enclawed-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "enclawed");
        const pkgRoot = path.join(project, "node_modules", "enclawed");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "enclawed");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "enclawed.mjs")));
    ...
    {
      name: "falls through from a non-enclawed moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-enclawed");
    ...
    {
      name: "returns null for non-enclawed package roots",
      setup: () => {
        const pkgRoot = fx("not-enclawed");
        setPackageRoot(pkgRoot, "not-enclawed");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "enclawed.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "enclawed");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveEnclawedPackageRootSync,
      resolveEnclawedPackageRoot,
      opts,
      expected,
    );
  });
