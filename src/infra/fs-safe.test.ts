...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("enclawed-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("enclawed-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("enclawed-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("enclawed-fs-safe-root-");
    const sourceDir = await tempDirs.make("enclawed-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("enclawed-fs-safe-root-");
    const outside = await tempDirs.make("enclawed-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    const originalEnclawedHome = process.env.ENCLAWED_HOME;
    const fakeHome = path.resolve(path.sep, "tmp", "fake-home-test");
    process.env.HOME = fakeHome;
    process.env.ENCLAWED_HOME = fakeHome;
    try {
      const result = expandHomePrefix("~/file.txt");
      expect(path.normalize(result)).toBe(path.join(fakeHome, "file.txt"));
    } finally {
      process.env.HOME = originalHome;
      process.env.ENCLAWED_HOME = originalEnclawedHome;
    }

    const root = await tempDirs.make("enclawed-tilde-test-");
    process.env.HOME = root;
    process.env.ENCLAWED_HOME = root;
    try {
    ...
    const outsideRoot = await tempDirs.make("enclawed-tilde-outside-");
    await expect(
      openFileWithinRoot({
        rootDir: outsideRoot,
        relativePath: "~/escape.txt",
