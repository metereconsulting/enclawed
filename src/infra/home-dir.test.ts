...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers ENCLAWED_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for ENCLAWED_HOME",
      env: { ENCLAWED_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when ENCLAWED_HOME is tilde-only and no fallback home exists",
      env: { ENCLAWED_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores ENCLAWED_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          ENCLAWED_HOME: "/srv/enclawed-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { ENCLAWED_HOME: "/srv/enclawed-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/enclawed-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/enclawed-home " },
      expected: "/srv/enclawed-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/enclawed-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of ENCLAWED_HOME", () => {
    expect(
