...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.ENCLAWED_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Enclawed. You can keep running the web gateway without it: \`${formatCliCommand("enclawed gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
