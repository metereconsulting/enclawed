...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.ENCLAWED_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-current-bindings-"));
    process.env.ENCLAWED_STATE_DIR = testStateDir;
