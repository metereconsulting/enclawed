...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Enclawed tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideEnclawed = path.join(os.tmpdir(), "outside-enclawed", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
