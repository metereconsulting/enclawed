...
// Keep aligned with @enclawed/plugin-package-contract ExternalPluginCompatibility.
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.ENCLAWED_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.ENCLAWED_CLAWHUB_TOKEN) ||
...
export async function downloadClawHubPackageArchive(params: {
  ...
  const target = await createTempDownloadTarget({
    prefix: "enclawed-clawhub-package",
