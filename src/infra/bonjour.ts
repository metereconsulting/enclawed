...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ENCLAWED_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Enclawed";
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.ENCLAWED_MDNS_HOSTNAME?.trim() || "enclawed";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
