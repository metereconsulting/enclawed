...
export function hasEnvHttpProxyAgentConfigured(
  ...
  return (
    hasEnvHttpProxyConfigured("http", env) || hasEnvHttpProxyConfigured("https", env)
  );
}
 ...
 */
export function resolveEnvHttpProxyAgentOptions(env: NodeJS.ProcessEnv = process.env): {
  httpProxy?: string;
  httpsProxy?: string;
  noProxy?: string;
} {
  const httpProxy = resolveEnvHttpProxyUrl("http", env);
  const httpsProxy = resolveEnvHttpProxyUrl("https", env);
  const noProxyValue = normalizeProxyEnvValue(env.no_proxy) ?? normalizeProxyEnvValue(env.NO_PROXY);
  return {
    ...(httpProxy ? { httpProxy } : {}),
    ...(httpsProxy ? { httpsProxy } : {}),
    ...(noProxyValue ? { noProxy: noProxyValue } : {}),
  };
}
