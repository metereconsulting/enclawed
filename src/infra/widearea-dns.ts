...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ENCLAWED_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*enclawed-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "enclawed";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "enclawed");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "enclawed-gw");
  ...
  records.push(`_enclawed-gw._tcp IN PTR ${instanceLabel}._enclawed-gw._tcp`);
  records.push(`${instanceLabel}._enclawed-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._enclawed-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; enclawed-content-hash: ${contentHash}\n${contentBody}`;
}
