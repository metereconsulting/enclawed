...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "enclawed.plugin.json");
const EXTENSION_PACKAGE = bundledPluginFile("demo", "package.json");
const EXTENSION_README = bundledPluginFile("demo", "README.md");
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "enclawed.plugin.json");
...
const BASE_PROJECT_FILES = {
  [ROOT_TSCONFIG]: "{}\n",
  [ROOT_PACKAGE]: '{"name":"enclawed-test"}\n',
...
function statusCommandSpawn() {
  return [process.execPath, "enclawed.mjs", "status"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "enclawed-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "enclawed.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "enclawed-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "enclawed.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "enclawed.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          ENCLAWED_FORCE_BUILD: "1",
          ENCLAWED_RUNNER_LOG: "1",
          ENCLAWED_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[enclawed]");
      expect(spawnCalls.at(-1)?.args).toEqual(["enclawed.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.ENCLAWED_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "enclawed-run-node-" }, async (tmp) => {
      ...
      expect(childArgs).toEqual(["enclawed.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.ENCLAWED_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
    ...
    await withTempDir({ prefix: "enclawed-run-node-" }, async (tmp) => {
      ...
      const requirement = resolveBuildRequirement(
        createBuildRequirementDeps(tmp, {
          env: { ENCLAWED_BUILD_PRIVATE_QA: "1" },
          gitHead: "abc123\n",
          gitStatus: "",
        }),
      );
  ...
  it("forwards wrapper SIGTERM to the active enclawed child and returns 143", async () => {
    await withTempDir({ prefix: "enclawed-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("skips rebuilding when extension package metadata is newer than the build stamp", async () => {
    await withTempDir({ prefix: "enclawed-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","enclawed":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","enclawed":{"extensions":["./stale.js"]}}\n',
        },
