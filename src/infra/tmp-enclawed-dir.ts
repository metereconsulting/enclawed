...
export const POSIX_ENCLAWED_TMP_DIR = "/tmp/enclawed";
...
export function resolvePreferredEnclawedTmpDir(
  options: ResolvePreferredEnclawedTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "enclawed" : `enclawed-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[enclawed] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Enclawed temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Enclawed temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_ENCLAWED_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_ENCLAWED_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_ENCLAWED_TMP_DIR)) {
      return POSIX_ENCLAWED_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_ENCLAWED_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_ENCLAWED_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_ENCLAWED_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_ENCLAWED_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_ENCLAWED_TMP_DIR;
