...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevEnclawedHome = process.env.ENCLAWED_HOME;
    const prevHome = process.env.HOME;
    process.env.ENCLAWED_HOME = "/srv/enclawed-home";
    process.env.HOME = "/home/other";
    const enclawedHome = path.join(path.resolve("/srv/enclawed-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", enclawedHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevEnclawedHome === undefined) {
        delete process.env.ENCLAWED_HOME;
      } else {
        process.env.ENCLAWED_HOME = prevEnclawedHome;
      }
