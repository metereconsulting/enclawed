...
const PRIMARY_PACKAGE_NAME = "enclawed";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const ENCLAWED_MAIN_PACKAGE_SPEC = "github:enclawed/enclawed#main";
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "enclawed.plugin.json")))
      ) {
        expectedFiles.add(relativePath);
      }
    }),
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Enclawed", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.ENCLAWED_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.ENCLAWED_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return ENCLAWED_MAIN_PACKAGE_SPEC;
  }
