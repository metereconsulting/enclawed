...
export function triggerEnclawedRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.ENCLAWED_SYSTEMD_UNIT,
      process.env.ENCLAWED_PROFILE,
    );
  ...
  const label =
    process.env.ENCLAWED_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ENCLAWED_PROFILE);
