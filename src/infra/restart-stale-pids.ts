 ...
 * returning control to the caller (typically `triggerEnclawedRestart` →
...
/**
 * Parse enclawed gateway PIDs from lsof -Fpc stdout.
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (
        currentPid != null &&
        currentCmd &&
        normalizeLowercaseStringOrEmpty(currentCmd).includes("enclawed")
      ) {
...
/**
 * Windows: find listening PIDs on the port, then verify each is an enclawed
 ...
 * Returns only PIDs that belong to enclawed gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only enclawed gateway processes.
