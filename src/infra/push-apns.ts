...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Enclawed to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "enclawed.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ENCLAWED_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ENCLAWED_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ENCLAWED_APNS_TEAM_ID and ENCLAWED_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ENCLAWED_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ENCLAWED_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ENCLAWED_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ENCLAWED_APNS_PRIVATE_KEY_P8 or ENCLAWED_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ENCLAWED_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
