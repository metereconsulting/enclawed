...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: EnclawedConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: EnclawedConfig) {
...
function resolveHeartbeatAgents(cfg: EnclawedConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: EnclawedConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: EnclawedConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: EnclawedConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
