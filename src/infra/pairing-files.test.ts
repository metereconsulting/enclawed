...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/enclawed-state", "devices")).toEqual({
      dir: path.join("/tmp/enclawed-state", "devices"),
      pendingPath: path.join("/tmp/enclawed-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/enclawed-state", "devices", "paired.json"),
    });
  });
