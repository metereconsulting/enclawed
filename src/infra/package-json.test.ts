...
describe("package-json helpers", () => {
  it("reads package version and trims package name", async () => {
    await withTempDir({ prefix: "enclawed-package-json-" }, async (root) => {
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ version: " 1.2.3 ", name: "  @enclawed/demo  " }),
        "utf8",
      );
  ...
  it.each([
    ...
    },
    {
      ...
      writePackageJson: async (root: string) => {
        await fs.writeFile(
          path.join(root, "package.json"),
          JSON.stringify({ version: "   ", name: "@enclawed/demo" }),
          "utf8",
        );
      },
      expectedVersion: null,
