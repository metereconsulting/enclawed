...
export type SpeechProviderPrepareSynthesisContext = {
  ...
  // Persona type is intentionally permissive in oss; upstream binds this to
  // ResolvedTtsPersona which depends on TtsPersonaConfig (not present in oss).
  // The structural subset below covers the fields provider plugins read while
  // keeping the contract provider-agnostic.
  persona?: {
    id: string;
    label?: string;
    prompt?: {
