      ...
      },
      {
        ...
        plugin: {
          ...
          config: {
            listAccountIds: () => [],
            resolveAccount: () => ({}),
            resolveAllowFrom: ({ cfg }: { cfg: EnclawedConfig }) =>
...
function makeCfg(overrides?: Partial<EnclawedConfig>): EnclawedConfig {
  return {
    bindings: [],
    channels: {},
    ...overrides,
...
function makeTelegramBoundCfg(accountId = "account-b"): EnclawedConfig {
  return makeCfg({
    bindings: [
      {
        agentId: AGENT_ID,
...
async function resolveLastTarget(cfg: EnclawedConfig) {
