...
const { makeStorePath } = createCronStoreHarness({ prefix: "enclawed-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ENCLAWED_HOME for tilde expansion", () => {
    vi.stubEnv("ENCLAWED_HOME", "/srv/enclawed-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/enclawed-home", "cron", "jobs.json"));
  });
});
