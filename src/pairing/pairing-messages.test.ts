...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ENCLAWED_CONTAINER_HINT", "ENCLAWED_PROFILE"]);
    delete process.env.ENCLAWED_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:enclawed|enclawed) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
