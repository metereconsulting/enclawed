...
export type PromptDefaultModelResult = { model?: string; config?: EnclawedConfig };
...
function resolveConfiguredModelRaw(cfg: EnclawedConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: EnclawedConfig): string[] {
...
export function applyModelAllowlist(cfg: EnclawedConfig, models: string[]): EnclawedConfig {
