...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<EnclawedConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<EnclawedConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: EnclawedConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: EnclawedConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: EnclawedConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: EnclawedConfig, result: EnclawedConfig): EnclawedConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as EnclawedConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with enclawed configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Enclawed will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.enclawed.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Enclawed will store a reference instead of the API key.",
