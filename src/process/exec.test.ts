...
import { ENCLAWED_CLI_ENV_VALUE } from "../infra/openclaw-exec-env.js";
...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        ENCLAWED_BASE_ENV: "base",
        ENCLAWED_TO_REMOVE: undefined,
      },
      env: {
        ENCLAWED_TEST_ENV: "ok",
      },
    });

    expect(resolved.ENCLAWED_BASE_ENV).toBe("base");
    expect(resolved.ENCLAWED_TEST_ENV).toBe("ok");
    expect(resolved.ENCLAWED_TO_REMOVE).toBeUndefined();
    expect(resolved.ENCLAWED_CLI).toBe(ENCLAWED_CLI_ENV_VALUE);
  });
