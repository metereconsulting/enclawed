...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "enclawed-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "enclawed-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "enclawed-bundled");
      const plugin = makeHookEntry("shared", "enclawed-plugin");
      const managed = makeHookEntry("shared", "enclawed-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("enclawed-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "enclawed-managed");
      const workspace = makeHookEntry("shared", "enclawed-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "enclawed-workspace");
      const second = makeHookEntry("shared", "enclawed-workspace");
      second.hook.handlerPath = "/tmp/enclawed-workspace/shared/handler-2.js";
