...
This directory contains hooks that ship with Enclawed. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.enclawed/workspace`)
...
homepage: https://docs.enclawed.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.enclawed/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.enclawed/enclawed.json`:
...
2. Restart gateway: `pkill -9 -f 'enclawed.*gateway' && pnpm enclawed gateway`
3. Enable the hook: `enclawed hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.enclawed.ai/automation/hooks
