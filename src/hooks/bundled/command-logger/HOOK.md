...
homepage: https://docs.enclawed.ai/automation/hooks#command-logger
metadata:
  {
    "enclawed":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.enclawed/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.enclawed/logs/commands.log ~/.enclawed/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/enclawed`:
   ```
   /home/username/.enclawed/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
