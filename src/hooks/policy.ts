import type { EnclawedConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "enclawed-bundled": {
    ...
    canOverride: ["enclawed-bundled"],
    canBeOverriddenBy: ["enclawed-managed", "enclawed-plugin"],
  },
  "enclawed-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["enclawed-bundled", "enclawed-plugin"],
    canBeOverriddenBy: ["enclawed-managed"],
  },
  "enclawed-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["enclawed-bundled", "enclawed-managed", "enclawed-plugin"],
    canBeOverriddenBy: ["enclawed-managed"],
  },
  "enclawed-workspace": {
    precedence: 40,
    ...
    canOverride: ["enclawed-workspace"],
    canBeOverriddenBy: ["enclawed-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "enclawed-plugin") {
    return { enabled: true };
  }
