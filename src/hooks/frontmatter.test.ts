...
`;
    ...
    expect(parsed.enclawed.emoji).toBe("💾");
    expect(parsed.enclawed.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.enclawed.emoji).toBe("📝");
    expect(parsed.enclawed.events).toEqual(["command"]);
    expect(parsed.enclawed.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.enclawed.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"enclawed": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"enclawed": {"events": ["test"]}}');
  });
...
describe("resolveEnclawedMetadata", () => {
  it("extracts enclawed metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveEnclawedMetadata(frontmatter);
  ...
  it("returns undefined when enclawed key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        enclawed: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Enclawed" },
            { id: "npm", kind: "npm", package: "@enclawed/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@enclawed/hook");
  });
...
`;
    ...
    const enclawed = resolveEnclawedMetadata(frontmatter);
    expect(enclawed).toBeDefined();
    expect(enclawed?.emoji).toBe("💾");
    expect(enclawed?.events).toEqual(["command:new", "command:reset"]);
    expect(enclawed?.requires?.config).toEqual(["workspace.dir"]);
    expect(enclawed?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const enclawed = resolveEnclawedMetadata(frontmatter);
    expect(enclawed?.emoji).toBe("disk");
    expect(enclawed?.events).toEqual(["command:new"]);
  });
});
