...
/**
 * Selects how the source channel surfaces an agent reply that the message
 * tool could also have delivered. `automatic` lets the host deliver the
 * reply directly; `message_tool_only` defers visible delivery to the
 * message-tool path so duplicate sends do not happen in chat contexts where
 * the agent invokes the message tool explicitly.
 */
...
export type GetReplyOptions = {
  ...
  /**
   * Source-channel visible-reply delivery mode. `message_tool_only` defers
   * visible delivery to the message tool instead of streaming block replies to
   * the source channel.
   */
  sourceReplyDeliveryMode?: SourceReplyDeliveryMode;
  /**
   * If true, suppress the default tool progress placeholder messages for this
   * run (used by channels that render their own progress UI).
   */
