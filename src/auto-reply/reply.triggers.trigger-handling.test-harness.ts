...
const piEmbeddedMocks = getSharedMocks("enclawed.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("enclawed.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("enclawed.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("enclawed.trigger-handling.web-session-mocks", () => ({
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.ENCLAWED_STATE_DIR = join(home, ".enclawed");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "enclawed-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".enclawed", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): EnclawedConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "enclawed"),
