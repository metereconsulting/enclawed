...
export function findCommandByNativeName(
  name: string,
  provider?: string,
  options?: { includeBundledChannelFallback?: boolean },
): ChatCommandDefinition | undefined {
  const normalized = normalizeOptionalLowercaseString(name);
  if (!normalized) {
    return undefined;
  }
  // `includeBundledChannelFallback` lets callers opt out of matching commands
  // that are only present via a bundled-channel fallback registration. The
  // active chat-command registry does not currently expose fallback-only
  // entries, so the lookup set is the same regardless; the option is honored
  // by callers that pre-filter before reaching this resolver.
  void options;
...
function resolveDefaultCommandContext(cfg?: EnclawedConfig): {
  provider: string;
  model: string;
} {
