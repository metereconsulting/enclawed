...
const { buildStatusReplyMock, createEnclawedToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createEnclawedToolsMock: vi.fn(),
...
vi.mock("../../agents/enclawed-tools.runtime.js", () => ({
  createEnclawedTools: (...args: unknown[]) => createEnclawedToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createEnclawedToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
