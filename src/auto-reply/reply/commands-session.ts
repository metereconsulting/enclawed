...
import { scheduleGatewaySigusr1Restart, triggerEnclawedRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Enclawed in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerEnclawedRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Enclawed via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
