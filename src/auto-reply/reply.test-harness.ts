...
type HomeEnvSnapshot = {
  ...
  ENCLAWED_STATE_DIR: string | undefined;
  ENCLAWED_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    ENCLAWED_AGENT_DIR: process.env.ENCLAWED_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".enclawed", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ENCLAWED_STATE_DIR = path.join(home, ".enclawed");
    process.env.ENCLAWED_AGENT_DIR = path.join(home, ".enclawed", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".enclawed", "agent");
  ...
  return withFastReplyConfig({
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "enclawed"),
      },
    },
    channels: {
      whatsapp: {
