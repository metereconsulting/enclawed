...
import type { SpeechProviderPlugin, SpeechSynthesisRequest } from "@enclawed/plugin-sdk/speech-core";
...
vi.mock("@enclawed/plugin-sdk/channel-targets", () => ({
...
function createTtsConfig(prefsName: string): EnclawedConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    const cfg = createTtsConfig("enclawed-speech-core-tts-test");
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    const cfg = createTtsConfig("enclawed-speech-core-tts-slack-test");
    const payload: ReplyPayload = {
