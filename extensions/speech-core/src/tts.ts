...
import { normalizeChannelId, type ChannelId } from "@enclawed/plugin-sdk/channel-targets";
import type {
  EnclawedConfig,
  TtsAutoMode,
  TtsConfig,
  ...
  TtsProvider,
} from "@enclawed/plugin-sdk/config-runtime";
import { formatErrorMessage } from "@enclawed/plugin-sdk/error-runtime";
import { redactSensitiveText } from "@enclawed/plugin-sdk/logging-core";
import {
  resolveSendableOutboundReplyParts,
  type ReplyPayload,
} from "@enclawed/plugin-sdk/reply-payload";
import { isVerbose, logVerbose } from "@enclawed/plugin-sdk/runtime-env";
import { resolvePreferredEnclawedTmpDir } from "@enclawed/plugin-sdk/sandbox";
import {
  ...
  resolveUserPath,
  stripMarkdown,
} from "@enclawed/plugin-sdk/text-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.ENCLAWED_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: EnclawedConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: EnclawedConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: EnclawedConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: EnclawedConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: EnclawedConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: EnclawedConfig): TtsProvider[] {
