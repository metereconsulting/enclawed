import { createScopedDmSecurityResolver } from "@enclawed/plugin-sdk/channel-config-helpers";
...
function resolveZalouserOutboundChunkMode(cfg: EnclawedConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: EnclawedConfig, accountId?: string) {
...
export const zalouserPairingTextAdapter = {
  ...
  notify: async ({ cfg, id, message }: { cfg: EnclawedConfig; id: string; message: string }) => {
    const { sendMessageZalouser } = await loadZalouserChannelRuntime();
