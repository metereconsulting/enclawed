...
const ENCLAWED_OPENSHELL_E2E = process.env.ENCLAWED_E2E_OPENSHELL === "1";
const ENCLAWED_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const ENCLAWED_OPENSHELL_COMMAND =
  process.env.ENCLAWED_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "enclawed-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && ENCLAWED_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: ENCLAWED_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(ENCLAWED_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(ENCLAWED_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-openshell-e2e-"));
      ...
      const gatewayName = `enclawed-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `enclawed-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/enclawed-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: ENCLAWED_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("enclawed-openshell-e2e");
