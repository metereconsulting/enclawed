...
import type { PluginLogger } from "@enclawed/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "@enclawed/plugin-sdk/runtime-env";
...
// See https://github.com/enclawed/enclawed/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.ENCLAWED_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Enclawed"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Enclawed";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Enclawed\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.ENCLAWED_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "enclawed";
    ...
    const recreateAdvertiser = async (reason: string) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or ENCLAWED_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
