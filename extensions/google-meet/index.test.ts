...
import type { RealtimeVoiceProviderPlugin } from "@enclawed/plugin-sdk/realtime-voice";
...
vi.mock("@enclawed/plugin-sdk/ssrf-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@enclawed/plugin-sdk/ssrf-runtime")>();
  return {
    ...actual,
...
describe("google-meet plugin", () => {
  ...
  it("defaults to chrome realtime with safe read-only tools", () => {
    ...
    expect(resolveGoogleMeetConfig({}).realtime.instructions).toContain("enclawed_agent_consult");
  });
  ...
  it("uses env fallbacks for OAuth, preview, and default meeting values", () => {
    expect(
      resolveGoogleMeetConfigWithEnv(
        {},
        {
          ENCLAWED_GOOGLE_MEET_CLIENT_ID: "client-id",
          GOOGLE_MEET_CLIENT_SECRET: "client-secret",
          ENCLAWED_GOOGLE_MEET_REFRESH_TOKEN: "refresh-token",
          GOOGLE_MEET_ACCESS_TOKEN: "access-token",
          ENCLAWED_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: "123456",
          GOOGLE_MEET_DEFAULT_MEETING: "https://meet.google.com/abc-defg-hij",
          ENCLAWED_GOOGLE_MEET_PREVIEW_ACK: "true",
        },
      ),
    ).toMatchObject({
  ...
  it("reports setup status through the tool", async () => {
    ...
    try {
      const { tools } = setup({
        chrome: {
          audioInputCommand: ["enclawed-audio-bridge", "capture"],
          audioOutputCommand: ["enclawed-audio-bridge", "play"],
        },
      });
      const tool = tools[0] as {
  ...
  it("writes export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "enclawed-google-meet-tool-export-"));
    const { tools } = setup();
    const tool = tools[0] as {
      execute: (
        id: string,
  ...
  it("dry-runs export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const parentDir = mkdtempSync(path.join(tmpdir(), "enclawed-google-meet-tool-dry-run-"));
  ...
  it("retries caption enable until the captions button is available", () => {
    ...
    const stateAfterFirst = windowState.__enclawedMeetCaptions as { enabledAttempted?: boolean };
    ...
    const stateAfterSecond = windowState.__enclawedMeetCaptions as { enabledAttempted?: boolean };
              ...
              payload: {
                result: {
                  result: JSON.stringify({
                    ...
                    manualActionMessage: "Admit the Enclawed browser participant in Google Meet.",
                    title: "Meet",
                    url: "https://meet.google.com/abc-defg-hij?authuser=me@example.com",
                  }),
                },
    ...
    const { tools } = setup(
      ...
      },
      {
        browserActResult: {
          ...
          manualActionMessage:
            "Sign in to Google in the Enclawed browser profile, then retry the Meet join.",
  ...
  it("explains when chrome-node has no capable paired node", async () => {
    ...
    expect(result.details.error).toContain("enclawed node run");
  });
