...
async function listGoogleMeetNodes(
  ...
  const nodesRuntime = runtime.nodes;
  if (!nodesRuntime) {
    throw new Error("Google Meet node inventory unavailable: runtime node access is not enabled");
  }
  try {
    const result = await nodesRuntime.list(
      params?.connected === true ? { includeOffline: false } : { includeOffline: true },
    );
    return {
      nodes: result.nodes.map((node) => ({
        nodeId: node.nodeId,
        displayName: node.displayName,
        caps: node.capabilities ? [...node.capabilities] : undefined,
...
export async function resolveChromeNodeInfo(params: {
  ...
  if (requested) {
    ...
    if (matches.length === 1) {
      ...
      throw new Error(
        `Configured Google Meet node ${requested} is not usable (${formatNodeLabel(node)}): ${describeNodeUsabilityIssues(node).join("; ")}. Start or reinstall \`enclawed node run\` on that Chrome host, approve pairing, and allow googlemeet.chrome plus browser.proxy.`,
      );
    }
    if (matches.length > 1) {
      throw new Error(
    ...
    throw new Error(
      `Configured Google Meet node ${requested} was not found. Run \`enclawed nodes status\` and start or approve the Chrome node.`,
    );
  }
  ...
  if (nodes.length === 0) {
    throw new Error(
      "No connected Google Meet-capable node with browser proxy. Run `enclawed node run` on the Chrome host with browser proxy enabled, approve pairing, and allow googlemeet.chrome plus browser.proxy.",
    );
  }
  if (nodes.length === 1) {
    return nodes[0];
...
export async function callBrowserProxyOnNode(params: {
  ...
  if (!nodesRuntime) {
    throw new Error("Google Meet browser proxy unavailable: runtime node access is not enabled");
  }
