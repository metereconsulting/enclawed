...
import { callGatewayFromCli } from "@enclawed/plugin-sdk/gateway-runtime";
import type { PluginRuntime } from "@enclawed/plugin-sdk/plugin-runtime";
import type { RuntimeLogger } from "@enclawed/plugin-sdk/plugin-runtime";
  ...
  if (result.code !== 0 || !outputMentionsBlackHole2ch(output)) {
    ...
    throw new Error(
      [
        ...
        "Install BlackHole 2ch and route Chrome input/output through the Enclawed audio bridge.",
        hint,
      ]
        .filter(Boolean)
        .join(" "),
...
function meetStatusScript(params: {
  ...
  const captionState = (() => {
    ...
    const w = window;
    if (!inCall && !w.__enclawedMeetCaptions) return undefined;
    if (!w.__enclawedMeetCaptions) {
      w.__enclawedMeetCaptions = {
    ...
    return w.__enclawedMeetCaptions;
  })();
  ...
  if (!inCall && (host === "accounts.google.com" || /use your google account|to continue to google meet|choose an account|sign in to (join|continue)/i.test(pageText))) {
    ...
    manualActionMessage = "Sign in to Google in the Enclawed browser profile, then retry the Meet join.";
    ...
    manualActionMessage = "Admit the Enclawed browser participant in Google Meet, then retry speech.";
  } else if (permissionNeeded) {
    manualActionReason = "meet-permission-required";
    manualActionMessage = allowMicrophone
      ? "Allow microphone/camera/speaker permissions for Meet in the Enclawed browser profile, then retry."
      : "Join without microphone/camera permissions in the Enclawed browser profile, then retry.";
    ...
    manualActionMessage = allowMicrophone
      ? "Meet is showing the microphone choice. Click Use microphone in the Enclawed browser profile, then retry."
      : "Meet is showing the microphone choice. Choose the no-microphone option in the Enclawed browser profile, then retry.";
  }
  ...
  do {
    try {
      ...
      browser = {
        ...
        manualActionMessage:
          "Open the Enclawed browser profile, finish Google Meet login, admission, or permission prompts, then retry.",
        notes: [
          ...permissionNotes,
...
export async function launchChromeMeetOnNode(params: {
  ...
  const nodesRuntime = params.runtime.nodes;
  if (!nodesRuntime) {
    throw new Error("[google-meet] runtime node access is unavailable");
  }
  const nodeId = await resolveChromeNode({
    runtime: params.runtime,
    requestedNode: params.config.chromeNode.node,
  ...
  const raw = await nodesRuntime.invoke({
    nodeId,
    command: "googlemeet.chrome",
    params: {
      action: "start",
