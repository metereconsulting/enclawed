import { createChannelPairingController } from "@enclawed/plugin-sdk/channel-pairing";
import {
...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as EnclawedConfig);
  ...
  if (message.isGroup && commandGate.shouldBlock) {
    const { logInboundDrop } = await import("@enclawed/plugin-sdk/channel-inbound");
    logInboundDrop({
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as EnclawedConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as EnclawedConfig);
  ...
  const { dispatchInboundReplyWithBase } =
    await import("@enclawed/plugin-sdk/inbound-reply-dispatch");
