import { createSendCfgThreadingRuntime } from "@enclawed/plugin-sdk/channel-test-helpers";
...
vi.mock("@enclawed/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("@enclawed/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("@enclawed/plugin-sdk/text-runtime", async () => {
  const original = (await vi.importActual("@enclawed/plugin-sdk/text-runtime")) as Record<
    string,
    unknown
  >;
  return {
