...
function resolveQianfanPreset(cfg: EnclawedConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: EnclawedConfig) => {
...
export function applyQianfanProviderConfig(cfg: EnclawedConfig): EnclawedConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: EnclawedConfig): EnclawedConfig {
