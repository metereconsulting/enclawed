...
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "@enclawed/plugin-sdk/webhook-request-guards";
import { beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { createMockIncomingRequest } from "@enclawed/plugin-sdk/test-env";
...
vi.mock("@enclawed/plugin-sdk/reply-runtime", () => ({
...
vi.mock("@enclawed/plugin-sdk/channel-reply-pipeline", () => ({
  createChannelReplyPipeline: vi.fn(() => ({})),
}));

vi.mock("@enclawed/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/webhook-ingress")>(
    "@enclawed/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
