...
import { patchChannelConfigForAccount } from "@enclawed/plugin-sdk/setup-runtime";
import { formatDocsLink } from "@enclawed/plugin-sdk/setup-tools";
import type { ResolvedSlackAccount } from "./accounts.js";
import type { EnclawedConfig } from "./channel-api.js";

export const SLACK_CHANNEL = "slack" as const;

function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Enclawed";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Enclawed`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "Enclawed"): string[] {
  return [
