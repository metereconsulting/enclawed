import { hasConfiguredSecretInput } from "@enclawed/plugin-sdk/secret-input";
import {
  ...
  type EnclawedConfig,
} from "@enclawed/plugin-sdk/setup-runtime";
import { formatDocsLink } from "@enclawed/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: EnclawedConfig, accountId: string): EnclawedConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: EnclawedConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: EnclawedConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: EnclawedConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: EnclawedConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: EnclawedConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: EnclawedConfig; accountId: string }) =>
    ...
    disable: (cfg: EnclawedConfig) => setSetupChannelEnabled(cfg, channel, false),
