import { CURRENT_MESSAGE_MARKER } from "@enclawed/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "@enclawed/plugin-sdk/channel-test-helpers";
import { resetInboundDedupe } from "@enclawed/plugin-sdk/reply-dedupe";
import { HISTORY_CONTEXT_MARKER } from "@enclawed/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\benclawed\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("enclawed: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("enclawed: hello <@U2>");
  });
