...
vi.mock("@enclawed/plugin-sdk/target-resolver-runtime", async (orig) => {
...
vi.mock("@enclawed/plugin-sdk/extension-shared", async (orig) => {
  const original = (await orig()) as Record<string, unknown>;
  return {
    ...original,
...
function makeMinimalSlackConfig(
  ...
  return { channels: { slack } } as EnclawedConfig;
}
