import { expectChannelInboundContextContract } from "@enclawed/plugin-sdk/channel-contract-testing";
import type { EnclawedConfig } from "@enclawed/plugin-sdk/config-types";
import { createTempHomeEnv } from "@enclawed/plugin-sdk/test-env";
...
describe("Slack inbound context contract", () => {
  it("keeps inbound context finalized", async () => {
    const tempHome = await createTempHomeEnv("enclawed-slack-inbound-contract-");
    try {
