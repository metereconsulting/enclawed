...
// Prefix for Enclawed-generated action IDs to scope our handler
const ENCLAWED_ACTION_PREFIX = "enclawed:";
...
export function registerSlackInteractionEvents(params: {
  ...
  const modalMatcher = new RegExp(`^${ENCLAWED_ACTION_PREFIX}`);

  // Handle Enclawed modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
