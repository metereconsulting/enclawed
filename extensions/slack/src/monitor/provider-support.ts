...
const ENCLAWED_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
...
export function shouldSkipEnclawedSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout:
      params.socketMode?.clientPingTimeout ?? ENCLAWED_SLACK_CLIENT_PING_TIMEOUT_MS,
    installerOptions: {
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping Enclawed's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
  });
  app.use(async (args) => {
    if (shouldSkipEnclawedSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
