...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): EnclawedConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as EnclawedConfig;
  ...
  async function resolveReadToken(cfg: EnclawedConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: EnclawedConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/enclawed-media/report.pdf",
    ...
    expect(result.content[0]).toEqual(
      expect.objectContaining({
        type: "text",
        text: expect.stringContaining("/tmp/enclawed-media/report.pdf"),
      }),
    );
    ...
    expect(result.details).toEqual(
      expect.objectContaining({
        ...
        contentType: "application/pdf",
        media: {
          mediaUrl: "/tmp/enclawed-media/report.pdf",
          contentType: "application/pdf",
        },
      }),
    );
