...
}): Promise<EnclawedPluginNodeInvokePolicyResult | null> {
  const nodeDisplayName = input.ctx.node?.displayName;
  const missingCode =
...
type PreflightResult =
  ...
  | {
      ok: false;
      result: EnclawedPluginNodeInvokePolicyResult;
    };
...
async function handleFileTransferInvoke(
  ctx: EnclawedPluginNodeInvokePolicyContext,
): Promise<EnclawedPluginNodeInvokePolicyResult> {
...
export function createFileTransferNodeInvokePolicy(): EnclawedPluginNodeInvokePolicy {
  return {
    commands: COMMANDS,
