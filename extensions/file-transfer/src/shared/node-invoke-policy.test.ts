...
import type { EnclawedPluginNodeInvokePolicyContext } from "@enclawed/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: EnclawedPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<EnclawedPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<EnclawedPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
