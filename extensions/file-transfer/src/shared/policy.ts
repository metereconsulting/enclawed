...
// a config block to ~/.enclawed/enclawed.json under
...
import { mutateConfigFile } from "@enclawed/plugin-sdk/config-mutation";
...
export async function persistAllowAlways(input: {
  ...
  await mutateConfigFile({
    ...
    mutate: (draft) => {
      // Plugin config is intentionally plugin-owned; the root EnclawedConfig
