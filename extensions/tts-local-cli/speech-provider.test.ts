...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "@enclawed/plugin-sdk/speech-core";
...
vi.mock("@enclawed/plugin-sdk/media-runtime", () => ({
...
function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "enclawed-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.ENCLAWED_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("@enclawed/plugin-sdk/media-runtime")>(
