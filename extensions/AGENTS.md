...
- Extension production code should import from `@enclawed/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `enclawed.plugin.json` and the package
  `enclawed` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `@enclawed/plugin-sdk/*`.
