...
} from "@enclawed/plugin-sdk/memory-host-events";
...
describe("syncMemoryWikiBridgeSources", () => {
  ...
  it("imports public memory artifacts and stays idempotent across reruns", async () => {
    ...
    const appConfig: EnclawedConfig = {
      agents: {
    ...
    const logLines = (await fs.readFile(path.join(vaultDir, ".enclawed-wiki", "log.jsonl"), "utf8"))
      .trim()
      .split("\n");
