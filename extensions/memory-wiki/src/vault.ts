...
import {
  ...
  withTrailingNewline,
} from "@enclawed/plugin-sdk/memory-host-markdown";
...
function buildIndexMarkdown(): string {
  return withTrailingNewline(
    replaceManagedMarkdownBlock({
      original: "# Wiki Index\n",
      heading: "## Generated",
      startMarker: "<!-- enclawed:wiki:index:start -->",
      endMarker: "<!-- enclawed:wiki:index:end -->",
...
- Use \`.enclawed-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the Enclawed memory-wiki plugin.
...
- \`.enclawed-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- enclawed:human:start -->
<!-- enclawed:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".enclawed-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".enclawed-wiki", "log.jsonl"), "", createdFiles);
