...
const AGENT_DIGEST_PATH = ".enclawed-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".enclawed-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- enclawed:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- enclawed:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- enclawed:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- enclawed:wiki:${group.dir}:index:end -->`,
