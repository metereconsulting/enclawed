...
- Start from `enclawed wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `enclawed wiki obsidian status` before shelling out, then prefer the dedicated helpers like `enclawed wiki obsidian search`, `enclawed wiki obsidian open`, `enclawed wiki obsidian command`, and `enclawed wiki obsidian daily`.
