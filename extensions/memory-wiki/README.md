# @enclawed/memory-wiki

Persistent wiki compiler and Obsidian-friendly knowledge vault for **Enclawed**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.enclawed-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
```bash
enclawed wiki status
enclawed wiki doctor
enclawed wiki init
enclawed wiki ingest ./notes/alpha.md
enclawed wiki compile
enclawed wiki lint
enclawed wiki search "alpha"
enclawed wiki get entity.alpha --from 1 --lines 80

enclawed wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

enclawed wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

enclawed wiki bridge import
enclawed wiki unsafe-local import

enclawed wiki obsidian status
enclawed wiki obsidian search "alpha"
enclawed wiki obsidian open syntheses/alpha-summary.md
enclawed wiki obsidian command workspace:quick-switcher
enclawed wiki obsidian daily
```
...
- The compiled agent digests in `.enclawed-wiki/cache/agent-digest.json` and `.enclawed-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
