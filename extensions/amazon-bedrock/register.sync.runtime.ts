...
import { resolvePluginConfigObject } from "@enclawed/plugin-sdk/plugin-config-runtime";
import type { EnclawedPluginApi, ProviderThinkingProfile } from "@enclawed/plugin-sdk/plugin-entry";
import {
...
} from "@enclawed/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When Enclawed's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if Enclawed identifies this as an Anthropic model via the ARN heuristic.
 ...
 * the Enclawed config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: EnclawedPluginApi): void {
