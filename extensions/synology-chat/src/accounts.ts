...
import {
  ...
  type EnclawedConfig,
} from "@enclawed/plugin-sdk/account-resolution";
import { resolveDangerousNameMatchingEnabled } from "@enclawed/plugin-sdk/dangerous-name-runtime";
import type {
...
/** Extract the channel config from the full Enclawed config object. */
function getChannelConfig(cfg: EnclawedConfig): SynologyChatChannelConfig | undefined {
 ...
 */
export function listAccountIds(cfg: EnclawedConfig): string[] {
...
export function resolveAccount(
  ...
  const envBotName = process.env.ENCLAWED_BOT_NAME ?? "Enclawed";
