...
function getChannelConfig(cfg: EnclawedConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: EnclawedConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: EnclawedConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: EnclawedConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Enclawed uses to send replies back to Chat.",
      ],
