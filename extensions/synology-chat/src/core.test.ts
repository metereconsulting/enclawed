...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: EnclawedConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  beforeEach(() => {
    ...
    delete process.env.ENCLAWED_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("Enclawed");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.ENCLAWED_BOT_NAME = "TestBot";
