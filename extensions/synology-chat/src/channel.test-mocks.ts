...
vi.mock("@enclawed/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("@enclawed/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("@enclawed/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("@enclawed/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/enclawed/synology-chat-sessions.json"),
