...
import type { ChannelSetupWizard } from "@enclawed/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "@enclawed/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<EnclawedConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: EnclawedConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: EnclawedConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: EnclawedConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: EnclawedConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Enclawed can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Enclawed" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("enclawed channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
