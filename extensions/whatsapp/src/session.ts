...
import { VERSION } from "@enclawed/plugin-sdk/cli-runtime";
import {
  resolveEnvHttpProxyUrl,
  shouldUseEnvHttpProxyForUrl,
} from "@enclawed/plugin-sdk/fetch-runtime";
import { danger, success } from "@enclawed/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "@enclawed/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "@enclawed/plugin-sdk/text-runtime";
import {
  readCredsJsonRaw,
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("enclawed channels login")}`,
              ),
            );
          }
        }
