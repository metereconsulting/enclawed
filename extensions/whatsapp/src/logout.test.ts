...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "enclawed-test-web-logout-"));
    previousOAuthDir = process.env.ENCLAWED_OAUTH_DIR;
    process.env.ENCLAWED_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.ENCLAWED_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the Enclawed auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Enclawed auth root", async () => {
