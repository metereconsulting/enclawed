import { createQueuedWizardPrompter } from "@enclawed/plugin-sdk/plugin-test-runtime";
import { DEFAULT_ACCOUNT_ID } from "@enclawed/plugin-sdk/routing";
  ...
  return {
    ...
    splitSetupEntries: (raw: string) =>
      raw
        ...
        .filter(Boolean),
    setSetupChannelEnabled: (cfg: EnclawedConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<EnclawedConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
