...
let resetLogger: typeof import("@enclawed/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("@enclawed/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: EnclawedConfig = {
  channels: { whatsapp: {} },
};
...
vi.mock("@enclawed/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/media-runtime")>(
    "@enclawed/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("@enclawed/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `enclawed-outbound-${crypto.randomUUID()}.log`);
