...
import { captureEnv } from "@enclawed/plugin-sdk/test-env";
import { mockPinnedHostnameResolution } from "@enclawed/plugin-sdk/test-env";
import { optimizeImageToPng } from "@enclawed/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredEnclawedTmpDir(), "enclawed-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub ENCLAWED_STATE_DIR.
    // Also keep it outside the Enclawed temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["ENCLAWED_STATE_DIR"]);
    process.env.ENCLAWED_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "enclawed-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredEnclawedTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Enclawed state workspace and sandbox roots", async () => {
  ...
  it("rejects default Enclawed state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
