...
vi.mock("@enclawed/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/channel-pairing")>(
...
vi.mock("@enclawed/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/media-store")>(
    "@enclawed/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `enclawed-inbound-media-${crypto.randomUUID()}`);
