import { resolveAccountEntry } from "@enclawed/plugin-sdk/account-core";
import { resolveInboundDebounceMs } from "@enclawed/plugin-sdk/channel-inbound-debounce";
import { formatCliCommand } from "@enclawed/plugin-sdk/cli-runtime";
import { hasControlCommand } from "@enclawed/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "@enclawed/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "@enclawed/plugin-sdk/reply-history";
import { resolveAgentRoute } from "@enclawed/plugin-sdk/routing";
import { logVerbose } from "@enclawed/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "@enclawed/plugin-sdk/runtime-env";
import { getChildLogger } from "@enclawed/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "@enclawed/plugin-sdk/runtime-env";
import { enqueueSystemEvent } from "@enclawed/plugin-sdk/system-event-runtime";
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("enclawed channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("enclawed channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("enclawed channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("enclawed channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
