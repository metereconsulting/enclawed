...
import { generateSecureUuid } from "@enclawed/plugin-sdk/core";
import { redactIdentifier } from "@enclawed/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "@enclawed/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "@enclawed/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "@enclawed/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "@enclawed/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: EnclawedConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`enclawed channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: EnclawedConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
