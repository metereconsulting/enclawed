...
import { readJsonFileWithFallback, writeJsonFileAtomically } from "@enclawed/plugin-sdk/json-store";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit ENCLAWED_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.ENCLAWED_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.enclawed.
  // Use resolvePreferredEnclawedTmpDir + string concat (mirrors
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    const name = "enclawed-vitest-" + process.pid;
    return path.join(resolvePreferredEnclawedTmpDir(), name);
  }
  // Canonical Enclawed state dir: honors `~` expansion + legacy/new
