...
import { type ClaimableDedupe, createClaimableDedupe } from "@enclawed/plugin-sdk/persistent-dedupe";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  if (env.VITEST || env.NODE_ENV === "test") {
    // Isolate tests from real ~/.enclawed state without sharing across tests.
  ...
  // Canonical Enclawed state dir: honors ENCLAWED_STATE_DIR (with `~` expansion
  // via resolveUserPath), plus legacy/new fallback. Using the shared helper
  // keeps this plugin's persistence aligned with the rest of Enclawed state.
