...
import { sleepWithAbort } from "@enclawed/plugin-sdk/runtime-env";
import {
  createComputedAccountStatusAdapter,
  createDefaultChannelRuntimeState,
} from "@enclawed/plugin-sdk/status-helpers";
import { resolveTargetsWithOptionalToken } from "@enclawed/plugin-sdk/target-resolver-runtime";
...
function resolveDiscordStartupDelayMs(cfg: EnclawedConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Enclawed adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
      ...
      status: createComputedAccountStatusAdapter<ResolvedDiscordAccount, DiscordProbe>({
        ...
        buildCapabilitiesDiagnostics: async ({ account, target }) => {
          ...
          const statusCfg: EnclawedConfig = {
            channels: {
              discord: {
                accounts: {
                  [account.accountId]: {
