...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "@enclawed/plugin-sdk/media-runtime";
import { unlinkIfExists } from "@enclawed/plugin-sdk/media-runtime";
...
async function generateWaveformFromPcm(filePath: string): Promise<string> {
  const tempDir = resolvePreferredEnclawedTmpDir();
