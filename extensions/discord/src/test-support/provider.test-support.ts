...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: EnclawedConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): EnclawedConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("@enclawed/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/acp-runtime")>(
    "@enclawed/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/command-auth")>(
...
vi.mock("@enclawed/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("@enclawed/plugin-sdk/native-command-config-runtime")
  >("@enclawed/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/runtime-env")>(
    "@enclawed/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/error-runtime")>(
    "@enclawed/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
