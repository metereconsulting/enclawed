...
const DISCORD_COMPONENT_ENTRIES_KEY = Symbol.for("enclawed.discord.componentEntries");
const DISCORD_MODAL_ENTRIES_KEY = Symbol.for("enclawed.discord.modalEntries");
...
function getPersistentComponentStore(): DiscordRegistryStore<DiscordComponentEntry> | undefined {
  ...
  if (!runtime?.state.openKeyedStore) {
    return undefined;
  }
  try {
