...
function createCfg(): EnclawedConfig {
  return {
    channels: {
      discord: {
        enabled: true,
...
function resolveAccount(cfg: EnclawedConfig, accountId = "default"): ResolvedDiscordAccount {
  return discordPlugin.config.resolveAccount(cfg, accountId);
}

function startDiscordAccount(cfg: EnclawedConfig, accountId = "default") {
