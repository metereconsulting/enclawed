...
vi.mock("@enclawed/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@enclawed/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<EnclawedConfig["channels"]>["discord"]>["execApprovals"],
): EnclawedConfig {
  return {
    channels: {
      discord: {
