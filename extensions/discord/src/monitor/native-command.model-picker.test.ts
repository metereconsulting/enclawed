...
import * as commandRegistryModule from "@enclawed/plugin-sdk/command-auth";
import type { ChatCommandDefinition, CommandArgsParsing } from "@enclawed/plugin-sdk/command-auth";
import type { ModelsProviderData } from "@enclawed/plugin-sdk/command-auth";
import type { EnclawedConfig } from "@enclawed/plugin-sdk/config-types";
import * as globalsModule from "@enclawed/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  saveSessionStore,
} from "@enclawed/plugin-sdk/session-store-runtime";
import * as commandTextModule from "@enclawed/plugin-sdk/text-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "enclawed-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          {
            id: "pi",
            label: "Enclawed Pi Default",
            description: "Use the built-in Enclawed Pi runtime.",
          },
          {
            id: "codex",
            label: "codex",
