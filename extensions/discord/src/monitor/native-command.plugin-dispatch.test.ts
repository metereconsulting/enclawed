...
import type { NativeCommandSpec } from "@enclawed/plugin-sdk/command-auth";
import { resolveDirectStatusReplyForSession } from "@enclawed/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "@enclawed/plugin-sdk/reply-dispatch-runtime";
...
function createConfig(): EnclawedConfig {
  return {
    channels: {
      discord: {
...
async function createNativeCommand(cfg: EnclawedConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: EnclawedConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("@enclawed/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("@enclawed/plugin-sdk/plugin-runtime").executePluginCommand,
    );
