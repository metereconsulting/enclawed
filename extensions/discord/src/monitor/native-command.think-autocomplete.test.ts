...
} from "@enclawed/plugin-sdk/plugin-test-runtime";
import { clearSessionStoreCacheForTest } from "@enclawed/plugin-sdk/session-store-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { ChannelType, type AutocompleteInteraction } from "../internal/discord.js";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("@enclawed/plugin-sdk/conversation-binding-runtime");
...
vi.mock("@enclawed/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("@enclawed/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("@enclawed/plugin-sdk/conversation-binding-runtime")>(
        "@enclawed/plugin-sdk/conversation-binding-runtime",
      ),
  );
});

vi.mock("@enclawed/plugin-sdk/agent-runtime", () => ({
  normalizeProviderId: (value: string) => value.trim().toLowerCase(),
  resolveDefaultModelForAgent: (params: { cfg: EnclawedConfig }) => {
...
vi.mock("@enclawed/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `enclawed-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("@enclawed/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("@enclawed/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("@enclawed/plugin-sdk/command-auth");
