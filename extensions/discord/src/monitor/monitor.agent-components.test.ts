...
import { peekSystemEvents, resetSystemEventsForTest } from "@enclawed/plugin-sdk/test-fixtures";
...
describe("agent components", () => {
  ...
  const createCfg = (): EnclawedConfig => ({}) as EnclawedConfig;
  ...
  it("sends pairing reply when DM sender is not allowlisted", async () => {
    ...
    expect(pairingText).toContain(`enclawed pairing approve discord ${code}`);
