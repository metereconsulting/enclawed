...
let listNativeCommandSpecs: typeof import("@enclawed/plugin-sdk/command-auth").listNativeCommandSpecs;
...
function createNativeCommand(
  ...
  opts?: {
    cfg?: EnclawedConfig;
    discordConfig?: NonNullable<EnclawedConfig["channels"]>["discord"];
  },
  ...
  const baseCfg: EnclawedConfig = opts?.cfg ?? {};
  const discordConfig: NonNullable<EnclawedConfig["channels"]>["discord"] =
...
describe("createDiscordNativeCommand option wiring", () => {
  beforeAll(async () => {
    ({ listNativeCommandSpecs } = await import("@enclawed/plugin-sdk/command-auth"));
  ...
  it("returns no autocomplete choices for group DMs outside dm.groupChannels", async () => {
    ...
    } satisfies NonNullable<EnclawedConfig["channels"]>["discord"];
    const command = createNativeCommand("think", {
      cfg: {
        commands: {
          allowFrom: {
  ...
  it("truncates Discord command and option descriptions to Discord's limit", () => {
    ...
    const discordConfig = {} as NonNullable<EnclawedConfig["channels"]>["discord"];
