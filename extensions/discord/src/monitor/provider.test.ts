...
let runtimeEnvModule: typeof import("@enclawed/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): EnclawedConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("@enclawed/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("@enclawed/plugin-sdk/runtime-env");
  ...
  it("passes Enclawed event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
