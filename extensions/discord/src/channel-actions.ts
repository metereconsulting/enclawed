import { createUnionActionGate } from "@enclawed/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "@enclawed/plugin-sdk/channel-contract";
import type { DiscordActionConfig, EnclawedConfig } from "@enclawed/plugin-sdk/config-types";
...
function listDiscoverableDiscordAccounts(cfg: EnclawedConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: EnclawedConfig) {
