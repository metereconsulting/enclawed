import { createTestWizardPrompter } from "@enclawed/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "@enclawed/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "@enclawed/plugin-sdk/runtime-env";
import { withEnv, withEnvAsync } from "@enclawed/plugin-sdk/test-env";
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      await expect(maybeTool.execute({ query: "Enclawed" })).resolves.toMatchObject({
