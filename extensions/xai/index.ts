...
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "@enclawed/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "@enclawed/plugin-sdk/provider-stream-shared";
import { jsonResult, readProviderEnvValue } from "@enclawed/plugin-sdk/provider-web-search";
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          docs: "https://docs.enclawed.ai/tools/code-execution",
        });
      }
