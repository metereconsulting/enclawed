import { buildPluginConfigSchema } from "@enclawed/plugin-sdk/plugin-entry";
import { z } from "@enclawed/plugin-sdk/zod";
import type { EnclawedPluginConfigSchema } from "../api.js";
import {
...
type DiffsPluginConfig = {
  ...
  /**
   * Per-file maximum hunk-line count. If any file inside a rendered patch
   * exceeds this number of unified/split diff lines, rendering aborts with
   * an actionable error. 0 means "no per-file cap" (the global
   * MAX_PATCH_TOTAL_LINES still applies). Default: 0 (off).
   */
  maxHunkLines?: number;
  defaults?: {
    fontFamily?: string;
...
const DiffsPluginJsonSchemaSource = z.strictObject({
  viewerBaseUrl: z
    ...
    .optional(),
  maxHunkLines: z.number().int().min(0).max(1_000_000).optional(),
  defaults: z
    .strictObject({
...
export const diffsPluginConfigSchema = {
...
} satisfies EnclawedPluginConfigSchema;
...
function buildDiffsPluginConfigShape(config: DiffsPluginConfig): DiffsPluginConfig {
  ...
  const maxHunkLines = resolveDiffsPluginMaxHunkLines(config);
  return {
    ...(viewerBaseUrl !== undefined ? { viewerBaseUrl } : {}),
    ...(maxHunkLines !== undefined ? { maxHunkLines } : {}),
...
export function resolveDiffsPluginMaxHunkLines(config: unknown): number | undefined {
  if (!config || typeof config !== "object" || Array.isArray(config)) {
    return undefined;
  }
  const value = (config as DiffsPluginConfig).maxHunkLines;
