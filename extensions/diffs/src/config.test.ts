...
import {
  ...
  resolveDiffsPluginMaxHunkLines,
...
function compileManifestConfigSchema() {
  const manifest = JSON.parse(
    fs.readFileSync(new URL("../enclawed.plugin.json", import.meta.url), "utf8"),
...
describe("resolveDiffsPluginMaxHunkLines", () => {
  it("returns undefined when missing", () => {
    expect(resolveDiffsPluginMaxHunkLines(undefined)).toBeUndefined();
    expect(resolveDiffsPluginMaxHunkLines({})).toBeUndefined();
  });

  it("reads positive integers as a per-file cap", () => {
    expect(resolveDiffsPluginMaxHunkLines({ maxHunkLines: 500 })).toBe(500);
  });

  it("ignores non-numeric / negative values", () => {
    expect(resolveDiffsPluginMaxHunkLines({ maxHunkLines: -3 })).toBeUndefined();
    expect(
      resolveDiffsPluginMaxHunkLines({ maxHunkLines: Number.NaN as unknown as number }),
    ).toBeUndefined();
  });

  it("rejects through the strict schema at validation time when negative", () => {
    const result = diffsPluginConfigSchema.safeParse({ maxHunkLines: -1 });
    expect(result.success).toBe(false);
  });

  it("accepts maxHunkLines=0 (cap disabled) through the schema", () => {
    const result = diffsPluginConfigSchema.safeParse({ maxHunkLines: 0 });
...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/enclawed/",
      }),
    ).toBe("https://example.com/enclawed");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/enclawed?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/enclawed#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/enclawed/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/enclawed",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/enclawed/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("enclawedDiffsReady");
