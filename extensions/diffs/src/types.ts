...
export type DiffRenderOptions = {
  ...
  /**
   * Per-file unified/split diff-line cap. When set, render aborts if any
   * single file in a patch exceeds this number. 0 / undefined means no
   * per-file cap (only the global MAX_PATCH_TOTAL_LINES applies). Sourced
   * from `extensions.diffs.maxHunkLines` in the host config.
   */
  maxHunkLines?: number;
};
