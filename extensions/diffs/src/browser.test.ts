...
import type { EnclawedPluginApi, EnclawedPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("enclawed-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<EnclawedPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: EnclawedPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<EnclawedPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as EnclawedPluginApi);
