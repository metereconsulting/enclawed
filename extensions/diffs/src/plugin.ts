...
import { resolvePreferredEnclawedTmpDir, type EnclawedPluginApi } from "../api.js";
import {
...
export function registerDiffsPlugin(api: EnclawedPluginApi): void {
  ...
  const maxHunkLines = resolveDiffsPluginMaxHunkLines(api.pluginConfig);
  const store = new DiffArtifactStore({
    rootDir: path.join(resolvePreferredEnclawedTmpDir(), "enclawed-diffs"),
    logger: api.logger,
  });
  ...
  api.registerTool(
    (ctx) =>
      createDiffsTool({
        ...
        defaults,
        viewerBaseUrl,
        ...(typeof maxHunkLines === "number" ? { maxHunkLines } : {}),
        context: ctx,
      }),
    {
      name: "diffs",
