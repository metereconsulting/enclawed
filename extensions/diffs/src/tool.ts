...
import type { AnyAgentTool, EnclawedPluginApi, EnclawedPluginToolContext } from "../api.js";
...
export function createDiffsTool(params: {
  ...
  /**
   * Per-file unified/split line cap, sourced from
   * `extensions.diffs.maxHunkLines`. 0 / undefined disables the cap.
   */
  maxHunkLines?: number;
    ...
    execute: async (_toolCallId, rawParams) => {
      ...
      const rendered = await renderDiffDocument(
        input,
        {
          ...
          image,
          expandUnchanged,
          ...(typeof params.maxHunkLines === "number" && params.maxHunkLines > 0
            ? { maxHunkLines: params.maxHunkLines }
            : {}),
        },
        renderTarget,
      );
...
function buildArtifactContext(
  context: EnclawedPluginToolContext | undefined,
