...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-enclawed-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-enclawed-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
async function renderPatchDiff(
  ...
  const perFileCap = options.maxHunkLines;
  if (typeof perFileCap === "number" && perFileCap > 0) {
    for (const fileDiff of files) {
      const splitLines = Number.isFinite(fileDiff.splitLineCount) ? fileDiff.splitLineCount : 0;
      const unifiedLines = Number.isFinite(fileDiff.unifiedLineCount)
        ? fileDiff.unifiedLineCount
        : 0;
      const fileLines = Math.max(splitLines, unifiedLines, 0);
      if (fileLines > perFileCap) {
        const name = (fileDiff as { path?: string; newPath?: string; oldPath?: string }).path
          ?? (fileDiff as { newPath?: string }).newPath
          ?? (fileDiff as { oldPath?: string }).oldPath
          ?? "(unknown)";
        throw new Error(
          `Patch file ${name} exceeds the configured maxHunkLines cap (${fileLines} > ${perFileCap}). ` +
            `Raise extensions.diffs.maxHunkLines in your config to allow larger diffs.`,
        );
      }
    }
  }
