...
Read-only diff viewer plugin for **Enclawed** agents.
...
Set plugin-wide defaults in `~/.enclawed/enclawed.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/enclawed",
        },
      },
    },
  },
...
Before:
Enclawed supports plugins.

After:
Enclawed supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/enclawed-diffs`).
