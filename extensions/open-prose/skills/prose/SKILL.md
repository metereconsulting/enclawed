...
metadata: { "enclawed": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Enclawed `sessions_spawn`
- **File I/O** == Enclawed `read`/`write`
- **Remote fetch** == Enclawed `web_fetch` (or `exec` with curl when POST is required)
