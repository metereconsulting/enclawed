...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): EnclawedConfig {
    const defaults: NonNullable<NonNullable<EnclawedConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: EnclawedConfig) {
