...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "@enclawed/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("@enclawed/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createQmdCfg(agentId: string): EnclawedConfig {
  return {
...
function createBuiltinCfg(agentId: string): EnclawedConfig {
  return {
    agents: {
      defaults: {
...
describe("getMemorySearchManager caching", () => {
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-qmd-workspace-"));
