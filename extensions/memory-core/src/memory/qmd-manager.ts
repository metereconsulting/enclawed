...
import { withFileLock } from "@enclawed/plugin-sdk/file-lock";
import {
  ...
  writeFileWithinRoot,
  type EnclawedConfig,
} from "@enclawed/plugin-sdk/memory-core-host-engine-foundation";
import { resolveAgentContextLimits } from "@enclawed/plugin-sdk/memory-core-host-engine-foundation";
import {
  buildSessionEntry,
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so Enclawed search semantics match direct qmd search.
  return trimmed;
}
...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private readonly cfg: EnclawedConfig;
