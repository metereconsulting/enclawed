...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("enclawed.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("enclawed.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("enclawed.qmdEmbedQueueTail");
...
vi.mock("@enclawed/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("@enclawed/plugin-sdk/memory-core-host-engine-foundation")
  >("@enclawed/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/file-lock")>(
    "@enclawed/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
} from "@enclawed/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: EnclawedConfig }) {
