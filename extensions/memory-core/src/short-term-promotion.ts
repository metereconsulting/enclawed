...
import { formatMemoryDreamingDay } from "@enclawed/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "@enclawed/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "enclawed-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*enclawed-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*enclawed-memory-promotion:([^\n]+?)\s*-->/gi);
