...
import { resolveMemoryDreamingConfig } from "@enclawed/plugin-sdk/memory-core-host-status";
import { normalizeLowercaseStringOrEmpty } from "@enclawed/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: EnclawedConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: EnclawedConfig, enabled: boolean): EnclawedConfig {
...
function formatStatus(cfg: EnclawedConfig): string {
...
export function registerDreamingCommand(api: EnclawedPluginApi): void {
  api.registerCommand({
    name: "dreaming",
