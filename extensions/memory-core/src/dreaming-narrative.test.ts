...
import * as configRuntimeModule from "@enclawed/plugin-sdk/config-runtime";
import {
  RequestScopedSubagentRuntimeError,
  SUBAGENT_RUNTIME_REQUEST_SCOPE_ERROR_CODE,
} from "@enclawed/plugin-sdk/error-runtime";
import * as memoryCoreHostRuntimeCoreModule from "@enclawed/plugin-sdk/memory-core-host-runtime-core";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("enclawed.memoryCore.dreamingNarrative.fileLocks");
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("enclawed:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("enclawed-dreaming-backfill-");
    ...
    expect(content.match(/enclawed:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("enclawed-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- enclawed:dreaming:diary:start -->");
    expect(content).toContain("<!-- enclawed:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- enclawed:dreaming:diary:start -->");
    const end = content.indexOf("<!-- enclawed:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- enclawed:dreaming:light:start -->\n- Candidate: test\n<!-- enclawed:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("enclawed-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("enclawed-dreaming-narrative-");
    const stateDir = await createTempWorkspace("enclawed-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("enclawed-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("enclawed-dreaming-narrative-");
