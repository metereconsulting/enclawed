...
import { resolveMemoryRemDreamingConfig } from "@enclawed/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "@enclawed/plugin-sdk/routing";
...
function resolveMemoryPluginConfig(cfg: EnclawedConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredEnclawedTmpDir(), "enclawed-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: EnclawedConfig): string {
...
function resolveAgent(cfg: EnclawedConfig, agent?: string) {
...
function resolveAgentIds(cfg: EnclawedConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: enclawed memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredEnclawedTmpDir(), "enclawed-rem-backfill-"),
      );
      try {
