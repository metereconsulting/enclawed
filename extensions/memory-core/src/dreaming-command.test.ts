...
function resolveStoredDreaming(config: EnclawedConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: EnclawedConfig = {}) {
  const registered: { command?: EnclawedPluginCommandDefinition } = {};
  let runtimeConfig: EnclawedConfig = initialConfig;

  const runtime = {
    config: {
      loadConfig: vi.fn(() => runtimeConfig),
      writeConfigFile: vi.fn(async (nextConfig: EnclawedConfig) => {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: EnclawedPluginCommandDefinition) => {
      registered.command = definition;
    }),
  } as unknown as EnclawedPluginApi;
