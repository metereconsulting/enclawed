import { createProviderUsageFetch, makeResponse } from "@enclawed/plugin-sdk/test-env";
...
vi.mock("@enclawed/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("@enclawed/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/enclawed-state",
}));

import type { ProviderResolveDynamicModelContext } from "@enclawed/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/enclawed-state/credentials/github-copilot.token.json";
