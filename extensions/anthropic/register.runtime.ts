import { formatCliCommand, parseDurationMs } from "@enclawed/plugin-sdk/cli-runtime";
import type {
  EnclawedPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type EnclawedConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "@enclawed/plugin-sdk/provider-auth";
import {
  cloneFirstTemplateModel,
  isClaudeOpus47ModelId,
  type ProviderPlugin,
  resolveClaudeThinkingProfile,
} from "@enclawed/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "@enclawed/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("enclawed models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("enclawed models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("enclawed doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: EnclawedPluginApi): void {
