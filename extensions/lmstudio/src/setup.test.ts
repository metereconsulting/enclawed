import { CUSTOM_LOCAL_AUTH_MARKER } from "@enclawed/plugin-sdk/provider-auth";
import type { EnclawedConfig } from "@enclawed/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "@enclawed/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "@enclawed/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "@enclawed/plugin-sdk/provider-setup";
...
vi.mock("@enclawed/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@enclawed/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@enclawed/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
function buildConfig(): EnclawedConfig {
  return {
    models: {
      providers: {
        lmstudio: {
