...
import { withAgentModelAliases } from "@enclawed/plugin-sdk/provider-onboard";
import {
...
import { WizardCancelledError, type WizardPrompter } from "@enclawed/plugin-sdk/setup";
import {
...
function stripLmstudioStoredAuthConfig(cfg: EnclawedConfig): EnclawedConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<EnclawedConfig["agents"]>["defaults"]>["models"];
  discoveredModels: ModelDefinitionConfig[];
}) {
...
export async function prepareLmstudioDynamicModels(
  ...
  return discoveredModels.map((model) => {
    const { input, api: _modelApi, baseUrl: _modelBaseUrl, ...rest } = model;
    const textImageInput = input.filter(
