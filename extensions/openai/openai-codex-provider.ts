...
import { buildOauthProviderAuthResult } from "@enclawed/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "@enclawed/plugin-sdk/provider-auth-login";
import {
...
import { fetchCodexUsage } from "@enclawed/plugin-sdk/provider-usage";
import { normalizeLowercaseStringOrEmpty, readStringValue } from "@enclawed/plugin-sdk/text-runtime";
import {
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.enclawed.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `enclawed models auth login --provider openai-codex` or `enclawed configure`.";
}
