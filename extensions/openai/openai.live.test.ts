...
import type { ResolvedTtsConfig } from "@enclawed/plugin-sdk/agent-runtime";
import type { EnclawedConfig } from "@enclawed/plugin-sdk/config-types";
import { encodePngRgba, fillPixel } from "@enclawed/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "@enclawed/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "@enclawed/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.ENCLAWED_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.ENCLAWED_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.ENCLAWED_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.ENCLAWED_LIVE_TEST === "1";
...
function createLiveConfig(): EnclawedConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "Enclawed integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing Enclawed OpenAI realtime transcription integration test OK.";
