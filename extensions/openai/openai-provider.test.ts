...
vi.mock("@enclawed/plugin-sdk/provider-stream-family", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@enclawed/plugin-sdk/provider-stream-family")>();
...
describe("buildOpenAIProvider", () => {
  ...
  it("owns Codex wrapper composition for responses payloads", () => {
    ...
    expect(result.options?.headers).toMatchObject({
      originator: "enclawed",
      "User-Agent": expect.stringMatching(/^enclawed\//u),
    });
