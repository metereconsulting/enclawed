...
} from "@enclawed/plugin-sdk/proxy-capture";
...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds Enclawed attribution headers to native OpenAI speech requests", async () => {
      vi.stubEnv("ENCLAWED_VERSION", "2026.3.22");
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.ENCLAWED_DEBUG_PROXY_ENABLED = "1";
      process.env.ENCLAWED_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
      process.env.ENCLAWED_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
      process.env.ENCLAWED_DEBUG_PROXY_SESSION_ID = "tts-session";
      ...
      const store = getDebugProxyCaptureStore(
        process.env.ENCLAWED_DEBUG_PROXY_DB_PATH,
        process.env.ENCLAWED_DEBUG_PROXY_BLOB_DIR,
      );
      store.upsertSession({
        id: "tts-session",
        startedAt: Date.now(),
        mode: "test",
        sourceScope: "enclawed",
        sourceProcess: "enclawed",
        dbPath: process.env.ENCLAWED_DEBUG_PROXY_DB_PATH,
        blobDir: process.env.ENCLAWED_DEBUG_PROXY_BLOB_DIR,
      });
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.ENCLAWED_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
      process.env.ENCLAWED_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
      process.env.ENCLAWED_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
