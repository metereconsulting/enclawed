import { getExecApprovalReplyMetadata } from "@enclawed/plugin-sdk/approval-runtime";
...
import { toGatewayAccount, writeEnclawedConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped enclawed.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // enclawed.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeEnclawedConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe enclawed.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeEnclawedConfigThroughRuntime(getQQBotRuntime(), nextCfg as EnclawedConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as EnclawedConfig, accountId);
