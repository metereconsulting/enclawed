 ...
 *   2. `$ENCLAWED_STATE_DIR` / `$CLAWDBOT_STATE_DIR`
 *   3. `~/.enclawed/agents/{agentId}/sessions/sessions.json`
 */
function resolveSessionStorePath(
  ...
  const stateDir =
    process.env.ENCLAWED_STATE_DIR?.trim() ||
    process.env.CLAWDBOT_STATE_DIR?.trim() ||
    path.join(process.env.HOME || process.env.USERPROFILE || "", ".enclawed");
