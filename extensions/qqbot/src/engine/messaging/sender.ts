...
let _enclawedVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${_pluginVersion} (Node/${process.versions.node}; ${os.platform()}; Enclawed/${_enclawedVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; enclawedVersion?: string }): void {
  if (options.pluginVersion) {
    _pluginVersion = options.pluginVersion;
  }
  if (options.enclawedVersion) {
    _enclawedVersion = options.enclawedVersion;
  }
}

/** Update the Enclawed framework version in the User-Agent (called after runtime injection). */
export function setEnclawedVersion(version: string): void {
  if (version) {
    _enclawedVersion = version;
  }
}
