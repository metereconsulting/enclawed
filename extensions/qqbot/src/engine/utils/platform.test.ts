...
describe("qqbot local media path remapping", () => {
  ...
  function createEnclawedTestRoot() {
    const actualHome = getHomeDir();
    const enclawedDir = path.join(actualHome, ".enclawed");
    fs.mkdirSync(enclawedDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(enclawedDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createEnclawedTestRoot();
  ...
  it("allows structured payload files inside sibling Enclawed media subdirectories", () => {
    ...
    // trust the shared `~/.enclawed/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".enclawed", "media", "outbound");
