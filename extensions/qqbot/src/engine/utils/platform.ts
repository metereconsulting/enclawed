 ...
 * `@enclawed/plugin-sdk`. The temp-directory fallback is delegated
...
/** Return a path under `~/.enclawed/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".enclawed", "qqbot", ...subPaths);
}

/** Return a path under `~/.enclawed/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.enclawed/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Enclawed's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".enclawed", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.enclawed/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.enclawed/media`, Enclawed's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `enclawed/src/media/local-roots.ts`). Using it as a
 ...
 */
function getEnclawedMediaDir(): string {
  return path.join(getHomeDir(), ".enclawed", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".enclawed", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Enclawed's shared `~/.enclawed/media`
  ...
  const allowedRoots = [getEnclawedMediaDir(), getQQBotMediaPath()];
