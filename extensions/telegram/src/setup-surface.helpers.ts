...
export function shouldShowTelegramDmAccessWarning(cfg: EnclawedConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`enclawed config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`enclawed config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
