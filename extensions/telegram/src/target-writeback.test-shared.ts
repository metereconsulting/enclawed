...
vi.mock("@enclawed/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/config-mutation")>(
    "@enclawed/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/cron-store-runtime")>(
    "@enclawed/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      readConfigFileSnapshotForWrite.mockResolvedValue({
        ...
        writeOptions: { expectedConfigPath: "/tmp/enclawed.json" },
      });
      ...
      expect(writeConfigFile).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ expectedConfigPath: "/tmp/enclawed.json" }),
      );
