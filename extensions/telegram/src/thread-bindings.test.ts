...
import { getSessionBindingService } from "@enclawed/plugin-sdk/conversation-runtime";
import { resolveStateDir } from "@enclawed/plugin-sdk/state-paths";
import { importFreshModule } from "@enclawed/plugin-sdk/test-fixtures";
...
vi.mock("@enclawed/plugin-sdk/json-store", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/json-store")>(
    "@enclawed/plugin-sdk/json-store",
  );
...
describe("telegram thread bindings", () => {
  ...
  beforeEach(async () => {
    ...
    const acpRuntime = await vi.importActual<typeof import("@enclawed/plugin-sdk/acp-runtime")>(
      "@enclawed/plugin-sdk/acp-runtime",
    );
  ...
  it("does not persist lifecycle updates when manager persistence is disabled", async () => {
    stateDirOverride = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-telegram-bindings-"));
    process.env.ENCLAWED_STATE_DIR = stateDirOverride;
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-03-06T10:00:00.000Z"));
  ...
  it("persists unbinds before restart so removed bindings do not come back", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:enclawed-codex-app-server:abc123",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
  ...
  it("keeps plugin-owned bindings when ACP cleanup runs on startup", async () => {
    ...
    await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:enclawed-codex-app-server:still-valid",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
    ...
    expect(reloaded.getByConversationId("plugin-binding-convo")?.targetSessionKey).toBe(
      "plugin-binding:enclawed-codex-app-server:still-valid",
    );
