...
function resolveDefaultAgentId(cfg: EnclawedConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: EnclawedConfig): string[] {
...
function listBoundAccountIds(cfg: EnclawedConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: EnclawedConfig, channelId: string): string | null {
...
export function resolveDefaultTelegramAccountSelection(cfg: EnclawedConfig): {
  accountId: string;
  accountIds: string[];
