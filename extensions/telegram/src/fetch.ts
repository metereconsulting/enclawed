...
} from "@enclawed/plugin-sdk/fetch-runtime";
import {
  captureHttpExchange,
  resolveEffectiveDebugProxyUrl,
} from "@enclawed/plugin-sdk/proxy-capture";
import { resolveRequestUrl } from "@enclawed/plugin-sdk/request-url";
...
// fix for the leak observed in enclawed#68128 is the transport lifecycle that
...
function resolveEnclawedProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.ENCLAWED_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveEnclawedProxyUrlForTelegram() : undefined;
