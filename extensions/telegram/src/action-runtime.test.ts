...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): EnclawedConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as EnclawedConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): EnclawedConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as EnclawedConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: EnclawedConfig, accountId = "media") {
    await handleTelegramAction(
