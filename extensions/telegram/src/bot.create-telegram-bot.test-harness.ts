...
import type { GetReplyOptions, MsgContext } from "@enclawed/plugin-sdk/reply-runtime";
...
type GetRuntimeConfigFn =
  typeof import("@enclawed/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("@enclawed/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("@enclawed/plugin-sdk/session-store-runtime").resolveStorePath;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/enclawed-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("@enclawed/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: EnclawedConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: EnclawedConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: EnclawedConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: EnclawedConfig) => {
