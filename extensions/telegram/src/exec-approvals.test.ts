...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<EnclawedConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<EnclawedConfig["channels"]>["telegram"]>>,
): EnclawedConfig {
  return {
    channels: {
      telegram: {
