...
const getAgentScopedMediaLocalRoots = vi.hoisted(() =>
  vi.fn((_cfg: unknown, agentId: string) => [`/tmp/.enclawed/workspace-${agentId}`]),
);
...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  function createContext(overrides?: Partial<TelegramMessageContext>): TelegramMessageContext {
    ...
    base.turn = {
      storePath: "/tmp/enclawed/telegram-sessions.json",
    ...
    expect(deliverReplies).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        mediaLocalRoots: expect.arrayContaining([
          expect.stringMatching(/[\\/]\.enclawed[\\/]workspace-work$/u),
        ]),
      }),
    );
