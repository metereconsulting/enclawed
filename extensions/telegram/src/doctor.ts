...
export function maybeRepairTelegramApiRoots(cfg: EnclawedConfig): {
...
async function repairTelegramConfig(params: { cfg: EnclawedConfig }): Promise<{
  config: EnclawedConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: EnclawedConfig): Promise<{
  config: EnclawedConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("@enclawed/plugin-sdk/runtime-secret-resolution");
