...
vi.mock("@enclawed/plugin-sdk/runtime-secret-resolution", () => {
  return {
...
describe("telegram doctor", () => {
  ...
  it("formats invalid allowFrom warnings", () => {
    ...
    expect(warnings[1]).toContain("enclawed doctor --fix");
  });
  ...
  it("warns and repairs Telegram apiRoot values that include the bot endpoint", () => {
    ...
    expect(
      collectTelegramApiRootWarnings({ hits, doctorFixCommand: "enclawed doctor --fix" }),
    ).toContain(
