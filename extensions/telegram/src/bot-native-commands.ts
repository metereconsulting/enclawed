...
import { resolveDefaultModelForAgent } from "@enclawed/plugin-sdk/agent-runtime";
...
import type { ChannelGroupPolicy } from "@enclawed/plugin-sdk/config-types";
import type {
  ReplyToMode,
...
import { resolveMarkdownTableMode } from "@enclawed/plugin-sdk/markdown-table-runtime";
import { resolveAgentRoute } from "@enclawed/plugin-sdk/routing";
import { getRuntimeConfigSnapshot } from "@enclawed/plugin-sdk/runtime-config-snapshot";
...
type TelegramChunkMode = ReturnType<
  typeof import("@enclawed/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("@enclawed/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): EnclawedConfig => telegramDeps.getRuntimeConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: EnclawedConfig): TelegramAccountConfig => {
    try {
