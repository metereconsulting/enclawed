...
import * as runtimeEnvModule from "@enclawed/plugin-sdk/runtime-env";
import { withEnv } from "@enclawed/plugin-sdk/test-env";
...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", ENCLAWED_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): EnclawedConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): EnclawedConfig => ({
    channels: {
      telegram: {
