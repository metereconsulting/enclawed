...
import type { SpeechVoiceOption } from "@enclawed/plugin-sdk/speech";
import { resolveActiveTalkProviderConfig } from "@enclawed/plugin-sdk/talk-config-runtime";
import {
  ...
  register(api: EnclawedPluginApi) {
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        const cfg = api.runtime.config.current() as EnclawedConfig;
