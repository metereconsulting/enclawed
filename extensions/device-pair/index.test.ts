...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredEnclawedTmpDir: vi.fn(() => path.join(os.tmpdir(), "enclawed-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredEnclawedTmpDir: pluginApiMocks.resolvePreferredEnclawedTmpDir,
...
function createApi(params?: {
  runtime?: EnclawedPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as EnclawedPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: EnclawedPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredEnclawedTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredEnclawedTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Enclawed iOS app:");
    expect(text).toContain("![Enclawed pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Enclawed iOS app:");
