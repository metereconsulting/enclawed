...
import type { EnclawedConfig, RuntimeEnv } from "./runtime-api.js";
...
function createRuntimeCore(cfg: EnclawedConfig) {
  ...
  return {
    ...
    channel: {
      ...
      session: {
        resolveStorePath: () => "/tmp/enclawed-test-sessions.json",
...
describe("mattermost inbound user posts", () => {
  ...
  it("pins direct-message main route updates to the configured owner", async () => {
    ...
    const abortController = new AbortController();
    mockState.abortController = abortController;
    const directConfig: EnclawedConfig = {
      channels: {
        mattermost: {
          enabled: true,
          baseUrl: "https://mattermost.example.com",
