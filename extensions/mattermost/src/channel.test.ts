...
vi.mock("@enclawed/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("@enclawed/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
function getDescribedActions(cfg: EnclawedConfig, accountId?: string): string[] {
