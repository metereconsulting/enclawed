...
const { tokenjuiceFactory, createTokenjuiceEnclawedEmbeddedExtension } = vi.hoisted(() => {
  const tokenjuiceFactory = vi.fn();
  const createTokenjuiceEnclawedEmbeddedExtension = vi.fn(() => tokenjuiceFactory);
  return {
    tokenjuiceFactory,
...
describe("tokenjuice bundled plugin", () => {
  beforeEach(() => {
    createTokenjuiceEnclawedEmbeddedExtension.mockClear();
  ...
  it("registers tokenjuice tool result middleware for Pi and Codex runtimes", () => {
    ...
    expect(createTokenjuiceEnclawedEmbeddedExtension).toHaveBeenCalledTimes(1);
