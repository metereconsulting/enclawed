...
function normalizeDenyList(cfg: EnclawedPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: EnclawedPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: EnclawedPluginApi) {
    ...
    const timerService: EnclawedPluginService = {
