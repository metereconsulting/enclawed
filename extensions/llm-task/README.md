...
`enclawed.invoke --each`) without adding new Enclawed code per workflow.
...
This extension depends on Enclawed internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Enclawed extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.enclawed/extensions` as a standalone plugin directory.
