...
import { runPluginCommandWithTimeout } from "@enclawed/plugin-sdk/run-command";
import type { RuntimeEnv } from "@enclawed/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "@enclawed/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredEnclawedTmpDir(), "enclawed-signal-"));
