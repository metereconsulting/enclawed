import type { MockFn } from "@enclawed/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/enclawed-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("@enclawed/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("@enclawed/plugin-sdk/runtime-config-snapshot")
  >("@enclawed/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/session-store-runtime")>(
    "@enclawed/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/reply-runtime")>(
...
vi.mock("@enclawed/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/conversation-runtime")>(
    "@enclawed/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/security-runtime")>(
    "@enclawed/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/system-event-runtime")>(
    "@enclawed/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
vi.mock("@enclawed/plugin-sdk/transport-ready-runtime", () => ({
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("@enclawed/plugin-sdk/reply-runtime"),
      import("@enclawed/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
