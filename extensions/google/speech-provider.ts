import { transcodeAudioBufferToOpus } from "@enclawed/plugin-sdk/media-runtime";
import {
...
} from "@enclawed/plugin-sdk/speech-core";
import { asObject, trimToUndefined } from "@enclawed/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: EnclawedConfig): string | undefined {
...
function isEnclawedGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isEnclawedGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
