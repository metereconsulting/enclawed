...
describe("matrix harness runtime", () => {
  ...
  it("starts the harness, waits for versions, and exposes a stop command", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml down --remove-orphans @/repo/enclawed`,
        `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml up -d @/repo/enclawed`,
        `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml ps --format json matrix-qa-homeserver @/repo/enclawed`,
      ]);
      ...
      expect(calls).toContain(
        `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml restart matrix-qa-homeserver @/repo/enclawed`,
      );
    } finally {
      await rm(outputDir, { recursive: true, force: true });
    }
  ...
  it("falls back to the container IP when the host port is unreachable", async () => {
    ...
    try {
      ...
      expect(calls).toContain(
        `docker compose -f ${outputDir}/docker-compose.matrix-qa.yml ps -q matrix-qa-homeserver @/repo/enclawed`,
      );
      expect(calls).toContain(
        "docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} container-123 @/repo/enclawed",
      );
    } finally {
      await rm(outputDir, { recursive: true, force: true });
    }
