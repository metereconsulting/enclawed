...
import { loadQaRuntimeModule } from "@enclawed/plugin-sdk/qa-runner-runtime";
...
function shouldWriteMatrixQaProgress() {
  const override = process.env.ENCLAWED_QA_MATRIX_PROGRESS;
  if (override === "0") {
    return false;
  }
  if (override === "1") {
...
async function startMatrixQaLiveLaneGateway(params: {
  ...
  transport: {
    ...
    createGatewayConfig: (params: {
      baseUrl: string;
    }) => Pick<EnclawedConfig, "channels" | "messages">;
  };
...
export async function runMatrixQaLive(params: {
  ...
  const topology = buildMatrixQaTopologyForScenarios({
    defaultRoomName: `Enclawed Matrix QA ${runSuffix}`,
    scenarios,
  });
  const observedEvents: MatrixQaObservedEvent[] = [];
  const includeObservedEventContent = process.env.ENCLAWED_QA_MATRIX_CAPTURE_CONTENT === "1";
    ...
    try {
      return await measureMatrixQaStep(() =>
        provisionMatrixQaRoom({
          ...
          roomName: `Enclawed Matrix QA ${runSuffix}`,
