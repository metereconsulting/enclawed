...
import { normalizeOptionalLowercaseString, sleep } from "@enclawed/plugin-sdk/text-runtime";
import { loadWebMedia } from "@enclawed/plugin-sdk/web-media";
import type { MarkdownTableMode, MSTeamsReplyStyle, EnclawedConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as EnclawedConfig,
      channel: "msteams",
    });
