...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["enclawedPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["enclawed", "pollId"]) ??
    readNestedString(value, ["enclawed", "poll", "id"]) ??
    readNestedString(value, ["data", "enclawedPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "enclawed", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          enclawedPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "enclawed poll vote",
          displayText: "Vote recorded",
          value: { enclawedPollId: pollId, pollId },
        },
      },
    ],
  };
