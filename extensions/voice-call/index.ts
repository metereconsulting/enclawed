...
import { ErrorCodes, errorShape } from "@enclawed/plugin-sdk/gateway-runtime";
...
const voiceCallConfigSchema = {
  ...
  uiHints: {
    ...
    "realtime.toolPolicy": {
      ...
      help: "Controls the shared enclawed_agent_consult tool.",
      advanced: true,
    },
...
function isCliOnlyProcess(): boolean {
  return process.env.ENCLAWED_CLI === "1" && !process.argv.slice(2).includes("gateway");
}

const VOICE_CALL_RUNTIME_KEY = Symbol.for("enclawed.voice-call.runtime");
const VOICE_CALL_RUNTIME_PROMISE_KEY = Symbol.for("enclawed.voice-call.runtimePromise");
const VOICE_CALL_RUNTIME_STOP_PROMISE_KEY = Symbol.for("enclawed.voice-call.runtimeStopPromise");
  ...
  register(api: EnclawedPluginApi) {
    ...
    if (api.pluginConfig && typeof api.pluginConfig === "object") {
      for (const warning of formatVoiceCallLegacyConfigWarnings({
        ...
        doctorFixCommand: "enclawed doctor --fix",
      })) {
