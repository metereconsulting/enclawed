...
vi.mock("@enclawed/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: enclawed", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "enclawed" });
  });

  it("runTavilyExtract sends X-Client-Source: enclawed", async () => {
