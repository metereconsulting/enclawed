...
import { canResolveEnvSecretRefInReadOnlyPath } from "@enclawed/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "@enclawed/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<EnclawedConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<EnclawedConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: EnclawedConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: EnclawedConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: EnclawedConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: EnclawedConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: EnclawedConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: EnclawedConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: EnclawedConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: EnclawedConfig, override?: number): number {
