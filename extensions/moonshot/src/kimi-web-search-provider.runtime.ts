...
function resolveKimiBaseUrl(kimi?: KimiConfig, enclawedConfig?: EnclawedConfig): string {
  ...
  const moonshotBaseUrl = enclawedConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: EnclawedConfig; searchConfig?: SearchConfigRecord },
