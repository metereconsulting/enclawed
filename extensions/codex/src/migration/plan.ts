...
} from "@enclawed/plugin-sdk/migration";
import type {
  MigrationItem,
  MigrationPlan,
      ...
      createMigrationItem({
        ...
        reason: collides
          ...
          : targetExists && !params.overwrite
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: `Copy ${item.skill.sourceLabel} into this Enclawed agent workspace.`,
        details: {
          skillName: item.name,
...
export async function buildCodexMigrationPlan(
  ...
  for (const [index, plugin] of source.plugins.entries()) {
    items.push(
      createMigrationManualItem({
        ...
        recommendation:
          "Review the plugin bundle first, then install trusted compatible plugins with enclawed plugins install <path>.",
      }),
    );
  }
  ...
  const warnings = [
    ...
    ...(source.plugins.length > 0
      ? [
          "Codex native plugins are reported for manual review only. Enclawed does not auto-activate plugin bundles, hooks, MCP servers, or apps from another Codex home.",
        ]
      : []),
    ...(source.archivePaths.length > 0
      ? [
          "Codex config and hook files are archive-only. They are preserved in the migration report, not loaded into Enclawed automatically.",
        ]
      : []),
  ];
  return {
    ...
    warnings,
    nextSteps: [
      "Run enclawed doctor after applying the migration.",
      "Review skipped Codex plugin/config/hook items before installing or recreating them in Enclawed.",
    ],
    metadata: {
