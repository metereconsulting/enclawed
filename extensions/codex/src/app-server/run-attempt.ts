...
type EnclawedCodingToolsOptions = NonNullable<
  Parameters<(typeof import("@enclawed/plugin-sdk/agent-harness"))["createEnclawedCodingTools"]>[0]
>;
...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `Enclawed dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `Enclawed dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedDynamicToolResponse("Enclawed dynamic tool call aborted before execution.");
  }
  ...
  const abortFromRun = () => {
    const message = "Enclawed dynamic tool call aborted.";
  ...
  try {
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("Enclawed dynamic tool call finished."));
    }
  }
}
...
function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("enclawed:codex:native-hook-relay:v1");
  hash.update("\0");
...
async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const agentDir = params.agentDir ?? resolveEnclawedAgentDir();
  const { createEnclawedCodingTools } = await import("@enclawed/plugin-sdk/agent-harness");
  const allTools = createEnclawedCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as EnclawedCodingToolsOptions["modelCompat"])
        : undefined,
