...
import { createMockPluginRegistry } from "@enclawed/plugin-sdk/plugin-test-runtime";
...
function threadStartResult(threadId = "thread-1") {
  return {
    thread: {
      ...
      status: { type: "idle" },
      path: null,
      cwd: tempDir || "/tmp/enclawed-codex-test",
      cliVersion: "0.125.0",
      source: "unknown",
      agentNickname: null,
      agentRole: null,
...
describe("runCodexAppServerAttempt", () => {
  beforeEach(async () => {
    resetAgentEventsForTest();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-codex-run-"));
  });
  ...
  it("allows Codex dynamic tool filtering to opt back into Enclawed compatibility", () => {
    const tools = ["read", "exec", "message", "custom_tool"].map((name) => ({ name }));

    expect(
      __testing
        .applyCodexDynamicToolProfile(tools, {
          codexDynamicToolsProfile: "enclawed-compat",
  ...
  it("starts Codex threads without duplicate Enclawed workspace tools by default", async () => {
  ...
  it("returns a failed dynamic tool response when an app-server tool call exceeds the deadline", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Enclawed dynamic tool call timed out after 1ms while running tool message.",
        },
      ],
    });
  ...
  it("logs process poll timeout context separately from session idle", async () => {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Enclawed dynamic tool call timed out after 1ms while waiting for process action=poll sessionId=rapid-crustacean. This is a tool RPC timeout, not a session idle timeout.",
        },
      ],
    });
  ...
  it("releases the session when Codex never completes after a dynamic tool response", async () => {
    ...
    await expect(
      ...
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown Enclawed tool: message" }],
    });
  ...
  it("releases completion when a projector callback throws during turn/completed", async () => {
    // Regression for enclawed/enclawed#67996: a throw inside the projector's
  ...
  it("builds resume and turn params from the currently selected Enclawed model", () => {
