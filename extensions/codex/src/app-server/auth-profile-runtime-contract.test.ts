...
import { AUTH_PROFILE_RUNTIME_CONTRACT } from "@enclawed/plugin-sdk/agent-runtime-test-contracts";
...
describe("Auth profile runtime contract - Codex app-server adapter", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-codex-auth-contract-"));
  });
