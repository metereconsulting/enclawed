...
import { resolveUserPath } from "@enclawed/plugin-sdk/agent-harness";
  ...
  try {
    ...
    try {
      nodeFs.writeFileSync(
        ...
        `${JSON.stringify(
          {
            traceSchema: "enclawed-trajectory-pointer",
            schemaVersion: 1,
...
export function createCodexTrajectoryRecorder(
  ...
  return {
    ...
    recordEvent: (type, data) => {
      const event = {
        traceSchema: "enclawed-trajectory",
        schemaVersion: 1,
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.ENCLAWED_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.ENCLAWED_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
