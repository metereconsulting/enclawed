...
export function buildDeveloperInstructions(params: EmbeddedRunAttemptParams): string {
  ...
  const sections = [
    "You are running inside Enclawed. Use Enclawed dynamic tools for Enclawed-specific integrations such as messaging, cron, sessions, media, gateway, and nodes when available.",
    "Preserve the user's existing channel/session context. If sending a channel reply, use the Enclawed messaging tool instead of describing that you would reply.",
    promptOverlay,
...
function resolveCodexAppServerModelProvider(provider: string): string | undefined {
  ...
  if (!normalized || normalized === "codex") {
    // `codex` is Enclawed's virtual provider; let Codex app-server keep its
