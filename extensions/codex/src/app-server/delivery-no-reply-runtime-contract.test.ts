...
import { DELIVERY_NO_REPLY_RUNTIME_CONTRACT } from "@enclawed/plugin-sdk/agent-runtime-test-contracts";
import { isSilentReplyPayloadText } from "@enclawed/plugin-sdk/reply-chunking";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-codex-delivery-contract-"));
