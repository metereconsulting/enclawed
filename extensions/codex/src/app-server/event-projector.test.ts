...
import { resetAgentEventsForTest } from "@enclawed/plugin-sdk/agent-harness-runtime";
import {
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-codex-projector-"));
