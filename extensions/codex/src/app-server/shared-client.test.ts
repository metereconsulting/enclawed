...
const mocks = vi.hoisted(() => ({
  ...
  resolveEnclawedAgentDir: vi.fn(() => "/tmp/enclawed-agent"),
}));
...
vi.mock("@enclawed/plugin-sdk/agent-harness-runtime", () => ({
  embeddedAgentLog: mocks.embeddedAgentLog,
  ENCLAWED_VERSION: "test",
}));
...
describe("shared Codex app-server client", () => {
  ...
  afterEach(() => {
    ...
    mocks.resolveEnclawedAgentDir.mockClear();
  });
  ...
  it("closes the shared app-server when the version gate fails", async () => {
    ...
    await sendInitializeResult(harness, "enclawed/0.117.9 (macOS; test)");
  ...
  it("closes and clears a shared app-server when initialize times out", async () => {
    ...
    await sendInitializeResult(second, "enclawed/0.125.0 (macOS; test)");
  ...
  it("passes the selected auth profile through the bridge helper", async () => {
    ...
    await sendInitializeResult(harness, "enclawed/0.125.0 (macOS; test)");
  ...
  it("uses the selected agent dir for shared app-server auth bridging", async () => {
    ...
    const listPromise = listCodexAppServerModels({
      timeoutMs: 1000,
      authProfileId: "openai-codex:work",
      agentDir: "/tmp/enclawed-agent-nova",
    });
    await sendInitializeResult(harness, "enclawed/0.125.0 (macOS; test)");
    await sendEmptyModelList(harness);
  ...
  it("restarts the shared client when the bridged auth token changes", async () => {
    ...
    await sendInitializeResult(first, "enclawed/0.125.0 (macOS; test)");
    ...
    server.on("connection", (socket, request) => {
      ...
      socket.on("message", (data) => {
        ...
        if (message.method === "initialize") {
          socket.send(
            JSON.stringify({ id: message.id, result: { userAgent: "enclawed/0.125.0" } }),
          );
          return;
        }
