import type { AnyAgentTool } from "@enclawed/plugin-sdk/agent-harness";
import { wrapToolWithBeforeToolCallHook } from "@enclawed/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installEnclawedOwnedToolHooks,
  mediaToolResult,
  resetEnclawedOwnedToolHooks,
  textToolResult,
...
describe("Enclawed-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetEnclawedOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installEnclawedOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installEnclawedOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installEnclawedOwnedToolHooks();
