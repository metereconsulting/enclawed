...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage Enclawed's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via Enclawed's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated Enclawed-managed browser (`enclawed`).",
...
export const browserPluginNodeHostCommands: EnclawedPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: EnclawedPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): EnclawedPluginService {
  let service: EnclawedPluginService | null = null;
...
export function registerBrowserPlugin(api: EnclawedPluginApi) {
  api.registerTool(((ctx: EnclawedPluginToolContext) =>
