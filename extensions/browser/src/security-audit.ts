import type { EnclawedPluginSecurityAuditContext } from "@enclawed/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "@enclawed/plugin-sdk/secret-input";
import { formatCliCommand } from "@enclawed/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "@enclawed/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: EnclawedPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("enclawed security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.ENCLAWED_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.ENCLAWED_GATEWAY_PASSWORD) ||
