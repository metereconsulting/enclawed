...
describe("registerBrowserCli lazy browser subcommands", () => {
  ...
  it("registers browser placeholders without loading handlers for help", () => {
    ...
    registerBrowserCli(program, ["node", "enclawed", "browser", "--help"]);
  ...
  it("registers only the requested browser group before dispatch", async () => {
    ...
    registerBrowserCli(program, ["node", "enclawed", "browser", "status"]);
  ...
  it("loads browser doctor from the manage group so --deep is available", async () => {
    ...
    registerBrowserCli(program, ["node", "enclawed", "browser", "doctor", "--deep"]);
  ...
  it("preserves parent --json while reparsing lazy manage commands", async () => {
    ...
    registerBrowserCli(program, ["node", "enclawed", "browser", "--json", "open", "about:blank"]);
    ...
    tabsProgram.name("enclawed");
    registerBrowserCli(tabsProgram, ["node", "enclawed", "browser", "--json", "tabs"]);
  ...
  it("can eagerly register all browser groups for compatibility", async () => {
    vi.stubEnv("ENCLAWED_DISABLE_LAZY_SUBCOMMANDS", "1");
    const program = new Command();
    program.name("enclawed");
