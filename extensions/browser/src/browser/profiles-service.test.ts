...
import { resolveEnclawedUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveEnclawedUserDataDir: vi.fn(() => "/tmp/enclawed-test/enclawed/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "enclawed-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveEnclawedUserDataDir).mockReturnValue(userDataDir);
