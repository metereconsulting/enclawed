...
const { stopEnclawedChromeMock } = vi.hoisted(() => ({
  stopEnclawedChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopEnclawedChrome: stopEnclawedChromeMock,
}));
...
beforeEach(() => {
  ...
  stopEnclawedChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["enclawed", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      enclawed: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.enclawed).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopEnclawedChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("enclawed browser stop failed: Error: oops");
  });
});
