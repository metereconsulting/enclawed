...
function localEnclawedProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "enclawed",
...
function createLocalEnclawedResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localEnclawedProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveEnclawedUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-reset-"));
    const profileDir = path.join(tempRoot, "enclawed");
    ...
    const ops = createLocalEnclawedResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveEnclawedUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-reset-no-own-"));
    const profileDir = path.join(tempRoot, "enclawed");
    fs.mkdirSync(profileDir, { recursive: true });
