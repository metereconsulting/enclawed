...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".enclawed-profile-decorated");
}
 ...
 */
export function decorateEnclawedProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ENCLAWED_BROWSER_COLOR).toUpperCase();
