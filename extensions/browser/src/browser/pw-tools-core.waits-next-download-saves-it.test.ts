...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredEnclawedTmpDir: vi.fn(() => "/tmp/enclawed"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-enclawed-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-enclawed-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredEnclawedTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredEnclawedTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredEnclawedTmpDir.mockReturnValue("/tmp/enclawed");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".enclawed-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredEnclawedTmpDir.mockReturnValue("/tmp/enclawed-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "enclawed-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "enclawed-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredEnclawedTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "enclawed-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "enclawed-preferred", "downloads")}${path.sep}`),
    );
  });
