...
const resolvePreferredEnclawedTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/enclawed"));
const ENCLAWED_TMP_ROOT = "/tmp/enclawed";
const TRASH_SOURCE = `${ENCLAWED_TMP_ROOT}/demo`;
...
describe("browser trash", () => {
  beforeEach(() => {
    ...
    resolvePreferredEnclawedTmpDirMock.mockReturnValue("/tmp/enclawed");
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    await expect(movePathToTrash("/etc/enclawed-demo")).rejects.toThrow(
