...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    // Start with only enclawed profile
    ...
    // 2. Simulate adding a new profile to config (like user editing enclawed.json)
  ...
  it("forProfile refreshes existing profile config after getRuntimeConfig cache updates", async () => {
    ...
    mockState.cfgProfiles.enclawed = { cdpPort: 19999, color: "#FF4500" };
    ...
    expect(state.resolved.profiles.enclawed?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", async () => {
    const cfg = getRuntimeConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const enclawedProfile = resolveProfile(resolved, "enclawed");
    expect(enclawedProfile).toBeTruthy();
    ...
    const runtime = state.profiles.get("enclawed");
  ...
  it("marks local managed runtime state for reconcile when profile headless changes", async () => {
    ...
    const enclawedProfile = resolveProfile(resolved, "enclawed");
    expect(enclawedProfile).toBeTruthy();
    expect(enclawedProfile?.headless).toBe(true);
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
    ...
    mockState.cfgProfiles.enclawed = {
      cdpPort: 18800,
      color: "#FF4500",
      headless: false,
    };
  ...
  it("marks local managed runtime state for reconcile when profile executablePath changes", async () => {
    ...
    const enclawedProfile = resolveProfile(resolved, "enclawed");
    expect(enclawedProfile).toBeTruthy();
    expect(enclawedProfile?.executablePath).toBe("/usr/bin/chrome-old");
    const state: BrowserServerState = {
      server: null,
      port: 18791,
      resolved,
  ...
  it("does not reconcile remote cdp runtime when only headless changes", async () => {
    ...
    const remoteProfile = resolveProfile(resolved, "remote");
    expect(remoteProfile).toBeTruthy();
    expect(remoteProfile?.driver).toBe("enclawed");
    ...
    const runtime = state.profiles.get("remote");
    expect(runtime).toBeTruthy();
    expect(runtime?.profile.driver).toBe("enclawed");
