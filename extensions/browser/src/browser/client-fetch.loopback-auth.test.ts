...
vi.mock("@enclawed/plugin-sdk/ssrf-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/ssrf-runtime")>(
    "@enclawed/plugin-sdk/ssrf-runtime",
  );
  return {
    ...actual,
...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => EnclawedConfig>(() => ({
    gateway: {
      auth: {
...
describe("fetchBrowserJson loopback auth", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("ENCLAWED_GATEWAY_TOKEN", "loopback-token");
  ...
  it("preserves dispatcher timeout context without no-retry hint", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      contains: ["Chrome CDP handshake timeout", "Restart the Enclawed gateway"],
      omits: ["Can't reach the Enclawed browser control service", "Do NOT retry the browser tool"],
    });
  });
  ...
  it("preserves dispatcher abort context without no-retry hint", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      contains: ["operation aborted", "Restart the Enclawed gateway"],
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=manual"),
      {
        contains: [
          ...
          "browser profile is external to Enclawed",
          "Restarting the Enclawed gateway will not launch it",
        ],
        omits: ["Restart the Enclawed gateway", "Do NOT retry the browser tool"],
      },
    );
  });
  ...
  it("keeps restart-gateway guidance for managed local dispatcher timeouts", async () => {
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=enclawed"),
      {
        contains: ["Chrome CDP handshake timeout", "Restart the Enclawed gateway"],
        omits: ["browser profile is external to Enclawed", "Do NOT retry the browser tool"],
      },
    );
  });
  ...
  it("keeps no-retry hint but not restart guidance for persistent external profile failures", async () => {
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=manual"),
      {
        ...
        omits: ["Restart the Enclawed gateway"],
      },
    );
  });
  ...
  it("keeps no-retry hint for persistent dispatcher failures", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      ...
      omits: ["Can't reach the Enclawed browser control service"],
    });
  });
  ...
  it("keeps absolute URL failures wrapped as reachability errors", async () => {
    ...
    await expectThrownBrowserFetchError(
      ...
      {
        contains: [
          "Can't reach the Enclawed browser control service",
          "Do NOT retry the browser tool",
        ],
      },
    );
