...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        enclawed: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ENCLAWED_GATEWAY_PORT;
    process.env.ENCLAWED_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ENCLAWED_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ENCLAWED_GATEWAY_PASSWORD;
