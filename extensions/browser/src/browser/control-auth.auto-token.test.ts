...
const mocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => EnclawedConfig>(),
  writeConfigFile: vi.fn<(cfg: EnclawedConfig) => Promise<void>>(async (_cfg) => {}),
  replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: EnclawedConfig }) => {
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<EnclawedConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: EnclawedConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): EnclawedConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: EnclawedConfig) {
