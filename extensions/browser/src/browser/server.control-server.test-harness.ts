...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "enclawed" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.ENCLAWED_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "enclawed",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/enclawed" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    enclawed: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchEnclawedChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveEnclawedUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopEnclawedChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "enclawed";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.ENCLAWED_GATEWAY_PORT;
  process.env.ENCLAWED_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.ENCLAWED_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.ENCLAWED_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.ENCLAWED_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.ENCLAWED_GATEWAY_PASSWORD;
  } else {
    process.env.ENCLAWED_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
