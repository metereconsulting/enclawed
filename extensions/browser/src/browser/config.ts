...
export const ENCLAWED_BROWSER_HEADLESS_ENV = "ENCLAWED_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_ENCLAWED_BROWSER_COLOR;
  }
...
function isLocalManagedProfile(profile: ResolvedBrowserProfile): boolean {
  return profile.driver === "enclawed" && profile.cdpIsLoopback && !profile.attachOnly;
}
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_ENCLAWED_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME]
        ? DEFAULT_ENCLAWED_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "enclawed";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[ENCLAWED_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  const sourceHint =
    mode.source === "request"
      ? "request override"
      : mode.source === "env"
        ? `${ENCLAWED_BROWSER_HEADLESS_ENV}=0`
        : mode.source === "profile"
  ...
  return (
    ...
    `Set ${ENCLAWED_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
