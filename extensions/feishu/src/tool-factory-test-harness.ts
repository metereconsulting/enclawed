...
export function createToolFactoryHarness(cfg: EnclawedPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<EnclawedPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
