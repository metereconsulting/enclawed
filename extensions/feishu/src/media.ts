...
import { mediaKindFromMime } from "@enclawed/plugin-sdk/media-mime";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS, runFfmpeg } from "@enclawed/plugin-sdk/media-runtime";
import {
  resolvePreferredEnclawedTmpDir,
  withTempDownloadPath,
} from "@enclawed/plugin-sdk/temp-path";
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "enclawed-feishu-img-",
...
async function downloadMessageResourceWithType(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "enclawed-feishu-resource-",
