...
The bundled ACPX extension is a thin Enclawed wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `enclawed/acpx`, not in this extension.
...
Use this flow when Enclawed needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `enclawed/acpx` repo first.
2. In Enclawed, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Enclawed and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Enclawed ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Enclawed-specific glue, move it to `enclawed/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
