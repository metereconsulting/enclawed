...
import type { z } from "@enclawed/plugin-sdk/zod";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "enclawed-plugin-tools";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "enclawed.plugin.json")) &&
...
function resolveAcpxPluginRootFromEnclawedLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Enclawed layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromEnclawedLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveEnclawedRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const enclawedRoot = resolveEnclawedRoot(pluginRoot);
  const distEntry = path.join(enclawedRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(enclawedRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
