...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: EnclawedPluginServiceContext): Promise<void> {
      if (process.env.ENCLAWED_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (ENCLAWED_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (process.env.ENCLAWED_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: EnclawedPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
