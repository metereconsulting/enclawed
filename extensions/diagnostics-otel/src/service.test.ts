...
import type { EnclawedPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/enclawed-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): EnclawedPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("enclawed.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("enclawed.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("enclawed.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("enclawed.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("enclawed.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("enclawed.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("enclawed.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("enclawed.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("enclawed.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("enclawed.webhook.processed");
    expect(spanNames).toContain("enclawed.message.processed");
    expect(spanNames).toContain("enclawed.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["enclawed.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("enclawed.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "enclawed.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["enclawed.reason"]).toBe("string");
    expect(String(attrs?.["enclawed.reason"])).not.toContain(
