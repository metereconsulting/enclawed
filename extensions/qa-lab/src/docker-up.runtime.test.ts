...
describe("runQaDockerUp", () => {
  it("builds the QA UI, writes the harness, starts compose, and waits for health", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (args.join(" ").includes("ps --format json enclawed-qa-gateway")) {
      ...
      expect(calls).toEqual([
        "pnpm qa:lab:build @/repo/enclawed",
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/enclawed`,
        expect.stringContaining(
          `docker compose -f ${outputDir}/docker-compose.qa.yml up --build -d @/repo/enclawed`,
        ),
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json enclawed-qa-gateway @/repo/enclawed`,
      ]);
  ...
  it("skips UI build and compose --build for prebuilt images", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/enclawed`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/enclawed`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json enclawed-qa-gateway @/repo/enclawed`,
      ]);
  ...
  it("uses a repo-root-relative default output dir when none is provided", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${path.join(repoRoot, ".artifacts/qa-docker/docker-compose.qa.yml")} ps --format json enclawed-qa-gateway @${repoRoot}`,
      ]);
    } finally {
  ...
  it("falls back to the container IP when the host gateway port is unreachable", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (joined.includes("ps --format json enclawed-qa-gateway")) {
              return { stdout: '{"Health":"healthy","State":"running"}\n', stderr: "" };
            }
            if (joined.includes("ps -q enclawed-qa-gateway")) {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/enclawed`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json enclawed-qa-gateway @/repo/enclawed`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps -q enclawed-qa-gateway @/repo/enclawed`,
        "docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} gateway-container @/repo/enclawed",
      ]);
      expect(fetchCalls).toEqual([
        "http://127.0.0.1:43124/healthz",
        "http://127.0.0.1:18889/healthz",
