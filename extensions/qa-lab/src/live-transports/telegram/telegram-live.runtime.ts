...
const TELEGRAM_QA_ENV_KEYS = [
  "ENCLAWED_QA_TELEGRAM_GROUP_ID",
  "ENCLAWED_QA_TELEGRAM_DRIVER_BOT_TOKEN",
  "ENCLAWED_QA_TELEGRAM_SUT_BOT_TOKEN",
] as const;
...
export function resolveTelegramQaRuntimeEnv(
  ...
  const groupId = resolveEnvValue(env, "ENCLAWED_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("ENCLAWED_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "ENCLAWED_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "ENCLAWED_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
export async function runTelegramQaLive(params: {
  ...
  const includeObservedMessageContent = process.env.ENCLAWED_QA_TELEGRAM_CAPTURE_CONTENT === "1";
