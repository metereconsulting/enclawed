...
const QA_LAB_UI_OVERLAY_DIR = "/opt/enclawed-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        ENCLAWED_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${params.bindUiDist ? `    volumes:\n      - ${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro\n` : ""}    healthcheck:
    ...
    command:
      ...
      - "http://enclawed-qa-gateway:18789/"
      - --control-ui-token
      - "${params.gatewayToken}"
...
${imageBlock}    pull_policy: never
    ...
    environment:
      ENCLAWED_CONFIG_PATH: /tmp/enclawed/enclawed.json
      ...
      ENCLAWED_PROFILE: ""
    volumes:
      - ./state:/opt/enclawed-scaffold:ro
      - ${repoMount}:/opt/enclawed-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/enclawed/workspace /tmp/enclawed/state && cp /opt/enclawed-scaffold/enclawed.json /tmp/enclawed/enclawed.json && cp -R /opt/enclawed-scaffold/seed-workspace/. /tmp/enclawed/workspace/ && ln -snf /opt/enclawed-repo /tmp/enclawed/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t enclawed:qa-local-prebaked --build-arg ENCLAWED_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "enclawed:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "enclawed.json"),
  ];
