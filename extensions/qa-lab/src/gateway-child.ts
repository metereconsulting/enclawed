...
import { fetchWithSsrFGuard } from "@enclawed/plugin-sdk/ssrf-runtime";
...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    ENCLAWED_HOME: params.homeDir,
    ENCLAWED_CONFIG_PATH: params.configPath,
    ENCLAWED_STATE_DIR: params.stateDir,
    ENCLAWED_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    ENCLAWED_NO_RESPAWN: "1",
    ENCLAWED_TEST_FAST: "1",
    ENCLAWED_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    ENCLAWED_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { ENCLAWED_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { ENCLAWED_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: EnclawedConfig) => EnclawedConfig;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredEnclawedTmpDir(), "enclawed-qa-suite-"),
  );
  ...
  const configPath = path.join(tempRoot, "enclawed.json");
  ...
  const keepTemp = process.env.ENCLAWED_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
