...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/enclawed-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/enclawed";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/enclawed-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `enclawed-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "ENCLAWED_CONFIG_PATH" &&
          key !== "ENCLAWED_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `ENCLAWED_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `ENCLAWED_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  await writeFile(
    plan.hostLogPath,
    `# Enclawed QA Multipass host log\nvmName=${plan.vmName}\noutputDir=${plan.outputDir}\n\n`,
    "utf8",
  );
  await writeFile(
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm enclawed qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredEnclawedTmpDir(), `${plan.vmName}-qa-suite-`),
  );
