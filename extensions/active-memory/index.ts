...
import { definePluginEntry, type EnclawedPluginApi } from "@enclawed/plugin-sdk/plugin-entry";
...
function resolvePersistentTranscriptBaseDir(api: EnclawedPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: EnclawedPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: EnclawedConfig): boolean {
...
function resolveActiveMemoryPluginConfigFromConfig(cfg: EnclawedConfig): unknown {
...
async function runRecallSubagent(params: {
  ...
  const tempDir = params.config.persistTranscripts
    ? undefined
    : await fs.mkdtemp(path.join(resolvePreferredEnclawedTmpDir(), "enclawed-active-memory-"));
