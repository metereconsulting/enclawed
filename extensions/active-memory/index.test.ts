...
import type { EnclawedPluginApi } from "@enclawed/plugin-sdk/plugin-entry";
...
vi.mock("@enclawed/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("@enclawed/plugin-sdk/config-runtime")>(
...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedPiAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/enclawed-session-store.json"),
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
    ...
    plugin.register(api as unknown as EnclawedPluginApi);
  });
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    const mkdtempSpy = vi
      .spyOn(fs, "mkdtemp")
      .mockResolvedValue("/tmp/enclawed-active-memory-temp");
    ...
    expect(runEmbeddedPiAgent.mock.calls.at(-1)?.[0]?.sessionFile).toBe(
      "/tmp/enclawed-active-memory-temp/session.jsonl",
    );
    expect(rmSpy).toHaveBeenCalledWith("/tmp/enclawed-active-memory-temp", {
      recursive: true,
      force: true,
    });
  });
