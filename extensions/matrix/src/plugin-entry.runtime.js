...
const ENCLAWED_PLUGIN_SDK_PACKAGE_NAMES = [
  ["enclawed", "plugin-sdk"].join("/"),
  ["@enclawed", "plugin-sdk"].join("/"),
];
...
function hasTrustedEnclawedRootIndicator(packageRoot, packageJson) {
  ...
  const hasEnclawedBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("enclawed")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.enclawed === "string");
  const hasEnclawedEntrypoint = fs.existsSync(path.join(packageRoot, "enclawed.mjs"));
  return hasCliEntryExport || hasEnclawedBin || hasEnclawedEntrypoint;
}

function findEnclawedPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "enclawed" && hasTrustedEnclawedRootIndicator(cursor, pkg)) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findEnclawedPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    for (const packageName of ENCLAWED_PLUGIN_SDK_PACKAGE_NAMES) {
  ...
  if (extensionApi) {
    aliasMap["@enclawed/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findEnclawedPackageRoot(moduleDir);
  if (location) {
