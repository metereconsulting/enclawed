...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<EnclawedConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<EnclawedConfig["channels"]>["matrix"]>>,
): EnclawedConfig {
  return {
    channels: {
      matrix: {
