...
import type { ChannelSetupInput } from "@enclawed/plugin-sdk/setup";
...
import { isEnclawedManagedMatrixDevice } from "./matrix/device-health.js";
...
function resolveMatrixCliAccountContext(accountId?: string): {
  ...
  const cfg = readMatrixRuntimeConfig() as CoreConfig;
  return {
...
function formatMatrixCliCommandParts(parts: string[], accountId?: string): string {
  const normalizedAccountId = normalizeAccountId(accountId);
  const command = ["enclawed", "matrix", ...parts];
...
async function addMatrixAccount(params: {
  ...
  const cfg = readMatrixRuntimeConfig(runtime) as CoreConfig;
  ...
  if (desiredDisplayName || desiredAvatarUrl) {
    try {
      ...
      if (synced.convertedAvatarFromHttp && synced.resolvedAvatarUrl) {
        const latestCfg = readMatrixRuntimeConfig(runtime) as CoreConfig;
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleEnclawedDeviceIds: addedDevices
        .filter((device) => !device.current && isEnclawedManagedMatrixDevice(device.displayName))
...
function printMatrixVerificationSummary(summary: MatrixCliVerificationSummary): void {
  ...
  console.log(`Initiated by Enclawed: ${summary.initiatedByMe ? "yes" : "no"}`);
...
function buildVerificationGuidance(
  ...
  if (status.serverDeviceKnown === false) {
    nextSteps.add(
      `This Matrix device is no longer listed on the homeserver. Create a new Enclawed Matrix device with ${formatMatrixCliCommand("account add --homeserver <url> --user-id <@user:server> --password <password> --device-name Enclawed-Gateway", accountId)}. If you use token auth, create a fresh Matrix access token in your Matrix client or admin UI, then run ${formatMatrixCliCommand("account add --homeserver <url> --access-token <token>", accountId)}.`,
    );
  }
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.enclawed.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleEnclawedDeviceIds.length > 0) {
              const staleDeviceIds = result.deviceHealth.staleEnclawedDeviceIds
                .map((deviceId) => formatMatrixCliText(deviceId))
                .join(", ");
              console.log(
                `Matrix device hygiene warning: stale Enclawed devices detected (${staleDeviceIds}). Run ${formatMatrixCliCommand("devices prune-stale", result.accountId)}.`,
              );
            }
            ...
            const bindHint = `enclawed agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale Enclawed-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
      ...
      await runMatrixCliCommand({
        ...
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale Enclawed devices: ${
