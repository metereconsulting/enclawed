...
const matrixWrapperGlobal = globalThis as typeof globalThis & {
  __enclawedMatrixWrapperJitiOptions?: unknown;
};
const PLUGIN_SDK_ROOT = ["enclawed", "plugin-sdk"].join("/");
const SCOPED_PLUGIN_SDK_ROOT = ["@enclawed", "plugin-sdk"].join("/");
...
function writeCapturingJitiFixture(fixtureRoot: string) {
  writeFixtureFile(
    ...
    [
      ...
      "  globalThis.__enclawedMatrixWrapperJitiOptions = options;",
...
function writeEnclawedPackageFixture(fixtureRoot: string) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
  ...
  writeFixtureFile(fixtureRoot, "enclawed.mjs", "export {};\n");
  writeFixtureFile(fixtureRoot, "dist/plugin-sdk/index.js", "export {};\n");
}

function writeEnclawedAliasFixture(fixtureRoot: string, extraExports?: Record<string, string>) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
...
function writeTrustedEnclawedBinFixture(
  fixtureRoot: string,
...
function writeCapturingSourceRuntimeWrapperFixture(fixtureRoot: string) {
  delete matrixWrapperGlobal.__enclawedMatrixWrapperJitiOptions;
  writeEnclawedAliasFixture(fixtureRoot);
...
function expectSourcePluginSdkAliases(fixtureRoot: string) {
  expect(matrixWrapperGlobal.__enclawedMatrixWrapperJitiOptions).toMatchObject({
    alias: {
...
it("loads the source-checkout runtime wrapper through native ESM import", async () => {
  ...
  writeEnclawedPackageFixture(fixtureRoot);
...
it("resolves extension-api aliases through the same source extension family", async () => {
  ...
  expect(matrixWrapperGlobal.__enclawedMatrixWrapperJitiOptions).toMatchObject({
    alias: {
      "@enclawed/extension-api": path.join(fixtureRoot, "src", "extensionAPI.mts"),
    },
  });
}, 240_000);
...
it("keeps wrapper plugin-sdk aliases deterministic and ignores unsafe subpaths", async () => {
  ...
  const aliasKeys = Object.keys(
    (
      (matrixWrapperGlobal.__enclawedMatrixWrapperJitiOptions ?? {}) as {
...
it("ignores nearby untrusted enclawed package stubs when resolving the wrapper root", async () => {
...
it("treats string bin hints case-insensitively when trusting wrapper package roots", async () => {
  ...
  delete matrixWrapperGlobal.__enclawedMatrixWrapperJitiOptions;
  writeTrustedEnclawedBinFixture(fixtureRoot, "Enclawed.MJS");
  writeCapturingJitiFixture(fixtureRoot);
  writeSourceRuntimeWrapperFixture(fixtureRoot);
  await importFixtureModule(fixtureRoot, "extensions/matrix/src/plugin-entry.runtime.js");
