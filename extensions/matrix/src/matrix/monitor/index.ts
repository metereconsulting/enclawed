...
import type { ChannelRuntimeSurface } from "@enclawed/plugin-sdk/channel-contract";
import { waitUntilAbort } from "@enclawed/plugin-sdk/channel-lifecycle";
...
import { getMatrixRuntime, matrixReplaceConfigFile } from "../../runtime.js";
import type {
  CoreConfig,
  MatrixConfig,
...
export type MonitorMatrixOpts = {
  ...
  setStatus?: (next: import("@enclawed/plugin-sdk/channel-contract").ChannelAccountSnapshot) => void;
};
...
export async function monitorMatrixProvider(opts: MonitorMatrixOpts = {}): Promise<void> {
  ...
  let cfg = (core.config.current?.() ?? core.config.loadConfig()) as CoreConfig;
  ...
  try {
    ...
    await runMatrixStartupMaintenance({
      ...
      logger,
      logVerboseMessage,
      getRuntimeConfig: () => (core.config.current?.() ?? core.config.loadConfig()) as CoreConfig,
