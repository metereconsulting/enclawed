...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'enclawed matrix verify status --verbose --account ${accountId}' and 'enclawed matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Enclawed runtime is using the same account. " +
    `Check 'enclawed matrix verify status --verbose --account ${accountId}' and 'enclawed matrix devices list --account ${accountId}'.`
  );
}
