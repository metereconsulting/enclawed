import { resolveControlCommandGate } from "@enclawed/plugin-sdk/command-gating";
import {
  evaluateSupplementalContextVisibility,
  resolveChannelContextVisibilityMode,
} from "@enclawed/plugin-sdk/context-visibility-runtime";
import { hasFinalInboundReplyDispatch } from "@enclawed/plugin-sdk/inbound-reply-dispatch";
import type { GetReplyOptions } from "@enclawed/plugin-sdk/reply-runtime";
...
import { normalizeOptionalString } from "@enclawed/plugin-sdk/string-coerce-runtime";
import type {
  CoreConfig,
  MatrixRoomConfig,
  MatrixStreamingMode,
...
import { MATRIX_ENCLAWED_FINALIZED_PREVIEW_KEY } from "../send/types.js";
...
let acpBindingRuntimePromise:
  | Promise<typeof import("@enclawed/plugin-sdk/acp-binding-runtime")>
  | undefined;
let sessionBindingRuntimePromise:
  | Promise<typeof import("@enclawed/plugin-sdk/session-binding-runtime")>
  | undefined;
...
function loadAcpBindingRuntime(): Promise<
  typeof import("@enclawed/plugin-sdk/acp-binding-runtime")
> {
  acpBindingRuntimePromise ??= import("@enclawed/plugin-sdk/acp-binding-runtime");
  return acpBindingRuntimePromise;
}

function loadSessionBindingRuntime(): Promise<
  typeof import("@enclawed/plugin-sdk/session-binding-runtime")
> {
  sessionBindingRuntimePromise ??= import("@enclawed/plugin-sdk/session-binding-runtime");
...
function buildMatrixFinalizedPreviewContent(): Record<string, unknown> {
  return { [MATRIX_ENCLAWED_FINALIZED_PREVIEW_KEY]: true };
}
  ...
  return async (roomId: string, event: MatrixRawEvent) => {
    ...
    try {
      ...
      const continueIngress = async (params: {
        ...
        const liveCfg = (core.config.current?.() ?? core.config.loadConfig()) as CoreConfig;
