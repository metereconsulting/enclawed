...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleEnclawedDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Enclawed devices detected for ${params.auth.userId}: ${deviceHealth.staleEnclawedDevices.map((device) => device.deviceId).join(", ")}. Run 'enclawed matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
