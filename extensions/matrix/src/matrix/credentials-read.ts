...
import { getMatrixRuntime, readMatrixRuntimeConfig } from "../runtime.js";
import {
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  try {
    ...
    const homeDir = env.ENCLAWED_HOME?.trim() || env.HOME?.trim() || os.homedir();
    return path.join(homeDir, ".enclawed");
  }
}
...
function shouldReadLegacyCredentialsForAccount(accountId?: string | null): boolean {
  ...
  const cfg = readMatrixRuntimeConfig() as EnclawedConfig;
