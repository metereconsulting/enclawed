...
import type { EnclawedPluginApi, EnclawedPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<EnclawedPluginApi> = {}): EnclawedPluginApi {
...
function fakeCtx(overrides: Partial<EnclawedPluginToolContext> = {}): EnclawedPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: EnclawedPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
