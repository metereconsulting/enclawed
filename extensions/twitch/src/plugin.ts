/**
 * Twitch channel plugin for Enclawed.
 *
...
import { buildChannelConfigSchema } from "@enclawed/plugin-sdk/channel-config-schema";
import { createChatChannelPlugin } from "@enclawed/plugin-sdk/channel-core";
import {
  createLoggedPairingApprovalNotifier,
  createPairingPrefixStripper,
} from "@enclawed/plugin-sdk/channel-pairing";
import type { EnclawedConfig } from "@enclawed/plugin-sdk/config-types";
import { buildPassiveProbedChannelStatusSummary } from "@enclawed/plugin-sdk/extension-shared";
import {
 ...
 * for Enclawed. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: EnclawedConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: EnclawedConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: EnclawedConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: EnclawedConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("@enclawed/plugin-sdk/runtime-env").RuntimeEnv;
