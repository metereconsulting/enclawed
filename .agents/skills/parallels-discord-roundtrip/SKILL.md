...
```bash
export ENCLAWED_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.enclawed/enclawed.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env ENCLAWED_PARALLELS_DISCORD_TOKEN \
...
- Use the `enclawed` wrapper for guest `message send/read`; `node enclawed.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/enclawed-parallels-smoke.*`
...
- host inbound nonce appears in `enclawed message read` output
