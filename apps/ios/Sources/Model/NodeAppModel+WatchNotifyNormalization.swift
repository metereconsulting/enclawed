...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: EnclawedWatchNotifyParams) -> EnclawedWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [EnclawedWatchAction]?,
        kind: String?,
        promptId: String?) -> [EnclawedWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> EnclawedWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return EnclawedWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                EnclawedWatchAction(id: "approve", label: "Approve"),
                EnclawedWatchAction(id: "decline", label: "Decline", style: "destructive"),
                EnclawedWatchAction(id: "open_phone", label: "Open iPhone"),
                EnclawedWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            EnclawedWatchAction(id: "done", label: "Done"),
            EnclawedWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            EnclawedWatchAction(id: "open_phone", label: "Open iPhone"),
            EnclawedWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: EnclawedWatchRisk?,
        priority: EnclawedNotificationPriority?) -> EnclawedWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: EnclawedNotificationPriority?,
        risk: EnclawedWatchRisk?) -> EnclawedNotificationPriority?
    {
