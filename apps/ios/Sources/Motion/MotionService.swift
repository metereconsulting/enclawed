...
final class MotionService: MotionServicing {
    func activities(params: EnclawedMotionActivityParams) async throws -> EnclawedMotionActivityPayload {
        ...
        let mapped: [EnclawedMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return EnclawedMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: EnclawedPedometerParams) async throws -> EnclawedPedometerPayload {
        ...
        let payload: EnclawedPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = EnclawedPedometerPayload(
