...
enum WatchMessagingPayloadCodec {
    ...
    static func encodeNotificationPayload(
        id: String,
        params: EnclawedWatchNotifyParams) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": EnclawedWatchPayloadType.notify.rawValue,
            "id": id,
            "title": params.title,
            "body": params.body,
            "priority": params.priority?.rawValue ?? EnclawedNotificationPriority.active.rawValue,
            "sentAtMs": nowMs(),
        ]
    ...
    static func encodeExecApprovalItem(_ item: EnclawedWatchExecApprovalItem) -> [String: Any] {
        var payload: [String: Any] = [
            "id": item.id,
    ...
    static func encodeExecApprovalPromptPayload(
        _ message: EnclawedWatchExecApprovalPromptMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": EnclawedWatchPayloadType.execApprovalPrompt.rawValue,
    ...
    static func encodeExecApprovalResolvedPayload(
        _ message: EnclawedWatchExecApprovalResolvedMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": EnclawedWatchPayloadType.execApprovalResolved.rawValue,
    ...
    static func encodeExecApprovalExpiredPayload(
        _ message: EnclawedWatchExecApprovalExpiredMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": EnclawedWatchPayloadType.execApprovalExpired.rawValue,
            "approvalId": message.approvalId,
    ...
    static func encodeExecApprovalSnapshotPayload(
        _ message: EnclawedWatchExecApprovalSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": EnclawedWatchPayloadType.execApprovalSnapshot.rawValue,
    ...
    {
        guard (payload["type"] as? String) == EnclawedWatchPayloadType.reply.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == EnclawedWatchPayloadType.execApprovalResolve.rawValue else {
            return nil
        }
        guard let approvalId = nonEmpty(payload["approvalId"] as? String),
              let rawDecision = nonEmpty(payload["decision"] as? String),
              let decision = EnclawedWatchExecApprovalDecision(rawValue: rawDecision)
        else {
            return nil
        }
        let replyId = nonEmpty(payload["replyId"] as? String) ?? UUID().uuidString
    ...
    {
        guard (payload["type"] as? String) == EnclawedWatchPayloadType.execApprovalSnapshotRequest.rawValue else {
            return nil
        }
