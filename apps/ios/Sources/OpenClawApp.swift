...
@MainActor
enum EnclawedAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class EnclawedAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.enclawed.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.enclawed.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.enclawed.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? EnclawedAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "enclawed.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "enclawed.watch.promptId"
    static let sessionKeyKey = "enclawed.watch.sessionKey"
    static let actionPrimaryIDKey = "enclawed.watch.action.primary.id"
    static let actionPrimaryLabelKey = "enclawed.watch.action.primary.label"
    static let actionSecondaryIDKey = "enclawed.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "enclawed.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "enclawed.watch.action.primary"
    static let actionSecondaryIdentifier = "enclawed.watch.action.secondary"
    static let actionIdentifierPrefix = "enclawed.watch.action."
    static let actionIDKeyPrefix = "enclawed.watch.action.id."
    static let actionLabelKeyPrefix = "enclawed.watch.action.label."
    static let categoryPrefix = "enclawed.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: EnclawedWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> EnclawedWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return EnclawedWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Enclawed" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [EnclawedWatchAction]) -> [UNNotificationAction] {
...
@main
struct EnclawedApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(EnclawedAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        EnclawedAppModelRegistry.appModel = appModel
...
extension EnclawedApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Enclawed: installing uncaught exception handler")
