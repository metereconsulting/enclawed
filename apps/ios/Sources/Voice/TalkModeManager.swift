...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ENCLAWED_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<EnclawedTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.enclawed", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = EnclawedTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> EnclawedTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return EnclawedTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> EnclawedTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> EnclawedTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return EnclawedTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> EnclawedTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = EnclawedTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: EnclawedTalkPTTStopPayload) {
