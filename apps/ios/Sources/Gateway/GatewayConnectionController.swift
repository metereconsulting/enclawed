...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [EnclawedCapability.canvas.rawValue, EnclawedCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(EnclawedCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(EnclawedCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = EnclawedLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(EnclawedCapability.location.rawValue) }

        caps.append(EnclawedCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(EnclawedCapability.watch.rawValue)
        }
        caps.append(EnclawedCapability.photos.rawValue)
        caps.append(EnclawedCapability.contacts.rawValue)
        caps.append(EnclawedCapability.calendar.rawValue)
        caps.append(EnclawedCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(EnclawedCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            EnclawedScreenCommand.record.rawValue,
            EnclawedSystemCommand.notify.rawValue,
            EnclawedChatCommand.push.rawValue,
        ...
        if caps.contains(EnclawedCapability.camera.rawValue) {
        ...
        if caps.contains(EnclawedCapability.location.rawValue) {
            commands.append(EnclawedLocationCommand.get.rawValue)
        }
        if caps.contains(EnclawedCapability.device.rawValue) {
            commands.append(EnclawedDeviceCommand.status.rawValue)
            commands.append(EnclawedDeviceCommand.info.rawValue)
        }
        if caps.contains(EnclawedCapability.watch.rawValue) {
            commands.append(EnclawedWatchCommand.status.rawValue)
            commands.append(EnclawedWatchCommand.notify.rawValue)
        }
        if caps.contains(EnclawedCapability.photos.rawValue) {
            commands.append(EnclawedPhotosCommand.latest.rawValue)
        }
        if caps.contains(EnclawedCapability.contacts.rawValue) {
            commands.append(EnclawedContactsCommand.search.rawValue)
            commands.append(EnclawedContactsCommand.add.rawValue)
        }
        if caps.contains(EnclawedCapability.calendar.rawValue) {
            commands.append(EnclawedCalendarCommand.events.rawValue)
            commands.append(EnclawedCalendarCommand.add.rawValue)
        }
        if caps.contains(EnclawedCapability.reminders.rawValue) {
            commands.append(EnclawedRemindersCommand.list.rawValue)
            commands.append(EnclawedRemindersCommand.add.rawValue)
        }
        if caps.contains(EnclawedCapability.motion.rawValue) {
            commands.append(EnclawedMotionCommand.activity.rawValue)
            commands.append(EnclawedMotionCommand.pedometer.rawValue)
        }
