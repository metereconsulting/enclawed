...
final class RemindersService: RemindersServicing {
    func list(params: EnclawedRemindersListParams) async throws -> EnclawedRemindersListPayload {
        ...
        let payload: [EnclawedReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return EnclawedReminderPayload(
        ...
        return EnclawedRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: EnclawedRemindersAddParams) async throws -> EnclawedRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = EnclawedReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return EnclawedRemindersAddPayload(reminder: payload)
    }
