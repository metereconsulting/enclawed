...
final class ContactsService: ContactsServicing {
    ...
    func search(params: EnclawedContactsSearchParams) async throws -> EnclawedContactsSearchPayload {
        ...
        return EnclawedContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: EnclawedContactsAddParams) async throws -> EnclawedContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return EnclawedContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return EnclawedContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> EnclawedContactPayload {
        EnclawedContactPayload(
