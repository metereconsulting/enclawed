...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "enclawed.exec-approval"
    static let reviewActionIdentifier = "enclawed.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.enclawedPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.enclawedPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func enclawedPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
