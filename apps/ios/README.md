...
- Recommended gateway-host storage for the APNs `.p8` file is `~/.openclaw/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `ENCLAWED_APNS_PRIVATE_KEY_PATH` at that file.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `openclaw.json`. `ENCLAWED_APNS_RELAY_BASE_URL` remains a temporary env override only.
