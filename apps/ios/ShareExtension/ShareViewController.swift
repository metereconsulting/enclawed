...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.enclawed.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Enclawed", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Enclawed gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Enclawed.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "EnclawedShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Enclawed is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Enclawed Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("enclawed-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
