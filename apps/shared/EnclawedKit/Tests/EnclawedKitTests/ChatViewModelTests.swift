...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> EnclawedChatHistoryPayload
{
    EnclawedChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> EnclawedChatSessionEntry {
    EnclawedChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> EnclawedChatSessionEntry
{
    EnclawedChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> EnclawedChatModelChoice {
    EnclawedChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [EnclawedChatHistoryPayload],
    sessionsResponses: [EnclawedChatSessionsListResponse] = [],
    modelResponses: [[EnclawedChatModelChoice]] = [],
    ...
    -> (TestChatTransport, EnclawedChatViewModel)
{
...
private func sendUserMessage(_ vm: EnclawedChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, EnclawedChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [EnclawedChatHistoryPayload]
    private let sessionsResponses: [EnclawedChatSessionsListResponse]
    private let modelResponses: [[EnclawedChatModelChoice]]
    ...
    private let stream: AsyncStream<EnclawedChatTransportEvent>
    private let continuation: AsyncStream<EnclawedChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<EnclawedChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<EnclawedChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> EnclawedChatHistoryPayload {
        ...
        return self.historyResponses.last ?? EnclawedChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [EnclawedChatAttachmentPayload]) async throws -> EnclawedChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return EnclawedChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> EnclawedChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? EnclawedChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [EnclawedChatModelChoice] {
    ...
    func emit(_ evt: EnclawedChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = EnclawedChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = EnclawedChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: EnclawedChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = EnclawedChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: EnclawedChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(EnclawedChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == EnclawedChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = EnclawedChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: EnclawedChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = EnclawedChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = EnclawedChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = EnclawedChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { EnclawedChatViewModel(sessionKey: "main", transport: transport) }
