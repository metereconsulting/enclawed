...
private let chatUILogger = Logger(subsystem: "ai.enclawed", category: "EnclawedChatUI")

@MainActor
@Observable
public final class EnclawedChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [EnclawedChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [EnclawedChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [EnclawedPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [EnclawedChatPendingToolCall] = []
    public private(set) var sessions: [EnclawedChatSessionEntry] = []
    private let transport: any EnclawedChatTransport
    private var sessionDefaults: EnclawedChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: EnclawedChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any EnclawedChatTransport,
    ...
    public var sessionChoices: [EnclawedChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [EnclawedChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: EnclawedPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [EnclawedChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: EnclawedChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: EnclawedChatMessage) -> EnclawedChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> EnclawedChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return EnclawedChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: EnclawedChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: EnclawedChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: EnclawedChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [EnclawedChatMessage],
        incoming: [EnclawedChatMessage]) -> [EnclawedChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [EnclawedChatMessage]) -> [EnclawedChatMessage] {
        var result: [EnclawedChatMessage] = []
    ...
    private static func dedupeKey(for message: EnclawedChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [EnclawedChatMessageContent] = [
            EnclawedChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> EnclawedChatAttachmentPayload in
            EnclawedChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> EnclawedChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = EnclawedChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: EnclawedChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: EnclawedChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: EnclawedAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = EnclawedChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> EnclawedPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
