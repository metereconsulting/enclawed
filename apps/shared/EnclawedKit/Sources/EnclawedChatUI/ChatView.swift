...
@MainActor
public struct EnclawedChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: EnclawedChatViewModel
    ...
    public init(
        viewModel: EnclawedChatViewModel,
    ...
    private var visibleMessages: [EnclawedChatMessage] {
        let base: [EnclawedChatMessage]
    ...
    private func mergeToolResults(in messages: [EnclawedChatMessage]) -> [EnclawedChatMessage] {
        ...
        for message in messages {
            ...
            let merged = EnclawedChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: EnclawedChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: EnclawedChatMessage) -> Bool {
    ...
    private func primaryText(in message: EnclawedChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: EnclawedChatMessage) -> Bool {
    ...
    private func toolCalls(in message: EnclawedChatMessage) -> [EnclawedChatMessageContent] {
    ...
    private func inlineToolResults(in message: EnclawedChatMessage) -> [EnclawedChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: EnclawedChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: EnclawedChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(EnclawedChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
