...
struct ChatMessageBubble: View {
    let message: EnclawedChatMessage
    let style: EnclawedChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? EnclawedChatTheme.userText : EnclawedChatTheme.assistantText
    ...
    private var inlineAttachments: [EnclawedChatMessageContent] {
    ...
    private var toolCalls: [EnclawedChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [EnclawedChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? EnclawedChatTheme.userBubble
        }
        if self.style == .onboarding {
            return EnclawedChatTheme.onboardingAssistantBubble
        }
        return EnclawedChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return EnclawedChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: EnclawedChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? EnclawedChatTheme.userText : EnclawedChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: EnclawedChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(EnclawedChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [EnclawedChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: EnclawedChatTheme.assistantText)
            }
        }
    }
}
