...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.enclawed.mac`) even if the shipped
    ...
    {
        if environment["ENCLAWED_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "enclawed.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "enclawed.nixMode") { return true }
