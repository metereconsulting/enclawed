...
enum CommandResolver {
    private static let projectRootDefaultsKey = "enclawed.gatewayProjectRootPath"
    private static let helperName = "enclawed"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let enclawedEntry = root.appendingPathComponent("enclawed.mjs").path
        if FileManager().isReadableFile(atPath: enclawedEntry) { return enclawedEntry }
        let binEntry = root.appendingPathComponent("bin/enclawed.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/enclawed")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let enclawedPaths = self.enclawedManagedPaths(home: home)
        if !enclawedPaths.isEmpty {
            extras.insert(contentsOf: enclawedPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + enclawedPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func enclawedManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".enclawed"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func enclawedExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectEnclawedExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("enclawed.mjs").path,
            root.appendingPathComponent("bin/enclawed.js").path,
        ]
    ...
    static func hasAnyEnclawedInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.enclawedExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func enclawedNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let enclawedPath = self.projectEnclawedExecutable(projectRoot: root) {
            return [enclawedPath, subcommand] + extraArgs
        }
        if let enclawedPath = self.enclawedExecutable(searchPaths: searchPaths) {
            return [enclawedPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "enclawed", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            enclawed entrypoint missing (looked for dist/index.js or enclawed.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real enclawed CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/enclawed"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/enclawed.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/enclawed.mjs"
            node "$PRJ/enclawed.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/enclawed.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/enclawed.js"
            node "$PRJ/bin/enclawed.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent enclawed"
          pnpm --silent enclawed \(quotedArgs);
        else
          echo "enclawed CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? EnclawedConfigFile.loadDict()
