...
enum EnclawedLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = EnclawedOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = EnclawedFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        EnclawedLogging.bootstrapIfNeeded()
        let label = EnclawedLogging.makeLabel(subsystem: subsystem, category: category)
...
struct EnclawedOSLogHandler: AppLogLevelBackedHandler {
...
struct EnclawedFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = EnclawedLogging.parseLabel(self.label)
