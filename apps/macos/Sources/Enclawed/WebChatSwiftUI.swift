...
private let webChatSwiftLogger = Logger(subsystem: "ai.enclawed", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "enclawed.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: EnclawedChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> EnclawedChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(EnclawedChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            EnclawedChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return EnclawedChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [EnclawedChatAttachmentPayload]) async throws -> EnclawedChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> EnclawedChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                EnclawedGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    EnclawedAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: EnclawedProtocol.ModelChoice) -> EnclawedChatModelChoice {
        EnclawedChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<EnclawedChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any EnclawedChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = EnclawedChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: EnclawedChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<EnclawedChatView>) -> NSViewController
    {
