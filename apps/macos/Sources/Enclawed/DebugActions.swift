...
enum DebugActions {
    private static let verboseDefaultsKey = "enclawed.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "enclawed.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = EnclawedPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Enclawed", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = EnclawedPaths.configURL
        guard
