...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: EnclawedProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.enclawed", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: EnclawedKit.AnyCodable]()) {
                $0[$1.key] = EnclawedKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: EnclawedProtocol.AnyCodable?) -> [String: EnclawedProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: EnclawedProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: EnclawedKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: EnclawedProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("enclawed.control.heartbeat")
    static let controlAgentEvent = Notification.Name("enclawed.control.agent")
}
