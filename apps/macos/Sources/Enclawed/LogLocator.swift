...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ENCLAWED_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/enclawed")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("enclawed-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("enclawed-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/enclawed/ (rolling or stdout), or nil if none exist.
