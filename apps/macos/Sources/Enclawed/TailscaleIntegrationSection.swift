...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.enclawed/enclawed.json. Prefer ENCLAWED_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.enclawed/enclawed.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.enclawed/enclawed.json. Restart the gateway to apply."
        }
