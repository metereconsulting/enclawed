...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.enclawed", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> EnclawedSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: EnclawedSessionPreviewEntry,
    ...
    private static func cache(payload: EnclawedSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: EnclawedChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [EnclawedKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [EnclawedKit.AnyCodable]) -> [EnclawedChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(EnclawedChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: EnclawedChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: EnclawedChatMessage) -> Bool {
    ...
    private static func toolNames(for message: EnclawedChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: EnclawedChatMessage) -> String? {
