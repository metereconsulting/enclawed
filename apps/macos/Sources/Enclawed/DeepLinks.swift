...
private let deepLinkLogger = Logger(subsystem: "ai.enclawed", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Enclawed is paused", message: "Unpause Enclawed to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Enclawed agent?", message: body) else { return }
        }
