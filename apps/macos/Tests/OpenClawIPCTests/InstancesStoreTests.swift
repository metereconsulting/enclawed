...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: EnclawedProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: EnclawedProtocol.AnyCodable] = [
            "presence": .init([EnclawedProtocol.AnyCodable(entry)]),
        ]
        let payload = EnclawedProtocol.AnyCodable(payloadMap)
