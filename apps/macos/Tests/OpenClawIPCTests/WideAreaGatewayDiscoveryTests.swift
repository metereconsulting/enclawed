...
import Foundation
import Testing
@testable import EnclawedDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("ENCLAWED_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("ENCLAWED_WIDE_AREA_DOMAIN", "enclawed.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("ENCLAWED_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("ENCLAWED_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._enclawed-gw._tcp.enclawed.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.enclawed.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Enclawed)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/enclawed/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Enclawed)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/enclawed/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._enclawed-gw._tcp.enclawed.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
