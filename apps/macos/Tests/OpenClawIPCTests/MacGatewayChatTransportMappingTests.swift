...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: EnclawedProtocol.AnyCodable(["ok": EnclawedProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: EnclawedProtocol.AnyCodable(["ok": EnclawedProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = EnclawedProtocol.AnyCodable([
            "runId": EnclawedProtocol.AnyCodable("run-1"),
            "sessionKey": EnclawedProtocol.AnyCodable("main"),
            "state": EnclawedProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: EnclawedProtocol.AnyCodable(["a": EnclawedProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
