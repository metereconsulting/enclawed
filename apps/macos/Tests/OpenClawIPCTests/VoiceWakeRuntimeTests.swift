...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["enclawed"]
        let text = "hey enclawed"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey enclawed do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["enclawed"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "enclawed", start: 0.2, duration: 0.1, range: transcript.range(of: "enclawed")),
