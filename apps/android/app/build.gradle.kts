...
val androidStoreFile = providers.gradleProperty("ENCLAWED_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("ENCLAWED_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("ENCLAWED_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("ENCLAWED_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set ENCLAWED_ANDROID_STORE_FILE, " +
            "ENCLAWED_ANDROID_STORE_PASSWORD, ENCLAWED_ANDROID_KEY_ALIAS, and " +
            "ENCLAWED_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/EnclawedKit/Sources/EnclawedKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.enclawed.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "ENCLAWED_ENABLE_SMS", "false")
            buildConfigField("boolean", "ENCLAWED_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "ENCLAWED_ENABLE_SMS", "true")
            buildConfigField("boolean", "ENCLAWED_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "enclawed-$versionName-$buildType.apk"
                    } else {
                        "enclawed-$versionName-$flavorName-$buildType.apk"
                    }
