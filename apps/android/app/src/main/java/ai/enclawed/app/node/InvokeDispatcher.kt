...
import ai.enclawed.app.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      EnclawedCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      EnclawedCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      EnclawedCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      EnclawedCanvasA2UICommand.Push.rawValue, EnclawedCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      EnclawedCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      EnclawedCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      EnclawedCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      EnclawedLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      EnclawedDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      EnclawedDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      EnclawedDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      EnclawedDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      EnclawedNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      EnclawedNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      EnclawedSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.enclawed.app.protocol.EnclawedPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      EnclawedContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      EnclawedContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      EnclawedCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      EnclawedCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      EnclawedMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      EnclawedMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      EnclawedSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      EnclawedSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      EnclawedCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
