...
internal fun enclawedAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Enclawed Android gateway connection failure.
    ...
    - tell me whether `enclawed devices list` should show a pending pairing request
    - if more signal is needed, ask for `enclawed qr --json`, `enclawed devices list`, and `enclawed nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${enclawedAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Enclawed gateway diagnostics", report))
