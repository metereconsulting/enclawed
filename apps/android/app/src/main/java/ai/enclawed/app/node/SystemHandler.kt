...
private const val NOTIFICATION_CHANNEL_BASE_ID = "enclawed.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Enclawed Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Enclawed Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Enclawed Active")
      }
