...
execution:
  kind: flow
  summary: Run with `ENCLAWED_LIVE_SETUP_TOKEN_VALUE=<setup-token> pnpm enclawed qa suite --provider-mode live-frontier --model anthropic/claude-opus-4-6 --alt-model anthropic/claude-opus-4-6 --scenario anthropic-opus-setup-token-smoke`.
  config:
