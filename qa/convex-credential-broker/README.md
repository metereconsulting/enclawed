...
- `ENCLAWED_QA_CONVEX_SECRET_MAINTAINER`
- `ENCLAWED_QA_CONVEX_SECRET_CI`

Client URL policy:

- `ENCLAWED_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `ENCLAWED_QA_ALLOW_INSECURE_HTTP=1`.
...
```bash
pnpm enclawed qa credentials add \
  --kind telegram \
...
Admin endpoints require `ENCLAWED_QA_CONVEX_SECRET_MAINTAINER`.
