...
process.env.ENCLAWED_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

function parseArgs(argv) {
  let packageRoot = process.env.ENCLAWED_BUNDLED_CHANNEL_SMOKE_ROOT;
...
const installedLayoutEnv = "ENCLAWED_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";

function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/enclawed");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "enclawed");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    if (!packageJson.enclawed?.channel) {
      continue;
    }

    const extensionEntries =
      Array.isArray(packageJson.enclawed.extensions) && packageJson.enclawed.extensions.length > 0
        ? packageJson.enclawed.extensions
        : ["./index.ts"];
    ...
    const setupEntry = packageJson.enclawed.setupEntry;
