...
read_config_gateway_token() {
  local config_path="$ENCLAWED_CONFIG_DIR/openclaw.json"
...
ENCLAWED_CONFIG_DIR="${ENCLAWED_CONFIG_DIR:-$HOME/.openclaw}"
ENCLAWED_WORKSPACE_DIR="${ENCLAWED_WORKSPACE_DIR:-$HOME/.openclaw/workspace}"
...
if [[ -z "${ENCLAWED_GATEWAY_TOKEN:-}" ]]; then
  ...
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ...
    echo "Reusing gateway token from $ENCLAWED_CONFIG_DIR/openclaw.json"
  else
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${ENCLAWED_CONFIG_DIR}:/home/node/.openclaw"
    gateway_workspace_mount="${ENCLAWED_WORKSPACE_DIR}:/home/node/.openclaw/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker openclaw-gateway --version >/dev/null 2>&1; then
    ...
    echo "  or use a local build (ENCLAWED_IMAGE=openclaw:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
echo "  ${COMPOSE_HINT} exec openclaw-gateway node dist/index.js health --token \"$ENCLAWED_GATEWAY_TOKEN\""
