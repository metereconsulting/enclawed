...
- regenerates apps/ios/Enclawed.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${ENCLAWED_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ENCLAWED_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ENCLAWED_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ENCLAWED_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ENCLAWED_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing ENCLAWED_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ENCLAWED_URL_SLASH)$(ENCLAWED_URL_SLASH)#g'
)"
...
ENCLAWED_CODE_SIGN_STYLE = Automatic
ENCLAWED_DEVELOPMENT_TEAM = ${TEAM_ID}
ENCLAWED_IOS_SELECTED_TEAM = ${TEAM_ID}
ENCLAWED_APP_BUNDLE_ID = ai.enclawed.client
ENCLAWED_SHARE_BUNDLE_ID = ai.enclawed.client.share
ENCLAWED_ACTIVITY_WIDGET_BUNDLE_ID = ai.enclawed.client.activitywidget
ENCLAWED_WATCH_APP_BUNDLE_ID = ai.enclawed.client.watchkitapp
ENCLAWED_WATCH_EXTENSION_BUNDLE_ID = ai.enclawed.client.watchkitapp.extension
ENCLAWED_APP_PROFILE =
ENCLAWED_SHARE_PROFILE =
ENCLAWED_PUSH_TRANSPORT = relay
ENCLAWED_PUSH_DISTRIBUTION = official
ENCLAWED_URL_SLASH = /
ENCLAWED_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ENCLAWED_PUSH_APNS_ENVIRONMENT = production
EOF
