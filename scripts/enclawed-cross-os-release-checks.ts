...
const PUBLISHED_INSTALLER_BASE_URL = "https://enclawed.ai";
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.ENCLAWED_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.ENCLAWED_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.ENCLAWED_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `enclawed@${previousVersion}` : "enclawed@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.ENCLAWED_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.ENCLAWED_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        ENCLAWED_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `enclawed-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".enclawed");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    ENCLAWED_HOME: lane.homeDir,
    ENCLAWED_STATE_DIR: lane.stateDir,
    ENCLAWED_CONFIG_PATH: join(lane.stateDir, "enclawed.json"),
    ENCLAWED_DISABLE_BONJOUR: "1",
    ENCLAWED_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  const updateEnv = { ...env };
  delete updateEnv.ENCLAWED_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  return updateEnv;
}

export function resolveExplicitBaselineVersion(baselineSpec) {
  const trimmed = baselineSpec.trim();
  if (!trimmed || trimmed === "enclawed@latest") {
    return "";
  }
  if (trimmed.startsWith("enclawed@")) {
    return trimmed.slice("enclawed@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "enclawed@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view enclawed@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'enclawed.cmd'),
      (Join-Path $npmPrefix 'enclawed.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command enclawed -ErrorAction Stop
...
Write-Output "__ENCLAWED_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__ENCLAWED_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed enclawed path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v enclawed >/dev/null 2>&1",
    'printf "__ENCLAWED_PATH__=%s\\n" "$(command -v enclawed)"',
    "enclawed --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__ENCLAWED_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed enclawed path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `enclawed update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.enclawed/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.ENCLAWED_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.ENCLAWED_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.ENCLAWED_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.ENCLAWED_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
async function runAgentTurn(params) {
  const sessionId = `cross-os-release-check-${params.label}-${Date.now()}`;
  const result = await runEnclawed({
    lane: params.lane,
    env: params.env,
    args: [
      "agent",
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>Enclawed Control</title>") &&
          html.includes("<enclawed-app></enclawed-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>Enclawed Control</title>")} app=${html.includes("<enclawed-app></enclawed-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runEnclawed(params) {
...
function installedPackageRoot(prefixDir) {
  return process.platform === "win32"
    ? join(prefixDir, "node_modules", "enclawed")
    : join(prefixDir, "lib", "node_modules", "enclawed");
}

function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "enclawed.mjs");
}
