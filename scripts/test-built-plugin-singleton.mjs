...
const { clearPluginCommands, getPluginCommandSpecs, loadEnclawedPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadEnclawedPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@enclawed/build-smoke-plugin",
      type: "module",
      enclawed: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "enclawed.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from '@enclawed/plugin-sdk';",
...
const registry = loadEnclawedPlugins({
  ...
  env: {
    ...process.env,
    ENCLAWED_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
