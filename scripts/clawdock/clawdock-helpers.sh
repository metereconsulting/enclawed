#!/usr/bin/env bash
# ClawDock - Docker helpers for Enclawed
# Inspired by Simon Willison's "Running Enclawed in Docker"
# https://til.simonwillison.net/llms/enclawed-docker
#
# Installation:
...
# Common paths to check for Enclawed
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/enclawed"
  "${HOME}/workspace/enclawed"
  "${HOME}/projects/enclawed"
  "${HOME}/dev/enclawed"
  "${HOME}/code/enclawed"
  "${HOME}/src/enclawed"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Enclawed at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/enclawed"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Enclawed not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/enclawed/enclawed.git ~/enclawed"
    echo "  cd ~/enclawed && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ENCLAWED_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d enclawed-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart enclawed-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f enclawed-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.enclawed"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show enclawed.json
  if [[ -f "${config_dir}/enclawed.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/enclawed.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/enclawed.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No enclawed.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.enclawed/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec enclawed-gateway \
    bash -c 'echo "alias enclawed=\"./enclawed.mjs\"" > /tmp/.bashrc_enclawed && bash --rcfile /tmp/.bashrc_enclawed'
}
...
clawdock-cli() {
  _clawdock_compose run --rm enclawed-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build enclawed-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d enclawed-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build enclawed-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ENCLAWED_GATEWAY_TOKEN=$token" enclawed-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" enclawed-gateway \
    bash -c './enclawed.mjs config set gateway.remote.token "$TOKEN" && ./enclawed.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec enclawed-gateway \
    bash -c "./enclawed.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart enclawed-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm enclawed-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec enclawed-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'enclawed config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Enclawed${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (enclawed alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to enclawed project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.enclawed)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'enclawed channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'enclawed status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.enclawed.ai${_CLR_RESET}"
  echo ""
}
