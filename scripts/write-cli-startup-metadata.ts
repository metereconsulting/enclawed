...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.enclawed?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.enclawed?.channel?.order;
      const labelRaw = parsed.enclawed?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".enclawed-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "enclawed-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "enclawed-build",
    ...
    ENCLAWED_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
    ENCLAWED_PLUGIN_DISCOVERY_CACHE_MS: "0",
    ENCLAWED_PLUGIN_MANIFEST_CACHE_MS: "0",
