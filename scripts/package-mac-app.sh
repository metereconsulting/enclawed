...
# Build and bundle Enclawed into a minimal .app we can open.
# Outputs to dist/Enclawed.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Enclawed.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Enclawed"
BUNDLE_ID="${BUNDLE_ID:-ai.enclawed.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/enclawed/enclawed/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Enclawed/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :EnclawedBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :EnclawedGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Enclawed"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Enclawed"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Enclawed"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Enclawed" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Enclawed/Resources/Enclawed.icns" "$APP_ROOT/Contents/Resources/Enclawed.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Enclawed/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying EnclawedKit resources"
ENCLAWEDKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/EnclawedKit_EnclawedKit.bundle"
if [ -d "$ENCLAWEDKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/EnclawedKit_EnclawedKit.bundle"
  cp -R "$ENCLAWEDKIT_BUNDLE" "$APP_ROOT/Contents/Resources/EnclawedKit_EnclawedKit.bundle"
else
  echo "WARN: EnclawedKit resource bundle not found at $ENCLAWEDKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Enclawed"
killall -q Enclawed 2>/dev/null || true
