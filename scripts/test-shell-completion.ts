 ...
 * `enclawed update`. Use it to verify the completion installation flow
...
\`enclawed update\`, \`enclawed doctor\`, and \`enclawed onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Enclawed Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`enclawed completion --install\` later to enable.`));
    return;
  }
