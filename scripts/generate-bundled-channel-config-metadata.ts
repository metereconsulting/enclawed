...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const enclawedMeta =
    ...
    "enclawed" in source.packageJson
      ? (source.packageJson.enclawed as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    enclawedMeta &&
    typeof enclawedMeta.channel === "object" &&
    enclawedMeta.channel &&
    !Array.isArray(enclawedMeta.channel)
      ? (enclawedMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
