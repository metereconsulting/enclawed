...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[enclawed] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.ENCLAWED_WATCH_MODE = "1";
  childEnv.ENCLAWED_WATCH_SESSION = watchSession;
  ...
  childEnv.ENCLAWED_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.ENCLAWED_WATCH_COMMAND = deps.args.join(" ");
  }
