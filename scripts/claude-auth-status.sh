...
# Checks both Claude Code and Enclawed auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ENCLAWED_AUTH="$HOME/.enclawed/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    enclawed models status --json 2>/dev/null || true
}
...
check_enclawed_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$ENCLAWED_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    enclawed_status=$(check_enclawed_auth 2>/dev/null || true)

    claude_expires=0
    enclawed_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        enclawed_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        enclawed_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ENCLAWED_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$enclawed_status" \
        --arg be "$enclawed_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            enclawed: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$enclawed_status" == EXPIRED* ]] || [[ "$enclawed_status" == MISSING* ]]; then
        echo "ENCLAWED_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$enclawed_status" == EXPIRING* ]]; then
        echo "ENCLAWED_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "Enclawed Auth (~/.enclawed/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ENCLAWED_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ENCLAWED_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'enclawed doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active enclawed >/dev/null 2>&1; then
    echo -e "Enclawed service: ${GREEN}running${NC}"
else
    echo -e "Enclawed service: ${RED}NOT running${NC}"
fi
