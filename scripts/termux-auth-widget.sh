...
# Enclawed Auth Widget for Termux
...
SERVER="${ENCLAWED_SERVER:-l36}"

# Check auth status
termux-toast "Checking Enclawed auth..."

STATUS=$(ssh "$SERVER" '$HOME/enclawed/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/enclawed/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ENCLAWED_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/enclawed/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ENCLAWED_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Enclawed won't work until re-auth"
                ;;
        esac
        ;;
