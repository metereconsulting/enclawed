...
show_usage() {
    cat << EOF
clawlog - Enclawed Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Enclawed logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Enclawed logs flow through the macOS unified log (subsystem: ai.enclawed).
