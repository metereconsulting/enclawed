...
install_openclaw() {
    ...
    if [[ -n "$resolved_version" ]]; then
        ...
        ui_info "Installing OpenClaw (${ENCLAWED_VERSION})"
    fi
    local install_spec=""
    ...
    if [[ "${ENCLAWED_VERSION}" == "latest" && "${package_name}" == "openclaw" ]]; then
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${ENCLAWED_CONFIG_PATH:-$HOME/.openclaw/openclaw.json}"
...
main() {
    ...
    ENCLAWED_BIN="$(resolve_openclaw_bin || true)"
