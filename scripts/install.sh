...
# enclawed installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://enclawed.com/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one enclawed."
...
GUM_VERSION="${ENCLAWED_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞🔒 enclawed installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://enclawed.com/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$ENCLAWED_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.enclawed.com/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_enclawed_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.enclawed-* "$npm_root"/enclawed 2>/dev/null || true
}

extract_enclawed_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_enclawed_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/enclawed" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/enclawed"|"/usr/local/bin/enclawed")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/enclawed/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale enclawed symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing enclawed binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing enclawed package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_enclawed_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "enclawed npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*enclawed" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_enclawed_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_enclawed_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "enclawed npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an enclawed binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${ENCLAWED_TAGLINE_INDEX:-}" ]]; then
        if [[ "${ENCLAWED_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((ENCLAWED_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${ENCLAWED_NO_ONBOARD:-0}
NO_PROMPT=${ENCLAWED_NO_PROMPT:-0}
DRY_RUN=${ENCLAWED_DRY_RUN:-0}
INSTALL_METHOD=${ENCLAWED_INSTALL_METHOD:-}
ENCLAWED_VERSION=${ENCLAWED_VERSION:-latest}
USE_BETA=${ENCLAWED_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/enclawed"
GIT_DIR=${ENCLAWED_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${ENCLAWED_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${ENCLAWED_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${ENCLAWED_VERBOSE:-0}"
VERIFY_INSTALL="${ENCLAWED_VERIFY_INSTALL:-0}"
ENCLAWED_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
enclawed installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://enclawed.com/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/enclawed)
...
Environment variables:
  ENCLAWED_INSTALL_METHOD=git|npm
  ENCLAWED_VERSION=latest|next|main|<semver>|<spec>
  ENCLAWED_BETA=0|1
  ENCLAWED_GIT_DIR=...
  ENCLAWED_GIT_UPDATE=0|1
  ENCLAWED_NO_PROMPT=1
  ENCLAWED_VERIFY_INSTALL=1
  ENCLAWED_DRY_RUN=1
  ENCLAWED_NO_ONBOARD=1
  ENCLAWED_VERBOSE=1
  ENCLAWED_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://enclawed.com/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://enclawed.com/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://enclawed.com/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://enclawed.com/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://enclawed.com/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected enclawed checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a enclawed source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_enclawed_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"enclawed"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://enclawed.com/install.sh | bash"
}
...
ensure_enclawed_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/enclawed" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/enclawed" ]]; then
        ln -sf "$npm_root/enclawed/dist/entry.js" "${npm_bin}/enclawed"
        ui_info "Created enclawed bin link at ${npm_bin}/enclawed"
    fi
    return 0
}

# Check for existing enclawed installation
check_existing_enclawed() {
    if [[ -n "$(type -P enclawed 2>/dev/null || true)" ]]; then
        ui_info "Existing enclawed installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make enclawed show as \"command not found\" in new terminals."
...
warn_enclawed_not_found() {
    ui_warn "Installed, but enclawed is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t enclawed 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named enclawed; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_enclawed_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P enclawed 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/enclawed" ]]; then
        echo "${npm_bin}/enclawed"
        return 0
    fi
...
install_enclawed_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/metereconsulting/enclawed.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing enclawed from git checkout: ${repo_dir}"
    else
        ui_info "Installing enclawed from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning enclawed" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building enclawed" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/enclawed" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/enclawed"
    ui_success "enclawed wrapper installed to \$HOME/.local/bin/enclawed"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view enclawed dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:metereconsulting/enclawed#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_enclawed() {
    local package_name="enclawed"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            ENCLAWED_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="enclawed"
        else
            ENCLAWED_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${ENCLAWED_VERSION}" ]]; then
        ENCLAWED_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${ENCLAWED_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${ENCLAWED_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing enclawed v${resolved_version}"
    else
        ui_info "Installing enclawed (${ENCLAWED_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${ENCLAWED_VERSION}")"

    if ! install_enclawed_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_enclawed_paths
        install_enclawed_npm "${install_spec}"
    fi

    if [[ "${ENCLAWED_VERSION}" == "latest" && "${package_name}" == "enclawed" ]]; then
        if ! resolve_enclawed_bin &> /dev/null; then
            ui_warn "npm install enclawed@latest failed; retrying enclawed@next"
            cleanup_npm_enclawed_paths
            install_enclawed_npm "enclawed@next"
        fi
    fi

    ensure_enclawed_bin_link || true

    ui_success "enclawed installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${ENCLAWED_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_enclawed_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (enclawed not on PATH yet)"
        warn_enclawed_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${ENCLAWED_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.enclawed/workspace-${profile}"
    else
        echo "${HOME}/.enclawed/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${ENCLAWED_CONFIG_PATH:-$HOME/.enclawed/enclawed.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run enclawed onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but enclawed not on PATH; skipping onboarding"
        warn_enclawed_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run enclawed onboard to retry"
        return
    }
}
...
resolve_enclawed_version() {
    local version=""
    local raw_version_output=""
    local claw="${ENCLAWED_BIN:-}"
    if [[ -z "$claw" ]] && command -v enclawed &> /dev/null; then
        claw="$(command -v enclawed)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_enclawed_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/enclawed/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/enclawed/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: enclawed not on PATH yet"
        warn_enclawed_not_found
        return 1
    fi

    run_quiet_step "Checking enclawed version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: enclawed gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_enclawed_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found enclawed checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set ENCLAWED_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_enclawed; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing enclawed"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g enclawed &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g enclawed 2>/dev/null || true
        ...
        install_enclawed_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/enclawed" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/enclawed"
    ...
    ENCLAWED_BIN="$(resolve_enclawed_bin || true)"
    ...
    installed_version=$(resolve_enclawed_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞🔒 enclawed installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞🔒 enclawed installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/enclawed"
        ui_kv "Update command" "enclawed update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://enclawed.com/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running enclawed doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                ENCLAWED_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                ENCLAWED_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                ENCLAWED_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run enclawed doctor and enclawed plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run enclawed onboard later"
        else
            local config_path="${ENCLAWED_CONFIG_PATH:-$HOME/.enclawed/enclawed.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (enclawed not on PATH yet)"
                    warn_enclawed_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run enclawed onboard to finish setup"
            return 0
        fi
    fi

    if command -v enclawed &> /dev/null; then
        local claw="${ENCLAWED_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_enclawed_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (enclawed daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if ENCLAWED_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: enclawed daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${ENCLAWED_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
