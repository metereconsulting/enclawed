...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/enclawed/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Enclawed Re-Auth" -c "After getting key, run: ssh $SERVER '~/enclawed/scripts/mobile-reauth.sh'" --id enclawed-auth
        ;;
    *)
