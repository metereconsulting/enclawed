...
#
# Suggested cron: */30 * * * * /home/admin/enclawed/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Enclawed notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., enclawed-alerts)
...
STATE_FILE="$HOME/.enclawed/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Enclawed if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use enclawed
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Enclawed to $NOTIFY_PHONE..."
            enclawed send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Enclawed is down. Run: ssh l36 '~/enclawed/scripts/mobile-reauth.sh'" "urgent"
    exit 1
