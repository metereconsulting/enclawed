#!/bin/bash
# Setup Enclawed Auth Management System
...
echo "=== Enclawed Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'enclawed-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Enclawed message: Send warning via Enclawed itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/enclawed-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/enclawed-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/enclawed-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now enclawed-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status enclawed-auth-monitor.timer"
echo ""
echo "Quick commands:"
