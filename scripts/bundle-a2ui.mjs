...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "EnclawedKit", "Tools", "CanvasA2UI");
...
export function getBundleHashRepoInputPaths(repoRoot = rootDir) {
  return [
    ...
    path.join(repoRoot, "apps", "shared", "EnclawedKit", "Tools", "CanvasA2UI"),
  ];
}
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.ENCLAWED_SPARSE_PROFILE || process.env.ENCLAWED_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because ENCLAWED_A2UI_SKIP_MISSING=1 or ENCLAWED_SPARSE_PROFILE is set.",
      );
      return;
    }
