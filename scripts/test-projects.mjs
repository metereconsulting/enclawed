...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.ENCLAWED_VITEST_MAX_WORKERS || env.ENCLAWED_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      ENCLAWED_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
  ...
  if (isFullSuiteRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.ENCLAWED_TEST_PROJECTS_PARALLEL &&
        !process.env.ENCLAWED_VITEST_MAX_WORKERS &&
        !process.env.ENCLAWED_TEST_WORKERS &&
