# enclawed Installer for Windows (PowerShell)
# Usage: iwr -useb https://enclawed.com/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://enclawed.com/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\enclawed",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞🔒 enclawed Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one enclawed.$NC" -Level info
    Write-Host ""
}
...
function Install-enclawedNpm {
    param([string]$Target = "latest")

    $installSpec = Resolve-PackageInstallSpec -Target $Target
    
    Write-Host "Installing enclawed ($installSpec)..." -Level info
    ...
    try {
        ...
        Write-Host "enclawed installed" -Level success
        return $true
    } catch {
...
function Install-enclawedGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing enclawed from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/enclawed/enclawed.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\enclawed\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\enclawed.cmd" -Encoding ASCII -Force
...
function Resolve-PackageInstallSpec {
    ...
    if ([string]::IsNullOrWhiteSpace($trimmed)) {
        return "enclawed@latest"
    }
    if ($trimmed.ToLowerInvariant() -eq "main") {
        return "github:metereconsulting/enclawed#main"
    }
    if (Test-ExplicitPackageInstallSpec -Target $trimmed) {
        return $trimmed
    }
    return "enclawed@$trimmed"
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install enclawed from git to $GitDir" -Level info
        } else {
            Install-enclawedGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install enclawed via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-enclawedNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'enclawed onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞🔒 enclawed installed successfully!" -Level success
}
