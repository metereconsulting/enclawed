...
const EXPECTED_REPOSITORY_URL = "https://github.com/enclawed/enclawed";
...
const skipPackValidationEnv = "ENCLAWED_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "enclawed") {
    errors.push(`package.json name must be "enclawed"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.enclawed !== "enclawed.mjs") {
    errors.push(
      `package.json bin.enclawed must be "enclawed.mjs"; found "${pkg.bin?.enclawed ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`enclawed-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `enclawed-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
