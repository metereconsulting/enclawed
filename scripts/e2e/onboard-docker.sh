...
IMAGE_NAME="enclawed-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/enclawed-bin:$PATH"
  mkdir -p /tmp/enclawed-bin
  cat > /tmp/enclawed-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/enclawed-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$ENCLAWED_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_enclawed_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/enclawed-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/enclawed-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$ENCLAWED_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/enclawed-e2e-$1.XXXXXX"
  }

  set_isolated_enclawed_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export ENCLAWED_HOME="$home_dir"
    export ENCLAWED_STATE_DIR="$home_dir/.enclawed"
    export ENCLAWED_CONFIG_PATH="$ENCLAWED_STATE_DIR/enclawed.json"
    mkdir -p "$ENCLAWED_STATE_DIR"
  }
  ...
  run_case_logged() {
    local label="$1"
    shift
    local log_path="/tmp/enclawed-onboard-${label}.log"
  ...
  run_case_local_basic() {
    ...
    set_isolated_enclawed_env "$home_dir"
    run_case_logged local-basic node "$ENCLAWED_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$ENCLAWED_STATE_DIR/workspace"
    config_path="$ENCLAWED_CONFIG_PATH"
    sessions_dir="$ENCLAWED_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_enclawed_env "$home_dir"
	    ...
	    run_case_logged remote-non-interactive node "$ENCLAWED_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
JSON
	    ...
	    run_case_logged reset-config node "$ENCLAWED_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$ENCLAWED_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$ENCLAWED_ENTRY\" configure --section skills" send_skills_flow
