...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-enclawed-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://enclawed.ai/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/enclawed-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-enclawed-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://enclawed.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run enclawed update --channel dev.
...
# enclawed.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$enclawed @args 2>&1
if (\$null -ne \$output) {
...
baseline_install_version() {
  ...
  npm view "enclawed@$INSTALL_VERSION" version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "enclawed-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_powershell "$(cat <<EOF
\$depsRoot = Join-Path \$env:LOCALAPPDATA 'Enclawed\deps'
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/enclawed-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
write_latest_install_runner_script() {
  ...
  WINDOWS_LATEST_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/enclawed-install-latest.ps1"
...
try {
  ...
  & (Join-Path \$env:APPDATA 'npm\enclawed.cmd') --version *>&1 | Tee-Object -FilePath \$LogPath -Append | Out-Null
  if (\$LASTEXITCODE -ne 0) {
    throw "enclawed --version failed with exit code \$LASTEXITCODE"
  }
...
write_baseline_npm_install_runner_script() {
  WINDOWS_BASELINE_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/enclawed-install-baseline-npm.ps1"
...
try {
  $portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Enclawed\deps') 'portable-git') ''
  $env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
  $enclawed = Join-Path $env:APPDATA 'npm\enclawed.cmd'

  Write-ProgressLog 'install.start'
  Invoke-Logged 'npm install baseline release' {
    & npm.cmd install -g "enclawed@$Version" --no-fund --no-audit --loglevel=error
  }

  Write-ProgressLog 'install.version'
  Invoke-Logged 'enclawed --version' { & $enclawed --version }
...
install_baseline_npm_release() {
  ...
  runner_name="enclawed-install-baseline-$RANDOM-$RANDOM.ps1"
  log_name="enclawed-install-baseline-$RANDOM-$RANDOM.log"
  done_name="enclawed-install-baseline-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-install-baseline-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-install-baseline-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
...
install_latest_release() {
  ...
  runner_name="enclawed-install-latest-$RANDOM-$RANDOM.ps1"
  log_name="enclawed-install-latest-$RANDOM-$RANDOM.log"
  done_name="enclawed-install-latest-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-install-latest-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
  startup_checked=0
...
install_main_tgz() {
  ...
  runner_name="enclawed-install-$RANDOM-$RANDOM.ps1"
  log_name="enclawed-install-$RANDOM-$RANDOM.log"
  done_name="enclawed-install-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-install-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-install-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
...
write_dev_update_runner_script() {
  WINDOWS_DEV_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/enclawed-update-dev.ps1"
...
try {
  ...
  $enclawed = Join-Path $env:APPDATA 'npm\enclawed.cmd'
  $gitRoot = Join-Path $env:USERPROFILE 'enclawed'
  $gitEntry = Join-Path $gitRoot 'enclawed.mjs'
...
"@ | Set-Content -Path $pnpmPsShim -Encoding UTF8
  ...
  Invoke-Logged 'enclawed update --channel dev --yes --json' {
    & $enclawed update --channel dev --yes --json
  }
  ...
  Invoke-Logged 'git enclawed --version' { & node.exe $gitEntry --version }
  Invoke-Logged 'git enclawed update status --json' { & node.exe $gitEntry update status --json }
...
run_dev_channel_update() {
  ...
  runner_name="enclawed-update-dev-$RANDOM-$RANDOM.ps1"
  log_name="enclawed-update-dev-$RANDOM-$RANDOM.log"
  done_name="enclawed-update-dev-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-update-dev-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
...
if ($busy) {
  throw 'dev update still has active npm/pnpm/enclawed processes'
}
$gitEntry = Join-Path $env:USERPROFILE 'enclawed\enclawed.mjs'
if (-not (Test-Path $gitEntry)) {
...
write_install_runner_script() {
  WINDOWS_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/enclawed-install-main.ps1"
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_enclawed "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/enclawed-onboard-$PROVIDER.ps1"
...
try {
  \$enclawed = Join-Path \$env:APPDATA 'npm\enclawed.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice ${AUTH_CHOICE} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json > "{1}" 2>&1' -f \$enclawed, \$LogPath)
...
run_ref_onboard() {
  ...
  runner_name="enclawed-onboard-$RANDOM-$RANDOM.ps1"
  log_name="enclawed-onboard-$RANDOM-$RANDOM.log"
  done_name="enclawed-onboard-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
verify_gateway() {
  guest_run_enclawed "" "" gateway status --deep --require-rpc
}
...
verify_gateway_reachable() {
  ...
  for attempt in 1 2 3 4 5 6; do
    probe_json="$(
      guest_run_enclawed "" "" gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json
    )"
    printf '%s\n' "$probe_json"
...
$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Enclawed\deps') 'portable-git') ''
$env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
$gitEntry = Join-Path (Join-Path $env:USERPROFILE 'enclawed') 'enclawed.mjs'
if (-not (Test-Path $gitEntry)) {
...
run_gateway_daemon_action() {
  ...
  local runner_name log_name done_name done_status launcher_state
  local poll_rc state_rc log_rc start_seconds poll_deadline startup_checked
  runner_name="enclawed-gateway-$action-$RANDOM-$RANDOM.ps1"
  log_name="enclawed-gateway-$action-$RANDOM-$RANDOM.log"
  done_name="enclawed-gateway-$action-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
try {
  \$enclawed = Join-Path \$env:APPDATA 'npm\enclawed.cmd'
  & \$enclawed gateway $action *>&1 | Tee-Object -FilePath \$log -Append | Out-Null
...
show_gateway_status_compat() {
  if guest_run_enclawed "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_enclawed "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_enclawed "" "" gateway status --deep
}

verify_turn() {
  guest_run_enclawed "" "" models set "$MODEL_ID"
  guest_run_enclawed "$API_KEY_ENV" "$API_KEY_VALUE" \
...
run_fresh_main_lane() {
  ...
  if phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "enclawed-main-fresh.tgz"; then
    ...
    phase_run "fresh.install-main-retry" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "enclawed-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-baseline-package" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "enclawed-main-upgrade.tgz" || return $?
