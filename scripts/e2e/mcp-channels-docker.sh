...
IMAGE_NAME="${ENCLAWED_IMAGE:-enclawed-mcp-channels-e2e}"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="enclawed-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t enclawed-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "ENCLAWED_SKIP_GMAIL_WATCHER=1" \
  -e "ENCLAWED_SKIP_CRON=1" \
  -e "ENCLAWED_SKIP_CANVAS_HOST=1" \
  -e "ENCLAWED_STATE_DIR=/tmp/enclawed-state" \
  -e "ENCLAWED_CONFIG_PATH=/tmp/enclawed-state/enclawed.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "ENCLAWED_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
