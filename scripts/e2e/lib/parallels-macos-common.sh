...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/enclawed-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/enclawed-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${ENCLAWED_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${ENCLAWED_SECRET_ENV_NAME:-}" ] && [ -n "${ENCLAWED_SECRET_FILE:-}" ] && [ -f "$ENCLAWED_SECRET_FILE" ]; then
  secret_value="$(cat "$ENCLAWED_SECRET_FILE")"
  rm -f "$ENCLAWED_SECRET_FILE"
  export "${ENCLAWED_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "ENCLAWED_SECRET_ENV_NAME=$api_key_env" \
      "ENCLAWED_SECRET_FILE=$secret_path" \
      "ENCLAWED_WRAPPER_FILE=$wrapper_path" \
