...
IMAGE_NAME="enclawed-openwebui-e2e"
...
MODEL="${ENCLAWED_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${ENCLAWED_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${ENCLAWED_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${ENCLAWED_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${ENCLAWED_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${ENCLAWED_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="enclawed-openwebui-e2e-$$"
GW_NAME="enclawed-openwebui-gateway-$$"
OW_NAME="enclawed-openwebui-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ENCLAWED_GATEWAY_TOKEN=$TOKEN" \
  -e "ENCLAWED_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/enclawed-openwebui-config.XXXXXX.json)"
    ENCLAWED_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.ENCLAWED_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.ENCLAWED_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.ENCLAWED_OPENWEBUI_MODEL },
];
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Enclawed E2E" \
  -e WEBUI_SECRET_KEY="enclawed-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Enclawed E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="enclawed/default" \
...
echo "Running Open WebUI -> Enclawed smoke..."
if ! docker exec \
