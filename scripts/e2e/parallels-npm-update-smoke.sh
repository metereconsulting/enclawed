...
UPDATE_TARGET=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/enclawed-parallels-npm-update.XXXXXX)"
...
PROGRESS_STALE_S=60
TIMEOUT_UPDATE_S="${ENCLAWED_PARALLELS_NPM_UPDATE_TIMEOUT_S:-900}"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: enclawed@latest
  --update-target <target>    Target passed to guest 'enclawed update --tag'.
                             Default: host-served tgz packed from current checkout.
                             Examples: latest, beta, 2026.4.10, http://host/enclawed.tgz
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/enclawed-main-$CURRENT_HEAD_SHORT.tgz"
...
write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/enclawed-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/enclawed warnings do not
...
function Wait-GatewayRpcReady {
  param(
    [Parameter(Mandatory = $true)][string]$EnclawedPath,
    [int]$Attempts = 20,
    [int]$SleepSeconds = 3
  )
  ...
  for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
    ...
    try {
      Invoke-Logged 'enclawed gateway status' { & $EnclawedPath gateway status --deep --require-rpc }
      return
    } catch {
...
function Stop-EnclawedGatewayProcesses {
  Write-ProgressLog 'update.stop-old-gateway'
  $patterns = @(
    'enclawed-gateway',
    'enclawed.*gateway --port 18789',
    'enclawed.*gateway run',
    'enclawed\.mjs gateway',
...
function Restart-GatewayWithRecovery {
  ...
  if ($null -ne $restartCompleted) {
    ...
    if ($restartResult.ExitCode -ne 0) {
      ...
      "enclawed gateway restart failed with exit code $($restartResult.ExitCode)" | Tee-Object -FilePath $LogPath -Append | Out-Null
    }
  } else {
    $restartFailed = $true
    Stop-Job $restartJob -ErrorAction SilentlyContinue
    Write-ProgressLog 'update.restart-gateway.timeout'
    'enclawed gateway restart timed out after 20s; continuing to RPC readiness checks' | Tee-Object -FilePath $LogPath -Append | Out-Null
  }
  Remove-Job $restartJob -Force -ErrorAction SilentlyContinue

  Write-ProgressLog 'update.gateway-status'
  try {
    Wait-GatewayRpcReady -EnclawedPath $EnclawedPath
    return
  } catch {
    ...
    Invoke-Logged 'enclawed gateway start' { & $EnclawedPath gateway start }
...
try {
  $env:PATH = "$env:LOCALAPPDATA\Enclawed\deps\portable-git\cmd;$env:LOCALAPPDATA\Enclawed\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\Enclawed\deps\portable-git\usr\bin;$env:PATH"
  ...
  $enclawed = Join-Path $env:APPDATA 'npm\enclawed.cmd'
  Stop-EnclawedGatewayProcesses
  Write-ProgressLog 'update.enclawed-update'
  Invoke-Logged 'enclawed update' { & $enclawed update --tag $UpdateTarget --yes --json }
  Write-ProgressLog 'update.verify-version'
  $version = Invoke-CaptureLogged 'enclawed --version' { & $enclawed --version }
  ...
  Invoke-Logged 'enclawed update status' { & $enclawed update status --json }
  Write-ProgressLog 'update.set-model'
  Invoke-Logged 'enclawed models set' { & $enclawed models set $ModelId }
  ...
  Restart-GatewayWithRecovery -EnclawedPath $enclawed
  Write-ProgressLog 'update.agent-turn'
  Invoke-CaptureLogged 'enclawed agent' { & $enclawed agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/enclawed-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_macos_update_after_transport_loss() {
  ...
  local script_path="/tmp/enclawed-npm-update-macos-recover.sh"
...
busy="\$(/bin/ps -axo command | /usr/bin/egrep 'enclawed update|npm install|pnpm install|pnpm run build' | /usr/bin/egrep -v 'egrep|enclawed-npm-update-macos-recover' || true)"
if [ -n "\$busy" ]; then
  printf 'update still has active npm/pnpm/enclawed processes\n%s\n' "\$busy" >&2
  exit 1
fi
version="\$(/opt/homebrew/bin/enclawed --version)"
printf '%s\n' "\$version"
...
/opt/homebrew/bin/enclawed gateway status --deep --require-rpc >/dev/null 2>&1 || /opt/homebrew/bin/enclawed gateway restart || true
gateway_ready=0
for _ in 1 2 3 4 5 6; do
  if /opt/homebrew/bin/enclawed gateway status --deep --require-rpc; then
    gateway_ready=1
    break
  fi
  sleep 2
...
if [ "\$gateway_ready" != "1" ]; then
  /opt/homebrew/bin/enclawed gateway start || true
  for _ in 1 2 3 4 5 6; do
    if /opt/homebrew/bin/enclawed gateway status --deep --require-rpc; then
      gateway_ready=1
      break
...
/opt/homebrew/bin/enclawed models set "$MODEL_ID"
/opt/homebrew/bin/enclawed agent --agent main --session-id "parallels-npm-update-macos-transport-recovery-$expected_needle" --message "Reply with exact ASCII text OK only." --json
EOF
...
\$ErrorActionPreference = 'Stop'
\$enclawed = Join-Path \$env:APPDATA 'npm\\enclawed.cmd'
if (-not (Test-Path \$enclawed)) {
  throw "enclawed shim missing: \$enclawed"
}
\$busy = Get-CimInstance Win32_Process |
  Where-Object {
    \$_.CommandLine -and
    (\$_.CommandLine -match 'enclawed update|npm install|pnpm install|pnpm run build')
  }
if (\$busy) {
  throw 'update still has active npm/pnpm/enclawed processes'
}
\$version = & \$enclawed --version
...
\$gatewayReady = \$false
for (\$i = 0; \$i -lt 6; \$i++) {
  & \$enclawed gateway status --deep --require-rpc
...
& \$enclawed models set '$MODEL_ID'
& \$enclawed agent --agent main --session-id 'parallels-npm-update-windows-transport-recovery-$expected_needle' --message 'Reply with exact ASCII text OK only.' --json
EOF
  )"
  local rc=$?
  set -e
...
matches = [match for match in matches if re.search(r"Enclawed \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="enclawed-update-$RANDOM-$RANDOM.ps1"
  log_name="enclawed-update-$RANDOM-$RANDOM.log"
  done_name="enclawed-update-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-update-log-state.XXXXXX")"
  : >"$log_state_path"
  provider_key_b64="$(
...
run_macos_update() {
  ...
  cat <<EOF | prlctl exec "$MACOS_VM" /usr/bin/tee /tmp/enclawed-main-update.sh >/dev/null
set -euo pipefail
export PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin
...
cd "\$HOME"
stop_enclawed_gateway_processes() {
  /opt/homebrew/bin/enclawed gateway stop >/dev/null 2>&1 || true
  /usr/bin/pkill -9 -f enclawed-gateway || true
  /usr/bin/pkill -9 -f 'enclawed gateway run' || true
  /usr/bin/pkill -9 -f 'enclawed.mjs gateway' || true
...
stop_enclawed_gateway_processes
/opt/homebrew/bin/enclawed update --tag "$update_target" --yes --json
...
/opt/homebrew/bin/enclawed update status --json
/opt/homebrew/bin/enclawed models set "$MODEL_ID"
...
/opt/homebrew/bin/enclawed gateway restart || true
gateway_ready=0
for _ in 1 2 3 4 5 6 7 8; do
  if /opt/homebrew/bin/enclawed gateway status --deep --require-rpc >/dev/null 2>&1; then
    gateway_ready=1
    break
  fi
  sleep 2
...
if [ "\$gateway_ready" != "1" ]; then
  stop_enclawed_gateway_processes
  /opt/homebrew/bin/enclawed gateway run --bind loopback --port 18789 --force >/tmp/enclawed-parallels-npm-update-macos-gateway.log 2>&1 </dev/null &
  for _ in 1 2 3 4 5 6 7 8; do
    if /opt/homebrew/bin/enclawed gateway status --deep --require-rpc >/dev/null 2>&1; then
      gateway_ready=1
      break
...
if [ "\$gateway_ready" != "1" ]; then
  tail -n 120 /tmp/enclawed-parallels-npm-update-macos-gateway.log 2>/dev/null || true
fi
/opt/homebrew/bin/enclawed gateway status --deep --require-rpc
/opt/homebrew/bin/enclawed agent --agent main --session-id parallels-npm-update-macos-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  macos_desktop_user_exec /bin/bash /tmp/enclawed-main-update.sh
}
...
run_linux_update() {
  local update_target="$1"
  local expected_needle="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/enclawed-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
stop_enclawed_gateway_processes() {
  enclawed gateway stop >/dev/null 2>&1 || true
  pkill -9 -f enclawed-gateway || true
  pkill -9 -f 'enclawed gateway run' || true
  pkill -9 -f 'enclawed.mjs gateway' || true
...
stop_enclawed_gateway_processes
enclawed update --tag "$update_target" --yes --json
...
enclawed update status --json
enclawed models set "$MODEL_ID"
enclawed agent --local --agent main --session-id parallels-npm-update-linux-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/enclawed-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="enclawed@$LATEST_VERSION"
fi
resolve_current_head
...
say "Run same-guest enclawed update to $UPDATE_TARGET_EFFECTIVE"
