...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/enclawed-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...process.env,
      ENCLAWED_ALLOW_INSECURE_PRIVATE_WS: "1",
      ENCLAWED_STATE_DIR: "/tmp/enclawed-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[enclawed mcp] ${String(chunk)}`);
  });
