...
async function main() {
  const stateDir = process.env.ENCLAWED_STATE_DIR?.trim() || path.join(os.homedir(), ".enclawed");
  const configPath =
    process.env.ENCLAWED_CONFIG_PATH?.trim() || path.join(stateDir, "enclawed.json");
