...
install_latest_release() {
  ...
  guest_exec /usr/bin/env ENCLAWED_NO_ONBOARD=1 bash /tmp/openclaw-install.sh "${version_args[@]}" --no-onboard
...
setsid sh -lc 'exec env ENCLAWED_HOME=/root ENCLAWED_STATE_DIR=/root/.openclaw ENCLAWED_CONFIG_PATH=/root/.openclaw/openclaw.json ${API_KEY_ENV}=${api_key_value_q} openclaw gateway run --bind loopback --port 18789 --force >/tmp/openclaw-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
