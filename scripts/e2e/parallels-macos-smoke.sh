...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_ENCLAWED_BIN="/opt/homebrew/bin/openclaw"
GUEST_ENCLAWED_ENTRY="/opt/homebrew/lib/node_modules/openclaw/openclaw.mjs"
...
send -- "cat >/tmp/openclaw-prl.sh <<'__ENCLAWED_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__ENCLAWED_SCRIPT__\r"
send -- "/bin/bash /tmp/openclaw-prl.sh; rc=\$?; rm -f /tmp/openclaw-prl.sh; printf '__ENCLAWED_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1
...
/usr/bin/env \\
  ...
  ENCLAWED_STATE_DIR=$(shell_quote "$guest_home/.openclaw") \\
  ENCLAWED_CONFIG_PATH=$(shell_quote "$guest_home/.openclaw/openclaw.json") \\
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/openclaw-discord-token <<'__ENCLAWED_TOKEN__'
$DISCORD_TOKEN_VALUE
__ENCLAWED_TOKEN__
cat >/tmp/openclaw-discord-guilds.json <<'__ENCLAWED_GUILDS__'
$guilds_json
__ENCLAWED_GUILDS__
