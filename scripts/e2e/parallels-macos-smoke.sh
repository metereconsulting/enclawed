...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://enclawed.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_ENCLAWED_BIN="/opt/homebrew/bin/enclawed"
GUEST_ENCLAWED_ENTRY="/opt/homebrew/lib/node_modules/enclawed/enclawed.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/enclawed-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/enclawed-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://enclawed.ai/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: enclawed@2026.3.13-beta.1
...
resolve_guest_git_enclawed_entry() {
  local guest_home
  guest_home="$(resolve_guest_current_user_home)"
  printf '%s/enclawed/enclawed.mjs\n' "$guest_home"
}
...
spawn {*}$cmd
send -- "printf '__ENCLAWED_READY__\\n'\r"
expect "__ENCLAWED_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/enclawed-prl.sh <<'__ENCLAWED_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__ENCLAWED_SCRIPT__\r"
send -- "/bin/bash /tmp/enclawed-prl.sh; rc=\$?; rm -f /tmp/enclawed-prl.sh; printf '__ENCLAWED_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
set saw_rc 0
expect {
  -re {__ENCLAWED_RC__:(-?[0-9]+)} {
...
guest_current_user_sh() {
  ...
  script+="$1"
  if headless_guest_fallback; then
    script_path="/tmp/enclawed-prl-${BASHPID:-$$}-$RANDOM.sh"
    local guest_home
    guest_home="$(parallels_macos_resolve_desktop_home "$VM_NAME" "$GUEST_CURRENT_USER")"
...
markers = [
    ...
    if line.startswith("__ENCLAWED_RC__:")
]
if not markers:
    raise SystemExit(1)
...
printf '__ENCLAWED_RC__:%s\n' "\$status"
printf '%s\n' "\$status" > "$done_path"
exit "\$status"
EOF
)"
  write_runner_cmd="/bin/rm -f $(shell_quote "$runner_path")"$'\n'
  write_runner_cmd+="cat > $(shell_quote "$runner_path") <<'__ENCLAWED_RUNNER__'"$'\n'
  write_runner_cmd+="$runner_body"$'\n'
  write_runner_cmd+="__ENCLAWED_RUNNER__"$'\n'
  ...
  guest_home="$(resolve_guest_current_user_home)"
  guest_log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-guest-log-state.XXXXXX")"
  latest_npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-guest-npm-log-state.XXXXXX")"
  npm_state_path="$(mktemp "${TMPDIR:-/tmp}/enclawed-guest-npm-log-path.XXXXXX")"
...
matches = re.findall(r"^__ENCLAWED_RC__:(-?\d+)$", text, flags=re.MULTILINE)
if not matches:
    raise SystemExit(1)
print(matches[-1])
PY
...
resolve_latest_version() {
  ...
  npm view enclawed version --userconfig "$(mktemp)"
}
...
install_latest_release() {
  ...
  guest_current_user_sh "$(cat <<EOF
export ENCLAWED_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/enclawed-install.sh
bash /tmp/enclawed-install.sh${version_arg_q}
$GUEST_ENCLAWED_BIN --version
EOF
)"
}
...
ensure_guest_pnpm_for_dev_update() {
  ...
  bootstrap_root="/tmp/enclawed-smoke-pnpm-bootstrap"
...
repair_legacy_dev_source_checkout_if_needed() {
  ...
  bootstrap_bin="/tmp/enclawed-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/enclawed"
  update_entry="$update_root/enclawed.mjs"
  ...
  guest_current_user_exec /usr/bin/git clone --depth 1 --branch main \
    https://github.com/enclawed/enclawed.git "$update_root"
...
run_dev_channel_update() {
  ...
  bootstrap_bin="/tmp/enclawed-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/enclawed"
  update_log="/tmp/enclawed-smoke-update-dev.log"
  update_done="/tmp/enclawed-smoke-update-dev.done"
  update_runner="/tmp/enclawed-smoke-update-dev.sh"
  ensure_guest_pnpm_for_dev_update
...
/usr/bin/env NODE_OPTIONS=--max-old-space-size=4096 \
  $GUEST_NODE_BIN $GUEST_ENCLAWED_ENTRY update --channel dev --yes --json
EOF
  ...
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_ENCLAWED_ENTRY" --version
  printf 'update-dev: git-status\n'
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_ENCLAWED_ENTRY" update status --json
}

verify_dev_channel_update() {
  local status_json
  status_json="$(guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_ENCLAWED_ENTRY" update status --json)"
  printf '%s\n' "$status_json"
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_ENCLAWED_BIN" --version 2>&1
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/enclawed-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/enclawed-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
check_path "\$root/enclawed"
check_path "\$root/enclawed/extensions"
if [ -d "\$root/enclawed/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "\$extension_dir"
  done < <(/usr/bin/find "\$root/enclawed/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi
EOF
)"
...
start_manual_gateway_if_needed() {
  ...
  guest_gateway_log="/tmp/enclawed-parallels-macos-gateway.log"
  runner_log="/tmp/enclawed-parallels-gateway-start.log"
  done_path="/tmp/enclawed-parallels-gateway-start.done"
  runner_path="/tmp/enclawed-parallels-gateway-start.sh"
  printf 'manual gateway launch transport=%s user=%s\n' "$GUEST_CURRENT_USER_TRANSPORT" "$GUEST_CURRENT_USER"
  launch_cmd="$(cat <<EOF
set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'enclawed.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'enclawed-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'enclawed.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env \\
  ...
  ENCLAWED_HOME=$(shell_quote "$guest_home") \\
  ENCLAWED_STATE_DIR=$(shell_quote "$guest_home/.enclawed") \\
  ENCLAWED_CONFIG_PATH=$(shell_quote "$guest_home/.enclawed/enclawed.json") \\
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_ENCLAWED_ENTRY") gateway run --bind loopback --port 18789 --force \\
...
verify_gateway() {
  ...
  for attempt in 1 2 3 4; do
    if guest_current_user_exec "$GUEST_ENCLAWED_BIN" gateway status --deep --require-rpc --timeout 5000; then
      return 0
    fi
    if (( attempt < 4 )); then
...
show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_ENCLAWED_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_ENCLAWED_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_ENCLAWED_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_ENCLAWED_ENTRY" models set "$MODEL_ID"
  guest_current_user_sh "$(cat <<EOF
export PATH=$(shell_quote "$GUEST_EXEC_PATH")
exec /usr/bin/env $(shell_quote "$API_KEY_ENV=$API_KEY_VALUE") \
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_ENCLAWED_ENTRY") agent \
  --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_ENCLAWED_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from enclawed dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
verify_dashboard_load() {
  ...
  # `enclawed dashboard --no-open` can hang under the Tahoe Parallels transport
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 "\$dashboard_http_url" >/tmp/enclawed-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Enclawed Control</title>' /tmp/enclawed-dashboard-smoke.html >/dev/null; then
      if grep -F '<enclawed-app></enclawed-app>' /tmp/enclawed-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>Enclawed Control</title>' /tmp/enclawed-dashboard-smoke.html >/dev/null
grep -F '<enclawed-app></enclawed-app>' /tmp/enclawed-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/enclawed-discord-token <<'__ENCLAWED_TOKEN__'
$DISCORD_TOKEN_VALUE
__ENCLAWED_TOKEN__
cat >/tmp/enclawed-discord-guilds.json <<'__ENCLAWED_GUILDS__'
$guilds_json
__ENCLAWED_GUILDS__
token="\$(tr -d '\n' </tmp/enclawed-discord-token)"
guilds_json="\$(cat /tmp/enclawed-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_ENCLAWED_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_ENCLAWED_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_ENCLAWED_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_ENCLAWED_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_ENCLAWED_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_ENCLAWED_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_ENCLAWED_ENTRY channels status --probe --json
rm -f /tmp/enclawed-discord-token /tmp/enclawed-discord-guilds.json
EOF
)"
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "enclawed-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "enclawed-main-upgrade.tgz"
