...
IMAGE_NAME="enclawed-plugins-e2e"
...
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/enclawed-plugins-run.XXXXXX.log")"
if ! docker run --rm "${DOCKER_ENV_ARGS[@]}" -i "$IMAGE_NAME" bash -s >"$RUN_LOG" 2>&1 <<'EOF'
set -euo pipefail

if [ -f dist/index.mjs ]; then
  ENCLAWED_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  ENCLAWED_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export ENCLAWED_ENTRY
...
home_dir=$(mktemp -d "/tmp/enclawed-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
ENCLAWED_PLUGIN_HOME="$HOME/.enclawed/$BUNDLED_PLUGIN_ROOT_DIR"
...
const configPath = path.join(process.env.HOME, ".enclawed", "enclawed.json");
...
run_logged() {
  local label="$1"
  shift
  local log_file="/tmp/enclawed-plugins-e2e-${label}.log"
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$ENCLAWED_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$ENCLAWED_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$ENCLAWED_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/enclawed.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/enclawed.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$ENCLAWED_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/enclawed.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$ENCLAWED_ENTRY" plugins list --json > /tmp/plugins.json
node "$ENCLAWED_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
pack_dir="$(mktemp -d "/tmp/enclawed-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@enclawed/demo-plugin-tgz",
  "version": "0.0.1",
  "enclawed": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/enclawed.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
run_logged install-tgz node "$ENCLAWED_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$ENCLAWED_ENTRY" plugins list --json > /tmp/plugins2.json
node "$ENCLAWED_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
dir_plugin="$(mktemp -d "/tmp/enclawed-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@enclawed/demo-plugin-dir",
  "version": "0.0.1",
  "enclawed": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/enclawed.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
run_logged install-dir node "$ENCLAWED_ENTRY" plugins install "$dir_plugin"
node "$ENCLAWED_ENTRY" plugins list --json > /tmp/plugins3.json
node "$ENCLAWED_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
file_pack_dir="$(mktemp -d "/tmp/enclawed-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@enclawed/demo-plugin-file",
  "version": "0.0.1",
  "enclawed": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/enclawed.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
run_logged install-file node "$ENCLAWED_ENTRY" plugins install "file:$file_pack_dir/package"
node "$ENCLAWED_ENTRY" plugins list --json > /tmp/plugins4.json
node "$ENCLAWED_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
bundle_root="$ENCLAWED_PLUGIN_HOME/$bundle_plugin_id"
...
gateway_log="/tmp/enclawed-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/enclawed-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@enclawed/slash-install-plugin",
  "version": "0.0.1",
  "enclawed": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/enclawed.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "enclawed/fixture-marketplace"
    }
  }
}
JSON

node "$ENCLAWED_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
run_logged install-marketplace-shortcut node "$ENCLAWED_ENTRY" plugins install marketplace-shortcut@claude-fixtures
run_logged install-marketplace-direct node "$ENCLAWED_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$ENCLAWED_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$ENCLAWED_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$ENCLAWED_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
run_logged update-marketplace-shortcut-dry-run node "$ENCLAWED_ENTRY" plugins update marketplace-shortcut --dry-run
run_logged update-marketplace-shortcut node "$ENCLAWED_ENTRY" plugins update marketplace-shortcut
node "$ENCLAWED_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$ENCLAWED_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
