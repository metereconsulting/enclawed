...
IMAGE_NAME="enclawed-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/enclawed-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/enclawed-bin/systemctl

  cat > /tmp/enclawed-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/enclawed-bin/loginctl
  ...
  npm_log="/tmp/enclawed-doctor-switch-npm-install.log"
  if ! npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz" >"$npm_log" 2>&1; then
    cat "$npm_log"
    exit 1
  fi

	  npm_bin="/tmp/npm-prefix/bin/enclawed"
	  npm_root="/tmp/npm-prefix/lib/node_modules/enclawed"
  ...
  run_flow() {
    ...
    local install_log="/tmp/enclawed-doctor-switch-${name}-install.log"
    local doctor_log="/tmp/enclawed-doctor-switch-${name}-doctor.log"

    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/enclawed-switch-${name}.XXXXXX")
    ...
    unit_path="$HOME/.config/systemd/user/enclawed-gateway.service"
    if [ ! -f "$unit_path" ]; then
