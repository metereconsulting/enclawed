...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.ENCLAWED_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@enclawed/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "enclawed.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "enclawed.plugin.json");
    ...
    // Copy the signed module manifest (enclawed.module.json) verbatim into
    // dist so the host admission gate can read its boot-time decision from the
    // installed package. The byte content matters: the signature is verified
    // against the canonical manifest fields, so re-serializing is avoided.
    const sourceModulePath = path.join(pluginDir, "enclawed.module.json");
    const distModulePath = path.join(distPluginDir, "enclawed.module.json");
    if (fs.existsSync(sourceModulePath)) {
      fs.mkdirSync(distPluginDir, { recursive: true });
      writeTextFileIfChanged(distModulePath, fs.readFileSync(sourceModulePath, "utf8"));
    } else {
      removeFileIfExists(distModulePath);
    }
    ...
    if (packageJson.enclawed && "extensions" in packageJson.enclawed) {
      packageJson.enclawed = {
        ...packageJson.enclawed,
        extensions: rewritePackageExtensions(packageJson.enclawed.extensions),
        ...(typeof packageJson.enclawed.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.enclawed.setupEntry) }
          : {}),
      };
    }
