...
- ENCLAWED_IOS_VERSION = exact canonical iOS version
- ENCLAWED_MARKETING_VERSION = short iOS/App Store version
- ENCLAWED_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    ENCLAWED_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    ENCLAWED_MARKETING_VERSION)
...
ENCLAWED_IOS_VERSION = ${IOS_VERSION}
ENCLAWED_MARKETING_VERSION = ${MARKETING_VERSION}
ENCLAWED_BUILD_VERSION = ${BUILD_NUMBER}
EOF
