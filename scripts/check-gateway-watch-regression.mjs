...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  ENCLAWED_SKIP_ACPX_RUNTIME: "1",
  ENCLAWED_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$ENCLAWED_WATCH_PID_FILE"',
    'mkdir -p "$ENCLAWED_HOME/.enclawed"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}}}' > "$ENCLAWED_HOME/.enclawed/enclawed.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    ENCLAWED_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    ENCLAWED_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-gateway-watch-"));
