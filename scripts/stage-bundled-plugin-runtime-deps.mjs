...
function makePluginOwnedTempDir(pluginDir, label) {
  return makeTempDir(pluginDir, `.enclawed-runtime-deps-${label}-`);
}
...
function replaceDirAtomically(targetPath, sourcePath) {
  ...
  const backupPath = makeTempDir(
    targetParentDir,
    `.enclawed-runtime-deps-backup-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function writeJsonAtomically(targetPath, value) {
  ...
  const tempDir = makeTempDir(
    targetParentDir,
    `.enclawed-runtime-deps-stamp-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.enclawed?.bundle?.stageRuntimeDependencies === true;
}
...
function createRuntimeInstallManifest(pluginId, pinnedGroups) {
  const manifest = {
    name: `enclawed-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}`,
    private: true,
    version: "0.0.0",
  };
...
function runNpmInstall(params) {
  ...
  // If a parent process is `npm publish --dry-run`, it exports
  // `npm_config_dry_run=true`, which this nested install would inherit and then
  // materialize no node_modules. This staging install (used as the fallback
  // when a plugin's runtime closure can't be resolved from the hoisted root
  // graph) must always run for real, so strip any inherited dry-run flag.
  delete npmEnv.npm_config_dry_run;
  delete npmEnv["npm_config_dry-run"];
  delete npmEnv.NPM_CONFIG_DRY_RUN;
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".enclawed-runtime-deps-stamp.json");
}
