...
const ENCLAWED_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncEnclawedDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.enclawed;
  if (!current || current === "workspace:*" || !ENCLAWED_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.enclawed?.compat;
  const current = compat?.pluginApi;
  if (!current || !ENCLAWED_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildEnclawedVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.enclawed?.build;
  const current = build?.enclawedVersion;
  if (!current) {
    return false;
  }
  ...
  build.enclawedVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncEnclawedDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncEnclawedDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildEnclawedVersionChanged = syncBuildEnclawedVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildEnclawedVersionChanged;
    if (!packageChanged) {
