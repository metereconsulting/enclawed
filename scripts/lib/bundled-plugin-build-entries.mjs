...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.enclawed?.extensions)
    ? packageJson.enclawed.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.enclawed?.setupEntry === "string" &&
    packageJson.enclawed.setupEntry.trim().length > 0
      ? packageJson.enclawed.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "enclawed.plugin.json"));
    }
    if (hasPackageJson) {
