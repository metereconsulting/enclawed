...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "enclawed-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "enclawed.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".enclawed", "workspace");
