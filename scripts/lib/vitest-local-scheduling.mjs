...
function isSystemThrottleDisabled(env) {
  const normalized = env.ENCLAWED_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.ENCLAWED_VITEST_MAX_WORKERS ?? env.ENCLAWED_TEST_WORKERS);
  if (override !== null) {
