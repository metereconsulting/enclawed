...
const extensionRestartMetadataFiles = new Set(["enclawed.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.ENCLAWED_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.ENCLAWED_BUILD_PRIVATE_QA === "1" &&
...
const BUILD_REASON_LABELS = {
  force_build: "forced by ENCLAWED_FORCE_BUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "ENCLAWED_RUN_NODE_OUTPUT_LOG";
...
const logRunner = (message, deps) => {
  if (deps.env.ENCLAWED_RUNNER_LOG === "0") {
    return;
  }
  const line = `[enclawed] ${message}\n`;
...
const runEnclawed = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["enclawed.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[enclawed] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const shouldSkipCleanWatchRuntimeSync = (deps) => deps.env.ENCLAWED_WATCH_MODE === "1";
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.ENCLAWED_BUILD_PRIVATE_QA = "1";
    deps.env.ENCLAWED_ENABLE_PRIVATE_QA_CLI = "1";
  }
  ...
  try {
    ...
    if (!buildRequirement.shouldBuild) {
      ...
      exitCode = await runEnclawed(deps);
