...
BASE_IMAGE="${BASE_IMAGE:-enclawed-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-enclawed-sandbox-common:bookworm-slim}"
...
ENCLAWED_DOCKER_BUILD_USE_BUILDX="${ENCLAWED_DOCKER_BUILD_USE_BUILDX:-0}"
ENCLAWED_DOCKER_BUILD_CACHE_FROM="${ENCLAWED_DOCKER_BUILD_CACHE_FROM:-}"
ENCLAWED_DOCKER_BUILD_CACHE_TO="${ENCLAWED_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${ENCLAWED_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${ENCLAWED_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${ENCLAWED_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${ENCLAWED_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${ENCLAWED_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=enclawed.sandbox=1)
NOTE
