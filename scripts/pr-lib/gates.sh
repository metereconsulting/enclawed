...
prepare_gates() {
  ...
  if [ "$reuse_gates" = "true" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      gates_mode="full"
      if [ -n "${ENCLAWED_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with ENCLAWED_VITEST_MAX_WORKERS=$ENCLAWED_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env ENCLAWED_VITEST_MAX_WORKERS="$ENCLAWED_VITEST_MAX_WORKERS" pnpm test
      else
