...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.enclawed) ? packageJson.enclawed : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.enclawed?.channel?.id) || left.name;
    const rightId = trimString(right.enclawed?.channel?.id) || right.name;
