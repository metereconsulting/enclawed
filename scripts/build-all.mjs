...
/**
 * The OSS tree does not vendor the a2ui renderer sources (the
 * `vendor/a2ui/` tree is a closed-tree component that ships in the
 * downstream enclawed-enclaved repo). When those sources are absent,
 * implicitly skip the canvas-a2ui bundle + copy steps by setting
 * ENCLAWED_A2UI_SKIP_MISSING=1 — both scripts already short-circuit on
 * that env var. This avoids requiring the developer to know about the
 * skip-flag for the common `npm pack` rehearsal.
 */
const A2UI_AWARE_STEP_LABELS = new Set(["canvas:a2ui:bundle", "canvas-a2ui-copy"]);
function vendorA2uiPresent(cwd = process.cwd()) {
  return existsSync(path.join(cwd, "vendor", "a2ui", "renderers", "lit"));
}
...
function resolveStepEnv(step, env, platform) {
  let baseEnv = env;
  if (A2UI_AWARE_STEP_LABELS.has(step.label) && !vendorA2uiPresent() &&
      !baseEnv.ENCLAWED_A2UI_SKIP_MISSING && !baseEnv.ENCLAWED_SPARSE_PROFILE) {
    baseEnv = { ...baseEnv, ENCLAWED_A2UI_SKIP_MISSING: "1" };
  }
  if (platform !== "win32" || !step.windowsNodeOptions) {
    return baseEnv;
  }
  const currentNodeOptions = baseEnv.NODE_OPTIONS?.trim() ?? "";
