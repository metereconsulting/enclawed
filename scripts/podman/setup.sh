...
ENCLAWED_IMAGE="${ENCLAWED_PODMAN_IMAGE:-${ENCLAWED_IMAGE:-openclaw:local}}"
ENCLAWED_CONTAINER_NAME="${ENCLAWED_PODMAN_CONTAINER:-openclaw}"
...
if [[ "$ENCLAWED_IMAGE" == "openclaw:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
...
if [[ -z "$ENCLAWED_CONFIG_DIR" ]]; then
  ENCLAWED_CONFIG_DIR="$ENCLAWED_HOME/.openclaw"
fi
...
if [[ "$ENCLAWED_IMAGE" == "openclaw:local" ]]; then
...
CONFIG_JSON="$ENCLAWED_CONFIG_DIR/openclaw.json"
...
echo "  openclaw --container $ENCLAWED_CONTAINER_NAME dashboard --no-open"
