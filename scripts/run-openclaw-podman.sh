#!/usr/bin/env bash
# Rootless Enclawed in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-enclawed-podman.sh launch        # Start gateway
#   ./scripts/run-enclawed-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   enclawed --container enclawed dashboard --no-open
#   enclawed --container enclawed channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      ENCLAWED_GATEWAY_TOKEN|ENCLAWED_PODMAN_CONTAINER|ENCLAWED_PODMAN_IMAGE|ENCLAWED_IMAGE|ENCLAWED_PODMAN_PULL|ENCLAWED_PODMAN_GATEWAY_HOST_PORT|ENCLAWED_GATEWAY_PORT|ENCLAWED_PODMAN_BRIDGE_HOST_PORT|ENCLAWED_BRIDGE_PORT|ENCLAWED_GATEWAY_BIND|ENCLAWED_PODMAN_USERNS|ENCLAWED_BIND_MOUNT_OPTIONS|ENCLAWED_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-enclawed-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${ENCLAWED_CONFIG_DIR:-$EFFECTIVE_HOME/.enclawed}"
ENV_FILE="${ENCLAWED_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${ENCLAWED_CONFIG_DIR:-$EFFECTIVE_HOME/.enclawed}"
ENV_FILE="${ENCLAWED_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${ENCLAWED_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${ENCLAWED_PODMAN_CONTAINER:-enclawed}"
ENCLAWED_IMAGE="${ENCLAWED_PODMAN_IMAGE:-${ENCLAWED_IMAGE:-enclawed:local}}"
PODMAN_PULL="${ENCLAWED_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${ENCLAWED_PODMAN_GATEWAY_HOST_PORT:-${ENCLAWED_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${ENCLAWED_PODMAN_BRIDGE_HOST_PORT:-${ENCLAWED_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${ENCLAWED_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$ENCLAWED_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v enclawed >/dev/null 2>&1; then
    return 0
  fi
  ENCLAWED_CONTAINER="" ENCLAWED_CONFIG_DIR="$config_dir" \
    enclawed config get gateway.bind 2>/dev/null || true
}
...
# ENCLAWED_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${ENCLAWED_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'ENCLAWED_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v enclawed >/dev/null 2>&1; then
    echo "Warning: enclawed not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    ENCLAWED_CONTAINER="" ENCLAWED_CONFIG_DIR="$config_dir" \
      enclawed config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    ENCLAWED_CONTAINER="" ENCLAWED_CONFIG_DIR="$config_dir" \
      enclawed config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  ENCLAWED_CONTAINER="" ENCLAWED_CONFIG_DIR="$config_dir" \
    enclawed config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${ENCLAWED_GATEWAY_TOKEN:-}" ]]; then
  export ENCLAWED_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "ENCLAWED_GATEWAY_TOKEN" "$ENCLAWED_GATEWAY_TOKEN"
  echo "Generated ENCLAWED_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/enclawed.json"
...
PODMAN_USERNS="${ENCLAWED_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported ENCLAWED_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (ENCLAWED_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${ENCLAWED_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${ENCLAWED_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$ENCLAWED_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.enclawed/.npm \
    -e ENCLAWED_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.enclawed:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.enclawed/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$ENCLAWED_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME enclawed dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME enclawed devices approve --latest  # if pairing required"
