...
#
# Run the Enclawed precise OpenGrep rulepack against the local working tree
...
# Second rulepack: enclawed-specific rules derived from enclawed/test/security
# pen-tests. Loaded alongside the upstream-mirror pack when present.
ENCLAWED_CONFIG="$REPO_ROOT/security/opengrep/precise-enclawed.yml"
...
if [[ -f "$ENCLAWED_CONFIG" ]]; then
  CONFIG_ARGS+=( --config "$ENCLAWED_CONFIG" )
fi
...
if (( PATHS_PASSED == 0 )); then
  if (( CHANGED_ONLY )); then
    mapfile -t SCAN_PATHS < <(
      {
        git diff --name-only --diff-filter=ACMRTUXB "${ENCLAWED_OPENGREP_BASE_REF:-origin/main...HEAD}" 2>/dev/null || true
...
if (( ${#CONFIG_ARGS[@]} > 2 )); then
  echo "  Loading rulepacks: precise.yml + precise-enclawed.yml" >&2
else
  echo "  Loading rulepacks: precise.yml" >&2
fi
