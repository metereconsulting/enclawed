...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${ENCLAWED_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$ENCLAWED_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.ENCLAWED_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `ENCLAWED_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${ENCLAWED_INSTALL_SMOKE_IMAGE:-enclawed-install-smoke:local}"
NONROOT_IMAGE="${ENCLAWED_INSTALL_NONROOT_IMAGE:-enclawed-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${ENCLAWED_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${ENCLAWED_INSTALL_URL:-https://enclawed.bot/install.sh}"
CLI_INSTALL_URL="${ENCLAWED_INSTALL_CLI_URL:-https://enclawed.bot/install-cli.sh}"
PACKAGE_NAME="${ENCLAWED_INSTALL_PACKAGE:-enclawed}"
SKIP_NONROOT="${ENCLAWED_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${ENCLAWED_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${ENCLAWED_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${ENCLAWED_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${ENCLAWED_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${ENCLAWED_INSTALL_SMOKE_UPDATE_BASELINE:-2026.4.10}"
UPDATE_PACKAGE_SPEC="${ENCLAWED_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${ENCLAWED_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${ENCLAWED_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${ENCLAWED_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${ENCLAWED_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${ENCLAWED_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (ENCLAWED_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  start_update_server
  ...
  docker run --rm -t \
    ...
    -e ENCLAWED_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ENCLAWED_INSTALL_METHOD=npm \
    -e ENCLAWED_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e ENCLAWED_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e ENCLAWED_INSTALL_LATEST_OUT="/out/latest" \
    -e ENCLAWED_NO_ONBOARD=1 \
    -e ENCLAWED_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${UPDATE_DOCKER_HOST_ARGS[@]}" \
    -e ENCLAWED_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ENCLAWED_INSTALL_SMOKE_MODE=update \
    -e ENCLAWED_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e ENCLAWED_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e ENCLAWED_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e ENCLAWED_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e ENCLAWED_NO_ONBOARD=1 \
    -e ENCLAWED_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (ENCLAWED_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${UPDATE_DOCKER_HOST_ARGS[@]}" \
      -e ENCLAWED_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e ENCLAWED_INSTALL_SMOKE_MODE=npm-global \
      -e ENCLAWED_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e ENCLAWED_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e ENCLAWED_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e ENCLAWED_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (ENCLAWED_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e ENCLAWED_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e ENCLAWED_INSTALL_METHOD=npm \
    -e ENCLAWED_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e ENCLAWED_NO_ONBOARD=1 \
    -e ENCLAWED_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${ENCLAWED_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (ENCLAWED_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e ENCLAWED_INSTALL_URL="$INSTALL_URL" \
  -e ENCLAWED_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e ENCLAWED_NO_ONBOARD=1 \
  -e ENCLAWED_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
