...
type DriverMode = "token" | "webhook" | "enclawed";
...
function resolveStateDir(): string {
  const override = process.env.ENCLAWED_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.ENCLAWED_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".enclawed");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver enclawed] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Enclawed spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|enclawed> Driver transport mode (default: token)\n" +
    ...
    "  --enclawed-bin <path>        Enclawed CLI binary for driver=enclawed (default: enclawed)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  ENCLAWED_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ENCLAWED_DISCORD_SMOKE_DRIVER\n" +
    "  ENCLAWED_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ENCLAWED_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ENCLAWED_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ENCLAWED_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ENCLAWED_DISCORD_SMOKE_AGENT\n" +
    "  ENCLAWED_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ENCLAWED_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ENCLAWED_DISCORD_SMOKE_POLL_MS\n" +
    "  ENCLAWED_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  ENCLAWED_DISCORD_SMOKE_ENCLAWED_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.ENCLAWED_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.ENCLAWED_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "enclawed"
        ? "enclawed"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.ENCLAWED_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ENCLAWED_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ENCLAWED_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.ENCLAWED_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.ENCLAWED_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.ENCLAWED_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.ENCLAWED_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ENCLAWED_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ENCLAWED_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.ENCLAWED_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const enclawedBin =
    resolveArg("--enclawed-bin") || process.env.ENCLAWED_DISCORD_SMOKE_ENCLAWED_BIN || "enclawed";
...
async function enclawedCliJson<T>(params: { enclawedBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.enclawedBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`enclawed ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithEnclawed(params: {
  enclawedBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await enclawedCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    enclawedBin: params.enclawedBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "enclawed") {
    return await readMessagesWithEnclawed({
      enclawedBin: params.args.enclawedBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `enclawed-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await enclawedCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("enclawed message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "enclawed"
            ? await readMessagesWithEnclawed({
                enclawedBin: args.enclawedBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Enclawed.`,
        diagnostics: {
          bindingCandidates: [
            {
