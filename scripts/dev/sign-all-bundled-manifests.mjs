...
// Extension directories that MUST remain unsigned in the OSS tree.
// The paper-conformance test in enclawed/test/paper-conformance.test.mjs
// (§3.2) asserts no signed enclawed.module.json exists for any of these
// directories: the load-time gate in the enclaved flavor admits a cloud
// module only if it carries a valid signature, so leaving these unsigned
// is what makes them deny-by-default in production. Signing them would
// silently break the unreachability property the paper claims and that
// CI guards. List mirrors the test's lists exactly; keep in sync if the
// test's lists change.
const UNSIGNED_BY_DESIGN = new Set([
  // Cloud channels (paper-conformance §3.2 cloudChannels).
  ...
  "xiaomi", "bluebubbles",
  // Cloud LLM / API providers (paper-conformance §3.2 cloudProviders).
