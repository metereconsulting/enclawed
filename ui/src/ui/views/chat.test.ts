...
function clearDeleteConfirmSkip() {
  try {
    getSafeLocalStorage()?.removeItem("enclawed:skipDeleteConfirm");
  } catch {
    /* noop */
  }
}
...
describe("chat view", () => {
  it("uses the assistant avatar URL or bundled logo fallbacks", () => {
    ...
    expect(groupedLogo?.getAttribute("src")).toBe("/enclawed/favicon.svg");
  });
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
            ...
            toolMessages: [
              {
                ...
                content: JSON.stringify({
                  ...
                  view: {
                    ...
                    url: `/__enclawed__/canvas/documents/cv_inline_${params.suffix}/index.html`,
                    title: "Inline demo",
    ...
    expect(iframe?.getAttribute("src")).toBe(
      "/__enclawed__/canvas/documents/cv_inline_default/index.html",
    );
          ...
          toolMessages: [
            {
              ...
              content: JSON.stringify({
                ...
                view: {
                  ...
                  url: "/__enclawed__/canvas/documents/cv_inline_visible/index.html",
                  title: "Inline demo",
                  preferred_height: 360,
                },
                presentation: {
          ...
          toolMessages: [
            {
              ...
              content: JSON.stringify({
                ...
                view: {
                  backend: "canvas",
                  id: "cv_nearest_turn",
                  url: "/__enclawed__/canvas/documents/cv_nearest_turn/index.html",
              ...
              content: JSON.stringify({
                selected_item: {
                  ...
                  view: {
                    ...
                    url: "/__enclawed__/canvas/documents/cv_generic_inline/index.html",
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        container,
        {
          ...
          role: "assistant",
          content:
            "Local image\nMEDIA:/tmp/enclawed/test image.png\nMEDIA:/tmp/enclawed/test-doc.pdf",
          timestamp: Date.now(),
        },
        {
          showToolCalls: false,
          ...
          localMediaPreviewRoots: ["/tmp/enclawed"],
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/enclawed/__enclawed__/assistant-media?source=%2Ftmp%2Fenclawed%2Ftest+image.png&token=session-token&meta=1",
    ...
    expect(image?.getAttribute("src")).toBe(
      "/enclawed/__enclawed__/assistant-media?source=%2Ftmp%2Fenclawed%2Ftest+image.png&token=session-token",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/enclawed/__enclawed__/assistant-media?source=%2Ftmp%2Fenclawed%2Ftest-doc.pdf&token=session-token",
    );
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/enclawed/__enclawed__/assistant-media?source=%2Ftmp%2Fenclawed%2Ftest+image.png&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/enclawed/__enclawed__/assistant-media?source=%2Ftmp%2Fenclawed%2Ftest+image.png&token=fresh-token&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(container.querySelector(".chat-message-image")).not.toBeNull();
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__enclawed__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(docLink?.getAttribute("href")).toBe("/__enclawed__/media/test-doc.pdf");
    expect(container.textContent).not.toContain("Unavailable");
  });
    ...
    const cases = [
      renderCase({
        ...
        message: {
          ...
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/enclawed/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/enclawed/__enclawed__/assistant-media?source=%2FC%3A%2Ftmp%2Fenclawed%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/enclawed/__enclawed__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/enclawed/__enclawed__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__enclawed__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasHostUrl: "http://127.0.0.1:19003/__enclawed__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__enclawed__/cap/cap_123/__enclawed__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
              ...
              content: [
                {
                  ...
                  text: JSON.stringify({
                    ...
                    view: {
                      ...
                      url: "/__enclawed__/canvas/documents/cv_canvas_live_history/index.html",
                ...
                },
                {
                  ...
                  text: JSON.stringify({
                    ...
                    view: {
                      ...
                      url: "/__enclawed__/canvas/documents/cv_streamed_artifact/index.html",
                      title: "Streamed demo",
                      preferred_height: 320,
                    },
                    presentation: {
