...
const SKIP_DELETE_CONFIRM_KEY = "enclawed:skipDeleteConfirm";
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__enclawed__|media)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__enclawed__/assistant-media?${params.toString()}`;
}
