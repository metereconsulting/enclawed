...
export function validateRuleMetadata(rules) {
  ...
  for (const [index, rule] of rules.entries()) {
    ...
    const expectedGhsaUrl = GHSA_RE.test(advisoryId)
      ? `https://github.com/enclawed/enclawed/security/advisories/${advisoryId}`
      : "";
...
// Cross-runtime entrypoint detection: import.meta.main is Bun-only and silently
// returns undefined on Node, so the script becomes a no-op when invoked as
// `node check-rule-metadata.mjs`. Resolve the script path explicitly instead.
const _isMain = (() => {
  try {
