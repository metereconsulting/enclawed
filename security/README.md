...
This directory holds the shipped OpenGrep security rulepack and the supporting
tooling that validates and runs it. The 147 precise rules are inherited from the
upstream OpenClaw repository (MIT-licensed, copyright (c) 2025 Peter Steinberger);
each rule is tied to a real OpenClaw GHSA advisory through its metadata, and
the durable advisory URLs in each rule's metadata block stay pointed at the
upstream source. Maintainer-only advisory triage and detector-generation prompts
live outside this public repo; this repo keeps the durable artifacts needed to
block regressions in PRs and support local rule validation.

Because enclawed is a hard fork of OpenClaw and ships the entire upstream
OpenClaw extension catalog, every rule in this rulepack is a regression
firewall: if a vulnerable shape from a past OpenClaw GHSA is reintroduced
anywhere in this repo (including the mirrored extensions/ tree), the
corresponding rule fires. Sync the rulepack from upstream when new advisories
are published.
