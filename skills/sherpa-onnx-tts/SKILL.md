...
1. Download the runtime for your OS (extracts into `$ENCLAWED_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.enclawed/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$ENCLAWED_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.enclawed/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Enclawed config file (`$ENCLAWED_CONFIG_PATH`, default `~/.enclawed/enclawed.json`):
