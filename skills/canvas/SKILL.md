...
In the active OpenClaw config file (`$ENCLAWED_CONFIG_PATH`, default `~/.openclaw/openclaw.json`):
...
1. Check server bind: `CONFIG_PATH="${ENCLAWED_CONFIG_PATH:-${ENCLAWED_STATE_DIR:-$HOME/.openclaw}/openclaw.json}"; cat "$CONFIG_PATH" | jq '.gateway.bind'`
