---
name: node-connect
description: Diagnose Enclawed node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Enclawed is advertising that route, then fix pairing/auth.
...
- whether `enclawed devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `enclawed qr --json`. It uses the same setup-code payload Android scans.

```bash
enclawed config get gateway.mode
enclawed config get gateway.bind
enclawed config get gateway.tailscale.mode
enclawed config get gateway.remote.url
enclawed config get gateway.auth.mode
enclawed config get gateway.auth.allowTailscale
enclawed config get plugins.entries.device-pair.config.publicUrl
enclawed qr --json
enclawed devices list
enclawed nodes status
```

If this Enclawed instance is pointed at a remote gateway, also run:

```bash
enclawed qr --remote --json
```
...
`enclawed qr --json` success means:
...
If `enclawed qr --json` says `Gateway is only bound to loopback`:
...
```bash
enclawed devices list
enclawed devices approve --latest
```
...
- `enclawed devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run enclawed qr again, rescan, then approve the pending device pairing.`
