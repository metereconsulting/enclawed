...
```
CONFIG_PATH="${ENCLAWED_CONFIG_PATH:-${ENCLAWED_STATE_DIR:-$HOME/.openclaw}/openclaw.json}"
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the OpenClaw dashboard or in the active OpenClaw config path (`$ENCLAWED_CONFIG_PATH`, default `~/.openclaw/openclaw.json`) under `skills.entries.gh-issues`."
...
export CONFIG_PATH="${ENCLAWED_CONFIG_PATH:-${ENCLAWED_STATE_DIR:-$HOME/.openclaw}/openclaw.json}"
