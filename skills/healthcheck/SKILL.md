---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Enclawed deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Enclawed cron scheduling for periodic checks, or version status checks on a machine running Enclawed (laptop, workstation, Pi, VPS).
---

# Enclawed Host Hardening

## Overview

Assess and harden the host running Enclawed, then align it to a user-defined risk tolerance without breaking access. Use Enclawed security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Enclawed changes the host firewall, SSH, or OS updates; it does not.
...
5. Enclawed gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Enclawed can access sensitive data.
...
### 2) Run Enclawed security audits (read-only)

As part of the default read-only checks, run `enclawed security audit --deep`. Only offer alternatives if the user requests them:

1. `enclawed security audit` (faster, non-probing)
2. `enclawed security audit --json` (structured output)

Offer to apply Enclawed safe defaults (numbered):

1. `enclawed security audit --fix`

Be explicit that `--fix` only tightens Enclawed defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Enclawed version/update status (read-only)

As part of the default read-only checks, run `enclawed update status`.
...
- Credential hygiene notes (location of Enclawed creds, prefer disk encryption)
...
- Enclawed security audit (re-run)
...
After Enclawed install or first hardening pass, run at least one baseline audit and version check:

- `enclawed security audit`
- `enclawed security audit --deep`
- `enclawed update status`

Ongoing monitoring is recommended. Use the Enclawed cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `enclawed cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `enclawed update status`
...
Before creating, `enclawed cron list` and match on exact `name`. If found, `enclawed cron edit <id> ...`.
If not found, `enclawed cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `enclawed update status` (preferred for source checkouts and channels)
2. `npm view enclawed version` (published npm version)

## Enclawed command accuracy

Use only supported commands and flags:

- `enclawed security audit [--deep] [--fix] [--json]`
- `enclawed status` / `enclawed status --deep`
- `enclawed health --json`
- `enclawed update status`
- `enclawed cron add|list|runs|run`

Do not invent CLI flags or imply Enclawed enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Enclawed compaction:
