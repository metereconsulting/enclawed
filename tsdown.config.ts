...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.ENCLAWED_BUILD_VERBOSE === "1") {
    return undefined;
  }
  ...
  function isSuppressedLog(log: {
    ...
    // NOTE: Previously, UNRESOLVED_IMPORT warnings from anywhere under
    // \`extensions/\` were silently swallowed (G1 allowlist). The post-bundle
    // smoke gate now covers that drift class explicitly via the doctor
    // pass, so we no longer hide warnings here. Any new unresolved-import
    // surfaces should be investigated as a real issue rather than masked.
    if (log.code !== "EVAL") {
      return false;
    }
...
const shouldBuildPrivateQaEntries = process.env.ENCLAWED_BUILD_PRIVATE_QA === "1";
...
function shouldStageBundledPluginRuntimeDependencies(packageJson: unknown): boolean {
  return (
    ...
    packageJson !== null &&
    (packageJson as { enclawed?: { bundle?: { stageRuntimeDependencies?: boolean } } }).enclawed
...
function isPluginSdkSelfReference(id: string): boolean {
  return (
    id === "@enclawed/plugin-sdk" ||
    id.startsWith("@enclawed/plugin-sdk/") ||
    id === "@enclawed/plugin-sdk" ||
    id.startsWith("@enclawed/plugin-sdk/")
  );
}
