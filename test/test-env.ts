...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.ENCLAWED_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "ENCLAWED_TEST_FAST", value: process.env.ENCLAWED_TEST_FAST },
    {
      key: "ENCLAWED_STRICT_FAST_REPLY_CONFIG",
      value: process.env.ENCLAWED_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "ENCLAWED_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.ENCLAWED_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "ENCLAWED_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.ENCLAWED_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "ENCLAWED_STATE_DIR", value: process.env.ENCLAWED_STATE_DIR },
    { key: "ENCLAWED_CONFIG_PATH", value: process.env.ENCLAWED_CONFIG_PATH },
    { key: "ENCLAWED_GATEWAY_PORT", value: process.env.ENCLAWED_GATEWAY_PORT },
    { key: "ENCLAWED_BRIDGE_ENABLED", value: process.env.ENCLAWED_BRIDGE_ENABLED },
    { key: "ENCLAWED_BRIDGE_HOST", value: process.env.ENCLAWED_BRIDGE_HOST },
    { key: "ENCLAWED_BRIDGE_PORT", value: process.env.ENCLAWED_BRIDGE_PORT },
    { key: "ENCLAWED_CANVAS_HOST_PORT", value: process.env.ENCLAWED_CANVAS_HOST_PORT },
    { key: "ENCLAWED_TEST_HOME", value: process.env.ENCLAWED_TEST_HOME },
    { key: "ENCLAWED_AGENT_DIR", value: process.env.ENCLAWED_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ENCLAWED_TEST_HOME = tempHome;
  process.env.ENCLAWED_TEST_FAST = "1";
  process.env.ENCLAWED_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.ENCLAWED_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.ENCLAWED_BRIDGE_ENABLED;
  delete process.env.ENCLAWED_BRIDGE_HOST;
  delete process.env.ENCLAWED_BRIDGE_PORT;
  delete process.env.ENCLAWED_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.ENCLAWED_STATE_DIR = path.join(tempHome, ".enclawed");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.ENCLAWED_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.ENCLAWED_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".enclawed");
  const priorIsolatedHome = params.env.ENCLAWED_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".enclawed")
  ) {
    realStateDir = path.join(params.realHome, ".enclawed");
  }
  const tempStateDir = path.join(params.tempHome, ".enclawed");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.ENCLAWED_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.ENCLAWED_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "enclawed.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "enclawed.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.ENCLAWED_LIVE_TEST === "1" ||
    process.env.ENCLAWED_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.ENCLAWED_LIVE_USE_REAL_HOME);
