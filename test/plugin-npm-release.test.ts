...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @enclawed/zalo, @enclawed/feishu  @enclawed/zalo "),
    ).toEqual(["@enclawed/feishu", "@enclawed/zalo"]);
  });
});
...
describe("collectPublishablePluginPackages", () => {
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "enclawed-plugin-npm-release-");
    ...
    writeJsonFile(join(repoDir, "extensions", "demo-plugin", "package.json"), {
      name: "@enclawed/demo-plugin",
      version: "2026.4.10",
      enclawed: {
        extensions: ["./index.ts"],
        install: {
          npmSpec: "@enclawed/demo-plugin",
        },
        release: {
          publishToNpm: true,
        },
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        packageName: "@enclawed/demo-plugin",
        version: "2026.4.10",
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@enclawed/demo-plugin",
      },
    ]);
  });
});
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@enclawed/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @enclawed/missing.");
  });
});
