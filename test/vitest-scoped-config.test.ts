...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ ENCLAWED_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ ENCLAWED_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ ENCLAWED_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  it("applies the non-isolated runner by default", () => {
    ...
    expect(normalizeConfigPaths(config.test?.setupFiles)).toEqual([
      "test/setup.ts",
      "test/setup-enclawed-runtime.ts",
    ]);
  });
  ...
  it("loads scoped include overrides from ENCLAWED_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-vitest-scoped-"));
    try {
      ...
      const config = createScopedVitestConfig(["src/utils/**/*.test.ts"], {
        dir: "src",
        env: {
          ENCLAWED_VITEST_INCLUDE_FILE: includeFile,
        },
      });
...
describe("scoped vitest configs", () => {
  ...
  it("keeps scoped lanes on threads with the shared non-isolated runner", () => {
    ...
    // UI lane runs on forks: jsdom's transitive @exodus/bytes (ESM) +
    // CJS callers (whatwg-url, html-encoding-sniffer) error out under
    // worker_threads. See test/vitest/vitest.ui.config.ts.
    expect(defaultUiConfig.test?.pool).toBe("forks");
    expect(defaultUiConfig.test?.isolate).toBe(true);
    expect(defaultUiConfig.test?.runner).toBeUndefined();
  });

  it("keeps the process lane off the enclawed runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the enclawed runtime setup", () => {
  ...
  it("keeps the ui lane off both the enclawed runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from ENCLAWED_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "enclawed-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
