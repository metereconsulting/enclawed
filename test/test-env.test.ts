...
function createTempHome(): string {
  return makeTempDir(tempDirs, "enclawed-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-enclawed.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.enclawed/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.enclawed/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".enclawed", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".enclawed", "external-plugins", "glueclaw", "enclawed.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".enclawed", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.ENCLAWED_LIVE_TEST_QUIET = "1";
    process.env.ENCLAWED_CONFIG_PATH = "~/custom-enclawed.json5";
    process.env.ENCLAWED_TEST_HOME = priorIsolatedHome;
    process.env.ENCLAWED_STATE_DIR = path.join(priorIsolatedHome, ".enclawed");
    ...
    expect(process.env.ENCLAWED_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".enclawed", "enclawed.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".enclawed", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".enclawed", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.ENCLAWED_LIVE_TEST = "1";
    process.env.ENCLAWED_LIVE_USE_REAL_HOME = "1";
    process.env.ENCLAWED_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.ENCLAWED_LIVE_TEST;
    delete process.env.ENCLAWED_LIVE_GATEWAY;
    delete process.env.ENCLAWED_LIVE_USE_REAL_HOME;
    delete process.env.ENCLAWED_LIVE_TEST_QUIET;
