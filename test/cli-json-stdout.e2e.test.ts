  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.ENCLAWED_HOME;
        delete env.ENCLAWED_STATE_DIR;
        delete env.ENCLAWED_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "enclawed.mjs");
