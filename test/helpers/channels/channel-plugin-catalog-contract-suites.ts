    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-catalog-"));
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@enclawed/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(os.tmpdir(), "enclawed-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-catalog-state-"));
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-catalog-home-"));
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@enclawed/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              ENCLAWED_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              ENCLAWED_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@enclawed/default-env-demo",
