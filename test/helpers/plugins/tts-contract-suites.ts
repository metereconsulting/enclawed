...
function asLegacyTtsConfig(value: unknown): EnclawedConfig {
  return value as EnclawedConfig;
}

function asLegacyEnclawedConfig(value: Record<string, unknown>): EnclawedConfig {
  return value as unknown as EnclawedConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): EnclawedConfig {
...
function createResolvedSummarizationConfig(cfg: EnclawedConfig): ResolvedTtsConfig {
  const rawConfig =
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withEnv(
          ...
          () => {
            const cfg = asLegacyEnclawedConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: EnclawedConfig = asLegacyEnclawedConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.ENCLAWED_TTS_PREFS;
      process.env.ENCLAWED_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.ENCLAWED_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: EnclawedConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
