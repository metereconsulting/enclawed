...
export function requireEnclawedAgentDir(): string {
  const agentDir = process.env.ENCLAWED_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ENCLAWED_AGENT_DIR not set");
  }
  return agentDir;
}
