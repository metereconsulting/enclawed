...
export type EnclawedVitestPool = "forks" | "threads";
...
export function resolveLocalVitestMaxWorkers(
  ...
  pool: EnclawedVitestPool = resolveDefaultVitestPool(env),
): number {
...
export const sharedVitestConfig = {
  ...
  resolve: {
    alias: [
      {
        find: "@enclawed/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `@enclawed/plugin-sdk/${subpath}`,
