...
Enclawed has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Linux VM-backed QA lane: `pnpm enclawed qa suite --runner multipass --scenario channel-chat-baseline`
...
- `pnpm enclawed qa suite`
  ...
  - Supports provider modes `live-frontier`, `mock-openai`, and `aimock`.
    ...
    `mock-openai` lane.
- `pnpm enclawed qa suite --runner multipass`
...
- `pnpm enclawed qa aimock`
  - Starts only the local AIMock provider server for direct protocol smoke
    testing.
- `pnpm enclawed qa matrix`
  - Runs the Matrix live QA lane against a disposable Docker-backed Tuwunel homeserver.
  - This QA host is repo/dev-only today. Packaged Enclawed installs do not ship
    `qa-lab`, so they do not expose `enclawed qa`.
  ...
  - Uses the pinned stable Tuwunel image `ghcr.io/matrix-construct/tuwunel:v1.5.1` by default. Override with `ENCLAWED_QA_MATRIX_TUWUNEL_IMAGE` when you need to test a different image.
...
- `pnpm enclawed qa telegram`
  - Runs the Telegram live QA lane against a real private group using the driver and SUT bot tokens from env.
  - Requires `ENCLAWED_QA_TELEGRAM_GROUP_ID`, `ENCLAWED_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `ENCLAWED_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `ENCLAWED_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
...
When `--credential-source convex` (or `ENCLAWED_QA_CREDENTIAL_SOURCE=convex`) is enabled for
`enclawed qa telegram`, QA lab acquires an exclusive lease from a Convex-backed pool, heartbeats
...
- `ENCLAWED_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `ENCLAWED_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `ENCLAWED_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `ENCLAWED_QA_CREDENTIAL_ROLE` (defaults to `maintainer`)

Optional env vars:

- `ENCLAWED_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `ENCLAWED_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `ENCLAWED_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `ENCLAWED_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `ENCLAWED_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `ENCLAWED_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `ENCLAWED_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`ENCLAWED_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`ENCLAWED_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm enclawed qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm enclawed qa credentials list --kind telegram
pnpm enclawed qa credentials remove --credential-id <credential-id>
```

Use `--json` for machine-readable output in scripts and CI utilities.

Default endpoint contract (`ENCLAWED_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
- how `enclawed qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `enclawed qa <runner>` instead of registering a competing root command.
   Runner plugins should declare `qaRunners` in `enclawed.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`.
...
- Projects note:
  ...
  - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-enclawed-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
...
- Pool note:
  ...
  - The shared `scripts/run-vitest.mjs` launcher now also adds `--no-maglev` for Vitest child Node processes by default to reduce V8 compile churn during big local runs. Set `ENCLAWED_VITEST_ENABLE_MAGLEV=1` if you need to compare against stock V8 behavior.
- Fast-local iteration note:
  ...
  - The config keeps `ENCLAWED_VITEST_FS_MODULE_CACHE` enabled on supported hosts; set `ENCLAWED_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want one explicit cache location for direct profiling.
- Perf-debug note:
...
- Useful overrides:
  - `ENCLAWED_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `ENCLAWED_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Enclawed's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `ENCLAWED_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `ENCLAWED_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `ENCLAWED_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.enclawed`.
- Set `ENCLAWED_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `ENCLAWED_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `ENCLAWED_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `ENCLAWED_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `ENCLAWED_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Optional target overrides:
  - `ENCLAWED_ANDROID_NODE_ID` or `ENCLAWED_ANDROID_NODE_NAME`.
  - `ENCLAWED_ANDROID_GATEWAY_URL` / `ENCLAWED_ANDROID_GATEWAY_TOKEN` / `ENCLAWED_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `ENCLAWED_LIVE_TEST=1` if invoking Vitest directly)
- Set `ENCLAWED_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `ENCLAWED_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.7, Grok 4)
  - `ENCLAWED_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `ENCLAWED_LIVE_MODELS="openai/gpt-5.4,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Modern/all sweeps default to a curated high-signal cap; set `ENCLAWED_LIVE_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers:
  - `ENCLAWED_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `ENCLAWED_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates “provider API is broken / key is invalid” from “gateway agent pipeline is broken”
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@enclawed" actually does)
...
- How to select models:
  ...
  - `ENCLAWED_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `ENCLAWED_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all gateway sweeps default to a curated high-signal cap; set `ENCLAWED_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers (avoid “OpenRouter everything”):
  - `ENCLAWED_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `ENCLAWED_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
  - `ENCLAWED_LIVE_CLI_BACKEND_COMMAND="/full/path/to/codex"`
  - `ENCLAWED_LIVE_CLI_BACKEND_ARGS='["exec","--json","--color","never","--sandbox","read-only","--skip-git-repo-check"]'`
  - `ENCLAWED_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
  - `ENCLAWED_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `ENCLAWED_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `ENCLAWED_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `ENCLAWED_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=0` to disable the default Claude Sonnet -> Opus same-session continuity probe (set to `1` to force it on when the selected model supports a switch target).
...
- It resolves CLI smoke metadata from the owning extension, then installs the matching Linux CLI package (`@anthropic-ai/claude-code`, `@openai/codex`, or `@google/gemini-cli`) into a cached writable prefix at `ENCLAWED_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/enclawed/docker-cli-tools`).
...
- Defaults:
  ...
  - ACP backend: `acpx`
- Overrides:
  - `ENCLAWED_LIVE_ACP_BIND_AGENT=claude`
  - `ENCLAWED_LIVE_ACP_BIND_AGENT=codex`
  - `ENCLAWED_LIVE_ACP_BIND_AGENT=gemini`
  - `ENCLAWED_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `ENCLAWED_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `ENCLAWED_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
...
- Use `ENCLAWED_LIVE_ACP_BIND_AGENTS=claude`, `ENCLAWED_LIVE_ACP_BIND_AGENTS=codex`, or `ENCLAWED_LIVE_ACP_BIND_AGENTS=gemini` to narrow the matrix.
...
- Inside Docker, the runner sets `ENCLAWED_LIVE_ACP_BIND_ACPX_COMMAND=$HOME/.npm-global/bin/acpx` so acpx keeps provider env vars from the sourced profile available to the child harness CLI.
...
- Goal: validate the plugin-owned Codex harness through the normal gateway
  ...
  - select `ENCLAWED_AGENT_RUNTIME=codex`
  - send a first gateway agent turn to `codex/gpt-5.4`
  - send a second turn to the same Enclawed session and verify the app-server
    thread can resume
  - run `/codex status` and `/codex models` through the same gateway command
    path
- Test: `src/gateway/gateway-codex-harness.live.test.ts`
- Enable: `ENCLAWED_LIVE_CODEX_HARNESS=1`
- Default model: `codex/gpt-5.4`
- Optional image probe: `ENCLAWED_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `ENCLAWED_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- The smoke sets `ENCLAWED_AGENT_HARNESS_FALLBACK=none` so a broken Codex
...
```bash
source ~/.profile
ENCLAWED_LIVE_CODEX_HARNESS=1 \
  ENCLAWED_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  ENCLAWED_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  ENCLAWED_LIVE_CODEX_HARNESS_MODEL=codex/gpt-5.4 \
...
- Docker enables the image and MCP/tool probes by default. Set
  `ENCLAWED_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `ENCLAWED_LIVE_CODEX_HARNESS_MCP_PROBE=0` when you need a narrower debug run.
- Docker also exports `ENCLAWED_AGENT_HARNESS_FALLBACK=none`, matching the live
...
- Single model, direct (no gateway):
  - `ENCLAWED_LIVE_MODELS="openai/gpt-5.4" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `ENCLAWED_LIVE_GATEWAY_MODELS="openai/gpt-5.4" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `ENCLAWED_LIVE_GATEWAY_MODELS="openai/gpt-5.4,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `ENCLAWED_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `ENCLAWED_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Gemini API vs Gemini CLI:
  - API: Enclawed calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: Enclawed shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
Include at least one image-capable model in `ENCLAWED_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.

### Aggregators / alternate gateways

If you have keys enabled, we also support testing via:

- OpenRouter: `openrouter/...` (hundreds of models; use `enclawed models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `enclawed models list` / model selection.

- Per-agent auth profiles: `~/.enclawed/agents/<agentId>/agent/auth-profiles.json` (this is what “profile keys” means in the live tests)
- Config: `~/.enclawed/enclawed.json` (or `ENCLAWED_CONFIG_PATH`)
- Legacy state dir: `~/.enclawed/credentials/` (copied into the staged live home when present, but not the main profile-key store)
...
- Enable: `ENCLAWED_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `ENCLAWED_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `ENCLAWED_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `ENCLAWED_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
- Optional auth behavior:
  - `ENCLAWED_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

## Music generation live

- Test: `extensions/music-generation-providers.live.test.ts`
- Enable: `ENCLAWED_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `ENCLAWED_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `ENCLAWED_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.5+"`
- Optional auth behavior:
  - `ENCLAWED_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `ENCLAWED_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
- Harness: `pnpm test:live:media video`
- Scope:
  - Exercises the shared bundled video-generation provider path
  - Defaults to the release-safe smoke path: non-FAL providers, one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `ENCLAWED_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `--video-providers fal` or `ENCLAWED_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` to run it explicitly
  - Loads provider env vars from your login shell (`~/.profile`) before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in `auth-profiles.json` do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `ENCLAWED_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `ENCLAWED_LIVE_TEST=1 ENCLAWED_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
...
- Optional narrowing:
  - `ENCLAWED_LIVE_VIDEO_GENERATION_PROVIDERS="google,openai,runway"`
  - `ENCLAWED_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `ENCLAWED_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `ENCLAWED_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `ENCLAWED_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `ENCLAWED_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `ENCLAWED_LIVE_GATEWAY_SMOKE=1`,
  `ENCLAWED_LIVE_GATEWAY_MAX_MODELS=8`,
  `ENCLAWED_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `ENCLAWED_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
...
`.pnpm-store`, `.worktrees`, `__enclawed_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `ENCLAWED_SKIP_CHANNELS=1` so gateway live probes do not start
...
`ENCLAWED_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
Enclawed gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `enclawed/default`, then sends a
...
This lane expects a usable live model key, and `ENCLAWED_PROFILE_FILE`
...
container, starts a second container that spawns `enclawed mcp serve`, then
...
- `ENCLAWED_CONFIG_DIR=...` (default: `~/.enclawed`) mounted to `/home/node/.enclawed`
- `ENCLAWED_WORKSPACE_DIR=...` (default: `~/.enclawed/workspace`) mounted to `/home/node/.enclawed/workspace`
- `ENCLAWED_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- `ENCLAWED_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `ENCLAWED_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `ENCLAWED_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/enclawed/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `ENCLAWED_LIVE_PROVIDERS` / `ENCLAWED_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `ENCLAWED_DOCKER_AUTH_DIRS=all`, `ENCLAWED_DOCKER_AUTH_DIRS=none`, or a comma list like `ENCLAWED_DOCKER_AUTH_DIRS=.claude,.codex`
- `ENCLAWED_LIVE_GATEWAY_MODELS=...` / `ENCLAWED_LIVE_MODELS=...` to narrow the run
- `ENCLAWED_LIVE_GATEWAY_PROVIDERS=...` / `ENCLAWED_LIVE_PROVIDERS=...` to filter providers in-container
- `ENCLAWED_SKIP_DOCKER_BUILD=1` to reuse an existing `enclawed:local-live` image for reruns that do not need a rebuild
- `ENCLAWED_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `ENCLAWED_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `ENCLAWED_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
