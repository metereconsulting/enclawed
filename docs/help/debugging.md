...
```bash
pnpm gateway:dev
ENCLAWED_PROFILE=dev openclaw tui
```
...
1. **Profile isolation** (global `--dev`)
   ...
   - `ENCLAWED_STATE_DIR=~/.openclaw-dev`
   - `ENCLAWED_CONFIG_PATH=~/.openclaw-dev/openclaw.json`
...
```bash
ENCLAWED_PROFILE=dev openclaw gateway --dev --reset
```
...
ENCLAWED_RAW_STREAM_PATH=~/.openclaw/logs/raw-stream.jsonl
```
