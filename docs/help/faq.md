...
<AccordionGroup>
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    2. Copy `$ENCLAWED_STATE_DIR` (default: `~/.openclaw`) from the old machine.
...
<AccordionGroup>
  ...
  <Accordion title="Where does OpenClaw store its data?">
    Everything lives under `$ENCLAWED_STATE_DIR` (default: `~/.openclaw`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$ENCLAWED_STATE_DIR/openclaw.json`                             | Main config (JSON5)                                                |
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    OpenClaw reads an optional **JSON5** config from `$ENCLAWED_CONFIG_PATH` (default: `~/.openclaw/openclaw.json`):
...
<AccordionGroup>
  <Accordion title="How does OpenClaw load environment variables?">
    ...
    - a global fallback `.env` from `~/.openclaw/.env` (aka `$ENCLAWED_STATE_DIR/.env`)
...
<AccordionGroup>
  ...
  <Accordion title="How do I completely reset OpenClaw but keep it installed?">
    ...
    - If you used profiles (`--profile` / `ENCLAWED_PROFILE`), reset each state dir (defaults are `~/.openclaw-<profile>`).
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$ENCLAWED_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.openclaw/logs/...`; profiles use `~/.openclaw-<profile>/logs/...`)
