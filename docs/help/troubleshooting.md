---
summary: "Symptom first troubleshooting hub for Enclawed"
read_when:
  - Enclawed is not working and you need the fastest path to a fix
...
- `enclawed status` → shows configured channels and no obvious auth errors.
- `enclawed status --all` → full report is present and shareable.
- `enclawed gateway probe` → expected gateway target is reachable (`Reachable: yes`). `RPC: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `enclawed gateway status` → `Runtime: running` and `RPC probe: ok`.
- `enclawed doctor` → no blocking config/service errors.
- `enclawed channels status --probe` → reachable gateway returns live per-account
...
- `enclawed logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in Enclawed

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `enclawed infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on Enclawed agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger Enclawed prompts crash the
...
## Plugin install fails with missing enclawed extensions

If install fails with `package.json missing enclawed.extensions`, the plugin package
is using an old shape that Enclawed no longer accepts.

Fix in the plugin package:

1. Add `enclawed.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `enclawed plugins install <package>` again.

Example:

```json
{
  "name": "@enclawed/my-plugin",
  "version": "1.2.3",
  "enclawed": {
...
```mermaid
flowchart TD
  A[Enclawed is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `enclawed gateway status`
    - `RPC probe: ok`
  ...
  <Accordion title="Cron or heartbeat did not fire or did not deliver">
    ...
    <Accordion title="Exec suddenly asks for approval">
      ```bash
      enclawed config get tools.exec.host
      enclawed config get tools.exec.security
      enclawed config get tools.exec.ask
    ...
    <Accordion title="Browser tool fails">
      ...
      - `enclawed` starts, or `user` can see local Chrome tabs.
      ...
      - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `enclawed browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
