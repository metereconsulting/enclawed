---
summary: "Sign in to GitHub Copilot from Enclawed using the device flow"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `enclawed models auth login-github-copilot` flow
...
models for your GitHub account and plan. Enclawed can use Copilot as a model
provider in two different ways.

## Two ways to use Copilot in Enclawed

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Enclawed runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        enclawed models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        enclawed models set github-copilot/gpt-4o
        ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Enclawed talks to
...
```bash
# Skip confirmation
enclawed models auth login-github-copilot --yes

# Login and set the default model in one step
enclawed models auth login --provider github-copilot --method device --set-default
```
...
<AccordionGroup>
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Enclawed
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Environment variable resolution order">
    Enclawed resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Enclawed uses the highest-priority one.
    The device-login flow (`enclawed models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Enclawed runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Enclawed can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, Enclawed discovers available
...
1. Enclawed resolves your GitHub token (from env vars or auth profile).
...
available, Enclawed skips Copilot and tries the next provider.
