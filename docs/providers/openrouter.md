---
summary: "Use OpenRouter's unified API to access many models in Enclawed"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Enclawed
title: "OpenRouter"
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    enclawed onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    enclawed models set openrouter/<provider>/<model>
    ```
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), Enclawed also adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Enclawed
...
<AccordionGroup>
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that Enclawed uses for
    better prompt-cache reuse on system/developer prompt blocks.
  </Accordion>

  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Enclawed maps the selected thinking level to
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Enclawed keeps
    Gemini thought-signature sanitation there, but does not enable native Gemini
    replay validation or bootstrap rewrites.
  </Accordion>

  <Accordion title="Provider routing metadata">
    If you pass OpenRouter provider routing under model params, Enclawed forwards
