---
summary: "Use Venice AI privacy-focused models in Enclawed"
read_when:
  - You want privacy-focused inference in Enclawed
...
<Steps>
  ...
  <Step title="Configure Enclawed">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        enclawed onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    enclawed agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Enclawed shows all available Venice models. Pick based on your needs:
...
```bash
enclawed models set venice/kimi-k2-5
enclawed models set venice/claude-opus-4-6
```

List all available models:

```bash
enclawed models list | grep venice
```

You can also run `enclawed configure`, select **Model/auth**, and choose **Venice AI**.
...
Enclawed automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
enclawed agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
enclawed agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
enclawed agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
enclawed agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
enclawed agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `enclawed models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
