---
summary: "Run Enclawed with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Enclawed against a local SGLang server
...
Enclawed can connect to SGLang using the `openai-completions` API.

Enclawed can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Enclawed will query:
