...
read_when:
  - You want to use Groq with Enclawed
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Enclawed connects
...
Groq's model catalog changes frequently. Run `enclawed models list | grep groq`
...
<Tip>
Use `enclawed models list --provider groq` for the most up-to-date list of
...
media-understanding provider, Enclawed uses Groq's `whisper-large-v3-turbo`
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    gateway processes. Use `~/.enclawed/.env` or `env.shellEnv` config for
