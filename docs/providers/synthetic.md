---
summary: "Use Synthetic's Anthropic-compatible API in Enclawed"
read_when:
...
Enclawed registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    enclawed onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Enclawed's Anthropic client appends `/v1` to the base URL automatically, so use
...
`enclawed models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Enclawed appends `/v1` automatically.
