---
summary: "Use Z.AI (GLM models) with Enclawed"
read_when:
  - You want Z.AI / GLM models in Enclawed
...
for authentication. Create your API key in the Z.AI console. Enclawed uses the `zai` provider
with a Z.AI API key.
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Enclawed detects the matching Z.AI endpoint from the key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        enclawed onboard --auth-choice zai-api-key
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        enclawed models list --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        enclawed onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        enclawed onboard --auth-choice zai-coding-cn

        # General API
        enclawed onboard --auth-choice zai-global

        # General API CN (China region)
        enclawed onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Enclawed currently seeds the bundled `zai` provider with:
