...
read_when:
  - You want to use Vercel AI Gateway with Enclawed
...
<Tip>
Enclawed auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    enclawed onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    enclawed models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Enclawed accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. Enclawed resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Enclawed Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.enclawed/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
