---
summary: "GLM model family overview + how to use it in Enclawed"
read_when:
  - You want GLM models in Enclawed
...
GLM is a **model family** (not a company) available through the Z.AI platform. In Enclawed, GLM
...
<Steps>
  ...
  <Step title="Set GLM as the default model">
    ```bash
    enclawed config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
...
<Tip>
`zai-api-key` lets Enclawed detect the matching Z.AI endpoint from the key and
...
Enclawed currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, Enclawed inspects the key format
