---
summary: "Use Mistral models and Voxtral transcription with Enclawed"
read_when:
  - You want to use Mistral models in Enclawed
...
Enclawed supports Mistral for both text/image model routing (`mistral/...`) and
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    enclawed onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    enclawed onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    enclawed models list --provider mistral
    ```
  </Step>
</Steps>
...
Enclawed currently ships this bundled Mistral catalog:
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    Enclawed maps the session **thinking** level to Mistral's API:

    | Enclawed thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
