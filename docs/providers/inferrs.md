---
summary: "Run Enclawed through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run Enclawed against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact Enclawed compat flags for inferrs
title: "inferrs"
---
...
OpenAI-compatible `/v1` API. Enclawed works with `inferrs` through the generic
`openai-completions` path.

`inferrs` is currently best treated as a custom self-hosted OpenAI-compatible
backend, not a dedicated Enclawed provider plugin.
...
<Steps>
  ...
  <Step title="Add an Enclawed provider entry">
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If Enclawed runs fail with an error like:
    ...
    Enclawed will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full Enclawed agent-runtime
    turns.
    ...
    That disables Enclawed's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal Enclawed agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than Enclawed's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but enclawed infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If Enclawed no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running Enclawed against local model servers.
  </Card>
