...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Enclawed
...
Running Enclawed via Node with `tsx` fails at startup with:

```
[enclawed] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `enclawed status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch enclawed.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node enclawed.mjs status` works on Node 25.
