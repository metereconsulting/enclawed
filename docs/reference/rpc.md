...
Enclawed integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Enclawed owns lifecycle when `channels.signal.autoStart=true`.
...
- Enclawed spawns `imsg rpc` as a child process (legacy iMessage integration).
