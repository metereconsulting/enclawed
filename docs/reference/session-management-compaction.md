...
This document explains how Enclawed manages sessions end-to-end:
...
Enclawed is designed around a single **Gateway process** that owns session state.
...
Enclawed persists sessions in two layers:
...
- Transcripts: `~/.enclawed/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Enclawed resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Enclawed reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
enclawed sessions cleanup --dry-run
enclawed sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.enclawed/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Enclawed intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Enclawed splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Enclawed
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Enclawed preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Enclawed consumes the events, but Pi decides when to compact).
...
Enclawed also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Enclawed bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Enclawed leaves it alone.
...
- If the provider fails or returns an empty result, Enclawed falls back to built-in LLM summarization automatically.
...
- `enclawed status` (CLI)
- `enclawed sessions` / `sessions --json`
...
Enclawed supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Enclawed strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Enclawed also suppresses **draft/typing streaming** when a
...
Enclawed uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Enclawed’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `enclawed status`.
