---
summary: "How Enclawed builds prompt context and reports token usage + costs"
read_when:
...
Enclawed tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Enclawed assembles its own system prompt on every run. It includes:
...
For images, Enclawed downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Enclawed session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `enclawed status --usage` and `enclawed channels list` show
...
available; otherwise Enclawed falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, Enclawed shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Enclawed can
...
Anthropic's 1M context window is currently beta-gated. Enclawed can inject the
...
Enclawed skips the `context-1m-*` beta header because Anthropic currently
