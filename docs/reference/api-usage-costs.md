...
Enclawed features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, Enclawed reads usage from
...
Anthropic still does not expose a per-message dollar estimate that Enclawed can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `enclawed status --usage` and `enclawed channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Enclawed inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Enclawed prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Enclawed falls back to matching OAuth/API-key
...
Enclawed can pick up credentials from:
...
Enclawed's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Enclawed Claude-login path with **Extra Usage** enabled.
...
- `enclawed status --usage`
- `enclawed models status --json`
...
`enclawed models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
