...
This page lists every configuration knob for Enclawed memory search. For
...
`enclawed.json` unless noted otherwise.
...
When `provider` is not set, Enclawed selects the first available:
...
If Enclawed runs on EC2 with a Bedrock-enabled instance role, just set the
provider and model:
...
When sqlite-vec is unavailable, Enclawed falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.enclawed/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
Enclawed prefers the current QMD collection and MCP query shapes, but keeps
older QMD releases working by falling back to legacy `--mask` collection flags
and older MCP tool names when needed.

QMD model overrides stay on the QMD side, not Enclawed config. If you need to
