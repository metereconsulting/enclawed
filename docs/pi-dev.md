...
This guide summarizes a sane workflow for working on the pi integration in Enclawed.
...
```bash
ENCLAWED_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm enclawed agent --message "Hello" --thinking low`
...
State lives under the Enclawed state directory. Default is `~/.enclawed`. If `ENCLAWED_STATE_DIR` is set, use that directory instead.

To reset everything:

- `enclawed.json` for config
