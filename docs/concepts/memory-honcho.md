...
[Honcho](https://honcho.dev) adds AI-native memory to Enclawed. It persists
...
```bash
enclawed plugins install @honcho-ai/enclawed-honcho
enclawed honcho setup
...
Settings live under `plugins.entries["enclawed-honcho"].config`:
    ...
    entries: {
      "enclawed-honcho": {
        config: {
          ...
          workspaceId: "enclawed", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `enclawed honcho setup` detects and
...
```bash
enclawed honcho setup                        # Configure API key and migrate files
enclawed honcho status                       # Check connection status
enclawed honcho ask <question>               # Query Honcho about the user
enclawed honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/enclawed-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Enclawed integration guide](https://docs.honcho.dev/v3/guides/integrations/enclawed)
- [Memory](/concepts/memory) -- Enclawed memory overview
