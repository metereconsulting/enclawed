...
Enclawed gives agents tools to work across sessions, inspect status, and
...
After the target responds, Enclawed can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned Enclawed
...
the completion origin only identifies a channel Enclawed can still reuse the
