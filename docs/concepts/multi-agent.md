...
- **Session store** (chat history + routing state) under `~/.enclawed/agents/<agentId>/sessions`.
...
`~/.enclawed/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.enclawed` (or `ENCLAWED_STATE_DIR`)
- Workspace: `~/.enclawed/workspace` (or `~/.enclawed/workspace-<agentId>`)
- Agent dir: `~/.enclawed/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.enclawed/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Enclawed runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.enclawed/workspace` (or `~/.enclawed/workspace-<profile>` when `ENCLAWED_PROFILE` is set).
- State defaults to `~/.enclawed/agents/main/agent`.
...
```bash
enclawed agents add coding
enclawed agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.enclawed/agents/<agentId>`.
...
```bash
enclawed channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.enclawed/workspace-alex" },
      { id: "mia", workspace: "~/.enclawed/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, Enclawed upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, Enclawed falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.enclawed/workspace-main" },
      { id: "coding", workspace: "~/.enclawed/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.enclawed/workspace-main" },
      { id: "alerts", workspace: "~/.enclawed/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
enclawed channels login --channel whatsapp --account personal
enclawed channels login --channel whatsapp --account biz
```

`~/.enclawed/enclawed.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.enclawed/workspace-home",
        agentDir: "~/.enclawed/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.enclawed/workspace-work",
        agentDir: "~/.enclawed/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.enclawed/credentials/whatsapp/personal
          // authDir: "~/.enclawed/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.enclawed/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.enclawed/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.enclawed/workspace-opus",
