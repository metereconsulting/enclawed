...
The private QA stack is meant to exercise Enclawed in a more realistic,
...
```bash
pnpm enclawed qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`ENCLAWED_RUN_NODE_OUTPUT_LOG=<path>` to a repo-local log file.
...
disposable server. It requires `ENCLAWED_QA_TELEGRAM_GROUP_ID`,
`ENCLAWED_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
`ENCLAWED_QA_TELEGRAM_SUT_BOT_TOKEN`, plus two distinct bots in the same
...
```bash
pnpm enclawed qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Enclawed
...
- `mock-openai` is the scenario-aware Enclawed mock. It remains the default
