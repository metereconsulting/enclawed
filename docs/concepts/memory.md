...
summary: "How Enclawed remembers things across sessions"
read_when:
...
Enclawed remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.enclawed/workspace`).
...
<Info>
Enclawed auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Enclawed
...
```bash
enclawed memory rem-backfill --rollback
enclawed memory rem-backfill --rollback-short-term
```

## CLI

```bash
enclawed memory status          # Check index status and provider
enclawed memory search "query"  # Search from the command line
enclawed memory index --force   # Rebuild the index
```
