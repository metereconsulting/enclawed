...
Start with this in `enclawed.json`:
...
With those enabled, Enclawed can show:
...
`config.promptOverride` replaces the default Active Memory prompt. Enclawed
...
5. Verify memory search itself works with `enclawed memory status --deep`.
...
If `memorySearch.provider` is unset, Enclawed auto-detects the first available
embedding provider.
...
- if an embedding provider is resolved and then fails at runtime, Enclawed does
...
- run `enclawed memory status --deep` to inspect the current memory-search
