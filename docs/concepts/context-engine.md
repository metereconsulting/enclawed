...
read_when:
  - You want to understand how Enclawed assembles model context
...
A **context engine** controls how Enclawed builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Enclawed ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.enclawed/enclawed.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Enclawed plugin. Install
...
enclawed plugins install -l ./my-context-engine
```
...
Every time Enclawed runs a model prompt, the context engine participates at
...
Enclawed currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Enclawed
...
The built-in `legacy` engine preserves Enclawed's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Enclawed uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Enclawed disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Enclawed automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `@enclawed/plugin-sdk/core` to use
  Enclawed's built-in compaction behavior.
...
Enclawed resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Enclawed's built-in summarization. Plugin engines can implement
...
- Use `enclawed doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Enclawed
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `enclawed plugins install -l ./my-engine` to link a
