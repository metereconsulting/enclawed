...
Enclawed selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Enclawed can use (plus aliases).
...
- If you omit the provider, Enclawed resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, Enclawed
...
enclawed models set <provider/model>
enclawed models set-image <provider/model>

enclawed models aliases list
enclawed models aliases add <alias> <provider/model>
enclawed models aliases remove <alias>

enclawed models fallbacks list
enclawed models fallbacks add <provider/model>
enclawed models fallbacks remove <provider/model>
enclawed models fallbacks clear

enclawed models image-fallbacks list
enclawed models image-fallbacks add <provider/model>
enclawed models image-fallbacks remove <provider/model>
enclawed models image-fallbacks clear
```

`enclawed models` (no subcommand) is a shortcut for `models status`.
...
`enclawed models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.enclawed/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Enclawed writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Enclawed regenerates `models.json`, including command-driven paths like `enclawed agent`.
