---
summary: "What the Enclawed system prompt contains and how it is assembled"
read_when:
...
Enclawed builds a custom system prompt for every agent run. The prompt is **Enclawed-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Enclawed and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full Enclawed-owned prompt. The provider runtime can:
...
- **Enclawed Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Enclawed docs (repo or npm package) and when to read them.
...
Enclawed can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **Enclawed
...
occurs, Enclawed can inject a warning block in Project Context; control this with
...
When eligible skills exist, Enclawed injects a compact **available skills list**
...
local Enclawed docs directory (either `docs/` in the repo workspace or the bundled npm
...
for Enclawed behavior, commands, configuration, or architecture, and to run
`enclawed status` itself when possible (asking the user only when it lacks access).
