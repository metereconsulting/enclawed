---
summary: "How Enclawed manages conversation sessions"
read_when:
...
Enclawed organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `enclawed security audit`.
...
Enclawed automatically bounds session storage over time. By default, it runs
...
Preview with `enclawed sessions cleanup --dry-run`.

## Inspecting sessions

- `enclawed status` -- session store path and recent activity.
- `enclawed sessions --json` -- all sessions (filter with `--active <minutes>`).
