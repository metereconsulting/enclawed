---
summary: "Delegate architecture: running Enclawed as a named agent on behalf of an organization"
...
Goal: run Enclawed as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Enclawed agent that:
...
Enclawed's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Enclawed's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.enclawed/workspace-delegate",
  tools: {
...
- Cron run history: `~/.enclawed/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.enclawed/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Enclawed's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
enclawed agents add delegate
```

This creates:

- Workspace: `~/.enclawed/workspace-delegate`
- State: `~/.enclawed/agents/delegate/agent`
- Sessions: `~/.enclawed/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.enclawed/workspace" },
      {
        id: "delegate",
        workspace: "~/.enclawed/workspace-delegate",
        tools: {
...
~/.enclawed/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.enclawed/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.enclawed/workspace-org",
        agentDir: "~/.enclawed/agents/org-assistant/agent",
...
recall view. Enclawed redacts credential/token-like text, truncates long
