---
summary: "Get Enclawed installed and run your first chat in minutes."
read_when:
...
Install Enclawed, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.enclawed/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.enclawed/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Enclawed as a service account or want custom paths:

- `ENCLAWED_HOME` — home directory for internal path resolution
- `ENCLAWED_STATE_DIR` — override the state directory
- `ENCLAWED_CONFIG_PATH` — override the config file path
