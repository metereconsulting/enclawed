...
CLI onboarding is the **recommended** way to set up Enclawed on macOS,
...
`enclawed dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`enclawed configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.enclawed/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `enclawed doctor` first.
</Note>
...
Use `enclawed agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.enclawed/workspace-<agentId>`.
...
- CLI command reference: [`enclawed onboard`](/cli/onboard)
