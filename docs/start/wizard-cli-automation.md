---
summary: "Scripted onboarding and agent setup for the Enclawed CLI"
read_when:
...
Use `--non-interactive` to automate `enclawed onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but Enclawed now prefers Claude CLI reuse when available.
