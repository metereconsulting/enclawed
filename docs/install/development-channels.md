...
Enclawed ships three update channels:
...
```bash
enclawed update --channel stable
...
- **`dev`**: ensures a git checkout (default `~/enclawed`, override with
  `ENCLAWED_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
enclawed update --tag 2026.4.1-beta.1
...
enclawed update --tag enclawed@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `enclawed update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Enclawed prompts for confirmation (skip with `--yes`).
...
Preview what `enclawed update` would do without making changes:

```bash
enclawed update --dry-run
enclawed update --channel beta --dry-run
enclawed update --tag 2026.4.1-beta.1 --dry-run
enclawed update --dry-run --json
```
...
When you switch channels with `enclawed update`, Enclawed also syncs plugin
sources:
