---
summary: "ClawDock shell helpers for Docker-based Enclawed installs"
read_when:
  - You run Enclawed with Docker often and want shorter day-to-day commands
...
ClawDock is a small shell-helper layer for Docker-based Enclawed installs.
...
| `clawdock-cli <command>`  | Run Enclawed CLI commands in Docker           |
...
- `~/.enclawed/.env` for env-backed provider keys and bot tokens
- `~/.enclawed/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.enclawed/enclawed.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `enclawed.json` quickly. It redacts `.env` values in its printed output.
