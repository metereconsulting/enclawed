...
<Steps>
  ...
  <Step title="Manage the running container from the host CLI">
    Set `ENCLAWED_CONTAINER=openclaw`, then use normal `openclaw` commands from the host.
  </Step>
</Steps>

Setup details:

- `./scripts/podman/setup.sh` builds `openclaw:local` in your rootless Podman store by default, or uses `ENCLAWED_IMAGE` / `ENCLAWED_PODMAN_IMAGE` if you set one.
- It creates `~/.openclaw/openclaw.json` with `gateway.mode: "local"` if missing.
- It creates `~/.openclaw/.env` with `ENCLAWED_GATEWAY_TOKEN` if missing.
...
- `ENCLAWED_IMAGE` or `ENCLAWED_PODMAN_IMAGE` -- use an existing/pulled image instead of building `openclaw:local`
...
```bash
export ENCLAWED_CONTAINER=openclaw
```
...
- `ENCLAWED_CONFIG_DIR` -> `/home/node/.openclaw`
- `ENCLAWED_WORKSPACE_DIR` -> `/home/node/.openclaw/workspace`
...
- `ENCLAWED_PODMAN_CONTAINER` -- container name (`openclaw` by default)
...
If you use a non-default `ENCLAWED_CONFIG_DIR` or `ENCLAWED_WORKSPACE_DIR`, set the same variables for both `./scripts/podman/setup.sh` and later `./scripts/run-openclaw-podman.sh launch` commands. The repo-local launcher does not persist custom path overrides across shells.
...
- It reads `~/.openclaw/.env` as a runtime `EnvironmentFile` for values such as `ENCLAWED_GATEWAY_TOKEN`, but it does not consume the manual launcher's Podman-specific override allowlist.
...
- **Container CLI commands hit the wrong target:** Use `openclaw --container <name> ...` explicitly, or export `ENCLAWED_CONTAINER=<name>` in your shell.
