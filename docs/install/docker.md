...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export ENCLAWED_IMAGE="ghcr.io/openclaw/openclaw:latest"
    ./scripts/docker/setup.sh
    ```
...
```bash
docker compose exec openclaw-gateway node dist/index.js health --token "$ENCLAWED_GATEWAY_TOKEN"
```
...
Docker Compose bind-mounts `ENCLAWED_CONFIG_DIR` to `/home/node/.openclaw` and
`ENCLAWED_WORKSPACE_DIR` to `/home/node/.openclaw/workspace`, so those paths
...
<AccordionGroup>
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export ENCLAWED_HOME_VOLUME="openclaw_home"`
