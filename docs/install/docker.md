---
summary: "Optional Docker-based setup and onboarding for Enclawed"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Enclawed on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export ENCLAWED_IMAGE="ghcr.io/enclawed/enclawed:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/enclawed/enclawed/pkgs/container/enclawed).
  ...
  <Step title="Complete onboarding">
    ...
    `enclawed-gateway` directly. `enclawed-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm enclawed-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm enclawed-cli channels login

    # Telegram
    docker compose run --rm enclawed-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm enclawed-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t enclawed:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node enclawed-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node enclawed-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d enclawed-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `ENCLAWED_EXTRA_MOUNTS`
or `ENCLAWED_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `enclawed-cli` shares `enclawed-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d enclawed-gateway`, run onboarding
and setup-time config writes through `enclawed-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `ENCLAWED_IMAGE`               | Use a remote image instead of building locally                   |
| `ENCLAWED_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `ENCLAWED_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `ENCLAWED_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `ENCLAWED_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `ENCLAWED_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `ENCLAWED_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec enclawed-gateway node dist/index.js health --token "$ENCLAWED_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `ENCLAWED_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `ENCLAWED_CONFIG_DIR` to `/home/node/.enclawed` and
`ENCLAWED_WORKSPACE_DIR` to `/home/node/.enclawed/workspace`, so those paths
survive container replacement.

That mounted config directory is where Enclawed keeps:

- `enclawed.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `ENCLAWED_GATEWAY_TOKEN`

For full persistence details on VM deployments, see
[Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** watch `media/`, session JSONL files, `cron/runs/*.jsonl`,
and rolling file logs under `/tmp/enclawed/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/enclawed/enclawed/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export ENCLAWED_SANDBOX=1
    export ENCLAWED_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm enclawed-cli gateway probe
    docker compose run -T --rm enclawed-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `enclawed-cli` uses `network_mode: "service:enclawed-gateway"` so CLI
    ...
    `no-new-privileges` on `enclawed-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.enclawed`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/enclawed-config /path/to/enclawed-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export ENCLAWED_HOME_VOLUME="enclawed_home"`
    2. **Bake system deps**: `export ENCLAWED_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `ENCLAWED_HOME_VOLUME` or `ENCLAWED_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/enclawed/enclawed/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Enclawed runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm enclawed-cli dashboard --no-open
    docker compose run --rm enclawed-cli devices list
    docker compose run --rm enclawed-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm enclawed-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm enclawed-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Enclawed up to date
