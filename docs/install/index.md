---
summary: "Install Enclawed — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Enclawed, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://enclawed.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Enclawed and Node kept under a local prefix such as
`~/.enclawed`, without depending on a system-wide Node install:

```bash
curl -fsSL https://enclawed.ai/install-cli.sh | bash
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g enclawed@latest
```
...
Or skip the link and use `pnpm enclawed ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:enclawed/enclawed#main
```
...
```bash
enclawed --version      # confirm the CLI is available
enclawed doctor         # check for config issues
enclawed gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `enclawed onboard --install-daemon` or `enclawed gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Enclawed on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Enclawed completely.
  </Card>
</CardGroup>

## Troubleshooting: `enclawed` not found

If the install succeeded but `enclawed` is not found in your terminal:
