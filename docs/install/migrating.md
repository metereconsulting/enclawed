---
summary: "Move (migrate) an Enclawed install from one machine to another"
read_when:
  - You are moving Enclawed to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Enclawed to a New Machine

This guide moves an Enclawed gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.enclawed/` by default) and your **workspace**, you preserve:

- **Config** -- `enclawed.json` and all gateway settings
...
<Tip>
Run `enclawed status` on the old machine to confirm your state directory path.
Custom profiles use `~/.enclawed-<profile>/` or a path set via `ENCLAWED_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf enclawed-state.tgz .enclawed
    ```

    If you use multiple profiles (e.g. `~/.enclawed-work`), archive each separately.

  </Step>

  <Step title="Install Enclawed on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.enclawed/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `ENCLAWED_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `enclawed doctor`.
  </Accordion>

  <Accordion title="Copying only enclawed.json">
...
- [ ] `enclawed status` shows the gateway running
