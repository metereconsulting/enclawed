---
summary: "Host Enclawed on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Enclawed on Oracle Cloud
  - Looking for free VPS hosting for Enclawed
...
Run a persistent Enclawed Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@enclawed`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=enclawed --reset` to re-authenticate.

**Gateway will not start** -- Run `enclawed doctor --non-interactive` and check logs with `journalctl --user -u enclawed-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep Enclawed up to date
