---
summary: "Deploy Enclawed on Render with Infrastructure-as-Code"
read_when:
  - Deploying Enclawed to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Enclawed on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/enclawed/enclawed)
...
repository configures everything needed to run Enclawed:
...
services:
  - type: web
    ...
    envVars:
      - key: ENCLAWED_GATEWAY_PORT
        value: "8080"
      - key: ENCLAWED_STATE_DIR
        value: /data/.enclawed
      - key: ENCLAWED_WORKSPACE_DIR
        value: /data/workspace
      - key: ENCLAWED_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Enclawed state
...
`ENCLAWED_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Enclawed repository, Render will not auto-deploy your Enclawed. To update it, run a manual Blueprint sync from the dashboard.
...
For Enclawed, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Enclawed state plus any configured
...
- Missing `ENCLAWED_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ENCLAWED_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `enclawed backup create` in the Render shell.
