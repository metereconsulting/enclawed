...
When `ENCLAWED_NIX_MODE=1` is set (automatic with nix-openclaw), OpenClaw enters a deterministic mode that disables auto-install flows.
...
OpenClaw reads JSON5 config from `ENCLAWED_CONFIG_PATH` and stores mutable data in `ENCLAWED_STATE_DIR`. When running under Nix, set these explicitly to Nix-managed locations so runtime state and config stay out of the immutable store.

| Variable               | Default                                 |
| ---------------------- | --------------------------------------- |
| `ENCLAWED_HOME`        | `HOME` / `USERPROFILE` / `os.homedir()` |
| `ENCLAWED_STATE_DIR`   | `~/.openclaw`                           |
| `ENCLAWED_CONFIG_PATH` | `$ENCLAWED_STATE_DIR/openclaw.json`     |
