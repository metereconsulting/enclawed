---
summary: "Deploy Enclawed Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Enclawed on a Kubernetes cluster
  - You want to test Enclawed in a Kubernetes environment
title: "Kubernetes"
---

# Enclawed on Kubernetes

A minimal starting point for running Enclawed on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Enclawed is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/enclawed 18789:18789 -n enclawed
...
```bash
kubectl get secret enclawed-secrets -n enclawed -o jsonpath='{.data.ENCLAWED_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: enclawed (configurable via ENCLAWED_NAMESPACE)
├── Deployment/enclawed        # Single pod, init container + gateway
├── Service/enclawed           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/enclawed-config  # enclawed.json + AGENTS.md
└── Secret/enclawed-secrets    # Gateway token + API keys
```
...
Edit `enclawed.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret enclawed-secrets -n enclawed \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/enclawed -n enclawed
```

### Custom namespace

```bash
ENCLAWED_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/enclawed/enclawed:latest # or pin to a specific version from https://github.com/enclawed/enclawed/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # enclawed.json + AGENTS.md
