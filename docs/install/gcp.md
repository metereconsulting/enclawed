---
summary: "Run Enclawed Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
...
# Enclawed on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Enclawed Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Enclawed 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Enclawed Gateway in Docker
- Persist `~/.enclawed` + `~/.enclawed/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.enclawed` state includes `enclawed.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-enclawed-project --name="Enclawed Gateway"
    gcloud config set project my-enclawed-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create enclawed-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh enclawed-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Enclawed repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    ENCLAWED_IMAGE=enclawed:latest
    ENCLAWED_GATEWAY_TOKEN=change-me-now
    ENCLAWED_GATEWAY_BIND=lan
    ENCLAWED_GATEWAY_PORT=18789

    ENCLAWED_CONFIG_DIR=/home/$USER/.enclawed
    ENCLAWED_WORKSPACE_DIR=/home/$USER/.enclawed/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.enclawed
    ```
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `ENCLAWED_GATEWAY_TOKEN`.
    ...
    services:
      enclawed-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - ENCLAWED_GATEWAY_BIND=${ENCLAWED_GATEWAY_BIND}
          - ENCLAWED_GATEWAY_PORT=${ENCLAWED_GATEWAY_PORT}
          - ENCLAWED_GATEWAY_TOKEN=${ENCLAWED_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${ENCLAWED_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`ENCLAWED_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm enclawed-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh enclawed-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop enclawed-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type enclawed-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start enclawed-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create enclawed-deploy \
     --display-name="Enclawed Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-enclawed-project \
     --member="serviceAccount:enclawed-deploy@my-enclawed-project.iam.gserviceaccount.com" \
