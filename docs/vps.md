---
summary: "Run Enclawed on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Enclawed
title: "Linux Server"
...
Run the Enclawed Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/enclawed-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `ENCLAWED_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/enclawed-compile-cache`
...
For the standard `enclawed onboard --install-daemon` path, edit the user unit:
...
`enclawed-gateway.service` via `sudo systemctl edit enclawed-gateway.service`.
