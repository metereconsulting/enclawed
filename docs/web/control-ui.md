...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/enclawed`)
...
If the page fails to load, start the Gateway first: `enclawed gateway`.
...
created. Re-run `enclawed devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `enclawed devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.enclawed/enclawed.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop enclawed`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Enclawed
...
```bash
enclawed gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Enclawed **blocks** Control UI connections without device identity.
...
```bash
ENCLAWED_CONTROL_UI_BASE_PATH=/enclawed/ pnpm ui:build
```
