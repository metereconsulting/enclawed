...
```bash
enclawed tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `enclawed tui --deliver`
...
- Local shell commands receive `ENCLAWED_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `ENCLAWED_THEME=light` before launching `enclawed tui`.
- To force the original dark palette instead, set `ENCLAWED_THEME=dark`.
...
- Check the Gateway logs: `enclawed logs --follow`.
- Confirm the agent can run: `enclawed status` and `enclawed models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `enclawed agents list` and your routing config.
