---
summary: "Run Enclawed on local LLMs (LM Studio, vLLM, LiteLLM, custom OpenAI endpoints)"
read_when:
...
Local is doable, but Enclawed expects large context + strong defenses against prompt injection. Small cards truncate context and leak safety. Aim high: **≥2 maxed-out Mac Studios or equivalent GPU rig (~$30k+)**. A single **24 GB** GPU works only for lighter prompts with higher latency. Use the **largest / full-size model variant you can run**; aggressively quantized or “small” checkpoints raise prompt-injection risk (see [Security](/gateway/security)).

If you want the lowest-friction local setup, start with [LM Studio](/providers/lmstudio) or [Ollama](/providers/ollama) and `enclawed onboard`. This page is the opinionated guide for higher-end local stacks and custom OpenAI-compatible local servers.
...
- Enclawed treats these as proxy-style OpenAI-compatible routes, not native
  OpenAI endpoints
...
- hidden Enclawed attribution headers (`originator`, `version`, `User-Agent`)
...
- Some servers accept only string `messages[].content` on Chat Completions, not
  ...
  those endpoints.
- Some smaller or stricter local backends are unstable with Enclawed's full
  ...
  Enclawed agent turns, first try
...
- If the backend still fails only on larger Enclawed runs, the remaining issue
  is usually upstream model/server capacity or a backend bug, not Enclawed's
  transport layer.
...
- Enclawed warns when the detected context window is below **32k** and blocks below **16k**. If you hit that preflight, raise the server/model context limit or choose a larger model.
- Context errors? Lower `contextWindow` or raise your server limit.
- OpenAI-compatible server returns `messages[].content ... expected a string`?
  Add `compat.requiresStringContent: true` on that model entry.
- Direct tiny `/v1/chat/completions` calls work, but `enclawed infer model run`
  ...
  on larger Enclawed prompts, treat it as an upstream server/model limitation.
