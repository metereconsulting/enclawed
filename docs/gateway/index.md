...
enclawed gateway --port 18789 --verbose
# force-kill listener on selected port, then start
enclawed gateway --force
```
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `ENCLAWED_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `ENCLAWED_GATEWAY_TOKEN` / `ENCLAWED_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Enclawed’s highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `enclawed`, `enclawed/default`, and `enclawed/<agentId>`.
- `enclawed/default` is the stable alias that always maps to the configured default agent.
- Use `x-enclawed-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `ENCLAWED_GATEWAY_PORT` → `gateway.port` → `18789` |
...
LaunchAgent labels are `ai.enclawed.gateway` (default) or `ai.enclawed.<profile>` (named profile). `enclawed doctor` audits and repairs service config drift.
...
```bash
enclawed gateway install
systemctl --user enable --now enclawed-gateway[-<profile>].service
enclawed gateway status
```
...
[Service]
ExecStart=/usr/local/bin/enclawed gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `Enclawed Gateway`
(or `Enclawed Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Enclawed falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now enclawed-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/enclawed-gateway[-<profile>].service` and adjust
`ExecStart=` if your `enclawed` binary lives elsewhere.
...
- Unique `gateway.port`
- Unique `ENCLAWED_CONFIG_PATH`
- Unique `ENCLAWED_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
ENCLAWED_CONFIG_PATH=~/.enclawed/a.json ENCLAWED_STATE_DIR=~/.enclawed-a enclawed gateway --port 19001
ENCLAWED_CONFIG_PATH=~/.enclawed/b.json ENCLAWED_STATE_DIR=~/.enclawed-b enclawed gateway --port 19002
```
...
```bash
enclawed --dev setup
enclawed --dev gateway --allow-unconfigured
enclawed --dev status
```
