...
enclawed nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.enclawed`):

- `~/.enclawed/nodes/paired.json`
- `~/.enclawed/nodes/pending.json`

If you override `ENCLAWED_STATE_DIR`, the `nodes/` folder moves with it.
