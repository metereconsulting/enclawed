...
read_when:
  - Running Enclawed behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Enclawed
...
- You run Enclawed behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Enclawed checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Enclawed extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that Enclawed can verify.
...
- Enclawed can stay on loopback HTTP behind the proxy.
...
If Enclawed itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://enclawed.example.com
    to: http://enclawed-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
enclawed.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy enclawed:18789 {
...
location / {
    ...
    proxy_pass http://enclawed:18789;
...
Enclawed rejects ambiguous configurations where both a `gateway.auth.token` (or `ENCLAWED_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-enclawed-scopes`.

Examples:

- `x-enclawed-scopes: operator.read`
- `x-enclawed-scopes: operator.read,operator.write`
- `x-enclawed-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Enclawed honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-enclawed-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-enclawed-scopes` explicitly when you want a trusted-proxy request to
...
`enclawed security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
Enclawed rejected a loopback-source trusted-proxy request.
...
3. Update Enclawed config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `enclawed security audit` and review findings
