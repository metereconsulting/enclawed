...
Enclawed can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Enclawed requires a shared password.
...
- `token` (default when `ENCLAWED_GATEWAY_TOKEN` is set)
- `password` (shared secret via `ENCLAWED_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Enclawed verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Enclawed only treats a request as Serve when it arrives from loopback with
...
Prefer `ENCLAWED_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
enclawed gateway --tailscale serve
enclawed gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Enclawed to undo `tailscale serve`
