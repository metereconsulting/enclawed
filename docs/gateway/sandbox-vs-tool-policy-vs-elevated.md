...
Enclawed has three related (but different) controls:
...
Use the inspector to see what Enclawed is _actually_ doing:
...
- Enclawed validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:enclawed`: all built-in Enclawed tools (excludes provider plugins)
