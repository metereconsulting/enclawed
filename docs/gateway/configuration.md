...
read_when:
  - Setting up Enclawed for the first time
...
Enclawed reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.enclawed/enclawed.json`.

If the file is missing, Enclawed uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `enclawed onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    enclawed onboard       # full onboarding flow
    enclawed configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    enclawed config get agents.defaults.workspace
    enclawed config set agents.defaults.heartbeat.every "2h"
    enclawed config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.enclawed/enclawed.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Enclawed only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

Schema tooling notes:

- `enclawed config schema` prints the same JSON Schema family used by Control UI
  and config validation.
- Treat that schema output as the canonical machine-readable contract for
  `enclawed.json`; this overview and the configuration reference summarize it.
...
- Only diagnostic commands work (`enclawed doctor`, `enclawed logs`, `enclawed health`, `enclawed status`)
- Run `enclawed doctor` to see exact issues
- Run `enclawed doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `enclawed.json`.
    ...
    ```bash
    enclawed config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `ENCLAWED_APNS_RELAY_BASE_URL` and `ENCLAWED_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `ENCLAWED_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-enclawed-token`); query-string tokens are rejected.
...
The Gateway watches `~/.enclawed/enclawed.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `enclawed config set` for single keys.
    </Warning>
    ...
    ```bash
    enclawed gateway call config.get --params '{}'  # capture payload.hash
    enclawed gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.enclawed/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    enclawed gateway call config.patch --params '{
...
Enclawed reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.enclawed/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Enclawed runs your login shell and imports only the missing keys:
...
Env var equivalent: `ENCLAWED_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${ENCLAWED_GATEWAY_TOKEN}" } },
