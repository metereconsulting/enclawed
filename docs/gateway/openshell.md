---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Enclawed agents"
read_when:
...
OpenShell is a managed sandbox backend for Enclawed. Instead of running Docker
containers locally, Enclawed delegates sandbox lifecycle to the `openshell` CLI,
...
- Enclawed Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Enclawed creates an OpenShell
...
- You edit files locally outside Enclawed and want those changes visible in the
...
- When the sandbox is first created, Enclawed seeds the remote workspace from
...
- Enclawed does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Enclawed after the initial seed,
the remote sandbox does **not** see those changes. Use
`enclawed sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"enclawed"`  | Sandbox source for first-time create                  |
...
1. Enclawed calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Enclawed calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `enclawed sandbox` commands
