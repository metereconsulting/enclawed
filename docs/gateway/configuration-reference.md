...
Mattermost ships as a plugin: `openclaw plugins install @enclawed/mattermost`.
...
```bash
ENCLAWED_CONFIG_PATH=~/.openclaw/a.json \
ENCLAWED_STATE_DIR=~/.openclaw-a \
...
- Hostname defaults to `openclaw`. Override with `ENCLAWED_MDNS_HOSTNAME`.
