...
- `enclawed gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `enclawed doctor` reports no blocking config/service issues.
- `enclawed channels status --probe` shows live per-account transport status and,
...
enclawed config get agents.defaults.models
```
...
- Enclawed model runs fail only on normal agent turns
...
enclawed infer model run --model <provider/model> --prompt "hi" --json
enclawed logs --follow
```

Look for:

- direct tiny calls succeed, but Enclawed runs fail only on larger prompts
...
- direct tiny requests succeed, but Enclawed agent runs fail with backend/model
  crashes (for example Gemma on some `inferrs` builds) → Enclawed transport is
...
2. Set `compat.supportsTools: false` for models/backends that cannot handle
   Enclawed's tool schema surface reliably.
3. Lower prompt pressure where possible: smaller workspace bootstrap, shorter
   ...
   support.
4. If tiny direct requests keep passing while Enclawed agent turns still crash
...
enclawed status
enclawed channels status --probe
enclawed pairing list --channel <channel> [--account <id>]
enclawed config get channels
enclawed logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `enclawed config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `enclawed devices list` then `enclawed devices approve <requestId>`.                                                                                                                                                                                            |
...
If `enclawed devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `enclawed devices rotate --scope ...` can only request operator scopes that
...
enclawed logs --follow
enclawed doctor
enclawed gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `enclawed onboard --mode local` / `enclawed setup` to restamp the expected local-mode config. If you are running Enclawed via Podman, the default config path is `~/.enclawed/enclawed.json`.
...
Use this when `enclawed gateway probe` reaches something, but still prints a warning block.

```bash
enclawed gateway probe
enclawed gateway probe --json
enclawed gateway probe --ssh user@gateway-host
```
...
```bash
enclawed cron status
enclawed cron list
enclawed cron runs --id <jobId> --limit 20
enclawed system heartbeat last
enclawed logs --follow
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Enclawed skips the model call.
...
enclawed approvals get --node <idOrNameOrIp>
enclawed logs --follow
enclawed status
```
...
enclawed browser start --browser-profile enclawed
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `enclawed browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
```bash
enclawed gateway status
enclawed config get gateway.mode
enclawed config get gateway.remote.url
enclawed config get gateway.auth.mode
```
...
```bash
enclawed config get gateway.bind
enclawed config get gateway.auth.mode
enclawed config get gateway.auth.token
enclawed gateway status
enclawed logs --follow
```
