...
`enclawed doctor` is the repair + migration tool for Enclawed. It fixes stale
...
```bash
enclawed doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
enclawed doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
enclawed doctor --non-interactive
```
...
- Extra workspace dir detection (`~/enclawed`).
...
doctor-style RPC methods, but they are **not** part of `enclawed doctor` CLI
repair/migration.
...
```bash
enclawed memory rem-backfill --path ./memory --stage-short-term
```
...
you to run `enclawed doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.enclawed/enclawed.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `enclawed doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.enclawed/sessions/` to `~/.enclawed/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.enclawed/agent/` to `~/.enclawed/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.enclawed/credentials/*.json` (except `oauth.json`)
  - to `~/.enclawed/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `enclawed doctor`. Talk provider/provider-map normalization now
...
Doctor also checks the cron job store (`~/.enclawed/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.enclawed` folders exist across
  home directories or when `ENCLAWED_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.enclawed/enclawed.json` is
...
that re-auth is required and prints the exact `enclawed models auth login --provider ...`
command to run.
...
Discord plugin runtime packages) are present in the Enclawed install root.
If any are missing, doctor reports the packages and installs them in
`enclawed doctor --fix` / `enclawed doctor --repair` mode.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the Enclawed service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Enclawed gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`enclawed doctor` without `--fix`) this check
is skipped entirely.
...
- **Legacy workspace dirs**: warns when `~/enclawed` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(enclawed completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `enclawed completion --write-state` to regenerate the cache manually.
...
- `enclawed doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `enclawed doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `enclawed memory status --deep` to verify embedding readiness at runtime.
...
- `enclawed doctor` prompts before rewriting supervisor config.
- `enclawed doctor --yes` accepts the default repair prompts.
- `enclawed doctor --repair` applies recommended fixes without prompts.
- `enclawed doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `enclawed gateway install --force`.
