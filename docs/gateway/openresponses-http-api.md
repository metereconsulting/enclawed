...
Enclawed’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`enclawed agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-enclawed-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-enclawed-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "enclawed"`, `model: "enclawed/default"`, `model: "enclawed/<agentId>"`, or `x-enclawed-agent-id`
- use `x-enclawed-model` when you want to override the selected agent's backend model
- use `x-enclawed-session-key` for explicit session routing
- use `x-enclawed-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Enclawed reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Enclawed normalizes common OpenAI-style aliases before those counters reach
