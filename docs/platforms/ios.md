...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `enclawed.internal.`), **or**
...
Run `enclawed devices list` again before approval.

4. Verify connection:

```bash
enclawed nodes status
enclawed gateway call node.list --params "{}"
```
...
- `ENCLAWED_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Enclawed iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Enclawed builds.
...
```bash
export ENCLAWED_APNS_TEAM_ID="TEAMID"
export ENCLAWED_APNS_KEY_ID="KEYID"
export ENCLAWED_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.enclawed/credentials/apns
chmod 700 ~/.enclawed/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.enclawed/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.enclawed/credentials/apns/AuthKey_KEYID.p8
export ENCLAWED_APNS_PRIVATE_KEY_PATH="$HOME/.enclawed/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_enclawed-gw._tcp` on `local.` and, when configured, the same
...
`enclawed.internal.`) and Tailscale split DNS.
...
```bash
enclawed nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__enclawed__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__enclawed__/canvas/` and `/__enclawed__/a2ui/`.
...
```bash
enclawed nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__enclawed; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
enclawed nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `enclawed devices list` and approve manually.
