---
summary: "Enclawed on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Enclawed on DigitalOcean
  - Looking for cheap VPS hosting for Enclawed
title: "DigitalOcean (Platform)"
---

# Enclawed on DigitalOcean

## Goal

Run a persistent Enclawed Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
enclawed config set gateway.bind tailnet
enclawed gateway restart
```
...
```bash
enclawed channels login whatsapp
# Scan QR code
```
...
- `~/.enclawed/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u enclawed-gateway.service --no-pager -n 50
```
