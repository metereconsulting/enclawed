...
> **Note:** The Android app has not been publicly released yet. The source code is available in the [Enclawed repository](https://github.com/enclawed/enclawed) under `apps/android`. You can build it yourself using Java 17 and the Android SDK (`./gradlew :app:assemblePlayDebug`). See [apps/android/README.md](https://github.com/enclawed/enclawed/blob/main/apps/android/README.md) for build instructions.
...
- You can run the CLI (`enclawed`) on the gateway machine (or via SSH).
...
```bash
dns-sd -B _enclawed-gw._tcp local.
```
...
```bash
enclawed gateway discover --json
```
...
1. Set up a DNS-SD zone (example `enclawed.internal.`) on the gateway host and publish `_enclawed-gw._tcp` records.
...
1. Create `~/.enclawed/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
enclawed nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__enclawed__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__enclawed__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__enclawed__/a2ui/`.
...
Android supports launching Enclawed from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
Enclawed..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set Enclawed as the default assistant app.
</Note>
