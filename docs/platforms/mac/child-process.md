...
service via the external `enclawed` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.enclawed.gateway`
  (or `ai.enclawed.<profile>` when using `--profile`/`ENCLAWED_PROFILE`; legacy `com.enclawed.*` is supported).
