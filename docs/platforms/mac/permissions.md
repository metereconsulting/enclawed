...
- Same path: run the app from a fixed location (for Enclawed, `dist/Enclawed.app`).
...
```bash
sudo tccutil reset Accessibility ai.enclawed.mac
sudo tccutil reset ScreenCapture ai.enclawed.mac
...
Workaround: move files into the Enclawed workspace (`~/.enclawed/workspace`) if you want to avoid per-folder grants.
