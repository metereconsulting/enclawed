---
summary: "Enclawed on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Enclawed on a Raspberry Pi
  - Running Enclawed on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Enclawed on Raspberry Pi

## Goal

Run a persistent, always-on Enclawed Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u enclawed-gateway.service -f
```

## 9) Access the Enclawed Dashboard
...
```bash
ssh user@gateway-host 'enclawed dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/enclawed-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/enclawed-compile-cache
mkdir -p /var/tmp/enclawed-compile-cache
export ENCLAWED_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `ENCLAWED_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Enclawed, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit enclawed-gateway.service
```

```ini
[Service]
Environment=ENCLAWED_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/enclawed-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Enclawed state/cache on SSD-backed storage to avoid SD-card
...
Most Enclawed features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled enclawed-gateway.service

# Enable if not
systemctl --user enable enclawed-gateway.service
...
```bash
# Check logs
journalctl --user -u enclawed-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/enclawed  # if using hackable install
npm run build
systemctl --user restart enclawed-gateway.service
```
