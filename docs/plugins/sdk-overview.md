...
read_when:
  ...
  - You want a reference for all registration methods on EnclawedPluginApi
...
prefer `@enclawed/plugin-sdk/channel-core`; keep `@enclawed/plugin-sdk/core` for
...
`@enclawed/plugin-sdk/slack`, `@enclawed/plugin-sdk/discord`,
`@enclawed/plugin-sdk/signal`, `@enclawed/plugin-sdk/whatsapp`, or
...
| `plugin-sdk/config-schema`  | `EnclawedSchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `enclawed.json` Zod schema export (`EnclawedSchema`) |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, Enclawed tool bridge helpers, and attempt result utilities |
...
The `register(api)` callback receives an `EnclawedPluginApi` object with these
methods:
...
- User config still wins. Enclawed merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `@enclawed/plugin-sdk/memory-host-core` instead of reaching into a specific
...
| `api.config`             | `EnclawedConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `@enclawed/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Enclawed is already running. If no runtime
...
- `@enclawed/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `@enclawed/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `@enclawed/plugin-sdk/speech`, `.../provider-model-shared`, or another
