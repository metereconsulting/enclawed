...
summary: "Step-by-step guide to building a model provider plugin for Enclawed"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Enclawed
...
(LLM) to Enclawed. By the end you will have a provider with a model catalog,
...
<Tip>
  Provider plugins add models to Enclawed's normal inference loop. If the model
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so Enclawed can detect
    ...
    is optional and lets Enclawed auto-load your provider plugin from shorthand
    model ids like `acme-large` before runtime hooks exist. If you publish the
    provider on ClawHub, those `enclawed.compat` and `enclawed.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "@enclawed/plugin-sdk/provider-auth";
    ...
    `enclawed onboard --acme-ai-api-key <key>` and select
    `acme-ai/acme-large` as their model.

    If the upstream provider uses different control tokens than Enclawed, add a
    ...
    Enclawed parses its own control markers or channel delivery.
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "@enclawed/plugin-sdk/provider-entry";
    ...
    `@enclawed/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `@enclawed/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    import { buildProviderStreamFamilyHooks } from "@enclawed/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "@enclawed/plugin-sdk/provider-tools";
    ...
    `@enclawed/plugin-sdk/provider-model-shared` also exports the replay-family
    ...
    `@enclawed/plugin-sdk/provider-stream` exposes both the family builder and
    ...
    example: `@enclawed/anthropic-provider` exports
    ...
    `@enclawed/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@enclawed/openai-provider`: `api.ts` exports provider builders,
      default-model helpers, and realtime provider builders
    - `@enclawed/openrouter-provider`: `api.ts` exports the provider builder
    ...
    <Accordion title="All available provider hooks">
      Enclawed calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Enclawed classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # enclawed.providers metadata
├── enclawed.plugin.json      # Manifest with provider auth metadata
