...
read_when:
  - You want to place an outbound voice call from Enclawed
...
Voice calls for Enclawed via a plugin. Supports outbound notifications and
...
- Use `enclawed voicecall ...` or the `voice_call` tool
...
enclawed plugins install "$PLUGIN_SRC"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `enclawed doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `enclawed doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `enclawed doctor --fix`:
...
```bash
enclawed voicecall call --to "+15555550123" --message "Hello from Enclawed"
enclawed voicecall start --to "+15555550123"   # alias for call
enclawed voicecall continue --call-id <id> --message "Any questions?"
enclawed voicecall speak --call-id <id> --message "One moment"
enclawed voicecall end --call-id <id>
enclawed voicecall status --call-id <id>
enclawed voicecall tail
enclawed voicecall latency                     # summarize turn latency from logs
enclawed voicecall expose --mode funnel
```
