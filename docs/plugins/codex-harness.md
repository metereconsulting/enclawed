---
title: "Codex Harness"
summary: "Run Enclawed embedded agent turns through the bundled Codex app-server harness"
read_when:
...
The bundled `codex` plugin lets Enclawed run embedded agent turns through the
...
Enclawed still owns chat channels, session files, model selection, tools,
...
force `embeddedHarness.runtime: "codex"` or `ENCLAWED_AGENT_RUNTIME=codex`.
...
Enclawed has separate routes for OpenAI and Codex-shaped access:

| Model ref              | Runtime path                                 | Use when                                                                |
| ---------------------- | -------------------------------------------- | ----------------------------------------------------------------------- |
| `openai/gpt-5.4`       | OpenAI provider through Enclawed/PI plumbing | You want direct OpenAI Platform API access with `OPENAI_API_KEY`.       |
...
- Enclawed with the bundled `codex` plugin available.
...
Enclawed on the protocol surface it has been tested against.
...
With fallback disabled, Enclawed fails early if the Codex plugin is disabled,
...
Enclawed session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the Enclawed session binding for that thread.
...
- `ENCLAWED_CODEX_APP_SERVER_BIN`
- `ENCLAWED_CODEX_APP_SERVER_ARGS`
- `ENCLAWED_CODEX_APP_SERVER_APPROVAL_POLICY`
- `ENCLAWED_CODEX_APP_SERVER_SANDBOX`
- `ENCLAWED_CODEX_APP_SERVER_GUARDIAN=1`
...
Model switching stays Enclawed-controlled. When an Enclawed session is attached
...
generic and works on any channel that supports Enclawed text commands.
...
- `/codex resume <thread-id>` attaches the current Enclawed session to an existing Codex thread.
...
normal turns. On the next message, Enclawed resumes that Codex thread, passes the
currently selected Enclawed `codex/*` model into app-server, and keeps extended
history enabled.
...
Enclawed still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal Enclawed delivery path.

When the selected model uses the Codex harness, native thread compaction is
delegated to Codex app-server. Enclawed keeps a transcript mirror for channel
...
**Enclawed falls back to PI:** set `embeddedHarness.fallback: "none"` or
`ENCLAWED_AGENT_HARNESS_FALLBACK=none` while testing.
