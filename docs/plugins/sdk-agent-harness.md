...
An **agent harness** is the low level executor for one prepared Enclawed agent
...
runtime and the normal Enclawed provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Enclawed
  session transcript
...
Before a harness is selected, Enclawed has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Enclawed transcript/session file
...
**Import:** `@enclawed/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "@enclawed/plugin-sdk/agent-harness";
...
Enclawed chooses a harness after provider/model resolution:

1. `ENCLAWED_AGENT_RUNTIME=<id>` forces a registered harness with that id.
2. `ENCLAWED_AGENT_RUNTIME=pi` forces the built-in PI harness.
3. `ENCLAWED_AGENT_RUNTIME=auto` asks registered harnesses if they support the
   resolved provider/model.
4. If no registered harness matches, Enclawed uses PI unless PI fallback is
   disabled.

Forced plugin harness failures surface as run failures. In `auto` mode,
Enclawed may fall back to PI when the selected plugin harness fails before a
turn has produced side effects. Set `ENCLAWED_AGENT_HARNESS_FALLBACK=none` or
...
Enclawed. The harness then claims that provider in `supports(...)`.
...
- app-server request: Enclawed sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs remain OpenAI provider
refs and continue to use the normal Enclawed provider path. Select `codex/gpt-*`
...
Enclawed requires Codex app-server `0.118.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Enclawed only runs against the protocol surface it has been tested with.

### Native Codex harness mode

The bundled `codex` harness is the native Codex mode for embedded Enclawed
agent turns. Enable the bundled `codex` plugin first, and include `codex` in
`plugins.allow` if your config uses a restrictive allowlist. It is different
from `openai-codex/*`:

- `openai-codex/*` uses ChatGPT/Codex OAuth through the normal Enclawed provider
  path.
- `codex/*` uses the bundled Codex provider and routes the turn through Codex
  app-server.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Enclawed still owns the chat channel,
...
By default, Enclawed runs embedded agents with `agents.defaults.embeddedHarness`
...
plugin harness fails before producing output, Enclawed falls back to PI.
...
an explicit `runtime: "pi"` or `ENCLAWED_AGENT_RUNTIME=pi`.
...
want Enclawed to silently fall back to PI, keep `runtime: "auto"` and disable
the fallback:
...
`ENCLAWED_AGENT_RUNTIME` still overrides the configured runtime. Use
`ENCLAWED_AGENT_HARNESS_FALLBACK=none` to disable PI fallback from the
environment.

```bash
ENCLAWED_AGENT_RUNTIME=codex \
ENCLAWED_AGENT_HARNESS_FALLBACK=none \
...
Keep that binding explicitly associated with the Enclawed session, and keep
mirroring user-visible assistant/tool output into the Enclawed transcript.

The Enclawed transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so Enclawed can
clear it when the owning Enclawed session is reset.

## Tool and media results

Core constructs the Enclawed tool list and passes it into the prepared attempt.
