...
read_when:
  - You are building an Enclawed plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (enclawed.plugin.json)

This page is for the **native Enclawed plugin manifest** only.
...
Enclawed auto-detects those bundle layouts too, but they are not validated
against the `enclawed.plugin.json` schema described here.

For compatible bundles, Enclawed currently reads bundle metadata plus declared
...
Enclawed runtime expectations.

Every native Enclawed plugin **must** ship a `enclawed.plugin.json` file in the
**plugin root**. Enclawed uses this manifest to validate configuration
...
`enclawed.plugin.json` is the metadata Enclawed reads before it loads your
plugin code.
...
- static capability ownership snapshots used for bundled compat wiring and
  contract coverage
- cheap QA runner metadata that the shared `enclawed qa` host can inspect
...
| `providerAuthEnvVars`               | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Enclawed can inspect without loading plugin code.                                                                                                                      |
| `providerAuthAliases`               | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                   |
| `channelEnvVars`                    | No       | `Record<string, string[]>`       | Cheap channel env metadata that Enclawed can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                     |
...
| `qaRunners`                         | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `enclawed qa` host before plugin runtime loads.                                                                                                               |
...
Enclawed reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Enclawed falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Enclawed
...
the shared `enclawed qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `enclawed qa`, for example `matrix`.    |
...
Use `contracts` only for static capability ownership metadata that Enclawed can
...
Use `modelSupport` when Enclawed should infer your provider plugin from
...
Enclawed applies this precedence:
...
Legacy top-level capability keys are deprecated. Use `enclawed doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `enclawed.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `enclawed` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Enclawed must know it before loading plugin code, put it in `enclawed.plugin.json`
...
`enclawed` block instead of `enclawed.plugin.json`.

Important examples:

| Field                                                             | What it means                                                                                                                                |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `enclawed.extensions`                                             | Declares native plugin entrypoints.                                                                                                          |
| `enclawed.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding and deferred channel startup.                                                       |
| `enclawed.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                         |
| `enclawed.channel.configuredState`                                | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime. |
| `enclawed.channel.persistedAuthState`                             | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.       |
| `enclawed.install.npmSpec` / `enclawed.install.localPath`         | Install/update hints for bundled and externally published plugins.                                                                           |
| `enclawed.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                                                                          |
| `enclawed.install.minHostVersion`                                 | Minimum supported Enclawed host version, using a semver floor like `>=2026.3.22`.                                                            |
| `enclawed.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                               |
| `enclawed.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                         |

`enclawed.install.minHostVersion` is enforced during install and manifest
registry loading. Invalid values are rejected; newer-but-valid values skip the
plugin on older hosts.

`enclawed.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `enclawed doctor --fix`.

`enclawed.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`enclawed.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
- The manifest is **required for native Enclawed plugins**, including local filesystem loads.
