...
summary: "Create your first Enclawed plugin in minutes"
read_when:
  - You want to create a new Enclawed plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to Enclawed
---
...
You do not need to add your plugin to the Enclawed repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`enclawed plugins install <package-name>`. Enclawed tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Enclawed to a messaging platform (Discord, IRC, etc.)
  </Card>
...
`@enclawed/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json enclawed.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to Enclawed",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    enclawed plugins install clawhub:@myorg/enclawed-my-plugin
    ```

    Enclawed also checks ClawHub before npm for bare package specs like
    `@myorg/enclawed-my-plugin`.
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, Enclawed retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `@enclawed/plugin-sdk/error-runtime`
...
Always import from focused `@enclawed/plugin-sdk/<subpath>` paths:
...
import { ... } from "@enclawed/plugin-sdk";
```
...
package-root seam instead of promoting it into `@enclawed/plugin-sdk/*`.

Some generated `@enclawed/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `enclawed` metadata</Check>
<Check>**enclawed.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [enclawed/enclawed](https://github.com/enclawed/enclawed/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Enclawed X account [@enclawed](https://x.com/enclawed) for release announcements.
