...
read_when:
  ...
  - You are defining plugin config schemas or package.json enclawed metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`enclawed.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `enclawed` field that tells the plugin system what
...
```json
{
  "name": "@myorg/enclawed-my-channel",
  "version": "1.0.0",
  "type": "module",
  "enclawed": {
    "extensions": ["./index.ts"],
...
```json enclawed-clawhub-package.json
{
...
`enclawed.channel` is cheap package metadata for channel discovery and setup
...
`enclawed.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported Enclawed version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `enclawed doctor --fix`.
...
When enabled, Enclawed loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `enclawed.plugin.json` in the package root.
Enclawed uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Enclawed",
  "configSchema": {
    "type": "object",
...
Enclawed loads when it only needs setup surfaces (onboarding, config repair,
...
`@enclawed/plugin-sdk/channel-entry-contract` instead of
...
**When Enclawed uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `@enclawed/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Enclawed validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "@enclawed/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `enclawed onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "@enclawed/plugin-sdk/channel-setup";
...
helpers from `@enclawed/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`@enclawed/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `@enclawed/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "@enclawed/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/enclawed-my-channel",
  docsPath: "/channels/my-channel",
});
...
```bash
enclawed plugins install @myorg/enclawed-my-plugin
```

Enclawed tries ClawHub first and falls back to npm automatically. You can also
force ClawHub explicitly:

```bash
enclawed plugins install clawhub:@myorg/enclawed-my-plugin   # ClawHub only
```
...
```bash
enclawed plugins install <package-name>
```

<Info>
  For npm-sourced installs, `enclawed plugins install` runs
