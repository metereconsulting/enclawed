...
read_when:
  - Building or debugging native Enclawed plugins
...
This page covers the internal architecture of the Enclawed plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Enclawed. Every
native Enclawed plugin registers against one or more capability types:
...
Enclawed classifies every loaded plugin into a shape based on its actual
...
Use `enclawed plugins inspect <id>` to see a plugin's shape and capability
...
When you run `enclawed doctor` or `enclawed plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `enclawed status --all` and `enclawed plugins doctor`.

## Architecture overview

Enclawed's plugin system has four layers:

1. **Manifest + discovery**
   Enclawed finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `enclawed.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Enclawed plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Enclawed reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Enclawed
...
That split lets Enclawed validate config, explain missing/disabled plugins, and
...
normal chat actions. Enclawed keeps one shared `message` tool in core, and
...
Enclawed treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Enclawed-facing
  surfaces
...
So if Enclawed adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Enclawed has shared
...
```ts
import type { EnclawedPluginDefinition } from "@enclawed/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "@enclawed/plugin-sdk/media-understanding";

const plugin: EnclawedPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Enclawed already treats image/audio/video understanding as one shared
...
`EnclawedPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Enclawed can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Enclawed knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `EnclawedPluginApi` or
  `api.runtime`
...
Native Enclawed plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Enclawed process

Compatible bundles are safer by default because Enclawed currently treats them
...
name: `@enclawed/<id>` by default, or an approved typed suffix such as
...
Enclawed exports capabilities, not implementation convenience.
...
At startup, Enclawed does roughly this:
...
The manifest is the control-plane source of truth. Enclawed uses it to:
...
Enclawed keeps short in-process caches for:
...
- Set `ENCLAWED_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `ENCLAWED_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `ENCLAWED_PLUGIN_DISCOVERY_CACHE_MS` and
  `ENCLAWED_PLUGIN_MANIFEST_CACHE_MS`.
...
Enclawed still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Enclawed calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | Enclawed tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on Enclawed's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Enclawed's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Enclawed adds those
...
- Enclawed only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Enclawed plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-enclawed-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-enclawed-scopes` only when the header is explicitly present
  - if `x-enclawed-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-enclawed-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `@enclawed/plugin-sdk` import when
authoring plugins:

- `@enclawed/plugin-sdk/plugin-entry` for plugin registration primitives.
- `@enclawed/plugin-sdk/core` for the generic shared plugin-facing contract.
- `@enclawed/plugin-sdk/config-schema` for the root `enclawed.json` Zod schema
  export (`EnclawedSchema`).
- Stable channel primitives such as `@enclawed/plugin-sdk/channel-setup`,
  `@enclawed/plugin-sdk/setup-runtime`,
  `@enclawed/plugin-sdk/setup-adapter-runtime`,
  `@enclawed/plugin-sdk/setup-tools`,
  `@enclawed/plugin-sdk/channel-pairing`,
  `@enclawed/plugin-sdk/channel-contract`,
  `@enclawed/plugin-sdk/channel-feedback`,
  `@enclawed/plugin-sdk/channel-inbound`,
  `@enclawed/plugin-sdk/channel-lifecycle`,
  `@enclawed/plugin-sdk/channel-reply-pipeline`,
  `@enclawed/plugin-sdk/command-auth`,
  `@enclawed/plugin-sdk/secret-input`, and
  `@enclawed/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `@enclawed/plugin-sdk/channel-config-helpers`,
  `@enclawed/plugin-sdk/allow-from`,
  `@enclawed/plugin-sdk/channel-config-schema`,
  `@enclawed/plugin-sdk/telegram-command-config`,
  `@enclawed/plugin-sdk/channel-policy`,
  `@enclawed/plugin-sdk/approval-gateway-runtime`,
  `@enclawed/plugin-sdk/approval-handler-adapter-runtime`,
  `@enclawed/plugin-sdk/approval-handler-runtime`,
  `@enclawed/plugin-sdk/approval-runtime`,
  `@enclawed/plugin-sdk/config-runtime`,
  `@enclawed/plugin-sdk/infra-runtime`,
  `@enclawed/plugin-sdk/agent-runtime`,
  `@enclawed/plugin-sdk/lazy-runtime`,
  `@enclawed/plugin-sdk/reply-history`,
  `@enclawed/plugin-sdk/routing`,
  `@enclawed/plugin-sdk/status-helpers`,
  `@enclawed/plugin-sdk/text-runtime`,
  `@enclawed/plugin-sdk/runtime-store`, and
  `@enclawed/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `@enclawed/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `@enclawed/plugin-sdk/*` subpaths. Enclawed core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  Enclawed is not yet serving a runtime snapshot.
...
- Avoid the root `@enclawed/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `@enclawed/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `@enclawed/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `@enclawed/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `@enclawed/plugin-sdk/channel-targets`; channel-specific
...
`@enclawed/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Enclawed writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Enclawed's
...
- if both `catalog` and `discovery` are registered, Enclawed uses `catalog`
...
- Read-only command paths such as `enclawed status`, `enclawed status --all`,
  `enclawed channels status`, `enclawed channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `enclawed.extensions`:
...
Security guardrail: every `enclawed.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `enclawed plugins install` installs plugin dependencies with
...
Optional: `enclawed.setupEntry` can point at a lightweight setup-only module.
When Enclawed needs setup surfaces for a disabled channel plugin, or
...
Optional: `enclawed.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Enclawed load the
...
Channel plugins can advertise setup/discovery metadata via `enclawed.channel` and
install hints via `enclawed.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@enclawed/nextcloud-talk",
  "enclawed": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@enclawed/nextcloud-talk",
...
Useful `enclawed.channel` fields beyond the minimal example:
...
Enclawed can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.enclawed/mpm/plugins.json`
- `~/.enclawed/mpm/catalog.json`
- `~/.enclawed/plugins/catalog.json`

Or point `ENCLAWED_PLUGIN_CATALOG_PATHS` (or `ENCLAWED_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "enclawed": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { buildMemorySystemPromptAddition } from "@enclawed/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `EnclawedPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Enclawed stays opinionated without becoming hardcoded to one
