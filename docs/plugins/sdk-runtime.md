...
`runEmbeddedAgent(...)` is the neutral helper for starting a normal Enclawed
...
Bind a Task Flow runtime to an existing Enclawed session key or trusted tool
...
trusted Enclawed session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "Enclawed plugin SDK", count: 5 },
});
```
