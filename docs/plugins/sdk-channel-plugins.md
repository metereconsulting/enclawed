...
summary: "Step-by-step guide to building a messaging channel plugin for Enclawed"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Enclawed to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Enclawed to a
...
<Info>
  If you have not built any Enclawed plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Enclawed keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `@enclawed/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `@enclawed/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `@enclawed/plugin-sdk/approval-auth-runtime`
- `@enclawed/plugin-sdk/approval-client-runtime`
- `@enclawed/plugin-sdk/approval-delivery-runtime`
...
- `@enclawed/plugin-sdk/approval-native-runtime`
- `@enclawed/plugin-sdk/approval-reply-runtime`
- `@enclawed/plugin-sdk/channel-runtime-context`

Likewise, prefer `@enclawed/plugin-sdk/setup-runtime`,
`@enclawed/plugin-sdk/setup-adapter-runtime`,
`@enclawed/plugin-sdk/reply-runtime`,
`@enclawed/plugin-sdk/reply-dispatch-runtime`,
`@enclawed/plugin-sdk/reply-reference`, and
`@enclawed/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `@enclawed/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `@enclawed/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `@enclawed/plugin-sdk/channel-setup` covers the optional-install setup
...
- use the broader `@enclawed/plugin-sdk/setup` seam only when you also need the
...
- `@enclawed/plugin-sdk/account-core`,
  `@enclawed/plugin-sdk/account-id`,
  `@enclawed/plugin-sdk/account-resolution`, and
  `@enclawed/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `@enclawed/plugin-sdk/inbound-envelope` and
  `@enclawed/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `@enclawed/plugin-sdk/messaging-targets` for target parsing/matching
- `@enclawed/plugin-sdk/outbound-media` and
  `@enclawed/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates
- `@enclawed/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `@enclawed/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `@enclawed/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `@enclawed/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `@enclawed/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
} from "@enclawed/plugin-sdk/channel-inbound";
...
`@enclawed/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`@enclawed/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#enclawed-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/enclawed-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "enclawed": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Enclawed to Acme Chat."
        }
      }
    }
    ```

    ```json enclawed.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { EnclawedConfig } from "@enclawed/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "@enclawed/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Enclawed
  ...
  <Step title="Add a setup entry">
    ...
    Enclawed loads this instead of the full entry when the channel is disabled
    ...
    `@enclawed/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Enclawed. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Enclawed.
...
├── package.json              # enclawed.channel metadata
├── enclawed.plugin.json      # Manifest with config schema
