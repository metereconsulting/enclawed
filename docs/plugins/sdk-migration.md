...
read_when:
  - You see the ENCLAWED_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the ENCLAWED_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Enclawed plugin
---

# Plugin SDK Migration

Enclawed has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`@enclawed/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`@enclawed/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`@enclawed/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `@enclawed/plugin-sdk/slack`, `@enclawed/plugin-sdk/discord`,
`@enclawed/plugin-sdk/signal`, `@enclawed/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`@enclawed/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `@enclawed/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `@enclawed/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "@enclawed/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "@enclawed/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "@enclawed/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `EnclawedSchema` |
...
```bash
ENCLAWED_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 enclawed gateway run
ENCLAWED_SUPPRESS_EXTENSION_API_WARNING=1 enclawed gateway run
```
