...
read_when:
  ...
  - Debugging why enclawed is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Enclawed-managed)"
---

# Browser (enclawed-managed)

Enclawed can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `enclawed` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **enclawed** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`enclawed`, `work`, `remote`, ...).
...
```bash
enclawed browser --browser-profile enclawed status
...
If `enclawed browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`enclawed browser`),
...
If `enclawed browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "enclawed"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.enclawed/enclawed.json`.
...
- If you override the Gateway port (`gateway.port` or `ENCLAWED_GATEWAY_PORT`),
...
- Default profile is `enclawed` (Enclawed-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `enclawed` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
Enclawed uses it automatically. Set `browser.executablePath` to override
auto-detection:

CLI example:

```bash
enclawed config set browser.executablePath "/usr/bin/google-chrome"
```
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Enclawed will not launch a local browser.

Stopping behavior differs by profile mode:

- local managed profiles: `enclawed browser stop` stops the browser process that
  Enclawed launched
- attach-only and remote CDP profiles: `enclawed browser stop` closes the active
  ...
  though no browser process was launched by Enclawed
...
Enclawed preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Enclawed can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Enclawed treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. Enclawed can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Enclawed
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Enclawed supports both:

- **HTTP(S) endpoints** — Enclawed calls `/json/version` to discover the
  WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — Enclawed connects directly,
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-enclawed-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Enclawed
  auto-generates `gateway.auth.token` on startup and persists it to config.
- Enclawed does **not** auto-generate that token when `gateway.auth.mode` is
...
Enclawed supports multiple named profiles (routing configs). Profiles can be:

- **enclawed-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `enclawed` profile is auto-created if missing.
...
Enclawed can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Enclawed attaches.
...
```bash
enclawed browser --browser-profile user start
enclawed browser --browser-profile user status
enclawed browser --browser-profile user tabs
enclawed browser --browser-profile user snapshot --format ai
```
...
- `enclawed doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `enclawed` profile because it can
  act inside your signed-in browser session.
- Enclawed does not launch the browser for this driver; it attaches to an
  existing session only.
- Enclawed uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, Enclawed passes it through to target that explicit
...
When launching locally, Enclawed picks the first available:
...
- `x-enclawed-password: <gateway password>` or HTTP Basic auth with that password
...
- ARIA snapshots
- Page screenshots for the managed `enclawed` browser when a per-tab CDP
...
```bash
docker compose run --rm enclawed-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`ENCLAWED_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `enclawed browser snapshot --interactive --compact --depth 6`
- `enclawed browser snapshot --efficient`
- `enclawed browser snapshot --labels`
- `enclawed browser snapshot --selector "#main" --interactive`
- `enclawed browser snapshot --frame "iframe#main" --interactive`
- `enclawed browser console --level error`

Lifecycle note:

- For attach-only and remote CDP profiles, `enclawed browser stop` is still the
...
- `enclawed browser requests --filter api --clear`
- `enclawed browser pdf`
- `enclawed browser responsebody "**/api" --max-chars 5000`
...
- `enclawed browser storage local set theme dark`
- `enclawed browser storage session clear`
- `enclawed browser set offline on`
- `enclawed browser set headers --headers-json '{"X-Debug":"1"}'`
- `enclawed browser set credentials user pass`
- `enclawed browser set credentials --clear`
- `enclawed browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `enclawed browser set geo --clear`
- `enclawed browser set media dark`
- `enclawed browser set timezone America/New_York`
- `enclawed browser set locale en-US`
...
- Download and trace output paths are constrained to Enclawed temp roots:
  - traces: `/tmp/enclawed` (fallback: `${os.tmpdir()}/enclawed`)
  - downloads: `/tmp/enclawed/downloads` (fallback: `${os.tmpdir()}/enclawed/downloads`)
- Upload paths are constrained to an Enclawed temp uploads root:
  - uploads: `/tmp/enclawed/uploads` (fallback: `${os.tmpdir()}/enclawed/uploads`)
...
Enclawed supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `enclawed browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `enclawed browser click 12`, `enclawed browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `enclawed browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `enclawed browser click e12`, `enclawed browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `enclawed browser wait --url "**/dash"`
- Wait for load state:
  - `enclawed browser wait --load networkidle`
- Wait for a JS predicate:
  - `enclawed browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `enclawed browser wait "#main"`
...
1. `enclawed browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `enclawed browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `enclawed browser trace start`
   - reproduce the issue
   - `enclawed browser trace stop` (prints `TRACE:<path>`)
...
```bash
enclawed browser status --json
enclawed browser snapshot --interactive --json
enclawed browser requests --filter api --json
enclawed browser cookies --json
```
...
- The enclawed browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `enclawed browser evaluate` and `wait --fn`
...
- **CDP startup or readiness failure** means Enclawed cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "enclawed" is not reachable after start`
...
- For the local loopback `enclawed` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Enclawed's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (enclawed, chrome, or remote CDP).
