---
summary: "Install, configure, and manage Enclawed plugins"
read_when:
...
Plugins extend Enclawed with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with Enclawed), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ```bash
    # From npm
    enclawed plugins install @enclawed/voice-call

    # From a local directory or archive
    enclawed plugins install ./my-plugin
    enclawed plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@enclawed/voice-call
...
`enclawed doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`enclawed.install.allowInvalidConfigRecovery`.

## Plugin types

Enclawed recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `enclawed.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to Enclawed features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `enclawed plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@enclawed/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@enclawed/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@enclawed/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@enclawed/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@enclawed/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@enclawed/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with Enclawed)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `enclawed browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `enclawed gateway` path), that
...
Enclawed scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.enclawed/<plugin-root>/*.ts` and `\<workspace\>/.enclawed/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.enclawed/<plugin-root>/*.ts` and `~/.enclawed/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with Enclawed. Many are enabled by default (model providers, speech).
...
```bash
enclawed plugins list                       # compact inventory
enclawed plugins list --enabled            # only loaded plugins
enclawed plugins list --verbose            # per-plugin detail lines
enclawed plugins list --json               # machine-readable inventory
enclawed plugins inspect <id>              # deep detail
enclawed plugins inspect <id> --json       # machine-readable
enclawed plugins inspect --all             # fleet-wide table
enclawed plugins info <id>                 # inspect alias
enclawed plugins doctor                    # diagnostics

enclawed plugins install <package>         # install (ClawHub first, then npm)
enclawed plugins install clawhub:<pkg>     # install from ClawHub only
enclawed plugins install <spec> --force    # overwrite existing install
enclawed plugins install <path>            # install from local path
enclawed plugins install -l <path>         # link (no copy) for dev
enclawed plugins install <plugin> --marketplace <source>
enclawed plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
enclawed plugins install <spec> --pin      # record exact resolved npm spec
enclawed plugins install <spec> --dangerously-force-unsafe-install
enclawed plugins update <id>             # update one plugin
enclawed plugins update <id> --dangerously-force-unsafe-install
enclawed plugins update --all            # update all
enclawed plugins uninstall <id>          # remove config/install records
enclawed plugins uninstall <id> --keep-files
enclawed plugins marketplace list <source>
enclawed plugins marketplace list <source> --json

enclawed plugins enable <id>
enclawed plugins disable <id>
```

Bundled plugins ship with Enclawed. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `enclawed plugins enable <id>`.
...
override instead, while `enclawed skills install` remains the separate ClawHub
...
`enclawed plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`enclawed plugins` CLI reference](/cli/plugins) for full details.
...
Enclawed loads the entry object and calls `register(api)` during plugin
