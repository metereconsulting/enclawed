...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.enclawed/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Enclawed picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    enclawed agent --message "give me a greeting"
    ```
...
The YAML frontmatter for the quickstart form supports these fields:

| Field                               | Required | Description                                 |
| ----------------------------------- | -------- | ------------------------------------------- |
| `name`                              | Yes      | Unique identifier (snake_case)              |
| `description`                       | Yes      | One-line description shown to the agent     |
| `metadata.enclawed.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.enclawed.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.enclawed.requires.config` | No       | Required config keys                        |

The quickstart form ships an `unverified` skill: every irreversible call
goes through a human-in-the-loop prompt. To raise the verification level
and relax HITL frequency, ship the full form documented at
[SKILL.md Manifest Specification](/tools/skill-manifest-spec) — that page
covers the seven mandatory fields (`v`, `id`, `label`, `caps`, `signer`,
`version`, `verification`), the capability vocabulary, the canonicalisation
and Ed25519 signing convention, the bootstrap re-check protocol, and the
proof-carrying bundle required for the `formal` level.
...
- **Test locally** — use `enclawed agent --message "..."` to test before sharing
...
| `~/.enclawed/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Enclawed) | Low        | Global                |
| `skills.load.extraDirs`         | Lowest     | Custom shared folders |

## Related

- [SKILL.md Manifest Specification](/tools/skill-manifest-spec) — full schema,
  signing, verification levels, proof-carrying bundle
