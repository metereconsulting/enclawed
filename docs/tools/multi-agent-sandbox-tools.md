...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `enclawed sandbox explain`.
...
Legacy `agent.*` configs are migrated by `enclawed doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=enclawed-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ENCLAWED_STATE_DIR:-$HOME/.enclawed}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
