...
Enclawed can convert outbound replies into audio using ElevenLabs, Google Gemini, Microsoft, MiniMax, or OpenAI.
It works anywhere Enclawed can send audio.
...
When `messages.tts.provider` is unset, Enclawed picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `enclawed.json`.
...
- If `provider` is **unset**, Enclawed uses the first configured speech provider in registry auto-select order.
...
`~/.enclawed/settings/tts.json`, override with `ENCLAWED_TTS_PREFS` or
...
- **Google Gemini**: Gemini API TTS returns raw 24kHz PCM. Enclawed wraps it as WAV for audio attachments and returns PCM directly for Talk/telephony. Native Opus voice-note format is not supported by this path.
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  - The bundled transport accepts an `outputFormat`, but not all formats are available from the service.
  - Output format values follow Microsoft Speech output formats (including Ogg/WebM Opus).
  - Telegram `sendVoice` accepts OGG/MP3/M4A; use OpenAI/ElevenLabs if you need
    guaranteed Opus voice messages.
  - If the configured Microsoft output format fails, Enclawed retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, Enclawed:
...
Discord note: `/tts` is a built-in Discord command, so Enclawed registers
