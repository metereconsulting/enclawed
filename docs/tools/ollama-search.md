...
- an Ollama host that is reachable from Enclawed
- `ollama signin`
...
If no explicit Ollama base URL is set, Enclawed uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, Enclawed reuses
...
- If the Ollama host is auth-protected, Enclawed reuses the normal Ollama
  provider API key when present.
- Enclawed warns during setup if Ollama is unreachable or not signed in, but
