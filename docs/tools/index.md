---
summary: "Enclawed tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools Enclawed provides
...
Enclawed has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). Enclawed ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    Enclawed), others are **external** (published on npm by the community).
...
These tools ship with Enclawed and are available without installing any plugins:
...
| `group:enclawed`   | All built-in Enclawed tools (excludes plugin tools)                                                       |
