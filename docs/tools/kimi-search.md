...
Enclawed supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Enclawed reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.enclawed/.env`.

If you omit `baseUrl`, Enclawed defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Enclawed defaults to `kimi-k2.5`.
