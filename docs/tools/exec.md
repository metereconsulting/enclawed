...
- `gateway`/`node` approvals are controlled by `~/.enclawed/exec-approvals.json`.
...
- Enclawed sets `ENCLAWED_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.enclawed/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#no-approval-yolo-mode).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Enclawed prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`enclawed security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `enclawed doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`enclawed security audit` and `enclawed doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
