...
`ask: "always"` in `~/.enclawed/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `enclawed approvals get`, `enclawed approvals get --gateway`, or
`enclawed approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `enclawed exec-policy show` exposes the same merged view and
`enclawed exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`enclawed exec-policy show` reports that scope as node-managed at runtime instead of
...
- **gateway host** → `enclawed` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Enclawed also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.enclawed/exec-approvals.sock",
...
- requested exec policy in Enclawed config (`tools.exec.*`)
- host-local approvals policy in `~/.enclawed/exec-approvals.json`
...
- In YOLO mode, Enclawed does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
enclawed config set tools.exec.host gateway
enclawed config set tools.exec.security full
enclawed config set tools.exec.ask off
...
```bash
enclawed approvals set --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `enclawed exec-policy` does not synchronize node approvals
- `enclawed exec-policy set --host node` is rejected
- node exec approvals are fetched from the node at runtime, so node-targeted updates must use `enclawed approvals --node ...`
...
When `tools.exec.strictInlineEval=true`, Enclawed treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.enclawed/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `enclawed approvals allowlist ...`).
- `enclawed security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `enclawed doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Enclawed still rejects the `env` builtin in safe-bin
...
`~/.enclawed/exec-approvals.json` directly.

CLI: `enclawed approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, Enclawed sends a followup `agent` turn to the same session.
...
topics, Enclawed preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Enclawed prevents the agent from reusing
