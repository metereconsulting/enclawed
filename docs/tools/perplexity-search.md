...
Enclawed supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Enclawed also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `enclawed configure --section web`. It stores the key in
`~/.enclawed/enclawed.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.enclawed/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
