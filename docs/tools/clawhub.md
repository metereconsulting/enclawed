---
summary: "ClawHub guide: public registry, native Enclawed install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Enclawed skills and plugins**.

- Use native `enclawed` commands to search/install/update skills and install
...
```bash
enclawed plugins install clawhub:<package>
...
Native `enclawed` commands install into your active workspace and persist source
...
`enclawed plugins install clawhub:...` only accepts installable plugin families.
If a ClawHub package is actually a skill, Enclawed stops and points you at
`enclawed skills install <slug>` instead.

## What ClawHub is

- A public registry for Enclawed skills and plugins.
...
4. Users browse, download, and install skills in Enclawed.
...
If you want to add new capabilities to your Enclawed agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `enclawed skills install <skill-slug>`
3. Start a new Enclawed session so it picks up the new skill.
...
## How it fits into Enclawed

Native `enclawed skills install` installs into the active workspace `skills/`
directory. `enclawed plugins install clawhub:...` records a normal managed
...
Community or other non-official channels can still install, but Enclawed warns
...
current working directory. If an Enclawed workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Enclawed loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.enclawed/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Enclawed how to perform a
...
Interested in becoming a moderator? Ask in the Enclawed Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Enclawed workspace).
...
Code plugins must include the required Enclawed metadata in `package.json`:

```json
{
  "name": "@myorg/enclawed-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "enclawed": {
    "extensions": ["./index.ts"],
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/enclawed/skills` and `~/.enclawed/skills`). This is designed to find older skill installs without extra flags.
