...
read_when:
  - Running Enclawed Gateway in WSL2 while Chrome lives on Windows
...
- Enclawed Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/enclawed/enclawed/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Enclawed and Chrome are on the same machine
...
- Enclawed points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Enclawed is not the problem yet.
...
Fix that before touching Enclawed config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Enclawed at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Enclawed to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Enclawed to succeed
...
```bash
enclawed browser open https://example.com --browser-profile remote
enclawed browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `enclawed browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `enclawed browser stop --browser-profile remote`
...
3. Enclawed config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Enclawed config or Control UI auth
