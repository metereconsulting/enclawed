---
summary: "CLI reference for `enclawed cron` (schedule and run background jobs)"
read_when:
...
Tip: run `enclawed cron --help` for the full command surface.
...
Note: `enclawed cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `enclawed cron runs --id <job-id>` to follow the eventual outcome.

Note: `enclawed cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.enclawed/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`enclawed doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
enclawed cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
enclawed cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
enclawed cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
enclawed cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
enclawed cron run <job-id>
enclawed cron run <job-id> --due
enclawed cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
enclawed cron edit <job-id> --agent ops
enclawed cron edit <job-id> --clear-agent
enclawed cron edit <job-id> --session current
enclawed cron edit <job-id> --session "session:daily-brief"
```
...
```bash
enclawed cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
enclawed cron edit <job-id> --best-effort-deliver
enclawed cron edit <job-id> --no-best-effort-deliver
enclawed cron edit <job-id> --no-deliver
```
