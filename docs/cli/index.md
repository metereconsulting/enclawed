---
summary: "Enclawed CLI reference for `enclawed` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.enclawed-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.enclawed-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `enclawed update` (source installs only).
...
Enclawed uses a lobster palette for CLI output.
...
```
enclawed [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `enclawed voicecall`).

## Security

- `enclawed security audit` — audit config + local state for common security foot-guns.
- `enclawed security audit --deep` — best-effort live Gateway probe.
- `enclawed security audit --fix` — tighten safe defaults and state/config permissions.
...
- `enclawed plugins list` — discover plugins (use `--json` for machine output).
- `enclawed plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `enclawed plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`; use `--force` to overwrite an existing install target).
- `enclawed plugins marketplace list <marketplace>` — list marketplace entries before install.
- `enclawed plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `enclawed plugins doctor` — report plugin load errors.
...
- `enclawed memory status` — show index stats; use `--deep` for vector + embedding readiness checks or `--fix` to repair stale recall/promotion artifacts.
- `enclawed memory index` — reindex memory files.
- `enclawed memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
- `enclawed memory promote` — rank short-term recalls and optionally append top entries into `MEMORY.md`.
...
- `--install` writes an `Enclawed Completion` block into your shell profile and points it at the cached script under the Enclawed state directory.
...
- `--workspace <dir>`: agent workspace path (default `~/.enclawed/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `enclawed config` with no
...
- `config set --dry-run`: validate assignments without writing `enclawed.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `enclawed.json`, including propagated field `title` / `description` docs metadata across nested object, wildcard, array-item, and composition branches, plus best-effort live plugin/channel schema metadata.
...
- `enclawed --update` rewrites to `enclawed update`.

### `backup`

Create and verify local backup archives for Enclawed state.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs live per-account probe/audit checks when the gateway is reachable; if not, it falls back to config-only channel summaries. Use `enclawed health` or `enclawed status --deep` for broader gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `enclawed doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Enclawed promotes account-scoped values into the channel account map before writing the new account. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```bash
enclawed channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
enclawed channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
enclawed channels remove --channel discord --account work --delete
enclawed channels status --probe
enclawed status --deep
```

### `directory`

Look up self, peer, and group IDs for channels that expose a directory surface. See [`enclawed directory`](/cli/directory).
...
Tip: use `enclawed skills search`, `enclawed skills install`, and `enclawed skills update` for ClawHub-backed skills.
...
Generate a mobile pairing QR and setup code from the current Gateway config. See [`enclawed qr`](/cli/qr).
...
- After scanning, approve the request with `enclawed devices list` / `enclawed devices approve <requestId>`.

### `clawbot`

Legacy alias namespace. Currently supports `enclawed clawbot qr`, which maps to [`enclawed qr`](/cli/qr).
...
- `hooks enable <name>`
- `hooks disable <name>`
- `hooks install <path-or-spec>` (deprecated alias for `enclawed plugins install`)
- `hooks update [id]` (deprecated alias for `enclawed plugins update`)
...
- Plugin-managed hooks cannot be enabled or disabled through `enclawed hooks`; enable or disable the owning plugin instead.
...
- `setup` configures the Gmail watch plus the Enclawed-facing push path.
...
- Without `--apply`, this is a planning helper that prints the recommended Enclawed + Tailscale DNS config.
...
- `enclawed message send --target +15555550123 --message "Hi"`
- `enclawed message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Running `enclawed agents` with no subcommand is equivalent to `enclawed agents list`.
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Enclawed may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Manage saved MCP server definitions and expose Enclawed channels over MCP stdio.

#### `mcp serve`

Expose routed Enclawed channel conversations over MCP stdio.
...
- `--gateway`
- `--json`
- node RPC options from `enclawed nodes`
...
Enclawed can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `enclawed status --usage` (prints full provider breakdown)
...
- MiniMax note: raw `usage_percent` / `usagePercent` means remaining quota, so Enclawed inverts it before display; count-based fields still win when present. `model_remains` responses prefer the chat-model entry, derive the window label from timestamps when needed, and include the model name in the plan label.
- Usage auth comes from provider-specific hooks when available; otherwise Enclawed falls back to matching OAuth/API-key credentials from auth profiles, env, or config. If none resolve, usage is hidden.
...
Legacy docs shortcut. Flow commands live under `enclawed tasks flow`:
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Enclawed services are treated as first-class and aren't flagged as "extra".
...
enclawed logs --limit 200
enclawed logs --plain
enclawed logs --json
enclawed logs --no-color
```
...
Anthropic note: Anthropic staff told us Enclawed-style Claude CLI usage is
allowed again, so Enclawed treats Claude CLI reuse and `claude -p` usage as
...
Anthropic setup-token remains available as a supported token-auth path, but Enclawed now prefers Claude CLI reuse and `claude -p` when available.

### `models` (root)

`enclawed models` is an alias for `models status`.
...
- Anthropic `setup-token` / `paste-token` remain available as a supported Enclawed token path, but Enclawed now prefers Claude CLI reuse and `claude -p` when available.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `ENCLAWED_GATEWAY_TOKEN` / `ENCLAWED_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `ENCLAWED_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`enclawed browser`](/cli/browser) and the [Browser tool](/tools/browser).
...
Plugin-provided voice-call utilities. Only appears when the voice-call plugin is installed and enabled. See [`enclawed voicecall`](/cli/voicecall).
...
Search the live Enclawed docs index.
