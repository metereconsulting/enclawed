---
summary: "CLI reference for `enclawed qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
enclawed qr
enclawed qr --setup-code-only
enclawed qr --json
enclawed qr --remote
enclawed qr --url wss://gateway.example/ws
```
...
- With `--remote`, Enclawed requires either `gateway.remote.url` or
