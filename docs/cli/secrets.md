---
summary: "CLI reference for `enclawed secrets` (reload, audit, configure, apply)"
read_when:
...
Use `enclawed secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
enclawed secrets reload
enclawed secrets reload --json
enclawed secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Enclawed state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `enclawed.json` refs)
...
```bash
enclawed secrets audit
enclawed secrets audit --check
enclawed secrets audit --json
enclawed secrets audit --allow-exec
```
...
```bash
enclawed secrets configure
enclawed secrets configure --plan-out /tmp/enclawed-secrets-plan.json
enclawed secrets configure --apply --yes
enclawed secrets configure --providers-only
enclawed secrets configure --skip-provider-setup
enclawed secrets configure --agent ops
enclawed secrets configure --json
```
...
- `configure` targets secret-bearing fields in `enclawed.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Enclawed fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
enclawed secrets apply --from /tmp/enclawed-secrets-plan.json --json
```
...
- `enclawed.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.enclawed/.env` known secret keys whose values were migrated
