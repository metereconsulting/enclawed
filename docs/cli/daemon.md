---
summary: "CLI reference for `enclawed daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `enclawed daemon ...` in scripts
...
`enclawed daemon ...` maps to the same service control surface as `enclawed gateway ...` service commands.

## Usage

```bash
enclawed daemon status
enclawed daemon install
enclawed daemon start
enclawed daemon stop
enclawed daemon restart
enclawed daemon uninstall
```
...
Use [`enclawed gateway`](/cli/gateway) for current docs and examples.
