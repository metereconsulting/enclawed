...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Enclawed Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`enclawed acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Enclawed channel
conversations instead of hosting an ACP harness session, use
[`enclawed mcp serve`](/cli/mcp) instead.
...
- Enclawed acts as an ACP server
- an IDE or ACP client connects to Enclawed
- Enclawed forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Enclawed runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Enclawed: use `enclawed acp`
- Enclawed should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Enclawed gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
enclawed acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
enclawed acp --url wss://gateway-host:18789 --token-file ~/.enclawed/gateway.token

# Attach to an existing session key
enclawed acp --session agent:main:main

# Attach by label (must already exist)
enclawed acp --session-label "support inbox"

# Reset the session key before the first prompt
enclawed acp --session agent:main:main --reset-session
```
...
enclawed acp client --server-args --url wss://gateway-host:18789 --token-file ~/.enclawed/gateway.token

# Override the server command (default: enclawed)
enclawed acp client --server "node" --server-args enclawed.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Enclawed through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Enclawed Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `enclawed acp` over stdio.

Example config (persisted):

```bash
enclawed config set gateway.remote.url wss://gateway-host:18789
enclawed config set gateway.remote.token <token>
```
...
```bash
enclawed acp --session agent:main:main
enclawed acp --session agent:design:main
enclawed acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Enclawed plugin tools, enable the
...
Enclawed bot over ACP, use `acpx` with its built-in `enclawed` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx enclawed` at `enclawed acp`.
3. Target the Enclawed session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Enclawed ACP session
acpx enclawed exec "Summarize the active Enclawed session state."

# Persistent named session for follow-up turns
acpx enclawed sessions ensure --name codex-bridge
acpx enclawed -s codex-bridge --cwd /path/to/repo \
  "Ask my Enclawed work agent for recent context relevant to this repo."
```

If you want `acpx enclawed` to target a specific Gateway and session key every
time, override the `enclawed` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "enclawed": {
      "command": "env ENCLAWED_HIDE_BANNER=1 ENCLAWED_SUPPRESS_NOTES=1 enclawed acp --url ws://127.0.0.1:18789 --token-file ~/.enclawed/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Enclawed checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env ENCLAWED_HIDE_BANNER=1 ENCLAWED_SUPPRESS_NOTES=1 node enclawed.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Enclawed agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Enclawed ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`ENCLAWED_GATEWAY_TOKEN`, `ENCLAWED_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`ENCLAWED_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `ENCLAWED_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `enclawed acp client` sets `ENCLAWED_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `enclawed`).
