...
```json
{
  "agents": {
    "openclaw": {
      "command": "env ENCLAWED_HIDE_BANNER=1 ENCLAWED_SUPPRESS_NOTES=1 openclaw acp --url ws://127.0.0.1:18789 --token-file ~/.openclaw/gateway.token --session agent:main:main"
    }
  }
}
```
...
```bash
env ENCLAWED_HIDE_BANNER=1 ENCLAWED_SUPPRESS_NOTES=1 node openclaw.mjs acp ...
```
...
- `openclaw acp client` sets `ENCLAWED_SHELL=acp-client` on the spawned bridge process.
