---
summary: "CLI reference for `enclawed wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `enclawed wiki`
title: "wiki"
---
...
Use `enclawed wiki` when you want a compiled knowledge vault with:
...
- `.enclawed-wiki/cache/agent-digest.json`
- `.enclawed-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `enclawed memory search` when the
...
```bash
enclawed wiki get entity.alpha
enclawed wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`enclawed wiki` behavior is shaped by:
