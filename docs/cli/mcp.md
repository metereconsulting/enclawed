---
summary: "Expose Enclawed channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Enclawed-backed channels
  - Running `enclawed mcp serve`
  - Managing Enclawed-saved MCP server definitions
title: "mcp"
---

# mcp

`enclawed mcp` has two jobs:

- run Enclawed as an MCP server with `enclawed mcp serve`
- manage Enclawed-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Enclawed acting as an MCP server
- `list` / `show` / `set` / `unset` is Enclawed acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`enclawed acp`](/cli/acp) when Enclawed should host a coding harness
session itself and route that runtime through ACP.

## Enclawed as an MCP server

This is the `enclawed mcp serve` path.

## When to use `serve`

Use `enclawed mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Enclawed-backed channel conversations
- you already have a local or remote Enclawed Gateway with routed sessions
- you want one MCP server that works across Enclawed's channel backends instead
  of running separate per-channel bridges

Use [`enclawed acp`](/cli/acp) instead when Enclawed should host the coding
runtime itself and keep the agent session inside Enclawed.

## How it works

`enclawed mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Enclawed Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `enclawed mcp serve`
...
conversations. A conversation appears when Enclawed already has session state
...
# Remote Gateway
enclawed mcp serve --url wss://gateway-host:18789 --token-file ~/.enclawed/gateway.token

# Remote Gateway with password auth
enclawed mcp serve --url wss://gateway-host:18789 --password-file ~/.enclawed/gateway.password

# Enable verbose bridge logs
enclawed mcp serve --verbose

# Disable Claude-specific push notifications
enclawed mcp serve --claude-channel-mode off
```
...
Enclawed equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Enclawed channel configuration
...
Enclawed ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `enclawed mcp serve`
...
## Enclawed as an MCP client registry

This is the `enclawed mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Enclawed over MCP. They manage Enclawed-owned MCP
server definitions under `mcp.servers` in Enclawed config.

Those saved definitions are for runtimes that Enclawed launches or configures
later, such as embedded Pi and other runtime adapters. Enclawed stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Enclawed config
...
Enclawed also stores a lightweight MCP server registry in config for surfaces
that want Enclawed-managed MCP definitions.

Commands:

- `enclawed mcp list`
- `enclawed mcp show [name]`
- `enclawed mcp set <name> <json>`
- `enclawed mcp unset <name>`
...
```bash
enclawed mcp list
enclawed mcp show context7 --json
enclawed mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
enclawed mcp set docs '{"url":"https://mcp.example.com"}'
enclawed mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Enclawed uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
