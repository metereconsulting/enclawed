---
summary: "CLI reference for `enclawed approvals` and `enclawed exec-policy`"
read_when:
...
## `enclawed exec-policy`

`enclawed exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.enclawed/exec-approvals.json`

Examples:

```bash
enclawed exec-policy show
enclawed exec-policy show --json

enclawed exec-policy preset yolo
enclawed exec-policy preset cautious --json

enclawed exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `enclawed exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `enclawed approvals set --gateway`
or `enclawed approvals set --node <id|name|ip>`.

## Common commands

```bash
enclawed approvals get
enclawed approvals get --node <id|name|ip>
enclawed approvals get --gateway
```

`enclawed approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
enclawed approvals set --file ./exec-approvals.json
enclawed approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
enclawed approvals set --node <id|name|ip> --file ./exec-approvals.json
enclawed approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Enclawed policy aligned, also set:
...
```bash
enclawed approvals allowlist add "~/Projects/**/bin/rg"
enclawed approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
enclawed approvals allowlist add --agent "*" "/usr/bin/uname"

enclawed approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `enclawed nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.enclawed/exec-approvals.json`.
