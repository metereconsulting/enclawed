---
summary: "CLI reference for `enclawed plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
enclawed plugins list --enabled
enclawed plugins list --verbose
enclawed plugins list --json
enclawed plugins install <path-or-spec>
enclawed plugins inspect <id>
enclawed plugins inspect <id> --json
enclawed plugins inspect --all
enclawed plugins info <id>
enclawed plugins enable <id>
enclawed plugins disable <id>
enclawed plugins uninstall <id>
enclawed plugins doctor
enclawed plugins update <id>
enclawed plugins update --all
enclawed plugins marketplace list <marketplace>
enclawed plugins marketplace list <marketplace> --json
```

Bundled plugins ship with Enclawed. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native Enclawed plugins must ship `enclawed.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: enclawed` or `Format: bundle`. Verbose list/info
...
```bash
enclawed plugins install <package>                      # ClawHub first, then npm
enclawed plugins install clawhub:<package>              # ClawHub only
enclawed plugins install <package> --force              # overwrite existing install
enclawed plugins install <package> --pin                # pin version
enclawed plugins install <package> --dangerously-force-unsafe-install
enclawed plugins install <path>                         # local path
enclawed plugins install <plugin>@<marketplace>         # marketplace
enclawed plugins install <plugin> --marketplace <name>  # marketplace (explicit)
...
run `enclawed doctor --fix` first. The only documented exception is a narrow
...
override, while `enclawed skills install` remains a separate ClawHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`enclawed.hooks` in `package.json`. Use `enclawed hooks` for filtered hook
...
those to a prerelease, Enclawed stops and asks you to opt in explicitly with a
prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as
`@1.2.3-beta.4`.

If a bare install spec matches a bundled plugin id (for example `diffs`), Enclawed
...
```bash
enclawed plugins install clawhub:enclawed-codex-app-server
enclawed plugins install clawhub:enclawed-codex-app-server@1.2.3
```

Enclawed now also prefers ClawHub for bare npm-safe plugin specs. It only falls
back to npm if ClawHub does not have that package or version:

```bash
enclawed plugins install enclawed-codex-app-server
```

Enclawed downloads the package archive from ClawHub, checks the advertised
...
```bash
enclawed plugins marketplace list <marketplace-name>
enclawed plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
enclawed plugins install <plugin-name> --marketplace <marketplace-name>
enclawed plugins install <plugin-name> --marketplace <owner/repo>
enclawed plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
enclawed plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Enclawed accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, Enclawed auto-detects:

- native Enclawed plugins (`enclawed.plugin.json`)
...
```bash
enclawed plugins install -l ./my-plugin
```
...
```bash
enclawed plugins uninstall <id>
enclawed plugins uninstall <id> --dry-run
...
```bash
enclawed plugins update <id-or-npm-spec>
enclawed plugins update --all
enclawed plugins update <id-or-npm-spec> --dry-run
enclawed plugins update @enclawed/voice-call@beta
enclawed plugins update enclawed-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Enclawed reuses the recorded install spec for that
...
or exact version. Enclawed resolves that package name back to the tracked plugin
record, updates that installed plugin, and records the new npm spec for future
id-based updates.

When a stored integrity hash exists and the fetched artifact hash changes,
Enclawed prints a warning and asks for confirmation before proceeding. Use
