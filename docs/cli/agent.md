---
summary: "CLI reference for `enclawed agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
enclawed agent --to +15555550123 --message "status update" --deliver
...
enclawed agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
enclawed agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Enclawed persists markers from the active source config snapshot, not from resolved runtime secret values.
