---
summary: "CLI reference for `enclawed message` (send + channel actions)"
read_when:
...
```
enclawed message <subcommand> [flags]
```
...
- On cache miss, Enclawed will attempt a live directory lookup when the provider supports it.
...
- `enclawed message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Enclawed does not force a `default` account SecretRef scope
...
```
enclawed message send --channel discord \
...
```bash
enclawed message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
enclawed message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
enclawed message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
enclawed message poll --channel msteams \
...
```
enclawed message react --channel slack \
...
```
enclawed message react --channel signal \
...
```
enclawed message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
enclawed message send --channel telegram --target @mychat \
