---
summary: "CLI reference for `enclawed config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `enclawed.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `enclawed configure`).

Root options:

- `--section <section>`: repeatable guided-setup section filter when you run `enclawed config` without a subcommand
...
```bash
enclawed config file
enclawed config --section model
enclawed config --section gateway --section daemon
enclawed config schema
enclawed config get browser.executablePath
...
enclawed config set secrets.providers.vaultfile --provider-source file --provider-path /etc/enclawed/secrets.json --provider-mode json
...
enclawed config validate
enclawed config validate --json
```

### `config schema`

Print the generated JSON schema for `enclawed.json` to stdout as JSON.
...
```bash
enclawed config schema > enclawed.schema.json
```
...
enclawed config get agents.list[0].id
```
...
enclawed config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
enclawed config set agents.defaults.heartbeat.every "0m"
enclawed config set gateway.port 19001 --strict-json
enclawed config set channels.whatsapp.groups '["*"]' --strict-json
```
...
`enclawed config set` supports four assignment styles:

1. Value mode: `enclawed config set <path> <value>`
2. SecretRef builder mode:

```bash
enclawed config set channels.discord.token \
...
```bash
enclawed config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/enclawed-vault \
  --provider-arg read \
...
```bash
enclawed config set --batch-json '[
  {
...
```bash
enclawed config set --batch-file ./config-set.batch.json --dry-run
```
...
enclawed config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/enclawed/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `enclawed.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.enclawed/enclawed.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
...
- `config file`: Print the active config file path (resolved from `ENCLAWED_CONFIG_PATH` or default location).
