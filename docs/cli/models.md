---
summary: "CLI reference for `enclawed models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
enclawed models set <model-or-alias>
enclawed models scan
```

`enclawed models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Enclawed falls back to matching OAuth/API-key
...
the command uses `ENCLAWED_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, Enclawed resolves the input as an alias first, then
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `ENCLAWED_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Enclawed could not resolve a probeable
...
```bash
enclawed models auth add
enclawed models auth login --provider <id>
enclawed models auth setup-token --provider <id>
enclawed models auth paste-token
```
...
`enclawed plugins list` to see which providers are installed.

Examples:

```bash
enclawed models auth login --provider openai-codex --set-default
```
