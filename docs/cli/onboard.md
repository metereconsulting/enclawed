---
summary: "CLI reference for `enclawed onboard` (interactive onboarding)"
read_when:
...
```bash
enclawed onboard
enclawed onboard --flow quickstart
enclawed onboard --flow manual
enclawed onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`ENCLAWED_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `enclawed gateway run`.
...
- Fastest first chat: `enclawed dashboard` (Control UI, no channel setup).
