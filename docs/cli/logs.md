---
summary: "CLI reference for `enclawed logs` (tail gateway logs via RPC)"
read_when:
...
`enclawed logs` also accepts the standard Gateway client flags:
...
enclawed logs --follow --interval 2000
enclawed logs --limit 500 --max-bytes 500000
enclawed logs --json
enclawed logs --plain
enclawed logs --no-color
enclawed logs --limit 500
enclawed logs --local-time
enclawed logs --follow --local-time
enclawed logs --url ws://127.0.0.1:18789 --token "$ENCLAWED_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `enclawed logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
