---
summary: "CLI reference for `enclawed configure` (interactive configuration prompts)"
read_when:
...
Tip: `enclawed config` without a subcommand opens the same wizard. Use
`enclawed config get|set|unset` for non-interactive edits.

For web search, `enclawed configure --section web` lets you choose a provider
...
enclawed configure --section model --section channels
enclawed configure --section gateway --section daemon
```
