---
summary: "Mattermost bot setup and Enclawed config"
read_when:
...
Mattermost ships as a bundled plugin in current Enclawed releases, so normal
...
```bash
enclawed plugins install @enclawed/mattermost
```

Local checkout (when running from a git repo):

```bash
enclawed plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Enclawed and start the gateway.
...
Native slash commands are opt-in. When enabled, Enclawed registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Enclawed derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when Enclawed registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Enclawed.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Enclawed.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Enclawed, not `404`.
...
- Approve via:
  - `enclawed pairing list mattermost`
  - `enclawed pairing approve mattermost <CODE>`
...
Use these target formats with `enclawed message send` or cron/webhooks:
...
Enclawed resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Enclawed sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Enclawed sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Enclawed derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Enclawed run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="enclawed-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"enclawed-mattermost-interactions",
...
`enclawed message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: Enclawed
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as Enclawed. Set an
