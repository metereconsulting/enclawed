...
Zalo ships as a bundled plugin in current Enclawed releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `enclawed plugins install @enclawed/zalo`
- Or from a source checkout: `enclawed plugins install ./path/to/local/zalo-plugin`
...
This page reflects current Enclawed behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `enclawed pairing list zalo`
  - `enclawed pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in Enclawed.
...
- Example: `enclawed message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `enclawed channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `enclawed logs --follow`
