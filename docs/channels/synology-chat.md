---
summary: "Synology Chat webhook setup and Enclawed config"
read_when:
  - Setting up Synology Chat with Enclawed
...
Synology Chat ships as a bundled plugin in current Enclawed releases, so normal
...
```bash
enclawed plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `enclawed onboard` now shows Synology Chat in the same channel setup list as `enclawed channels add`.
   - Non-interactive setup: `enclawed channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Enclawed gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Enclawed.
   - Guided: `enclawed onboard`
   - Direct: `enclawed channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Enclawed accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `enclawed pairing list synology-chat`
  - `enclawed pairing approve synology-chat <CODE>`
...
```bash
enclawed message send --channel synology-chat --target 123456 --text "Hello from Enclawed"
enclawed message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Enclawed now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Enclawed
- `Rate limit exceeded`:
