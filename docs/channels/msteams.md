...
Microsoft Teams ships as a bundled plugin in current Enclawed releases, so no
...
```bash
enclawed plugins install @enclawed/msteams
```

Local checkout (when running from a git repo):

```bash
enclawed plugins install ./path/to/local/msteams-plugin
```
...
3. Configure Enclawed with those credentials.
...
- Talk to Enclawed via Teams DMs, group chats, or channels.
...
- On startup, Enclawed resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.enclawed/enclawed.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Enclawed, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `enclawed-msteams` (must be unique) |
...
For production deployments, Enclawed supports **federated authentication** as a more secure alternative to client secrets. Two methods are available:
...
3. At runtime, Enclawed uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint (`169.254.169.254`).
...
**Default behavior:** When `authType` is not set, Enclawed defaults to client secret authentication. Existing configurations continue to work without changes.
...
1. **Ensure the Microsoft Teams plugin is available**
   ...
   - Older/custom installs can add it manually:
     - From npm: `enclawed plugins install @enclawed/msteams`
     - From a local checkout: `enclawed plugins install ./path/to/local/msteams-plugin`
...
Enclawed exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Enclawed in Teams", full: "Enclawed in Teams" },
...
Enclawed handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Enclawed only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/EnclawedShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Enclawed sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `enclawed message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.enclawed/msteams-polls.json`.
...
```bash
enclawed message send --channel msteams \
...
enclawed message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
enclawed message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
enclawed message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
enclawed message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
