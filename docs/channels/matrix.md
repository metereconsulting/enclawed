...
Matrix is a bundled channel plugin for Enclawed.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Bundled plugin

Matrix ships as a bundled plugin in current Enclawed releases, so normal
...
```bash
enclawed plugins install @enclawed/matrix
```

Install from a local checkout:

```bash
enclawed plugins install ./path/to/local/matrix-plugin
```
...
1. Ensure the Matrix plugin is available.
   - Current packaged Enclawed releases already bundle it.
...
```bash
enclawed channels add
enclawed configure --section channels
```
...
- To resolve room names before saving, use `enclawed channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "Enclawed Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.enclawed/credentials/matrix/`.
The default account uses `credentials.json`; named accounts use `credentials-<account>.json`.
When cached credentials exist there, Enclawed treats Matrix as configured for setup, doctor, and channel-status discovery even if current auth is not set directly in config.
...
`autoJoin` applies to all Matrix invites, including DM-style invites. Enclawed cannot reliably
...
Set `channels.matrix.streaming` to `"partial"` when you want Enclawed to send a single live preview
...
- `streaming: "off"` is the default. Enclawed waits for the final reply and sends it once.
...
- If the preview no longer fits in one Matrix event, Enclawed stops preview streaming and falls back to normal final delivery.
- Media replies still send attachments normally. If a stale preview can no longer be reused safely, Enclawed redacts it before sending the final media reply.
...
- bot user = the Enclawed Matrix account that sends the reply
...
1. Configure Enclawed to use quiet previews:
...
Enclawed marks finalized text-only preview edits with:

```json
{
  "com.enclawed.finalized_preview": true
}
```
...
```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/enclawed-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.enclawed\\.finalized_preview",
        "value": true
      },
...
- `enclawed-finalized-preview-botname`: a rule ID unique to this bot for this receiving user
- `@bot:example.org`: your Enclawed Matrix bot MXID, not the receiving user's MXID
...
- If one receiving user should notify for multiple Enclawed Matrix bot accounts, create one rule per bot with a unique rule ID for each sender match.
- A simple pattern is `enclawed-finalized-preview-<botname>`, such as `enclawed-finalized-preview-ops` or `enclawed-finalized-preview-support`.

The rule is evaluated against the event sender:

- authenticate with the receiving user's token
- match `sender` against the Enclawed bot MXID
...
- This only affects text-only preview edits that Enclawed can safely finalize in place. Media fallbacks and stale-preview fallbacks still use normal Matrix delivery.
...
- No special `homeserver.yaml` change is required for finalized Enclawed preview notifications.
...
By default, Matrix messages from other configured Enclawed Matrix accounts are ignored.
...
- Enclawed still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; Enclawed treats "bot-authored" as "sent by another configured Matrix account on this Enclawed gateway".
...
```bash
enclawed matrix verify status --include-recovery-key --json
```
...
```bash
enclawed matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
enclawed matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
enclawed matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
enclawed matrix verify backup restore
```

Verbose restore diagnostics:

```bash
enclawed matrix verify backup restore --verbose
```
...
```bash
enclawed matrix verify status --account assistant
enclawed matrix verify backup restore --account assistant
enclawed matrix devices list --account assistant
```
...
Enclawed treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `enclawed matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for Enclawed to treat the device as fully verified.

### What bootstrap does

`enclawed matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, Enclawed tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
backup baseline for future messages, use `enclawed matrix verify backup reset --yes`.
Do this only when you accept that unrecoverable old encrypted history will stay
unavailable and that Enclawed may recreate secret storage if the current backup
...
If startup still finds broken bootstrap state, Enclawed can attempt a guarded repair path even when `channels.matrix.password` is not configured.
If the homeserver requires password-based UIA for that repair, Enclawed logs a warning and keeps startup non-fatal instead of aborting the bot.
If the current device is already owner-signed, Enclawed preserves that identity instead of resetting it automatically.
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by Enclawed.
For self-verification flows, Enclawed also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, Enclawed auto-accepts the request and then waits for the SAS flow to proceed normally.
...
Enclawed does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old Enclawed-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
enclawed matrix devices list
```

Remove stale Enclawed-managed devices with:

```bash
enclawed matrix devices prune-stale
```
...
`~/.enclawed/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, Enclawed reuses the best existing
...
```bash
enclawed matrix profile set --name "Enclawed Assistant"
enclawed matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

Add `--account <id>` when you want to target a named Matrix account explicitly.

Matrix accepts `mxc://` avatar URLs directly. When you pass an `http://` or `https://` avatar URL, Enclawed uploads it to Matrix first and stores the resolved `mxc://` URL back into `channels.matrix.avatarUrl` (or the selected account override).
...
- Same-session DM user-target reuse only kicks in when the current session metadata proves the same DM peer on the same Matrix account; otherwise Enclawed falls back to normal user-scoped routing.
- When Enclawed sees a Matrix DM room collide with another DM room on the same shared Matrix DM session, it posts a one-time `m.notice` in that room with the `/focus` escape hatch when thread bindings are enabled and the `dm.sessionScope` hint.
...
- `threadBindings.spawnAcpSessions` is only required for `/acp spawn --thread auto|here`, where Enclawed needs to create or bind a child Matrix thread.
...
Ack reactions use the standard Enclawed resolution order:
...
- Matrix room history is pending-only: Enclawed buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
enclawed pairing list matrix
enclawed pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, Enclawed reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state gets out of sync, Enclawed can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
enclawed matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
enclawed matrix direct repair --user-id @alice:example.org
```
...
Set `defaultAccount` when you want Enclawed to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If multiple Matrix accounts are configured and one account id is `default`, Enclawed uses that account implicitly even when `defaultAccount` is unset.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `enclawed matrix verify ...` and `enclawed matrix devices ...` when you want to override that implicit selection for one command.
...
By default, Enclawed blocks private/internal Matrix homeservers for SSRF protection unless you
...
Enclawed uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere Enclawed asks you for a room or user target:
...
- `allowBots`: allow messages from other configured Enclawed Matrix accounts (`true` or `"mentions"`).
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; Enclawed does not trust alias state claimed by the invited room.
- `dm`: DM policy block (`enabled`, `policy`, `allowFrom`, `sessionScope`, `threadReplies`).
- `dm.policy`: controls DM access after Enclawed has joined the room and classified it as a DM. It does not change whether an invite is auto-joined.
