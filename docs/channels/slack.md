...
```json
{
  "display_information": {
    "name": "Enclawed",
    "description": "Slack connector for Enclawed"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/enclawed",
        "description": "Send a message to Enclawed",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `enclawed pairing approve slack <code>`.
...
- If streaming fails mid-reply, Enclawed falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Enclawed is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are Enclawed-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Enclawed falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `enclawed channels status --probe --json` shows `botTokenStatus` or
