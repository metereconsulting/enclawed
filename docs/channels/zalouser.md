...
read_when:
  - Setting up Zalo Personal for Enclawed
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Enclawed.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current Enclawed releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `enclawed plugins install @enclawed/zalouser`
- Or from a source checkout: `enclawed plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `enclawed channels login --channel zalouser`
...
```bash
enclawed directory self --channel zalouser
enclawed directory peers list --channel zalouser --query "name"
enclawed directory groups list --channel zalouser --query "work"
```
...
- `enclawed pairing list zalouser`
- `enclawed pairing approve zalouser <code>`
...
- On startup, Enclawed resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Enclawed stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Enclawed state. Example:
...
- Enclawed sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Enclawed sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `enclawed channels logout --channel zalouser && enclawed channels login --channel zalouser`
...
- The channel now runs fully in Enclawed without external CLI binaries.
