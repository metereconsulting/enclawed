...
> **Requires Enclawed 2026.4.10 or above.** Run `enclawed --version` to check. Upgrade with `enclawed update`.

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  enclawed channels login --channel feishu
  ```
...
```bash
enclawed pairing list feishu
enclawed pairing approve feishu <CODE>
```
...
4. Check logs: `enclawed logs --follow`
...
5. Ensure the gateway is running: `enclawed gateway status`
6. Check logs: `enclawed logs --follow`

### App Secret leaked

1. Reset the App Secret in Feishu Open Platform / Lark Developer
2. Update the value in your config
3. Restart the gateway: `enclawed gateway restart`
