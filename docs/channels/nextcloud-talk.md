...
Nextcloud Talk ships as a bundled plugin in current Enclawed releases, so
...
```bash
enclawed plugins install @enclawed/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
enclawed plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Enclawed" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `enclawed pairing list nextcloud-talk`
  - `enclawed pairing approve nextcloud-talk <CODE>`
