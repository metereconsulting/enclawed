...
read_when:
  ...
  - Reviewing Enclawed security posture
title: "Pairing"
---

# Pairing

“Pairing” is Enclawed’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `enclawed-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.enclawed/credentials/`:
...
3. On your phone, open the Enclawed iOS app → Settings → Gateway.
...
- The legacy `node.pair.*` API (CLI: `enclawed nodes pending|approve|reject|rename`) is a
