...
read_when:
  - Setting up Twitch chat integration for Enclawed
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. Enclawed connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current Enclawed releases, so normal
...
```bash
enclawed plugins install @enclawed/twitch
```

Local checkout (when running from a git repo):

```bash
enclawed plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `ENCLAWED_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "enclawed", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ENCLAWED_TWITCH_ACCESS_TOKEN env var)
...
```bash
ENCLAWED_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
