...
Tlon is a decentralized messenger built on Urbit. Enclawed connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Enclawed releases, so normal packaged
...
```bash
enclawed plugins install @enclawed/tlon
```

Local checkout (when running from a git repo):

```bash
enclawed plugins install ./path/to/local/tlon-plugin
```
...
By default, Enclawed blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `enclawed message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Enclawed replies in-thread.
