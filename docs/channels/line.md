...
read_when:
  - You want to connect Enclawed to LINE
...
LINE connects to Enclawed via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current Enclawed releases, so normal
...
```bash
enclawed plugins install @enclawed/line
```

Local checkout (when running from a git repo):

```bash
enclawed plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Enclawed applies strict pre-auth body limits and timeout before verification.
- Enclawed processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
enclawed pairing list line
enclawed pairing approve line <CODE>
```
