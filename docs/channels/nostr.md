...
read_when:
  - You want Enclawed to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Enclawed to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Enclawed releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`enclawed onboard`) and `enclawed channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
enclawed plugins install @enclawed/nostr
```

Use a local checkout (dev workflows):

```bash
enclawed plugins install --link <path-to-local-nostr-plugin>
```
...
enclawed channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "enclawed",
        displayName: "Enclawed",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "enclawed@example.com",
        lud16: "enclawed@example.com",
      },
    },
  },
}
