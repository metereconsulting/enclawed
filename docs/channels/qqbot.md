...
read_when:
  - You want to connect Enclawed to QQ
...
QQ Bot connects to Enclawed via the official QQ Bot API (WebSocket gateway). The
...
Current Enclawed releases bundle QQ Bot, so normal packaged builds do not need
a separate `enclawed plugins install` step.
...
```bash
enclawed channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `enclawed channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single Enclawed instance:
...
```bash
enclawed channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
| `/bot-version` | Show the Enclawed framework version  |
