...
```bash
enclawed nodes describe --node <idOrNameOrIp>
enclawed nodes canvas snapshot --node <idOrNameOrIp>
enclawed logs --follow
```
...
enclawed nodes status
enclawed approvals get --node <idOrNameOrIp>
enclawed approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`enclawed approvals get --node ...`), not in the gateway pairing record.
