...
Hooks are small scripts that run when something happens inside the Gateway. They are automatically discovered from directories and can be inspected with `enclawed hooks`.

There are two kinds of hooks in Enclawed:

- **Internal hooks** (this page): run inside the Gateway when agent events fire, like `/new`, `/reset`, `/stop`, or lifecycle events.
- **Webhooks**: external HTTP endpoints that let other systems trigger work in Enclawed. See [Webhooks](/automation/cron-jobs#webhooks).

Hooks can also be bundled inside plugins. `enclawed hooks list` shows both standalone hooks and plugin-managed hooks.
...
metadata:
  { "enclawed": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
**Metadata fields** (`metadata.enclawed`):
...
1. **Bundled hooks**: shipped with Enclawed
2. **Plugin hooks**: hooks bundled inside installed plugins
3. **Managed hooks**: `~/.enclawed/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
Hook packs are npm packages that export hooks via `enclawed.hooks` in `package.json`. Install with:
...
| command-logger        | `command`                      | Logs all commands to `~/.enclawed/logs/commands.log`  |
| boot-md               | `gateway:startup`              | Runs `BOOT.md` when the gateway starts                |

Enable any bundled hook:

```bash
enclawed hooks enable <hook-name>
```
...
Logs every slash command to `~/.enclawed/logs/commands.log`.
...
# Enable/disable
enclawed hooks enable <hook-name>
enclawed hooks disable <hook-name>
```
...
1. Verify the hook is enabled: `enclawed hooks list`
