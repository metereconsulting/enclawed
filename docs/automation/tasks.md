...
- `enclawed tasks list` shows all tasks; `enclawed tasks audit` surfaces issues.
...
enclawed tasks list --runtime acp
enclawed tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
enclawed tasks show <lookup>

# Cancel a running task (kills the child session)
enclawed tasks cancel <lookup>

# Change notification policy for a task
enclawed tasks notify <lookup> state_changes

# Run a health audit
enclawed tasks audit

# Preview or apply maintenance
enclawed tasks maintenance
enclawed tasks maintenance --apply

# Inspect TaskFlow state
enclawed tasks flow list
...
| CLI operations         | `cli`        | `enclawed agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `enclawed tasks cancel`                        |
...
When a task reaches a terminal state, Enclawed notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, Enclawed also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
```bash
enclawed tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
enclawed tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
enclawed tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `enclawed status` when issues are detected.
...
```bash
enclawed tasks maintenance [--json]
enclawed tasks maintenance --apply [--json]
```
...
```bash
enclawed tasks flow list [--status <status>] [--json]
enclawed tasks flow show <lookup> [--json]
enclawed tasks flow cancel <lookup>
```
...
For the full operator ledger, use the CLI: `enclawed tasks list`.

## Status integration (task pressure)

`enclawed status` includes an at-a-glance task summary:
...
```
$ENCLAWED_STATE_DIR/tasks/runs.sqlite
```
...
[Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `enclawed tasks` to inspect individual task records and `enclawed tasks flow` to inspect the orchestrating flow.

See [Task Flow](/automation/taskflow) for details.

### Tasks and cron

A cron job **definition** lives in `~/.enclawed/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
