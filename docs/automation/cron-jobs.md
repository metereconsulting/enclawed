...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into Enclawed
...
# See run history
enclawed cron runs --id <job-id>
```

## How cron works

- Cron runs **inside the Gateway** process (not inside the model).
- Jobs persist at `~/.enclawed/cron/jobs.json` so restarts do not lose schedules.
...
together`, and similar hints) and no descendant subagent run is still
  responsible for the final answer, Enclawed re-prompts once for the actual
...
This fires ~5–6 times per month instead of 0–1 times per month. Enclawed uses Croner's default OR behavior here. To require both conditions, use Croner's `+` day-of-week modifier (`0 9 15 * +1`) or schedule on one field and guard the other in your job's prompt or command.
...
running, Enclawed suppresses that partial parent update instead of announcing it.
...
Wire Gmail inbox triggers to Enclawed via Google PubSub.

**Prerequisites**: `gcloud` CLI, `gog` (gogcli), Enclawed hooks enabled, Tailscale for the public HTTPS endpoint.

### Wizard setup (recommended)

```bash
enclawed webhooks gmail setup --account enclawed@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `ENCLAWED_SKIP_GMAIL_WATCHER=1` to opt out.
...
# Edit a job
enclawed cron edit <jobId> --message "Updated prompt" --model "opus"

# Force run a job now
enclawed cron run <jobId>

# Run only if due
enclawed cron run <jobId> --due

# View run history
enclawed cron runs --id <jobId> --limit 50

# Delete a job
enclawed cron remove <jobId>

# Agent selection (multi-agent setups)
enclawed cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops
enclawed cron edit <jobId> --clear-agent
```

Model override note:

- `enclawed cron add|edit --model ...` changes the job's selected model.
...
Disable cron: `cron.enabled: false` or `ENCLAWED_SKIP_CRON=1`.
...
- Check `cron.enabled` and `ENCLAWED_SKIP_CRON` env var.
- Confirm the Gateway is running continuously.
- For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
- `reason: not-due` in run output means manual run was checked with `enclawed cron run <jobId> --due` and the job was not due yet.
...
- If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`),
  Enclawed suppresses direct outbound delivery and also suppresses the fallback
