...
If you believe you've found a security issue in Enclawed, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [enclawed/enclawed](https://github.com/enclawed/enclawed)
- **macOS desktop app** — [enclawed/enclawed](https://github.com/enclawed/enclawed) (apps/macos)
- **iOS app** — [enclawed/enclawed](https://github.com/enclawed/enclawed) (apps/ios)
- **Android app** — [enclawed/enclawed](https://github.com/enclawed/enclawed) (apps/android)
- **ClawHub** — [enclawed/clawhub](https://github.com/enclawed/clawhub)
- **Trust and threat model** — [enclawed/trust](https://github.com/enclawed/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@enclawed.ai](mailto:security@enclawed.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.enclawed.ai).
...
- Tested version details (Enclawed version and/or commit SHA).
...
- Demonstrated impact tied to Enclawed's documented trust boundaries.
- For exposed-secret reports: proof the credential is Enclawed-owned (or grants access to Enclawed-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at Enclawed. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

Enclawed is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
Enclawed does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-enclawed-scopes` headers are ignored for that shared-secret path
...
- Enclawed can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Enclawed, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Enclawed's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Enclawed's trusted computing base for a gateway.
...
- Using Enclawed in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped Enclawed production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.enclawed`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Enclawed-owned and not granting access to Enclawed-operated infrastructure/services) without demonstrated Enclawed impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

Enclawed security guidance assumes:

- The host where Enclawed runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.enclawed` state/config (including `enclawed.json`) is effectively a trusted operator.
...
Enclawed's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Enclawed separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Enclawed can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Enclawed process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Enclawed uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/enclawed` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/enclawed` (or `enclawed-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Enclawed-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Enclawed temp helpers (`resolvePreferredEnclawedTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-enclawed-dir.ts`
...
For threat model + hardening guidance (including `enclawed security audit --deep` and `--fix`), see:

- `https://docs.enclawed.ai/gateway/security`
...
Enclawed's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `enclawed gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Enclawed keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `enclawed security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__enclawed__/canvas/`, `/__enclawed__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__enclawed__/canvas/`, `/__enclawed__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Enclawed requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running Enclawed in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v enclawed-data:/app/data \
  enclawed/enclawed:latest
```
