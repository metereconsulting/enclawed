...
describe("resolveInitialTuiAgentId", () => {
  const cfg: EnclawedConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/enclawed" },
        { id: "ops", workspace: "/tmp/enclawed/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/enclawed/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run enclawed devices list");
    expect(state.pairingHint).toContain("enclawed devices list");
  });
