...
function createTelegramCfg(botToken: string, enabled?: boolean): EnclawedConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function createMSTeamsCatalogEntry(): ChannelPluginCatalogEntry {
  return {
    id: "external-chat",
    pluginId: "@enclawed/external-chat-plugin",
    meta: {
      id: "external-chat",
      label: "External Chat",
    ...
    install: {
      npmSpec: "@enclawed/external-chat",
    },
  };
}
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveConfigured: ({ cfg }: { cfg: EnclawedConfig }) =>
            ...
            credentials: [
              {
                ...
                inspect: ({ cfg }: { cfg: EnclawedConfig }) => ({
        ...
        plugin: {
          ...
          setupWizard: {
            ...
            status: {
              ...
              resolveSelectionHint: async ({ cfg }: { cfg: EnclawedConfig }) =>
            ...
            textInputs: [
              {
                ...
                required: true,
                applySet: ({ cfg, value }: { cfg: EnclawedConfig; value: string }) =>
                  ({
                    ...cfg,
                    channels: {
                      ...cfg.channels,
...
function patchTelegramAdapter(overrides: ChannelSetupWizardAdapterPatch) {
  ...
  const patch = {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: EnclawedConfig }) => ({
        channel: "telegram",
...
async function expectQuickstartPickerSkipsWithoutRuntime() {
  ...
  await expect(
    runSetupChannels({} as EnclawedConfig, prompter, {
      quickstartDefaults: true,
    }),
  ).resolves.toEqual({} as EnclawedConfig);
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as EnclawedConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("../commands/channel-setup/plugin-install.js", async () => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: EnclawedConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as EnclawedConfig, prompter, {
      quickstartDefaults: true,
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@enclawed/external-chat-plugin": { enabled: true },
          },
        },
      } as EnclawedConfig,
      prompter,
  ...
  it("treats installed external plugin channels as installed without reinstall prompts", async () => {
    ...
    manifestRegistryMocks.loadPluginManifestRegistry.mockReturnValue({
      plugins: [
        {
          id: "@enclawed/external-chat-plugin",
        ...
        if (channel === "external-chat") {
          registry.channels.push({
            ...
            plugin: {
              ...
              config: {
                listAccountIds: (cfg: EnclawedConfig) =>
                  Object.keys(
                    (
                      cfg.channels?.["external-chat"] as
                        | { accounts?: Record<string, unknown> }
                ...
                resolveAccount: (cfg: EnclawedConfig, accountId: string) =>
                  (
                    cfg.channels?.["external-chat"] as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: EnclawedConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: EnclawedConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
