...
let loadEnclawedPluginsMock: ReturnType<typeof vi.fn>;
...
function createWebSearchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    ENCLAWED_HOME: "/tmp/enclawed-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/enclawed.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/enclawed.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadEnclawedPluginsMock).toHaveBeenCalledTimes(count);
}
...
function expectRuntimeProviderResolution(
  ...
  expect(loadEnclawedPluginsMock).not.toHaveBeenCalled();
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadEnclawedPluginsMock = vi
      .spyOn(loaderModule, "loadEnclawedPlugins")
  ...
  it("retains the snapshot cache when config contents change in place", () => {
    ...
    const env = createWebSearchEnv({ ENCLAWED_HOME: "/tmp/enclawed-home-a" });
  ...
  it("invalidates the snapshot cache when env contents change in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.ENCLAWED_HOME = "/tmp/enclawed-home-b";
      },
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        ENCLAWED_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadEnclawedPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      ENCLAWED_PLUGIN_DISCOVERY_CACHE_MS: "5",
      ENCLAWED_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadEnclawedPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      ENCLAWED_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.ENCLAWED_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
