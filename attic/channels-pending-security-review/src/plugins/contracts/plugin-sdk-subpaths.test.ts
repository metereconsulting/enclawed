...
import type {
  ...
  EnclawedPluginApi as CoreEnclawedPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "@enclawed/plugin-sdk/core";
import * as providerEntrySdk from "@enclawed/plugin-sdk/provider-entry";
...
import type {
  ...
  EnclawedPluginApi as SharedEnclawedPluginApi,
...
import type { EnclawedPluginApi } from "../types.js";
...
const BROWSER_HELPER_EXPORT_PARITY_CONTRACTS: readonly BrowserHelperExportParityContract[] = [
  ...
  },
  {
    ...
    expectedExports: [
      ...
      "DEFAULT_ENCLAWED_BROWSER_ENABLED",
...
describe("plugin-sdk subpath exports", () => {
  ...
  it("keeps the deprecated channel-runtime shim unused in repo imports", () => {
    const matches = findRepoFilesContaining({
      ...
      pattern: /enclawed\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps shared plugin-sdk types aligned", () => {
    ...
    expectTypeOf<CoreEnclawedPluginApi>().toMatchTypeOf<EnclawedPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<PluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<ChannelMessageActionContext>();
    expectTypeOf<CoreEnclawedPluginApi>().toMatchTypeOf<SharedEnclawedPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<SharedPluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<SharedChannelMessageActionContext>();
  });

  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("@enclawed/plugin-sdk/core");
    ...
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "@enclawed/plugin-sdk/global-singleton",
    );
    const textRuntimeSdk = await importResolvedPluginSdkSubpath("@enclawed/plugin-sdk/text-runtime");
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("@enclawed/plugin-sdk/plugin-entry");
    ...
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`@enclawed/plugin-sdk/${id}`));
    }
