...
const PLUGIN_SDK_SUBPATH_PATTERN = /enclawed\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
function packEnclawedToTempDir(packDir: string): string {
...
async function readPackedRootPackageJson(archivePath: string): Promise<{
  ...
  const extractDir = makeTrackedTempDir("enclawed-packed-root-package-json", tempDirs);
  await tar.x({
    file: archivePath,
    cwd: extractDir,
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references @enclawed/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("keeps matrix crypto WASM in the packed artifact manifest", async () => {
    const tempRoot = makeTrackedTempDir("enclawed-matrix-wasm-pack", tempDirs);
    const packDir = join(tempRoot, "pack");
    mkdirSync(packDir, { recursive: true });

    const archivePath = packEnclawedToTempDir(packDir);
    ...
    expect(packedPackageJson.dependencies?.["@enclawed/plugin-package-contract"]).toBeUndefined();
  });
