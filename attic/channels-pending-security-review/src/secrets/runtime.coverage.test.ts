...
type SecretRegistryEntry = {
  id: string;
  configFile: "enclawed.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.ENCLAWED_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_ENCLAWED_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForEnclawedTarget(
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every enclawed.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) =>
        entry.configFile === "enclawed.json" &&
        !PLUGIN_OWNED_ENCLAWED_COVERAGE_EXCLUSIONS.has(entry.id),
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("enclawed.json", batch);
      const config = {} as EnclawedConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `ENCLAWED_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForEnclawedTarget(config, entry, envId, wildcardToken);
      }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `ENCLAWED_AUTH_SECRET_TARGET_${entry.id}`;
      ...
      const snapshot = await prepareAuthCoverageSnapshot({
        ...
        env,
        agentDirs: ["/tmp/enclawed-agent-main"],
