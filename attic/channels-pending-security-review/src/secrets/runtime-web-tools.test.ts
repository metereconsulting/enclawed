...
function setConfiguredFetchProviderKey(configTarget: EnclawedConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: EnclawedConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: EnclawedConfig, provider: ProviderUnderTest): unknown {
