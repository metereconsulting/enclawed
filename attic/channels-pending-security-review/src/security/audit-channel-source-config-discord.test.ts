...
function stubDiscordPlugin(params: {
  resolveAccount: (cfg: EnclawedConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: EnclawedConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: EnclawedConfig) => boolean;
}): ChannelPlugin {
  return {
    id: "discord",
    meta: {
...
describe("security audit channel source-config fallback discord", () => {
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    ...
    const resolvedConfig: EnclawedConfig = {
      commands: { native: true },
      channels: {
        discord: {
          enabled: true,
