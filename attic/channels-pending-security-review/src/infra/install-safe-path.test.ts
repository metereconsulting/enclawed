...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@enclawed/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@enclawed/matrix", id: "matrix", expected: true },
    { packageName: "@enclawed/matrix", id: "@enclawed/matrix", expected: true },
    { packageName: "@enclawed/matrix", id: "signal", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@enclawed/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@enclawed__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "enclawed-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "enclawed-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "enclawed-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    await withTempDir({ prefix: "enclawed-install-safe-" }, async (parentDir) => {
