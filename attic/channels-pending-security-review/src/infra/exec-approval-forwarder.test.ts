...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): EnclawedConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): EnclawedConfig {
  return {
