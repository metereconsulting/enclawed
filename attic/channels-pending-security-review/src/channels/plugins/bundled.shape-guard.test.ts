...
describe("bundled channel entry shape guards", () => {
  ...
  it("uses the active bundled plugin root override for channel entry loading", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-bundled-override-"));
    const previousBundledPluginsDir = process.env.ENCLAWED_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = path.join(tempRoot, "dist", "extensions");
      ...
      if (previousBundledPluginsDir === undefined) {
        ...
        process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
      }
  ...
  it("treats direct bundled plugin-tree overrides as scan roots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-bundled-direct-override-"));
    const previousBundledPluginsDir = process.env.ENCLAWED_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = pluginsRoot;
  ...
  it("partitions bundled channel lazy caches by active bundled root without re-importing", async () => {
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-bundled-root-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-bundled-root-b-"));
    const previousBundledPluginsDir = process.env.ENCLAWED_BUNDLED_PLUGINS_DIR;
    const testGlobal = globalThis as typeof globalThis & {
    ...
    try {
      ...
      process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = path.join(rootA, "dist", "extensions");
      ...
      process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = path.join(rootB, "dist", "extensions");
  ...
  it("loads setup-entry feature plugins without loading the main channel entry", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-bundled-setup-only-"));
    const previousBundledPluginsDir = process.env.ENCLAWED_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.ENCLAWED_BUNDLED_PLUGINS_DIR = path.join(root, "dist", "extensions");
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["index.ts", "channel-entry.ts", "setup-entry.ts"]) {
        ...
        if (
          !source.includes('from "@enclawed/plugin-sdk/channel-entry-contract"') ||
          source.includes('from "@enclawed/plugin-sdk/core"') ||
          source.includes('from "@enclawed/plugin-sdk/channel-core"')
        ) {
  ...
  it("keeps setup-entry legacy feature hints mirrored in package metadata", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      ...
      for (const feature of ["legacyStateMigrations", "legacySessionSurfaces"]) {
        ...
        const hasHint = packageJson.enclawed?.setupFeatures?.[feature] === true;
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "@enclawed/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("keeps bundled hot runtime barrels off the broad core SDK surface", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes("@enclawed/plugin-sdk/core"),
    );
  ...
  it("keeps bundled doctor surfaces off the broad runtime barrel", () => {
    const offenders = [
      ...
      fs
        ...
        .includes('from "@enclawed/plugin-sdk/runtime"'),
    );
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "enclawed-bundled-reentrant-"));
