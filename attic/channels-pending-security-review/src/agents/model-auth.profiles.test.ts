...
function buildOllamaProviderCfg(apiKey: string): EnclawedConfig {
  return {
    models: {
      providers: {
        ollama: {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-oauth-"));

    try {
      const agentDir = path.join(tempDir, "agent");
      await withEnvAsync(
        {
          ENCLAWED_STATE_DIR: tempDir,
          ENCLAWED_AGENT_DIR: agentDir,
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.ENCLAWED_AGENT_DIR, {
  ...
  it("resolveEnvApiKey('google-vertex') accepts ADC credentials from the provided env snapshot", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-google-adc-"));
  ...
  it("resolveEnvApiKey('anthropic-vertex') accepts GOOGLE_APPLICATION_CREDENTIALS with project_id", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-adc-"));
    const credentialsPath = path.join(tempDir, "adc.json");
