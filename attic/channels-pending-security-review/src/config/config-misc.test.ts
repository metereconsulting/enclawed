...
import { buildWebSearchProviderConfig, withTempHome, writeEnclawedConfig } from "./test-helpers.js";
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = EnclawedSchema.safeParse({
      $schema: "https://enclawed.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://enclawed.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = EnclawedSchema.safeParse({ $schema: 123 });
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@enclawed"]);
    }
  });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = EnclawedSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("accepts top-level memorySearch via auto-migration and reports legacyIssues", async () => {
    await withTempHome(async (home) => {
      await writeEnclawedConfig(home, {
        memorySearch: {
          provider: "local",
          fallback: "none",
          query: { maxResults: 7 },
  ...
  it("does not treat resolved-only gateway.bind aliases as source-literal legacy or invalid", async () => {
    await withTempHome(async (home) => {
      await writeEnclawedConfig(home, {
        gateway: { bind: "${ENCLAWED_BIND}" },
      });

      const prev = process.env.ENCLAWED_BIND;
      process.env.ENCLAWED_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(true);
        ...
        if (prev === undefined) {
          delete process.env.ENCLAWED_BIND;
        } else {
          process.env.ENCLAWED_BIND = prev;
        }
      }
    });
  });
