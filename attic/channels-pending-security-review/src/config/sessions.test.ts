...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "enclawed-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ENCLAWED_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-enclawed",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-enclawed#general");
  });
  ...
  it("derives session transcripts dir from ENCLAWED_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ENCLAWED_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ENCLAWED_STATE_DIR differs from stored paths", () => {
