...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir({ prefix: "enclawed-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "enclawed", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir({ prefix: "enclawed-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "enclawed", version: "2.3.4" });
  ...
  it("ignores non-enclawed package and blank build-info versions", async () => {
    await withTempDir({ prefix: "enclawed-version-" }, async (root) => {
  ...
  it("prefers ENCLAWED_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ENCLAWED_VERSION: "9.9.9",
        ENCLAWED_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale ENCLAWED_VERSION for compatibility checks", () => {
    const previous = process.env.ENCLAWED_VERSION;
    const previousService = process.env.ENCLAWED_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.ENCLAWED_VERSION = "2026.3.25";
      process.env.ENCLAWED_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.ENCLAWED_VERSION = previous;
      process.env.ENCLAWED_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ENCLAWED_VERSION: "2026.3.99",
        ENCLAWED_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        ENCLAWED_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        ENCLAWED_VERSION: "2026.3.99",
        ENCLAWED_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        ENCLAWED_VERSION: "   ",
        ENCLAWED_SERVICE_VERSION: "  2.0.0  ",
