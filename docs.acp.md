...
This document describes how the Enclawed ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`enclawed acp` exposes an ACP agent over stdio and forwards prompts to a running
Enclawed Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Enclawed Gateway
...
- ACP client spawns `enclawed acp` and speaks ACP messages over stdio.
...
`enclawed acp` resolves the Gateway URL and auth from CLI flags or config:
